...
class LayerGenerateNode(PipelineNode):
    ...
    async def _apply_mask(
        ...
        The mask is generated by the same VLM that created the content — it
        sees its own output and creates a precise silhouette. This avoids
        color-based assumptions (chroma key) and external ML dependencies
        (rembg). The VLM understands what is content vs background because
        it made the content.
        """
        import numpy as np
        ...
        try:
            ...
            mask_gray = mask_img.convert("L").resize(content_img.size)

            # Apply mask as alpha
            rgba = content_img.convert("RGBA")
            arr = np.array(rgba)
            arr[:, :, 3] = np.array(mask_gray)
