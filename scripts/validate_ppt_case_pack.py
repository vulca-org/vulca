...
RUN2_86_VISUAL_QUALITY_EVALUATION_STATUS = "run2_86_visual_quality_evaluation_public_blocked"
RUN2_86_VISUAL_QUALITY_QUESTION_IDS = {
...
RUN2_86_VISUAL_QUALITY_EXPECTED_ANSWERS = {
...
RUN2_86_ROOT_CAUSE_LAYERS = {
    "renderer_visual_primitives",
    "composition_engine",
    "design_motif_binding",
    "text_composition",
}
RUN2_86_DELTA_VALUES = {"improved", "partial", "unchanged", "regressed"}
...
RUN2_90_ASSET_SURFACE_COMPOSITION_STATUS = "run2_90_renderer_asset_surface_composition_rerun_generated_public_blocked"
RUN2_90_CONSUMED_SOURCE_PATHS = {
...
RUN2_91_VISUAL_QUALITY_EVALUATION_STATUS = "run2_91_visual_quality_evaluation_public_blocked"
RUN2_91_VISUAL_QUALITY_QUESTION_IDS = {
...
RUN2_91_VISUAL_QUALITY_EXPECTED_ANSWERS = {
...
def validate_run2_86_visual_quality_evaluation(pack_dir: Path, errors: list[str]) -> None:
    data = load_json(pack_dir / "results" / "run2_86_visual_quality_evaluation.json", errors)
    if not isinstance(data, dict):
        return
    label = "run2_86_visual_quality_evaluation"
    require_keys(
        label,
        data,
        [
    ...
    if data.get("artifact_id") != label:
        errors.append(f"{label}.artifact_id must be {label}")
    if data.get("part") != "Part Q":
        errors.append(f"{label}.part must be Part Q")
    if data.get("run_id") != "2.86":
        errors.append(f"{label}.run_id must be 2.86")
    if data.get("status") != RUN2_86_VISUAL_QUALITY_EVALUATION_STATUS:
        errors.append(f"{label}.status must be {RUN2_86_VISUAL_QUALITY_EVALUATION_STATUS}")
    for key in [
        "creates_new_ppt_deck",
        "starts_renderer_rerun",
        "updates_html_viewer",
        ...
        if data.get(key) is not False:
            errors.append(f"{label}.{key} must be false")
    if data.get("quality_claim_boundary") != "part_q_evaluation_only_no_public_release_no_renderer_rerun":
        errors.append(f"{label}.quality_claim_boundary must be part_q_evaluation_only_no_public_release_no_renderer_rerun")
    validate_run2_86_source_runs(f"{label}.source_runs", data.get("source_runs", {}), errors)
    validate_run2_86_input_chain(f"{label}.input_chain", data.get("input_chain", {}), errors)
    validate_run2_86_viewer_closure(f"{label}.viewer_comparison_closure", data.get("viewer_comparison_closure", {}), errors)
    validate_run2_86_gemini_summary(f"{label}.gemini_agent_review_summary", data.get("gemini_agent_review_summary", {}), errors)
    validate_run2_86_questions(f"{label}.evaluation_questions", data.get("evaluation_questions", {}), errors)
    validate_run2_86_assessment(f"{label}.visual_quality_assessment", data.get("visual_quality_assessment", {}), errors)
    validate_run2_86_role_assessments(f"{label}.role_assessments", data.get("role_assessments", []), errors)
    validate_run2_86_root_cause_summary(f"{label}.root_cause_summary", data.get("root_cause_summary", {}), errors)
    validate_run2_80_no_new_renderer_proof(f"{label}.no_new_renderer_proof", data.get("no_new_renderer_proof", {}), errors)
    if data.get("next_required_action") != "part_r_best_layout_recovery_and_visual_primitive_plan_from_q_evaluation":
        errors.append(f"{label}.next_required_action must be part_r_best_layout_recovery_and_visual_primitive_plan_from_q_evaluation")


def validate_run2_86_source_runs(label: str, value: Any, errors: list[str]) -> None:
    if not require_non_empty_dict(label, value, errors):
        return
    expected = {
...
def validate_run2_86_input_chain(label: str, value: Any, errors: list[str]) -> None:
    if not require_non_empty_dict(label, value, errors):
        return
    required = [
        "run2_85_result",
        "run2_84_p1_design_motif_plan",
        "run2_82_result",
        "run2_82_full_contact_sheet",
...
def validate_run2_86_viewer_closure(label: str, value: Any, errors: list[str]) -> None:
    if not require_non_empty_dict(label, value, errors):
        return
    require_keys(
        label,
        ...
        [
            "viewer_latest_run_id",
            "viewer_can_compare_2_82_and_2_85",
            "run2_82_full_preview_count",
            "run2_85_full_preview_count",
            "browser_check_required_for_handoff",
        ],
        errors,
    )
    if value.get("viewer_latest_run_id") != "2.85":
        errors.append(f"{label}.viewer_latest_run_id must be 2.85")
    if value.get("viewer_can_compare_2_82_and_2_85") is not True:
        errors.append(f"{label}.viewer_can_compare_2_82_and_2_85 must be true")
    for key in ["run2_82_full_preview_count", "run2_85_full_preview_count"]:
        if key in value and require_integer(f"{label}.{key}", value[key], errors) and value[key] != 6:
            errors.append(f"{label}.{key} must be 6")
    if value.get("browser_check_required_for_handoff") is not True:
        errors.append(f"{label}.browser_check_required_for_handoff must be true")
...
def validate_run2_86_gemini_summary(label: str, value: Any, errors: list[str]) -> None:
    if not require_non_empty_dict(label, value, errors):
        return
    require_keys(label, value, ["tool", "model", "review_count", "used_for_verdict", "run2_85_findings", "run2_85_risks"], errors)
    if value.get("tool") != "mcp__gemini_agent.gemini_artifact_review":
        errors.append(f"{label}.tool must be mcp__gemini_agent.gemini_artifact_review")
    if value.get("model") != "gemini-3.5-flash":
        errors.append(f"{label}.model must be gemini-3.5-flash")
    ...
    if value.get("used_for_verdict") is not True:
        errors.append(f"{label}.used_for_verdict must be true")
    if "run2_85_findings" in value:
        validate_string_list(f"{label}.run2_85_findings", value["run2_85_findings"], errors)
    if "run2_85_risks" in value:
        validate_string_list(f"{label}.run2_85_risks", value["run2_85_risks"], errors)


def validate_run2_86_questions(label: str, value: Any, errors: list[str]) -> None:
    if not require_non_empty_dict(label, value, errors):
        return
    for question_id in sorted(RUN2_86_VISUAL_QUALITY_QUESTION_IDS):
        if question_id not in value:
            errors.append(f"{label} missing key: {question_id}")
            continue
        question = value[question_id]
        ...
        if "answer" in question:
            require_non_empty_string(f"{question_label}.answer", question["answer"], errors)
    for question_id, expected in RUN2_86_VISUAL_QUALITY_EXPECTED_ANSWERS.items():
        question = value.get(question_id)
        if isinstance(question, dict) and question.get("answer") != expected:
            errors.append(f"{label}.{question_id}.answer must be {expected}")


def validate_run2_86_assessment(label: str, value: Any, errors: list[str]) -> None:
    if not require_non_empty_dict(label, value, errors):
        return
    expected_values = {
        "data_workflow_entry_gate": "pass_internal_only",
...
def validate_run2_86_role_assessments(label: str, value: Any, errors: list[str]) -> None:
    if not require_non_empty_list(label, value, errors):
        return
    roles: list[str] = []
    repair_count = 0
    for index, record in enumerate(value):
        ...
        if role in RUN2_73_VISUAL_GRAMMAR_PAGE_MODULE_MAP:
            ...
            if record.get("visual_grammar_module") != expected_module:
                errors.append(f"{record_label}.visual_grammar_module must be {expected_module} for {role}")
        if "delta_vs_2_82" in record:
            validate_choice(f"{record_label}.delta_vs_2_82", record["delta_vs_2_82"], RUN2_86_DELTA_VALUES, errors)
        if "motif_realization" in record:
            validate_choice(f"{record_label}.motif_realization", record["motif_realization"], RUN2_86_MOTIF_REALIZATION_VALUES, errors)
        if "layout_distinctiveness" in record:
            validate_choice(
                f"{record_label}.layout_distinctiveness",
                record["layout_distinctiveness"],
                RUN2_86_LAYOUT_DISTINCTIVENESS_VALUES,
                errors,
            )
        ...
        if "public_video_direction" in record:
            validate_choice(f"{record_label}.public_video_direction", record["public_video_direction"], RUN2_76_PUBLIC_VIDEO_DIRECTION_VALUES, errors)
        if "root_cause_layer" in record:
            validate_choice(f"{record_label}.root_cause_layer", record["root_cause_layer"], RUN2_86_ROOT_CAUSE_LAYERS, errors)
        if record.get("repair_required") is not True:
            errors.append(f"{record_label}.repair_required must be true")
        else:
            repair_count += 1
        ...
        trace = record.get("trace_support", {})
        if require_non_empty_dict(f"{record_label}.trace_support", trace, errors):
            for key in ["motif_family", "style_family", "scenario", "source_p1_primary_motif_id"]:
                if key in trace:
                    require_non_empty_string(f"{record_label}.trace_support.{key}", trace[key], errors)
                else:
                    errors.append(f"{record_label}.trace_support missing key: {key}")
            if "label_count" in trace and require_integer(f"{record_label}.trace_support.label_count", trace["label_count"], errors):
                if trace["label_count"] > 3:
                    errors.append(f"{record_label}.trace_support.label_count must be at most 3")
            elif "label_count" not in trace:
            ...
            if "preserved_visual_effects_rendered" in trace:
                validate_string_list(f"{record_label}.trace_support.preserved_visual_effects_rendered", trace["preserved_visual_effects_rendered"], errors)
            else:
                errors.append(f"{record_label}.trace_support missing key: preserved_visual_effects_rendered")
    if roles != RUN2_73_VISUAL_GRAMMAR_ROLES:
        errors.append(f"{label} roles must be {', '.join(RUN2_73_VISUAL_GRAMMAR_ROLES)}")
    if repair_count != 6:
        errors.append(f"{label} must mark all six pages repair_required")
...
def validate_run2_86_root_cause_summary(label: str, value: Any, errors: list[str]) -> None:
    if not require_non_empty_dict(label, value, errors):
        return
    require_keys(label, value, ["primary_layer", "not_primary_layer", "late_2_series_failure_mode"], errors)
    if value.get("primary_layer") != "renderer_visual_primitive_and_composition_engine":
        errors.append(f"{label}.primary_layer must be renderer_visual_primitive_and_composition_engine")
    if value.get("not_primary_layer") != "data_absence":
        errors.append(f"{label}.not_primary_layer must be data_absence")
    expected_failure_mode = "engineering_traceability_and_contract_layers_became_stronger_than_visual_execution_primitives"
...
def validate_run2_90_renderer_asset_surface_composition_rerun_result(pack_dir: Path, errors: list[str]) -> None:
    data = load_json(pack_dir / "results" / "run2_90_renderer_asset_surface_composition_rerun_result.json", errors)
    if not isinstance(data, dict):
        return
    label = "run2_90_renderer_asset_surface_composition_rerun_result"
    require_keys(
        label,
        data,
        [
    ...
    if data.get("artifact_id") != label:
        errors.append(f"{label}.artifact_id must be {label}")
    if data.get("part") != "Part U":
        errors.append(f"{label}.part must be Part U")
    if data.get("run_id") != "2.90":
        errors.append(f"{label}.run_id must be 2.90")
    if data.get("status") != RUN2_90_ASSET_SURFACE_COMPOSITION_STATUS:
        errors.append(f"{label}.status must be {RUN2_90_ASSET_SURFACE_COMPOSITION_STATUS}")
    if data.get("public_ready") is not False:
        errors.append(f"{label}.public_ready must be false")
    if data.get("public_release_started") is not False:
        errors.append(f"{label}.public_release_started must be false")
    expected_boundary = "asset_surface_composition_renderer_generated_viewer_check_only_no_quality_verdict"
    ...
    validate_exact_string_set(f"{label}.consumed_sources", data.get("consumed_sources", []), RUN2_90_CONSUMED_SOURCE_PATHS, errors)
    validate_run2_90_source_t_evaluation(f"{label}.source_t_evaluation", data.get("source_t_evaluation", {}), errors)
    validate_run2_90_source_run288(f"{label}.source_run2_88_renderer_result", data.get("source_run2_88_renderer_result", {}), errors)
    validate_run2_90_source_run287_plan(f"{label}.source_run2_87_plan", data.get("source_run2_87_plan", {}), errors)
    validate_run2_90_manifest(
        f"{label}.renderer_asset_surface_composition_manifest",
        data.get("renderer_asset_surface_composition_manifest", {}),
        errors,
    )
    validate_run2_90_rendered_pages(f"{label}.rendered_pages", data.get("rendered_pages", []), errors)
    validate_run2_90_checks(
        f"{label}.renderer_asset_surface_composition_checks",
        data.get("renderer_asset_surface_composition_checks", {}),
        errors,
    )
    if data.get("next_required_action") != "part_v_visual_quality_evaluation_for_run2_90":
        errors.append(f"{label}.next_required_action must be part_v_visual_quality_evaluation_for_run2_90")


def validate_run2_90_source_t_evaluation(label: str, value: Any, errors: list[str]) -> None:
    if not require_non_empty_dict(label, value, errors):
        return
    require_keys(label, value, ["status", "next_required_action", "source_result", "top_blocker", "next_layer_to_fix"], errors)
    if value.get("status") != RUN2_89_VISUAL_QUALITY_EVALUATION_STATUS:
        errors.append(f"{label}.status must be {RUN2_89_VISUAL_QUALITY_EVALUATION_STATUS}")
    if value.get("next_required_action") != "part_u_renderer_asset_surface_composition_repair_from_t_evaluation":
        errors.append(f"{label}.next_required_action must be part_u_renderer_asset_surface_composition_repair_from_t_evaluation")
    if value.get("source_result") != "docs/product/ppt-run2-data-skill-quality/results/run2_89_visual_quality_evaluation.json":
        errors.append(f"{label}.source_result must reference run2_89_visual_quality_evaluation.json")
    if value.get("next_layer_to_fix") != "renderer_asset_surface_and_composition_detail":
        errors.append(f"{label}.next_layer_to_fix must be renderer_asset_surface_and_composition_detail")


def validate_run2_90_source_run288(label: str, value: Any, errors: list[str]) -> None:
    if not require_non_empty_dict(label, value, errors):
        return
    require_keys(label, value, ["status", "next_required_action", "source_result"], errors)
    if value.get("status") != RUN2_88_BEST_LAYOUT_VISUAL_PRIMITIVE_STATUS:
        errors.append(f"{label}.status must be {RUN2_88_BEST_LAYOUT_VISUAL_PRIMITIVE_STATUS}")
    if value.get("next_required_action") != "part_t_visual_quality_evaluation_for_run2_88":
        errors.append(f"{label}.next_required_action must be part_t_visual_quality_evaluation_for_run2_88")
    if value.get("source_result") != "docs/product/ppt-run2-data-skill-quality/results/run2_88_best_layout_visual_primitive_rerun_result.json":
        errors.append(f"{label}.source_result must reference run2_88_best_layout_visual_primitive_rerun_result.json")


def validate_run2_90_source_run287_plan(label: str, value: Any, errors: list[str]) -> None:
    if not require_non_empty_dict(label, value, errors):
        return
    require_keys(label, value, ["status", "next_required_action", "source_result"], errors)
    if value.get("status") != RUN2_87_BEST_LAYOUT_PLAN_STATUS:
    ...
    if value.get("next_required_action") != "part_s_renderer_rerun_from_run2_87_best_layout_visual_primitive_plan":
        errors.append(f"{label}.next_required_action must be part_s_renderer_rerun_from_run2_87_best_layout_visual_primitive_plan")
    if value.get("source_result") != "docs/product/ppt-run2-data-skill-quality/run2_87_best_layout_recovery_visual_primitive_plan.json":
        errors.append(f"{label}.source_result must reference run2_87_best_layout_recovery_visual_primitive_plan.json")


def validate_run2_90_manifest(label: str, value: Any, errors: list[str]) -> None:
    if not require_non_empty_dict(label, value, errors):
        return
    require_keys(label, value, ["generator", "consumed_sources", "arms", "best_internal_arm", "outputs", "viewer_update"], errors)
    expected_generator = "scripts/generate_ppt_run2_90_renderer_asset_surface_composition_arms.mjs"
    if value.get("generator") != expected_generator:
        errors.append(f"{label}.generator must be {expected_generator}")
    validate_exact_string_set(f"{label}.consumed_sources", value.get("consumed_sources", []), RUN2_90_CONSUMED_SOURCE_PATHS, errors)
    validate_exact_string_set(f"{label}.arms", value.get("arms", []), RUN2_90_RENDERER_ARMS, errors)
    if value.get("best_internal_arm") != "run2_90_full_asset_surface_composition":
        errors.append(f"{label}.best_internal_arm must be run2_90_full_asset_surface_composition")
    outputs = value.get("outputs", {})
    if require_non_empty_dict(f"{label}.outputs", outputs, errors):
        for key in ["html_viewer", "pptx", "ppt_run_viewer", "four_arm_contact_sheet"]:
            if key not in outputs:
    ...
    viewer_update = value.get("viewer_update", {})
    if require_non_empty_dict(f"{label}.viewer_update", viewer_update, errors):
        if viewer_update.get("latest_run_id") != "2.90":
            errors.append(f"{label}.viewer_update.latest_run_id must be 2.90")
        if viewer_update.get("viewer_can_reference_new_run") is not True:
            errors.append(f"{label}.viewer_update.viewer_can_reference_new_run must be true")


def validate_run2_90_rendered_pages(label: str, value: Any, errors: list[str]) -> None:
    if not require_non_empty_list(label, value, errors):
        return
    roles: list[str] = []
    ...
    for index, page in enumerate(value):
        ...
        require_keys(
            ...
            [
                ...
                "slide_index",
                "visual_grammar_module",
                "source_t_root_cause_layer",
                "asset_surface_composition_id",
                "renderer_function_name",
                "renderer_repair_directives_applied",
                "anti_regression_gates",
                "asset_surface_rendered",
        ...
        if role in RUN2_73_VISUAL_GRAMMAR_PAGE_MODULE_MAP:
            ...
            if page.get("visual_grammar_module") != expected_module:
                errors.append(f"{page_label}.visual_grammar_module must be {expected_module} for {role}")
        if "source_t_root_cause_layer" in page:
            validate_choice(f"{page_label}.source_t_root_cause_layer", page["source_t_root_cause_layer"], RUN2_89_ROOT_CAUSE_LAYERS, errors)
        composition_id = page.get("asset_surface_composition_id")
        if require_non_empty_string(f"{page_label}.asset_surface_composition_id", composition_id, errors):
            composition_ids.add(composition_id)
        function_name = page.get("renderer_function_name")
        if require_non_empty_string(f"{page_label}.renderer_function_name", function_name, errors):
            if not function_name.startswith("drawRun290"):
                errors.append(f"{page_label}.renderer_function_name must start with drawRun290")
        validate_exact_string_set(
            f"{page_label}.renderer_repair_directives_applied",
            page.get("renderer_repair_directives_applied", []),
            RUN2_90_REPAIR_DIRECTIVES,
        ...
        for key in ["asset_surface_rendered", "composition_detail_added", "collision_avoidance_passed"]:
            if page.get(key) is not True:
                errors.append(f"{page_label}.{key} must be true")
        if page.get("wireframe_like") is not False:
            errors.append(f"{page_label}.wireframe_like must be false")
        if page.get("traceability_on_canvas") is not False:
            errors.append(f"{page_label}.traceability_on_canvas must be false")
        if "floating_label_count" in page and require_integer(f"{page_label}.floating_label_count", page["floating_label_count"], errors):
            if page["floating_label_count"] != 0:
        ...
        if "min_visible_label_font_size" in page and require_integer(f"{page_label}.min_visible_label_font_size", page["min_visible_label_font_size"], errors):
            if page["min_visible_label_font_size"] < 12:
                errors.append(f"{page_label}.min_visible_label_font_size must be at least 12")
        if "surface_detail_count" in page and require_integer(f"{page_label}.surface_detail_count", page["surface_detail_count"], errors):
            if page["surface_detail_count"] < 8:
                errors.append(f"{page_label}.surface_detail_count must be at least 8")
        if "filled_surface_count" in page and require_integer(f"{page_label}.filled_surface_count", page["filled_surface_count"], errors):
            if page["filled_surface_count"] < 4:
                errors.append(f"{page_label}.filled_surface_count must be at least 4")
        if "mock_asset_count" in page and require_integer(f"{page_label}.mock_asset_count", page["mock_asset_count"], errors):
            if page["mock_asset_count"] < 2:
                errors.append(f"{page_label}.mock_asset_count must be at least 2")
        if page.get("public_polish_claimed") is not False:
            errors.append(f"{page_label}.public_polish_claimed must be false")
    if roles != RUN2_73_VISUAL_GRAMMAR_ROLES:
        errors.append(f"{label} roles must be {', '.join(RUN2_73_VISUAL_GRAMMAR_ROLES)}")
    if len(composition_ids) != 6:
        errors.append(f"{label}.asset_surface_composition_id values must be unique for all six pages")


def validate_run2_90_checks(label: str, value: Any, errors: list[str]) -> None:
    if not require_non_empty_dict(label, value, errors):
        return
    require_keys(label, value, list(RUN2_90_CHECK_COUNTS) + ["public_quality_verdict_started"], errors)
    for key, expected in RUN2_90_CHECK_COUNTS.items():
        if key in value and require_integer(f"{label}.{key}", value[key], errors) and value[key] != expected:
            errors.append(f"{label}.{key} must be {expected}")
    if value.get("public_quality_verdict_started") is not False:
        errors.append(f"{label}.public_quality_verdict_started must be false")
...
def validate_run2_91_visual_quality_evaluation(pack_dir: Path, errors: list[str]) -> None:
    data = load_json(pack_dir / "results" / "run2_91_visual_quality_evaluation.json", errors)
    if not isinstance(data, dict):
        return
    label = "run2_91_visual_quality_evaluation"
    require_keys(
        label,
        data,
        [
    ...
    if data.get("artifact_id") != label:
        errors.append(f"{label}.artifact_id must be {label}")
    if data.get("part") != "Part V":
        errors.append(f"{label}.part must be Part V")
    if data.get("run_id") != "2.91":
        errors.append(f"{label}.run_id must be 2.91")
    if data.get("status") != RUN2_91_VISUAL_QUALITY_EVALUATION_STATUS:
    ...
    for key in [
        ...
        if data.get(key) is not False:
            errors.append(f"{label}.{key} must be false")
    if data.get("quality_claim_boundary") != "part_v_evaluation_only_no_public_release_no_renderer_rerun":
        errors.append(f"{label}.quality_claim_boundary must be part_v_evaluation_only_no_public_release_no_renderer_rerun")
    validate_run2_91_source_runs(f"{label}.source_runs", data.get("source_runs", {}), errors)
    validate_run2_91_input_chain(f"{label}.input_chain", data.get("input_chain", {}), errors)
    validate_run2_91_viewer_closure(f"{label}.viewer_comparison_closure", data.get("viewer_comparison_closure", {}), errors)
    validate_run2_91_gemini_summary(f"{label}.gemini_agent_review_summary", data.get("gemini_agent_review_summary", {}), errors)
    validate_run2_91_questions(f"{label}.evaluation_questions", data.get("evaluation_questions", {}), errors)
    validate_run2_91_assessment(f"{label}.visual_quality_assessment", data.get("visual_quality_assessment", {}), errors)
    validate_run2_91_role_assessments(f"{label}.role_assessments", data.get("role_assessments", []), errors)
    validate_run2_91_root_cause_summary(f"{label}.root_cause_summary", data.get("root_cause_summary", {}), errors)
    validate_run2_80_no_new_renderer_proof(f"{label}.no_new_renderer_proof", data.get("no_new_renderer_proof", {}), errors)
    if data.get("next_required_action") != "part_w_renderer_text_visual_binding_repair_from_v_evaluation":
        errors.append(f"{label}.next_required_action must be part_w_renderer_text_visual_binding_repair_from_v_evaluation")


def validate_run2_91_source_runs(label: str, value: Any, errors: list[str]) -> None:
    if not require_non_empty_dict(label, value, errors):
        return
    expected = {
...
def validate_run2_91_input_chain(label: str, value: Any, errors: list[str]) -> None:
    if not require_non_empty_dict(label, value, errors):
        return
    required = [
        "run2_90_result",
        "run2_89_t_evaluation",
        "run2_88_result",
        "run2_88_full_contact_sheet",
        "run2_90_full_contact_sheet",
        "run2_90_four_arm_contact_sheet",
        "ppt_run_viewer",
    ]
    require_keys(label, value, required, errors)
    for key in required:
...
def validate_run2_91_viewer_closure(label: str, value: Any, errors: list[str]) -> None:
    if not require_non_empty_dict(label, value, errors):
        return
    require_keys(
        label,
        ...
        [
            "viewer_latest_run_id",
            "viewer_can_compare_2_88_and_2_90",
            "run2_88_full_preview_count",
    ...
    if value.get("viewer_latest_run_id") != "2.90":
        errors.append(f"{label}.viewer_latest_run_id must be 2.90")
    if value.get("viewer_can_compare_2_88_and_2_90") is not True:
        errors.append(f"{label}.viewer_can_compare_2_88_and_2_90 must be true")
    for key in ["run2_88_full_preview_count", "run2_90_full_preview_count"]:
        if key in value and require_integer(f"{label}.{key}", value[key], errors) and value[key] != 6:
            errors.append(f"{label}.{key} must be 6")
    if "run2_90_arm_count" in value and require_integer(f"{label}.run2_90_arm_count", value["run2_90_arm_count"], errors):
        if value["run2_90_arm_count"] != 4:
            errors.append(f"{label}.run2_90_arm_count must be 4")
    if value.get("browser_check_required_for_handoff") is not True:
        errors.append(f"{label}.browser_check_required_for_handoff must be true")


def validate_run2_91_gemini_summary(label: str, value: Any, errors: list[str]) -> None:
    if not require_non_empty_dict(label, value, errors):
        return
    require_keys(label, value, ["tool", "model", "review_count", "used_for_verdict", "run2_90_findings", "run2_90_risks"], errors)
    if value.get("tool") != "gemini-agent artifact-review":
        errors.append(f"{label}.tool must be gemini-agent artifact-review")
    if value.get("model") != "gemini-3.5-flash":
        errors.append(f"{label}.model must be gemini-3.5-flash")
    ...
    if value.get("used_for_verdict") is not True:
        errors.append(f"{label}.used_for_verdict must be true")
    if "run2_90_findings" in value:
        validate_string_list(f"{label}.run2_90_findings", value["run2_90_findings"], errors)
    if "run2_90_risks" in value:
        validate_string_list(f"{label}.run2_90_risks", value["run2_90_risks"], errors)


def validate_run2_91_questions(label: str, value: Any, errors: list[str]) -> None:
    if not require_non_empty_dict(label, value, errors):
        return
    for question_id in sorted(RUN2_91_VISUAL_QUALITY_QUESTION_IDS):
        if question_id not in value:
            errors.append(f"{label} missing key: {question_id}")
            continue
        question = value[question_id]
        ...
        if "answer" in question:
            require_non_empty_string(f"{question_label}.answer", question["answer"], errors)
    for question_id, expected in RUN2_91_VISUAL_QUALITY_EXPECTED_ANSWERS.items():
        question = value.get(question_id)
        if isinstance(question, dict) and question.get("answer") != expected:
            errors.append(f"{label}.{question_id}.answer must be {expected}")


def validate_run2_91_assessment(label: str, value: Any, errors: list[str]) -> None:
    if not require_non_empty_dict(label, value, errors):
        return
    expected_values = {
        "data_workflow_entry_gate": "pass_internal_only",
...
def validate_run2_91_role_assessments(label: str, value: Any, errors: list[str]) -> None:
    if not require_non_empty_list(label, value, errors):
        return
    roles: list[str] = []
    repair_count = 0
    ...
    for index, record in enumerate(value):
        ...
        require_keys(
            ...
            [
                ...
                "asset_surface_delta",
                "text_visual_integration",
                "caption_anchor_quality",
                "proof_embedding_quality",
                "public_video_direction",
                "root_cause_layer",
                "repair_required",
                "visual_observation",
        ...
        if role in RUN2_73_VISUAL_GRAMMAR_PAGE_MODULE_MAP:
            ...
            if record.get("visual_grammar_module") != expected_module:
                errors.append(f"{record_label}.visual_grammar_module must be {expected_module} for {role}")
        if "asset_surface_delta" in record:
            validate_choice(f"{record_label}.asset_surface_delta", record["asset_surface_delta"], RUN2_91_ASSET_SURFACE_DELTA_VALUES, errors)
        if "text_visual_integration" in record:
            if validate_choice(
                f"{record_label}.text_visual_integration",
                record["text_visual_integration"],
                RUN2_91_TEXT_VISUAL_INTEGRATION_VALUES,
                errors,
            ):
                weak_or_partial_count += 1
        for key in ["caption_anchor_quality", "proof_embedding_quality"]:
            if key in record:
                validate_choice(f"{record_label}.{key}", record[key], RUN2_91_TEXT_VISUAL_INTEGRATION_VALUES, errors)
        if "public_video_direction" in record:
            validate_choice(f"{record_label}.public_video_direction", record["public_video_direction"], RUN2_76_PUBLIC_VIDEO_DIRECTION_VALUES, errors)
        if "root_cause_layer" in record:
            validate_choice(f"{record_label}.root_cause_layer", record["root_cause_layer"], RUN2_91_ROOT_CAUSE_LAYERS, errors)
        if record.get("repair_required") is not True:
            errors.append(f"{record_label}.repair_required must be true")
        else:
            repair_count += 1
        ...
        trace = record.get("trace_support", {})
        if require_non_empty_dict(f"{record_label}.trace_support", trace, errors):
            require_non_empty_string(f"{record_label}.trace_support.asset_surface_composition_id", trace.get("asset_surface_composition_id"), errors)
            function_name = trace.get("renderer_function_name")
            if require_non_empty_string(f"{record_label}.trace_support.renderer_function_name", function_name, errors):
                if not function_name.startswith("drawRun290"):
                    errors.append(f"{record_label}.trace_support.renderer_function_name must start with drawRun290")
            for key in ["asset_surface_rendered", "composition_detail_added"]:
                if trace.get(key) is not True:
                    errors.append(f"{record_label}.trace_support.{key} must be true")
    ...
    if repair_count != 6:
        errors.append(f"{label} must mark all six pages repair_required")
    if weak_or_partial_count != 6:
        errors.append(f"{label} must keep text_visual_integration blocked for all six pages")


def validate_run2_91_root_cause_summary(label: str, value: Any, errors: list[str]) -> None:
    if not require_non_empty_dict(label, value, errors):
        return
    require_keys(label, value, ["primary_layer", "not_primary_layer", "late_2_series_failure_mode"], errors)
    if value.get("primary_layer") != "object_bound_typography_and_text_visual_integration":
        errors.append(f"{label}.primary_layer must be object_bound_typography_and_text_visual_integration")
    if value.get("not_primary_layer") != "data_absence":
        errors.append(f"{label}.not_primary_layer must be data_absence")
    expected_failure_mode = "surface_layer_fixed_before_text_visual_binding_layer"
    if value.get("late_2_series_failure_mode") != expected_failure_mode:
        errors.append(f"{label}.late_2_series_failure_mode must be {expected_failure_mode}")
    secondary = value.get("secondary_layers")
    if secondary is not None:
