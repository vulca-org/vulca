...
const RUN2_40_POLICY = "trace_to_hidden_constraints_public_surface_composition";
const RUN2_40_VERDICT = "same_run2_38_run2_39_data_no_database_expansion_visual_compiler_improves_public_surface";
const RUN2_40_DATA_STATUS = "same_run2_38_run2_39_data_no_database_expansion";
...
const RUN2_40_DATA_INPUTS = Object.values(RUN2_40_INPUTS);

const baseSlides = [
  {
    role: "cover",
    title: "A launch deck should feel designed first.",
    claim: "The same data should become public-facing composition, not visible trace machinery.",
  },
  {
    role: "setup",
    title: "The problem is not missing data anymore.",
    claim: "The problem is that evidence is being shown as machinery instead of being compiled into taste.",
  },
  {
    role: "contrast",
    title: "Same data, different surface.",
    claim: "Holding Run 2.38 and Run 2.39 constant isolates the visual compiler layer.",
  },
  {
    role: "proof",
    title: "The product moment replaces the audit board.",
    claim: "Memory, recipe, and gates stay in trace while the slide shows one readable product state.",
  },
  {
    role: "climax",
    title: "The result has to own the frame.",
    claim: "The public high point is a cinematic generated-output object, not a system diagram.",
  },
  {
    role: "close",
    title: "Review the surface, not the machinery.",
    claim: "Public release remains blocked, but the next judgment should be about composition quality.",
  },
];
...
const armSpecs = [
  ...
  {
    armId: "run1_5_skill",
    slug: "ppt-run2-40-run1-5-skill",
    label: "Run 1.5 baseline",
    kicker: "RUN 2.40 / RUN 1.5",
  ...
  },
  {
    ...
    slug: "ppt-run2-40-full-vulca",
    label: "Run 2.40 full visual compiler",
    kicker: "RUN 2.40 / VISUAL COMPILER",
    footer: "run2_40_full_visual_compiler | same 2.38/2.39 data | trace hidden | public blocked",
    release: "public_blocked",
    allowed: [
      `${pack}/commercial_case.md`,
      RUN2_40_INPUTS.run239RerunResult,
      RUN2_40_INPUTS.publicVideoSlideDirectionMemory,
      RUN2_40_INPUTS.perSlideVisualRecipeMemory,
      RUN2_40_INPUTS.publicVideoWorkflowGates,
    ...
    visual_compiler_policy: RUN2_40_POLICY,
    palette: {
      bg: "#f8f4eb",
      rail: "#11161d",
      accent: "#11161d",
  ...
  },
  {
    ...
    slug: "ppt-run2-40-bad-trace-visible-visual-compiler",
    label: "Bad trace-visible visual compiler",
    kicker: "RUN 2.40 / SAME DATA CONTROL",
    footer: "bad_trace_visible_visual_compiler | same data but machinery visible | internal comparison",
    ...
    forbidden: ["new database records", "claiming hidden machinery policy"],
    visual_compiler_policy: "same_data_trace_visible_control",
    palette: {
      bg: "#f1eadb",
      rail: "#695d3a",
      accent: "#75683f",
    ...
    slides: baseSlides.map((slide, index) => ({
      ...slide,
      title: [
        "Same data, but the machinery is visible.",
        "The slide explains the compiler instead of the business.",
        "Trace terms compete with the transformation.",
        "The product moment becomes an audit board.",
        "The climax is swallowed by ids.",
        "The close shows gates instead of a decision.",
      ][index],
    })),
  },
];
...
function registerRun240Module(metrics, functionName) {
...
function armUsesRun240Data(arm) {
  return arm.armId === "run2_40_full_visual_compiler" || arm.armId === "bad_trace_visible_visual_compiler";
}

function assertRun240ArmInputBoundaries(arm) {
  const allowed = new Set(arm.allowed);
  const forbidden = new Set(arm.forbidden);
  if (armUsesRun240Data(arm)) {
    for (const input of RUN2_40_DATA_INPUTS) {
...
function readRun240PackJsonForArm(arm, relPath) {
  assertRun240ArmInputBoundaries(arm);
  if (!arm.allowed.includes(relPath) || arm.forbidden.includes(relPath) || !armUsesRun240Data(arm)) {
    throw new Error(`${arm.armId} input boundary does not permit reading ${relPath}`);
  }
  return readJson(relPath);
}

function validateRun240SameDataVisualCompiler(data) {
  const { result239, directionMemory, recipeMemory, workflowGates } = data;
  if (result239?.status !== "run2_39_public_video_visual_direction_rerun_public_blocked") {
    throw new Error("Run 2.40 must consume Run 2.39 rerun result");
  }
  if (directionMemory?.status !== "run2_38_public_video_slide_direction_memory_ready_public_blocked") {
    throw new Error("Run 2.40 direction memory status mismatch");
  }
  if (recipeMemory?.status !== "run2_38_per_slide_visual_recipe_memory_ready_public_blocked") {
    throw new Error("Run 2.40 recipe memory status mismatch");
  }
  if (workflowGates?.status !== "run2_38_public_video_workflow_gates_ready_public_blocked") {
    throw new Error("Run 2.40 workflow gate status mismatch");
  }
  if (result239.source_data_workflow_run_id !== "2.38" || result239.source_generated_run_id !== "2.36") {
    throw new Error("Run 2.40 source chain mismatch");
  }
  ...
  if (directions.length !== 6 || recipes.length !== 6 || gates.length !== 6) {
    throw new Error("Run 2.40 must reuse exactly six Run 2.38 records/gates");
  }
  ...
  for (const role of roles) {
    ...
    if (!direction || !recipe || !gate) throw new Error(`Run 2.40 missing same-data role ${role}`);
    if (recipe.source_direction_memory_id !== direction.direction_memory_id) {
      throw new Error(`Run 2.40 direction/recipe mismatch for ${role}`);
    }
    if (gate.required_public_video_slide_direction_memory_id !== direction.direction_memory_id) {
      throw new Error(`Run 2.40 direction/gate mismatch for ${role}`);
    }
  }
}

function loadRun240ContractData(arm) {
  const result239 = readRun240PackJsonForArm(arm, RUN2_40_INPUTS.run239RerunResult);
  const directionMemory = readRun240PackJsonForArm(arm, RUN2_40_INPUTS.publicVideoSlideDirectionMemory);
  const recipeMemory = readRun240PackJsonForArm(arm, RUN2_40_INPUTS.perSlideVisualRecipeMemory);
  const workflowGates = readRun240PackJsonForArm(arm, RUN2_40_INPUTS.publicVideoWorkflowGates);
  const data = { result239, directionMemory, recipeMemory, workflowGates };
  validateRun240SameDataVisualCompiler(data);
  return {
    ...data,
    status: "run2_40_same_data_visual_compiler_contract_ready",
  };
}

function selectRun240ForSlide(role, contractData) {
  ...
  if (!direction || !recipe || !gate) throw new Error(`Run 2.40 missing role ${role}`);
  return visualCompilerTransform({ direction, recipe, gate });
}

function visualCompilerTransform(selection) {
  const { direction, recipe, gate } = selection;
  return {
    direction,
    recipe,
    gate,
    ...
    hiddenConstraintPolicy: RUN2_40_POLICY,
    publicSurfaceMachineryHidden: true,
    sameDataStatus: RUN2_40_DATA_STATUS,
  };
}

function drawRun240EditorialPoster(slide, arm, spec, selection, metrics) {
  registerRun240Module(metrics, "drawRun240EditorialPoster");
...
function drawRun240UsecaseScene(slide, arm, spec, selection, metrics) {
  registerRun240Module(metrics, "drawRun240UsecaseScene");
...
function drawRun240TransformationSpread(slide, arm, spec, selection, metrics) {
  registerRun240Module(metrics, "drawRun240TransformationSpread");
  ...
  text(slide, "visible machinery", before.x + 24, before.y + 30, 160, 16, { fontSize: 12, mono: true, bold: true, color: arm.palette.muted });
  ...
  registerText(metrics, `visible machinery public-facing composition ${selection.publicObject}`);
  registerPresentationSurface(metrics, after.x, after.y, after.w, after.h, 0.74);
  registerProof(metrics, 3);
  registerZones(metrics, 2);
}

function drawRun240ProductMoment(slide, arm, spec, selection, metrics) {
  registerRun240Module(metrics, "drawRun240ProductMoment");
...
function drawRun240CinematicResult(slide, arm, spec, selection, metrics) {
  registerRun240Module(metrics, "drawRun240CinematicResult");
...
function drawRun240DecisionScene(slide, arm, spec, selection, metrics) {
  registerRun240Module(metrics, "drawRun240DecisionScene");
  ...
  registerPresentationSurface(metrics, pathArea.x, pathArea.y, pathArea.w, pathArea.h, 0.7);
  registerProof(metrics, 2);
  registerZones(metrics, 3);
}

const RUN2_40_MODULES = {
  cover: drawRun240EditorialPoster,
  setup: drawRun240UsecaseScene,
  contrast: drawRun240TransformationSpread,
  proof: drawRun240ProductMoment,
  climax: drawRun240CinematicResult,
  close: drawRun240DecisionScene,
};

function drawBadTraceVisibleSlide(slide, arm, spec, selection, metrics) {
  text(slide, spec.title, 76, 122, 500, 72, {
    fontSize: 34,
    bold: true,
    title: true,
    color: arm.palette.title,
  ...
  text(slide, spec.claim, 80, 214, 450, 58, { fontSize: 14, color: arm.palette.muted });
  registerText(metrics, spec.claim);
  const surface = { x: 84, y: 322, w: 470, h: 152 };
  rect(slide, surface.x, surface.y, surface.w, surface.h, arm.palette.panel, colorLine(arm.palette.rule, 1));
  text(slide, "same visual data, poor compiler", surface.x + 24, surface.y + 26, 330, 22, {
    fontSize: 20,
    title: true,
    bold: true,
    color: arm.palette.title,
  });
  text(slide, "The evidence is real, but it is exposed as labels instead of becoming composition.", surface.x + 24, surface.y + 72, 380, 42, {
    fontSize: 12,
    color: arm.palette.muted,
  });
  const trace = { x: 650, y: 116, w: 430, h: 430 };
  rect(slide, trace.x, trace.y, trace.w, trace.h, "#efe3c9", colorLine("#c9b789", 2));
  text(slide, "TRACE / GATE / MEMORY", trace.x + 28, trace.y + 28, 250, 16, {
    fontSize: 12,
    mono: true,
    bold: true,
    color: arm.palette.title,
  ...
  const rows = [
    ["memory", selection.direction.direction_memory_id],
    ["recipe", selection.recipe.recipe_memory_id],
    ["gate", selection.gate.gate_id],
    ["module", selection.gate.required_code_modules?.[0] ?? ""],
  ];
  rows.forEach(([label, value], index) => {
    const y = trace.y + 82 + index * 72;
    rect(slide, trace.x + 28, y, trace.w - 56, 48, index === 0 ? "#e4d3aa" : "#f7edda", colorLine("#d2be91", 1));
    text(slide, label, trace.x + 46, y + 12, 76, 10, {
      fontSize: 8,
      mono: true,
      bold: true,
      color: arm.palette.title,
    });
    text(slide, compactText(value, 38), trace.x + 134, y + 12, 230, 14, {
      fontSize: 8,
      mono: true,
      color: arm.palette.muted,
    });
  ...
  registerMachinery(metrics, 4);
  registerPresentationSurface(metrics, trace.x, trace.y, trace.w, trace.h, 0.42);
...
function renderRun240Slide(presentation, spec, arm, n, contractData, metricsByRole) {
  const slide = presentation.slides.add();
  base(slide, arm, n);
  const metrics = createSlideMetrics(spec.role);
  if (arm.armId === "run2_40_full_visual_compiler") {
    const selection = selectRun240ForSlide(spec.role, contractData);
    RUN2_40_MODULES[spec.role](slide, arm, spec, selection, metrics);
  } else if (arm.armId === "bad_trace_visible_visual_compiler") {
    const selection = selectRun240ForSlide(spec.role, contractData);
    drawBadTraceVisibleSlide(slide, arm, spec, selection, metrics);
  } else {
...
function traceFor(arm, context = {}) {
  assertRun240ArmInputBoundaries(arm);
  const usesData = armUsesRun240Data(arm);
  const fullRun240 = arm.armId === "run2_40_full_visual_compiler";
  const badTraceControl = arm.armId === "bad_trace_visible_visual_compiler";
  const fullData = usesData ? context.fullData ?? loadRun240ContractData(arm) : null;
  ...
  return {
    ...
    source_data_status: usesData ? RUN2_40_DATA_STATUS : "",
    database_expansion: false,
    run2_39_source_rerun_status: usesData ? fullData.result239.status : "",
    run2_40_visual_compiler_status: fullRun240
      ? "same_data_visual_compiler_applied"
      : badTraceControl
        ? "same_data_trace_visible_control"
        : "boundary_control_no_run2_40_visual_compiler",
    ...
    runtime_isolation: {
      ...
      prompt_context: "fresh Run 2.40 same-data visual compiler generation from scripts/generate_ppt_run2_40_visual_compiler_arms.mjs",
    ...
    slides: arm.slides.map((slide, index) => {
      ...
      const selection = usesData ? selectRun240ForSlide(slide.role, fullData) : null;
      const codeModuleIds = Array.from(roleMetrics.visualModuleIds);
      return {
        slide_id: `slide_${String(index + 1).padStart(2, "0")}`,
        role: slide.role,
        title: slide.title,
        run2_40_contract_status: usesData
          ? hasRenderedMetrics
            ? "same_data_visual_compiler_rendered"
            : "same_data_visual_compiler_preview_not_rendered"
          : "boundary_control_not_run2_40_same_data",
        ...
        run2_40_visual_compiler_policy: fullRun240 ? RUN2_40_POLICY : badTraceControl ? "same_data_trace_visible_control" : "",
        run2_40_public_surface_machinery_hidden: fullRun240,
        run2_40_visible_machinery_terms: fullRun240 ? 0 : badTraceControl ? Math.max(3, roleMetrics.visibleMachineryTerms) : 0,
        run2_40_code_module_ids: fullRun240
          ? hasRenderedMetrics
            ? codeModuleIds
            : [RUN2_40_MODULES[slide.role].name]
          : [],
        run2_40_same_data_control_status: badTraceControl
          ? "same_data_without_hidden_trace_visual_compiler"
          : fullRun240
            ? "same_data_with_hidden_trace_visual_compiler"
            : "",
        layout_metrics: {
          ...
          trace_panel_visible: fullRun240 ? false : roleMetrics.tracePanelVisible,
          gate_ribbon_visible: fullRun240 ? false : roleMetrics.gateRibbonVisible,
        },
      };
    }),
  };
...
function assertRun240VisualCompilerSelfCheck(trace) {
  if (trace.arm_id === "run2_40_full_visual_compiler") {
    if (trace.run2_40_visual_compiler_status !== "same_data_visual_compiler_applied") {
      throw new Error("Run 2.40 full trace did not apply visual compiler");
    }
    for (const slide of trace.slides) {
      if (!String(slide.run2_38_visual_direction_memory_id ?? "").startsWith("direction_2_38_")) {
        throw new Error(`Run 2.40 full slide ${slide.slide_id} missing direction memory`);
      }
      if (slide.run2_40_visual_compiler_policy !== RUN2_40_POLICY) {
        throw new Error(`Run 2.40 full slide ${slide.slide_id} missing compiler policy`);
      }
      if (slide.run2_40_public_surface_machinery_hidden !== true || slide.run2_40_visible_machinery_terms !== 0) {
        throw new Error(`Run 2.40 full slide ${slide.slide_id} leaked visible machinery`);
      }
      if ((slide.layout_metrics?.presentation_surface_weight ?? 0) < 0.7) {
        throw new Error(`Run 2.40 full slide ${slide.slide_id} presentation surface too weak`);
      }
      if (slide.layout_metrics?.trace_panel_visible !== false || slide.layout_metrics?.gate_ribbon_visible !== false) {
        throw new Error(`Run 2.40 full slide ${slide.slide_id} exposes trace/gate UI`);
      }
      if ((slide.run2_40_code_module_ids ?? []).length !== 1 || !slide.run2_40_code_module_ids[0].startsWith("drawRun240")) {
        throw new Error(`Run 2.40 full slide ${slide.slide_id} missing Run 2.40 module`);
      }
    }
  }
  if (trace.arm_id === "bad_trace_visible_visual_compiler") {
    for (const slide of trace.slides) {
      if (!String(slide.run2_38_visual_direction_memory_id ?? "").startsWith("direction_2_38_")) {
        throw new Error(`Run 2.40 bad slide ${slide.slide_id} did not use same data`);
      }
      if (slide.run2_40_public_surface_machinery_hidden !== false || slide.run2_40_visible_machinery_terms < 3) {
        throw new Error(`Run 2.40 bad slide ${slide.slide_id} did not expose machinery control`);
      }
      if (slide.layout_metrics?.trace_panel_visible !== true) {
        throw new Error(`Run 2.40 bad slide ${slide.slide_id} missing trace panel`);
      }
    }
  }
}
...
function buildRun240FourArmSheet(built) {
  ...
  return buildNamedContactSheet(
    path.join(outRoot, "run2-40-four-arm-contact-sheet.png"),
    "Run 2.40 same-data visual compiler comparison",
    sheets,
    sheets.length,
    labels,
  );
...
async function buildArm(arm) {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "required proof objects: same Run 2.38/Run 2.39 data, hidden trace constraints, public-facing composition, same-data trace-visible control",
      "source requirements: full and bad same-data control both read Run 2.39 result plus Run 2.38 direction/recipe/gates; prompt and run1.5 controls do not",
      "brand authenticity constraints: no copied source visuals, no screenshots, no raw tutorial media",
      "profile-specific QA gates: public surface machinery hidden in full arm; visible machinery only in bad same-data control; public release blocked",
  ...
  const fullData = armUsesRun240Data(arm) ? loadRun240ContractData(arm) : null;
  const slides = arm.slides.map((slide, index) => renderRun240Slide(presentation, slide, arm, index + 1, fullData, metricsByRole));
  ...
  assertRun240VisualCompilerSelfCheck(trace);
  ...
  const manifest = {
    ...
    slides: slides.map((_, index) => ({
      ...
      source: "scripts/generate_ppt_run2_40_visual_compiler_arms.mjs",
...
function writeRun240Result(runSummary) {
  const result = {
    schema_version: 1,
    run_id: "2.40",
    status: "run2_40_visual_compiler_rerun_public_blocked",
    public_ready: false,
    selected_usecase_id: selectedUsecaseId,
    source_data_workflow_run_id: "2.38",
    source_generated_run_id: "2.39",
    database_expansion: false,
    stage_policy: "repeat_same_five_layers_not_run3",
    input_chain: {
      run2_39_rerun_result: RUN2_40_INPUTS.run239RerunResult,
      public_video_slide_direction_memory: RUN2_40_INPUTS.publicVideoSlideDirectionMemory,
      per_slide_visual_recipe_memory: RUN2_40_INPUTS.perSlideVisualRecipeMemory,
      public_video_workflow_gates: RUN2_40_INPUTS.publicVideoWorkflowGates,
      source_data_layer: RUN2_40_INPUTS.run239RerunResult,
    },
    rerun: {
      generator: "scripts/generate_ppt_run2_40_visual_compiler_arms.mjs",
      arms: armSpecs.map((arm) => arm.armId),
      best_internal_arm: "run2_40_full_visual_compiler",
      best_internal_arm_verdict: RUN2_40_VERDICT,
    ...
    quality_delta: {
      target_layer: "visual_compiler_hidden_trace_public_composition",
      same_data_control: true,
      source_data_status: RUN2_40_DATA_STATUS,
      new_database_records_added: 0,
      public_surface_machinery_hidden_slides: 6,
      visible_machinery_same_data_control_slides: 6,
      visual_compiler_policy: RUN2_40_POLICY,
      repair_modules: [
    ...
    control_boundary: {
      bad_trace_visible_visual_compiler:
        "same Run 2.38 and Run 2.39 data, but trace/gate/memory/module terms stay visible on the public surface",
      prompt_only: "commercial_case_only_no_run2_38_run2_39_data",
      run1_5_skill: "prior_baseline_no_run2_38_run2_39_data",
    },
    visual_quality_boundary:
      "visual_compiler_surface_proof_only_not_final_human_approved_public_release_or_motion_review",
    ...
    next_required_action:
      "audit_run2_40_outputs_against_run2_10_and_run2_25_presentation_quality_then_continue_same_five_layers",
  };
  const resultJson = path.join(root, pack, "results", "run2_40_visual_compiler_rerun_result.json");
  const resultMd = path.join(root, pack, "results", "run2_40_visual_compiler_rerun_result.md");
...
async function main() {
  ...
  const fourArmSheet = buildRun240FourArmSheet(built);
  const fullSkillSeriesSheet = buildFullSkillSeriesSheet();
  const runSummary = {
    run_id: "run2_40_same_data_visual_compiler_four_arms",
    selected_usecase_id: selectedUsecaseId,
    arms: armSpecs.map((arm) => arm.armId),
  ...
  writeJson(path.join(outRoot, "run2_40_visual_compiler_rerun_summary.json"), runSummary);
  writeRun240Result(runSummary);
  return runSummary;
}
...
export {
  ...
  RUN2_40_DATA_INPUTS,
  armSpecs,
  assertRun240ArmInputBoundaries,
  assertRun240VisualCompilerSelfCheck,
  drawRun240EditorialPoster,
  drawRun240UsecaseScene,
  drawRun240TransformationSpread,
  drawRun240ProductMoment,
  drawRun240CinematicResult,
  drawRun240DecisionScene,
  loadRun240ContractData,
  main,
  readRun240PackJsonForArm,
  registerRun240Module,
  selectRun240ForSlide,
  traceFor,
