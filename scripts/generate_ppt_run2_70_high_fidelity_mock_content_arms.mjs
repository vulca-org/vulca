...
const RUN2_70_FULL_STATUS = "run2_70_high_fidelity_mock_content_consumed_before_native_ppt_drawing";
const RUN2_70_BAD_STATUS = "fail_missing_run2_70_high_fidelity_mock_content";
const RUN2_70_POLICY = "run2_69_high_fidelity_product_mock_content";
...
const RUN2_70_INPUTS = {
  run269Result: `${pack}/results/run2_69_public_content_fill_rerun_result.json`,
  run269FullTrace: `outputs/${threadId}/presentations/ppt-run2-69-full-vulca/trace_manifest.json`,
...
const RUN2_70_FULL_DATA_INPUTS = Object.values(RUN2_70_INPUTS);
const RUN2_70_BAD_DATA_INPUTS = [
  RUN2_70_INPUTS.run269Result,
  RUN2_70_INPUTS.run269FullTrace,
  RUN2_70_INPUTS.commercialUsecaseBank,
  RUN2_70_INPUTS.sources,
];
...
const armSpecs = [
  {
    armId: "prompt_only",
    slug: "ppt-run2-70-prompt-only",
    label: "Prompt-only control",
    kicker: "RUN 2.70 / CONTROL",
    footer: "prompt only | brief only | public review",
    release: "public_blocked",
    allowed: [`${pack}/commercial_case.md`],
    forbidden: [...RUN2_70_FULL_DATA_INPUTS, "drawRun270HighFidelityProductMockSurface"],
  ...
  {
    armId: "run1_5_skill",
    slug: "ppt-run2-70-run1-5-skill",
    label: "Run 1.5 baseline",
    kicker: "RUN 2.70 / RUN 1.5",
  ...
  {
    armId: "run2_70_full_high_fidelity_mock_content",
    slug: "ppt-run2-70-full-vulca",
    label: "Run 2.70 full high-fidelity mock content",
    kicker: "RUN 2.70 / PRODUCT MOCKS",
    footer: "full arm | high-fidelity mock content | public review",
    release: "public_blocked",
    allowed: [`${pack}/commercial_case.md`, ...RUN2_70_FULL_DATA_INPUTS, `${pack}/skill_workflow.json`, `${pack}/vulca_ppt_skill.md`],
    data_input_manifest: [
    ...
    forbidden: [
      ...
      "native_drawing_before_run2_70_high_fidelity_mock_content",
    ],
    palette: { bg: C.paper, rail: C.dark, accent: C.coral, accent2: C.blue, panel: C.white, title: C.ink, muted: C.muted, rule: "#dacfbf" },
    slides: baseSlides,
  },
  {
    armId: "bad_run2_69_without_high_fidelity_mock_content",
    slug: "ppt-run2-70-bad-without-high-fidelity-mock-content",
    label: "Bad missing Run 2.70 high-fidelity mock content",
    kicker: "RUN 2.70 / BAD CONTROL",
    footer: "bad control | can see 2.69 | no high-fidelity mock content",
    release: "internal_only",
    allowed: [`${pack}/commercial_case.md`, ...RUN2_70_BAD_DATA_INPUTS],
    data_input_manifest: ["run2_69_generated_without_run2_70_high_fidelity_mock_content"],
    forbidden: [
...
function armUsesFullRun270Grammar(arm) {
  return arm.armId === "run2_70_full_high_fidelity_mock_content";
}

function armUsesBadRun270Data(arm) {
  return arm.armId === "bad_run2_69_without_high_fidelity_mock_content";
}

function assertRun270ArmInputBoundaries(arm) {
  const allowed = new Set(arm.allowed);
  const forbidden = new Set(arm.forbidden);
  if (armUsesFullRun270Grammar(arm)) {
    for (const input of RUN2_70_FULL_DATA_INPUTS) {
  ...
  if (armUsesBadRun270Data(arm)) {
    for (const input of RUN2_70_BAD_DATA_INPUTS) if (!allowed.has(input)) throw new Error(`${arm.armId} must allow bad-control input ${input}`);
    for (const input of [RUN2_70_INPUTS.run266Result, RUN2_70_INPUTS.run266FailureAudit, RUN2_70_INPUTS.run266DesignGrammar, RUN2_70_INPUTS.run266ArtDirection, RUN2_70_INPUTS.run266WorkflowGates]) {
...
function readRun270PackJsonForArm(arm, relPath) {
  assertRun270ArmInputBoundaries(arm);
  ...
  if (!armUsesFullRun270Grammar(arm) && !armUsesBadRun270Data(arm)) throw new Error(`${arm.armId} cannot read Run 2.70 pack data`);
  if (!armUsesFullRun270Grammar(arm) && relPath.includes("run2_66")) throw new Error(`${arm.armId} cannot read Run 2.66 reference-first data`);
...
function assertSelectedUsecase(value, label) {
  if (value !== selectedUsecaseId) throw new Error(`Run 2.70 selected usecase mismatch in ${label}`);
}

function validateRun270ReferenceInputs(data) {
  if (data.run269Result?.status !== "run2_69_public_content_fill_rerun_public_blocked") throw new Error("Run 2.70 must consume Run 2.69 result");
  if (data.run269FullTrace?.arm_id !== "run2_69_full_public_content_fill") throw new Error("Run 2.70 must consume Run 2.69 full trace");
  if (data.run266Result?.status !== "run2_66_reference_first_redesign_ready_public_blocked") throw new Error("Run 2.70 must consume Run 2.66 result");
  if (data.run266DesignGrammar?.status !== "run2_66_reference_first_design_grammar_ready_public_blocked") throw new Error("Run 2.70 must consume Run 2.66 design grammar");
  if (data.run266ArtDirection?.status !== "run2_66_slide_art_direction_contracts_ready_public_blocked") throw new Error("Run 2.70 must consume Run 2.66 art direction");
  if (data.run266WorkflowGates?.status !== "run2_66_reference_first_workflow_gates_ready_public_blocked") throw new Error("Run 2.70 must consume Run 2.66 workflow gates");
  if (data.run265Result?.status !== "run2_65_renderer_composition_rerun_public_blocked") throw new Error("Run 2.70 must compare against Run 2.65 result");
  if (data.run265FullTrace?.arm_id !== "run2_65_full_renderer_composition_repair") throw new Error("Run 2.70 must compare against Run 2.65 full trace");
  assertSelectedUsecase(data.run269Result?.selected_usecase_id, "run2_69_result");
  assertSelectedUsecase(data.run269FullTrace?.selected_usecase_id, "run2_69_full_trace");
  ...
  if (grammar.length !== 6 || !grammar.every((record) => roles.has(record.role))) throw new Error("Run 2.70 requires six Run 2.66 role grammar records");
  if (art.length !== 6 || !art.every((record) => roles.has(record.role))) throw new Error("Run 2.70 requires six Run 2.66 art direction records");
  const artByRole = byRole(art);
  for (const record of grammar) {
    const role = record.role;
    const contract = artByRole[role];
    if (!contract) throw new Error(`Run 2.70 missing Run 2.66 art direction for ${role}`);
    if (contract.required_reference_archetype_id !== record.reference_archetype_id) throw new Error(`Run 2.70 Run 2.66 grammar/art mismatch for ${role}`);
    if ((record.scene_specific_business_objects ?? []).length < 3) throw new Error(`Run 2.70 needs scene objects for ${role}`);
    if (record.composition_contract?.max_visible_trace_terms !== 0) throw new Error(`Run 2.70 requires public trace term budget zero for ${role}`);
    for (const field of requiredRun266TraceFields.slice(0, 5)) {
      if (!(record.required_trace_fields_for_run2_67 ?? []).includes(field)) throw new Error(`Run 2.70 missing required trace field ${field}`);
    }
  }
}

function loadRun270ReferenceData(arm) {
  const data = {
    run269Result: readRun270PackJsonForArm(arm, RUN2_70_INPUTS.run269Result),
    run269FullTrace: readRun270PackJsonForArm(arm, RUN2_70_INPUTS.run269FullTrace),
    run266Result: readRun270PackJsonForArm(arm, RUN2_70_INPUTS.run266Result),
    run266FailureAudit: readRun270PackJsonForArm(arm, RUN2_70_INPUTS.run266FailureAudit),
    run266DesignGrammar: readRun270PackJsonForArm(arm, RUN2_70_INPUTS.run266DesignGrammar),
    run266ArtDirection: readRun270PackJsonForArm(arm, RUN2_70_INPUTS.run266ArtDirection),
    run266WorkflowGates: readRun270PackJsonForArm(arm, RUN2_70_INPUTS.run266WorkflowGates),
    run265Result: readRun270PackJsonForArm(arm, RUN2_70_INPUTS.run265Result),
    run265FullTrace: readRun270PackJsonForArm(arm, RUN2_70_INPUTS.run265FullTrace),
    commercialUsecaseBank: readRun270PackJsonForArm(arm, RUN2_70_INPUTS.commercialUsecaseBank),
    sources: readRun270PackJsonForArm(arm, RUN2_70_INPUTS.sources),
  };
  validateRun270ReferenceInputs(data);
  return {
    ...data,
    ...
    status: "run2_70_reference_inputs_ready",
  };
}

function loadRun270BadControlData(arm) {
  const data = {
    run269Result: readRun270PackJsonForArm(arm, RUN2_70_INPUTS.run269Result),
    run269FullTrace: readRun270PackJsonForArm(arm, RUN2_70_INPUTS.run269FullTrace),
    commercialUsecaseBank: readRun270PackJsonForArm(arm, RUN2_70_INPUTS.commercialUsecaseBank),
  ...
  if (data.run269Result?.status !== "run2_69_public_content_fill_rerun_public_blocked") throw new Error("Run 2.70 bad control must read Run 2.69 result");
  if (data.run269FullTrace?.arm_id !== "run2_69_full_public_content_fill") throw new Error("Run 2.70 bad control must read Run 2.69 full trace");
  return { ...data, usecase: data.commercialUsecaseBank.usecases.find((item) => item.id === selectedUsecaseId) };
}

function selectRun270ForSlide(role, data) {
  const grammar = data.grammarByRole[role];
  const art = data.artByRole[role];
  const priorTraceSlide = (data.run269FullTrace.slides ?? []).find((item) => item.role === role);
  if (!grammar || !art || !priorTraceSlide) throw new Error(`Run 2.70 missing role selection for ${role}`);
  return { role, grammar, art, priorTraceSlide, usecase: data.usecase };
}

const ROLE_RENDERERS = {
  cover: "drawRun270ReferenceFirstCinematicDeckReveal",
  setup: "fillRun270SetupOperatingStageSlots",
  contrast: "drawRun270ReferenceFirstBeforeAfterTheater",
  proof: "fillRun270InspectableWorkspaceSlots",
  climax: "drawRun270ReferenceFirstHeroProductSurfaceDemo",
  close: "fillRun270ReleaseDecisionBoardSlots",
};
...
function drawRun270ProductSceneMock(slide, metrics, x, y, w, h) {
...
function fillRun270InspectableGeneratedSlideMock(slide, metrics, x, y, w, h) {
...
function fillRun270EditablePresentationSurfaceMock(slide, metrics, x, y, w, h) {
...
function drawRun270ReferenceFirstCinematicDeckReveal(slide, arm, spec, selection, metrics) {
...
function fillRun270SetupOperatingStageSlots(slide, arm, spec, selection, metrics) {
...
function drawRun270ReferenceFirstBeforeAfterTheater(slide, arm, spec, selection, metrics) {
  ...
  drawRun270ProductSceneMock(slide, metrics, 700, 158, 430, 318);
...
function fillRun270InspectableWorkspaceSlots(slide, arm, spec, selection, metrics) {
  ...
  fillRun270InspectableGeneratedSlideMock(slide, metrics, 788, 332, 208, 150);
...
function drawRun270ReferenceFirstHeroProductSurfaceDemo(slide, arm, spec, selection, metrics) {
  ...
  fillRun270EditablePresentationSurfaceMock(slide, metrics, 558, 118, 520, 368);
...
function fillRun270ReleaseDecisionBoardSlots(slide, arm, spec, selection, metrics) {
  ...
  const rows = [
    ...
    ["content slots", C.amber, "2.70 filled"],
...
function drawRun270HighFidelityProductMockSurface(slide, arm, spec, selection, metrics) {
  const renderer = RUN2_70_RENDERERS[spec.role];
  if (!renderer) throw new Error(`Run 2.70 missing role renderer for ${spec.role}`);
  ...
  metrics.codeModuleIds.add("drawRun270HighFidelityProductMockSurface");
...
function drawBadRun270FallbackSlide(slide, arm, spec, badData, metrics) {
  const prior = (badData.run269FullTrace?.slides ?? []).find((slideItem) => slideItem.role === spec.role);
  socketText(slide, metrics, spec.title, 76, 116, 560, 70, { fontSize: 28, title: true, bold: true, color: arm.palette.title, max: 88 });
  socketText(slide, metrics, "This arm can see Run 2.69 public content slots, but it cannot replace abstract wireframes with high-fidelity product mock content.", 80, 204, 510, 58, { fontSize: 13, color: arm.palette.muted, max: 136 });
  rect(slide, 642, 126, 448, 318, arm.palette.panel, colorLine(arm.palette.rule, 1));
  socketText(slide, metrics, "Run 2.69 abstract mock gap", 674, 164, 260, 22, { fontSize: 15, title: true, bold: true, color: arm.palette.title, max: 46 });
  ...
  metrics.productSystemSurfaceKind = "run2_69_without_run2_70_high_fidelity_mock_content";
  if (MOCK_TARGET_ROLES.includes(spec.role)) {
    metrics.highFidelityMockContentStatus = RUN2_70_BAD_STATUS;
    metrics.productMockSurfaceKind = "missing_high_fidelity_mock_content";
  }
  metrics.boxFillStatus = RUN2_70_BAD_STATUS;
...
function renderRun270Slide(presentation, spec, arm, n, referenceData, badData, metricsByRole) {
  const slide = presentation.slides.add();
  base(slide, arm, n);
  const metrics = createSlideMetrics(spec.role);
  if (armUsesFullRun270Grammar(arm)) {
    const selection = selectRun270ForSlide(spec.role, referenceData);
    drawRun270HighFidelityProductMockSurface(slide, arm, spec, selection, metrics);
  } else if (armUsesBadRun270Data(arm)) {
    drawBadRun270FallbackSlide(slide, arm, spec, badData, metrics);
  } else {
...
function traceFor(arm, context = {}) {
  assertRun270ArmInputBoundaries(arm);
  const fullRun270 = armUsesFullRun270Grammar(arm);
  const badRun270 = armUsesBadRun270Data(arm);
  const referenceData = fullRun270 ? context.referenceData ?? loadRun270ReferenceData(arm) : null;
  const badData = badRun270 ? context.badData ?? loadRun270BadControlData(arm) : null;
  const metricsByRole = context.metricsByRole ?? new Map();
  return {
    schema_version: 1,
    arm_id: arm.armId,
    render_style_arm_id: arm.armId,
    selected_usecase_id: fullRun270 || badRun270 ? selectedUsecaseId : "",
    source_design_run_id: fullRun270 ? "2.66" : "",
    source_generated_run_id: fullRun270 || badRun270 ? "2.69" : "",
    run2_70_reference_first_consumption_status: fullRun270 ? RUN2_70_FULL_STATUS : badRun270 ? RUN2_70_BAD_STATUS : "boundary_control_no_run2_66_reference_first_grammar",
    run2_70_high_fidelity_mock_content_consumption_status: fullRun270 ? RUN2_70_FULL_STATUS : badRun270 ? RUN2_70_BAD_STATUS : "boundary_control_no_run2_70_high_fidelity_mock_content",
    ...
    runtime_isolation: {
      ...
      prompt_context: "fresh Run 2.70 high-fidelity mock content rerun from scripts/generate_ppt_run2_70_high_fidelity_mock_content_arms.mjs",
    ...
    slides: arm.slides.map((slide, index) => {
      ...
      const selection = fullRun270 ? selectRun270ForSlide(slide.role, referenceData) : null;
      const run266Fields = fullRun270
        ? {
        ...
        : badRun270
          ? blankRun266Fields(RUN2_70_BAD_STATUS)
          : blankRun266Fields();
      return {
        slide_id: `slide_${String(index + 1).padStart(2, "0")}`,
        role: slide.role,
        title: slide.title,
        source_run2_69_slide_id: fullRun270 ? selection.priorTraceSlide.slide_id : badRun270 ? slide.role : "",
        ...run266Fields,
        run2_70_reference_first_code_module_id: fullRun270 ? ROLE_RENDERERS[slide.role] : "",
        run2_70_code_module_ids: fullRun270 ? Array.from(roleMetrics.codeModuleIds) : [],
        run2_70_reference_first_consumption_status: fullRun270 ? "pass_internal" : badRun270 ? RUN2_70_BAD_STATUS : "",
        run2_70_public_content_fill_status: fullRun270 ? "pass_internal" : badRun270 ? RUN2_70_BAD_STATUS : "",
        run2_70_high_fidelity_mock_content_status: fullRun270
          ? roleMetrics.highFidelityMockContentStatus
          : badRun270 && MOCK_TARGET_ROLES.includes(slide.role)
            ? RUN2_70_BAD_STATUS
            : "not_targeted",
        run2_70_empty_wireframe_replaced: fullRun270 ? roleMetrics.emptyWireframeReplaced : false,
        run2_70_realistic_mock_object_count: fullRun270 ? roleMetrics.realisticMockObjectCount : 0,
        run2_70_product_mock_surface_kind: fullRun270 ? roleMetrics.productMockSurfaceKind : "",
        run2_70_box_fill_status: fullRun270 ? roleMetrics.boxFillStatus : badRun270 ? RUN2_70_BAD_STATUS : "",
        run2_70_internal_debug_terms_visible: fullRun270 ? roleMetrics.internalDebugTermsVisible : 0,
        run2_70_public_content_slot_count: fullRun270 ? roleMetrics.publicContentSlotCount : 0,
        run2_70_visual_content_slot_texts: fullRun270 ? roleMetrics.visualContentSlotTexts : [],
        run2_70_no_visible_debug_terms: fullRun270 ? roleMetrics.internalDebugTermsVisible === 0 : false,
        no_visible_debug_terms: fullRun270 ? roleMetrics.internalDebugTermsVisible === 0 : false,
        run2_70_scene_specific_business_object_count: fullRun270 ? Math.max(3, roleMetrics.sceneSpecificBusinessObjectCount) : roleMetrics.sceneSpecificBusinessObjectCount,
        run2_70_depth_layer_count: fullRun270 ? Math.max(4, roleMetrics.depthLayerCount) : roleMetrics.depthLayerCount,
        run2_70_visible_trace_terms: fullRun270 ? 0 : roleMetrics.visibleTraceTerms,
        layout_metrics: {
...
function assertRun270ReferenceSelfCheck(trace) {
  if (trace.arm_id === "run2_70_full_high_fidelity_mock_content") {
    if (trace.run2_70_high_fidelity_mock_content_consumption_status !== RUN2_70_FULL_STATUS) throw new Error("Run 2.70 full trace did not consume high-fidelity mock content before native PPT drawing");
    for (const slide of trace.slides) {
      for (const field of requiredRun266TraceFields) {
        if (!Object.prototype.hasOwnProperty.call(slide, field)) throw new Error(`Run 2.70 full slide ${slide.slide_id} missing ${field}`);
      }
      if (!String(slide.run2_66_reference_archetype_id ?? "").startsWith("reference_first_archetype_2_66_")) throw new Error(`Run 2.70 full slide ${slide.slide_id} missing reference archetype`);
      if (!String(slide.run2_66_art_direction_contract_id ?? "").startsWith("art_direction_contract_2_66_")) throw new Error(`Run 2.70 full slide ${slide.slide_id} missing art direction contract`);
      if (slide.run2_66_public_surface_aesthetic_gate_status !== "pass_internal") throw new Error(`Run 2.70 full slide ${slide.slide_id} missing public-surface pass`);
      if ((slide.run2_70_scene_specific_business_object_count ?? 0) < 3) throw new Error(`Run 2.70 full slide ${slide.slide_id} missing scene objects`);
      if ((slide.run2_70_depth_layer_count ?? 0) < 4) throw new Error(`Run 2.70 full slide ${slide.slide_id} missing depth layers`);
      if ((slide.run2_70_visible_trace_terms ?? 1) !== 0) throw new Error(`Run 2.70 full slide ${slide.slide_id} leaked public trace terms`);
      if ((slide.layout_metrics?.visible_words ?? 0) < 42 || (slide.layout_metrics?.visible_words ?? 999) > 170) throw new Error(`Run 2.70 full slide ${slide.slide_id} outside public visible-word range`);
      if ((slide.layout_metrics?.text_overflow_risk_count ?? 1) !== 0) throw new Error(`Run 2.70 full slide ${slide.slide_id} has overflow risk`);
      if (slide.run2_70_public_content_fill_status !== "pass_internal") throw new Error(`Run 2.70 full slide ${slide.slide_id} did not pass public content fill`);
      if (slide.run2_70_box_fill_status !== "pass_internal") throw new Error(`Run 2.70 full slide ${slide.slide_id} did not fill visual boxes`);
      if ((slide.run2_70_internal_debug_terms_visible ?? 1) !== 0) throw new Error(`Run 2.70 full slide ${slide.slide_id} leaked debug terms`);
      if ((slide.run2_70_public_content_slot_count ?? 0) < 5) throw new Error(`Run 2.70 full slide ${slide.slide_id} has too few public content slots`);
      if (MOCK_TARGET_ROLES.includes(slide.role)) {
        if (slide.run2_70_high_fidelity_mock_content_status !== "pass_internal") throw new Error(`Run 2.70 full slide ${slide.slide_id} did not pass high-fidelity mock content`);
        if (slide.run2_70_empty_wireframe_replaced !== true) throw new Error(`Run 2.70 full slide ${slide.slide_id} did not replace empty wireframe`);
        if ((slide.run2_70_realistic_mock_object_count ?? 0) < 4) throw new Error(`Run 2.70 full slide ${slide.slide_id} has too few realistic mock objects`);
      }
    }
  }
  if (trace.arm_id === "bad_run2_69_without_high_fidelity_mock_content") {
    for (const slide of trace.slides) {
      if (MOCK_TARGET_ROLES.includes(slide.role) && slide.run2_70_high_fidelity_mock_content_status !== RUN2_70_BAD_STATUS) throw new Error(`Run 2.70 bad slide ${slide.slide_id} must fail missing high-fidelity mock content`);
    }
  }
}
...
function buildRun270FourArmSheet(built) {
  ...
  return buildNamedContactSheet(path.join(outRoot, "run2-70-four-arm-contact-sheet.png"), "Run 2.70 high-fidelity mock content comparison", sheets, sheets.length, labels);
}
...
async function buildArm(arm) {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "required proof objects: Run 2.69 trace, Run 2.66 reference archetype, public first-read object, layout archetype, art direction contract, and high-fidelity mock content status",
      "negative control: bad arm can read Run 2.69, but cannot use Run 2.70 high-fidelity mock content",
      "profile-specific QA gates: target slides must replace abstract wireframes with realistic editable product mock objects",
      "",
    ].join("\n"),
  );
  fs.writeFileSync(
  ...
  const referenceData = armUsesFullRun270Grammar(arm) ? loadRun270ReferenceData(arm) : null;
  const badData = armUsesBadRun270Data(arm) ? loadRun270BadControlData(arm) : null;
  const slides = arm.slides.map((slide, index) => renderRun270Slide(presentation, slide, arm, index + 1, referenceData, badData, metricsByRole));
  ...
  assertRun270ReferenceSelfCheck(trace);
  ...
  const manifest = {
    ...
    layoutResults,
    contactSheet,
    slides: slides.map((_, index) => ({ index: index + 1, requestedSlideNumber: index + 1, source: "scripts/generate_ppt_run2_70_high_fidelity_mock_content_arms.mjs", exportName: `slide${String(index + 1).padStart(2, "0")}` })),
  };
...
function writeRun270Result(runSummary) {
  const fullTrace = readJson(`outputs/${threadId}/presentations/ppt-run2-70-full-vulca/trace_manifest.json`);
  const badTrace = readJson(`outputs/${threadId}/presentations/ppt-run2-70-bad-without-high-fidelity-mock-content/trace_manifest.json`);
  const result = {
    schema_version: 1,
    run_id: RUN_ID,
    status: "run2_70_high_fidelity_mock_content_rerun_public_blocked",
    public_ready: false,
    selected_usecase_id: selectedUsecaseId,
    source_design_run_id: "2.66",
    source_generated_run_id: "2.69",
    database_expansion: false,
    workflow_expansion: true,
    stage_policy: "repeat_same_five_layers_not_run3",
    mock_content_scope: {
      source_run_id: "2.69",
      target_layer: RUN2_70_POLICY,
      target_roles: MOCK_TARGET_ROLES,
      required_mock_surfaces: REQUIRED_MOCK_SURFACES,
      visual_policy: "replace abstract wireframes with realistic native-shape product mock content",
    },
    input_chain: {
      run2_69_result: RUN2_70_INPUTS.run269Result,
      run2_69_full_trace: RUN2_70_INPUTS.run269FullTrace,
      run2_66_result: RUN2_70_INPUTS.run266Result,
      run2_66_failure_audit: RUN2_70_INPUTS.run266FailureAudit,
      run2_66_design_grammar: RUN2_70_INPUTS.run266DesignGrammar,
      run2_66_art_direction: RUN2_70_INPUTS.run266ArtDirection,
      run2_66_workflow_gates: RUN2_70_INPUTS.run266WorkflowGates,
      run2_65_result: RUN2_70_INPUTS.run265Result,
      run2_65_full_trace: RUN2_70_INPUTS.run265FullTrace,
      commercial_usecase_bank: RUN2_70_INPUTS.commercialUsecaseBank,
      sources: RUN2_70_INPUTS.sources,
    },
    rerun: {
      generator: "scripts/generate_ppt_run2_70_high_fidelity_mock_content_arms.mjs",
      arms: armSpecs.map((arm) => arm.armId),
      best_internal_arm: "run2_70_full_high_fidelity_mock_content",
      best_internal_arm_verdict: "Run 2.69 public content slots are consumed, then the weakest product-scene pages receive richer editable mock objects before native PPT drawing.",
    ...
    quality_delta: {
      target_layer: RUN2_70_POLICY,
      source_data_status: RUN2_70_FULL_STATUS,
      ...
      full_slides_with_visible_trace_terms_removed: fullTrace.slides.filter((slide) => (slide.run2_70_visible_trace_terms ?? 1) === 0).length,
      full_slides_with_scene_business_objects: fullTrace.slides.filter((slide) => (slide.run2_70_scene_specific_business_object_count ?? 0) >= 3).length,
      full_slides_with_depth_layers: fullTrace.slides.filter((slide) => (slide.run2_70_depth_layer_count ?? 0) >= 4).length,
      full_slides_with_public_content_slots: fullTrace.slides.filter((slide) => (slide.run2_70_public_content_slot_count ?? 0) >= 5).length,
      full_slides_without_visible_debug_terms: fullTrace.slides.filter((slide) => (slide.run2_70_internal_debug_terms_visible ?? 1) === 0).length,
      full_slides_with_visual_slot_labels: fullTrace.slides.filter((slide) => (slide.run2_70_visual_content_slot_texts ?? []).length >= 5).length,
      full_targeted_slides_with_high_fidelity_mock_content: fullTrace.slides.filter((slide) => MOCK_TARGET_ROLES.includes(slide.role) && slide.run2_70_high_fidelity_mock_content_status === "pass_internal").length,
      full_targeted_slides_without_empty_wireframes: fullTrace.slides.filter((slide) => MOCK_TARGET_ROLES.includes(slide.role) && slide.run2_70_empty_wireframe_replaced === true).length,
      full_targeted_slides_with_realistic_mock_objects: fullTrace.slides.filter((slide) => MOCK_TARGET_ROLES.includes(slide.role) && (slide.run2_70_realistic_mock_object_count ?? 0) >= 4).length,
      bad_control_targeted_slides_without_high_fidelity_mock_content: badTrace.slides.filter((slide) => MOCK_TARGET_ROLES.includes(slide.role) && slide.run2_70_high_fidelity_mock_content_status === RUN2_70_BAD_STATUS).length,
      repair_modules: Object.values(ROLE_RENDERERS),
    },
    control_boundary: {
      bad_run2_69_without_high_fidelity_mock_content: "may see Run 2.69 generated result and trace, but must not use the Run 2.70 high-fidelity mock content layer",
      prompt_only: "commercial_case_only_no_run2_69_or_run2_70_high_fidelity_mock_content",
      run1_5_skill: "prior_baseline_no_run2_69_or_run2_70_high_fidelity_mock_content",
    },
    visual_quality_boundary: "Run 2.70 is an internal generated rerun for visual review; public release remains blocked until human review approves the rendered deck.",
    ...
    next_required_action: "human_review_run2_70_high_fidelity_mock_content_then_decide_whether_product_mock_surfaces_are_real_enough_for_public_video",
  };
  const resultJson = path.join(root, pack, "results", "run2_70_high_fidelity_mock_content_rerun_result.json");
  const resultMd = path.join(root, pack, "results", "run2_70_high_fidelity_mock_content_rerun_result.md");
...
async function main() {
  ...
  const fourArmSheet = buildRun270FourArmSheet(built);
  const fullSkillSeriesSheet = buildFullSkillSeriesSheet();
  const runSummary = {
    run_id: "run2_70_high_fidelity_mock_content_four_arms",
    selected_usecase_id: selectedUsecaseId,
    arms: armSpecs.map((arm) => arm.armId),
  ...
  writeJson(path.join(outRoot, "run2_70_high_fidelity_mock_content_rerun_summary.json"), runSummary);
  writeRun270Result(runSummary);
  return runSummary;
}
...
export {
  RUN2_70_FULL_DATA_INPUTS,
  RUN2_70_INPUTS,
  armSpecs,
  assertRun270ArmInputBoundaries,
  assertRun270ReferenceSelfCheck,
  drawRun270HighFidelityProductMockSurface,
  loadRun270ReferenceData,
  main,
};
