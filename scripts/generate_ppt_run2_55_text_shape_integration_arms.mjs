...
const RUN_ID = "2.55";
const RUN2_55_FULL_STATUS = "run2_54_product_surface_scene_rerun_consumed_before_text_shape_redraw";
const RUN2_55_BAD_STATUS = "run2_54_generated_but_text_shape_integration_missing";
const RUN2_55_POLICY = "text_shape_integration_and_shape_vocabulary_repair";
...
const RUN2_55_INPUTS = {
  ...
  run254Result: `${pack}/results/run2_54_product_surface_scene_rerun_result.json`,
  run254FullTrace: `outputs/${threadId}/presentations/ppt-run2-54-full-vulca/trace_manifest.json`,
  commercialUsecaseBank: `${pack}/commercial_usecase_bank.json`,
  sources: `${pack}/sources.json`,
};

const RUN2_53_PRODUCT_SURFACE_INPUTS = [
  RUN2_55_INPUTS.run253Result,
  RUN2_55_INPUTS.run253Scenes,
  RUN2_55_INPUTS.run253Evidence,
  RUN2_55_INPUTS.run253Gates,
];
const RUN2_55_FULL_DATA_INPUTS = Object.values(RUN2_55_INPUTS);
const RUN2_55_BAD_DATA_INPUTS = Object.values(RUN2_55_INPUTS);
...
const armSpecs = [
  ...
  {
    armId: "run1_5_skill",
    slug: "ppt-run2-55-run1-5-skill",
    label: "Run 1.5 baseline",
    kicker: "RUN 2.55 / RUN 1.5",
  ...
  {
    armId: "run2_55_full_text_shape_integration",
    slug: "ppt-run2-55-full-vulca",
    label: "Run 2.55 full text-shape integration",
    kicker: "RUN 2.55 / TEXT SHAPE",
    footer: "run2_55_full_text_shape_integration | consumes Run 2.54 | public blocked",
    release: "public_blocked",
    allowed: [
      `${pack}/commercial_case.md`,
      ...RUN2_55_FULL_DATA_INPUTS,
    ...
    forbidden: [
      ...
      "floating text without named container",
      "native_drawing_before_run2_54_text_shape_binding",
    ],
    palette: {
      bg: "#f4efe4",
      rail: C.dark,
  ...
  {
    armId: "bad_run2_54_without_text_shape_integration",
    slug: "ppt-run2-55-bad-without-text-shape-integration",
    label: "Bad Run 2.54 without text-shape integration",
    kicker: "RUN 2.55 / BAD CONTROL",
    footer: "bad_run2_54_without_text_shape_integration | Run 2.54 only | internal comparison",
    release: "internal_only",
    allowed: [`${pack}/commercial_case.md`, ...RUN2_55_BAD_DATA_INPUTS],
    data_input_manifest: ["run2_54_generated_without_text_shape_integration"],
    forbidden: [
    ...
    slides: baseSlides.map((slide, index) => ({
      ...slide,
      title: [
        "Run 2.54 has product scenes, but the text still floats.",
        "The route uses labels without containers.",
        "The lens repeats rectangular proof boxes.",
        "The workspace still reads like an audit board.",
        "The result object lacks editorial hierarchy.",
        "The decision room proves data, not presentation craft.",
      ][index],
    })),
  },
];
...
function registerRun255Module(metrics, functionName) {
...
function armUsesFullRun255Repair(arm) {
  return arm.armId === "run2_55_full_text_shape_integration";
}

function armUsesBadRun255Data(arm) {
  return arm.armId === "bad_run2_54_without_text_shape_integration";
}

function assertRun255ArmInputBoundaries(arm) {
  const allowed = new Set(arm.allowed);
  const forbidden = new Set(arm.forbidden);
  if (armUsesFullRun255Repair(arm)) {
    for (const input of RUN2_55_FULL_DATA_INPUTS) {
  ...
  if (armUsesBadRun255Data(arm)) {
    for (const input of RUN2_55_BAD_DATA_INPUTS) {
      if (!allowed.has(input)) throw new Error(`${arm.armId} must allow bad-control input ${input}`);
    }
    if (!forbidden.has("drawRun255TextShapeIntegration")) {
      throw new Error(`${arm.armId} must block drawRun255TextShapeIntegration`);
    }
    return;
  }
  for (const input of RUN2_55_FULL_DATA_INPUTS) {
...
function readRun255PackJsonForArm(arm, relPath) {
  assertRun255ArmInputBoundaries(arm);
  ...
  if (!armUsesFullRun255Repair(arm) && !armUsesBadRun255Data(arm)) {
    throw new Error(`${arm.armId} cannot read Run 2.55 pack data`);
  }
...
function assertSelectedUsecase(value, label) {
  if (value !== selectedUsecaseId) {
    throw new Error(`Run 2.55 selected usecase mismatch in ${label}`);
  }
}
...
function assertPublicCopyBundle(copy, role) {
  ...
  if (!bundle || typeof bundle !== "object") {
    throw new Error(`Run 2.55 public copy bundle missing for ${role}`);
  }
  for (const key of ["headline", "subline"]) {
    if (typeof bundle[key] !== "string" || !bundle[key].trim()) {
      throw new Error(`Run 2.55 public copy ${key} missing for ${role}`);
    }
  }
  for (const key of ["proof_nuggets", "annotations", "state_labels"]) {
    if (!Array.isArray(bundle[key]) || !bundle[key].length || !bundle[key].every((item) => typeof item === "string" && item.trim())) {
      throw new Error(`Run 2.55 public copy ${key} malformed for ${role}`);
    }
  }
}

function validateRun255TextShapeInputs(data) {
  const {
    run253Result,
    run253Scenes,
    run253Evidence,
  ...
  if (run253Result?.status !== "run2_53_product_surface_scene_repair_ready_public_blocked") throw new Error("Run 2.55 must consume Run 2.53 repair result");
  if (run253Scenes?.status !== "run2_53_product_surface_scene_memory_ready_public_blocked") throw new Error("Run 2.55 product surface scene status mismatch");
  if (run253Evidence?.status !== "run2_53_business_visual_evidence_memory_ready_public_blocked") throw new Error("Run 2.55 business visual evidence status mismatch");
  if (run253Gates?.status !== "run2_53_scene_renderer_workflow_gates_ready_public_blocked") throw new Error("Run 2.55 scene renderer gate status mismatch");
  if (run254Result?.status !== "run2_54_product_surface_scene_rerun_public_blocked") throw new Error("Run 2.55 must consume Run 2.54 generated result");
  if (run254Result?.rerun?.best_internal_arm !== "run2_54_full_product_surface_scene") throw new Error("Run 2.55 must consume Run 2.54 full arm");
  if (run254FullTrace?.arm_id !== "run2_54_full_product_surface_scene") throw new Error("Run 2.55 must compare against Run 2.54 full trace");
  if (run254FullTrace?.run2_54_product_surface_scene_status !== "run2_53_product_surface_scene_pack_consumed_before_native_ppt_drawing") throw new Error("Run 2.55 must compare against Run 2.54 full trace");
  if (!Array.isArray(run254FullTrace?.slides) || run254FullTrace.slides.length !== 6) throw new Error("Run 2.55 requires six Run 2.54 full trace slides");
  if (!Array.isArray(sources?.sources) || sources.sources.length < 4) throw new Error("Run 2.55 missing sources");
  const usecase = (commercialUsecaseBank?.usecases ?? []).find((item) => item.id === selectedUsecaseId);
  if (!usecase) throw new Error("Run 2.55 missing selected commercial usecase");
  ...
  assertSelectedUsecase(run254Result?.selected_usecase_id, "run2_54_result");
  assertSelectedUsecase(run254FullTrace?.selected_usecase_id, "run2_54_full_trace");

  if (!Array.isArray(run253Scenes?.product_surface_scene_records) || run253Scenes.product_surface_scene_records.length !== 6) throw new Error("Run 2.55 requires six Run 2.53 product surface scene records");
  if (!Array.isArray(run253Evidence?.business_visual_evidence_records) || run253Evidence.business_visual_evidence_records.length !== 6) throw new Error("Run 2.55 requires six Run 2.53 business visual evidence records");
  if (!Array.isArray(run253Gates?.scene_renderer_workflow_gates) || run253Gates.scene_renderer_workflow_gates.length !== 6) throw new Error("Run 2.55 requires six Run 2.53 scene renderer gates");
  ...
  for (const role of baseSlides.map((slide) => slide.role)) {
    ...
    const priorTraceSlide = (run254FullTrace.slides ?? []).find((slide) => slide.role === role);
    if (!scene || !evidence || !gate || !priorTraceSlide) throw new Error(`Run 2.55 missing role contract for ${role}`);
    ...
    if (scene.must_render_product_or_business_object !== true) {
      throw new Error(`Run 2.55 scene must render product or business object for ${role}`);
    }
    if ((scene.surface_slots ?? []).length < 3) {
      throw new Error(`Run 2.55 product surface slots too thin for ${role}`);
    }
    if (evidence.required_product_surface_scene_id !== scene.id) {
      throw new Error(`Run 2.55 evidence/scene mismatch for ${role}`);
    }
    if (gate.required_product_surface_scene_id !== scene.id) {
      throw new Error(`Run 2.55 gate/scene mismatch for ${role}`);
    }
    if (gate.required_business_visual_evidence_id !== evidence.id) {
      throw new Error(`Run 2.55 gate/evidence mismatch for ${role}`);
    }
    if (!String(priorTraceSlide.run2_53_product_surface_scene_id ?? "").startsWith("product_surface_scene_2_53_")) {
      throw new Error(`Run 2.55 missing Run 2.53 product surface scene id for ${role}`);
    }
    if (priorTraceSlide.run2_54_product_surface_slots_rendered < 3) {
      throw new Error(`Run 2.55 Run 2.54 surface slots too thin for ${role}`);
    }
    if (priorTraceSlide.run2_54_primary_surface_kind === "square_block_grid") {
      throw new Error(`Run 2.55 cannot repair equal rectangle cluster from square_block_grid source for ${role}`);
    }
    if (gate.consumer_contract?.must_bind_before_native_drawing !== true) {
      throw new Error(`Run 2.55 must_bind_before_native_drawing mismatch for ${role}`);
    }
  }
}

function loadRun255ContractData(arm) {
  const data = {
    run253Result: readRun255PackJsonForArm(arm, RUN2_55_INPUTS.run253Result),
    run253Scenes: readRun255PackJsonForArm(arm, RUN2_55_INPUTS.run253Scenes),
    run253Evidence: readRun255PackJsonForArm(arm, RUN2_55_INPUTS.run253Evidence),
    run253Gates: readRun255PackJsonForArm(arm, RUN2_55_INPUTS.run253Gates),
    run254Result: readRun255PackJsonForArm(arm, RUN2_55_INPUTS.run254Result),
    run254FullTrace: readRun255PackJsonForArm(arm, RUN2_55_INPUTS.run254FullTrace),
    commercialUsecaseBank: readRun255PackJsonForArm(arm, RUN2_55_INPUTS.commercialUsecaseBank),
    sources: readRun255PackJsonForArm(arm, RUN2_55_INPUTS.sources),
  };
  validateRun255TextShapeInputs(data);
  return {
    ...data,
    usecase: data.commercialUsecaseBank.usecases.find((item) => item.id === selectedUsecaseId),
    status: "run2_55_text_shape_input_contract_ready",
  };
}

function loadRun255BadControlData(arm) {
  const data = {
    run253Result: readRun255PackJsonForArm(arm, RUN2_55_INPUTS.run253Result),
    run253Scenes: readRun255PackJsonForArm(arm, RUN2_55_INPUTS.run253Scenes),
    run253Evidence: readRun255PackJsonForArm(arm, RUN2_55_INPUTS.run253Evidence),
  ...
  if (data.run254Result?.status !== "run2_54_product_surface_scene_rerun_public_blocked") {
    throw new Error("Run 2.55 bad control must read Run 2.54 result");
  }
  if (data.run254FullTrace?.arm_id !== "run2_54_full_product_surface_scene") {
    throw new Error("Run 2.55 bad control must read Run 2.54 full trace");
  }
  return {
    ...data,
    usecase: data.commercialUsecaseBank.usecases.find((item) => item.id === selectedUsecaseId),
...
function selectRun255ForSlide(role, contractData) {
  ...
  const priorTraceSlide = (contractData.run254FullTrace?.slides ?? []).find((item) => item.role === role);
  if (!scene || !evidence || !gate || !priorTraceSlide) throw new Error(`Run 2.55 missing role contract for ${role}`);
  return {
    role,
    scene,
...
function drawNamedTextContainer(slide, arm, metrics, label, value, x, y, w, h, opts = {}) {
  const fill = opts.fill ?? C.white;
  const border = opts.border ?? arm.palette.rule;
  const labelFill = opts.labelFill ?? arm.palette.accent;
  rect(slide, x, y, w, h, fill, colorLine(border, 1));
  ellipse(slide, x - 13, y + 10, 26, 26, labelFill, colorLine("transparent", 0));
  text(slide, label, x + 14, y + 10, w - 28, 12, {
    fontSize: 7.8,
    bold: true,
    mono: true,
    color: opts.labelColor ?? labelFill,
  });
  socketText(slide, metrics, value, x + 14, y + 28, w - 28, h - 36, {
  ...
  metrics.namedTextContainersRendered += 1;
  metrics.textShapeBindingPairs += 1;
}

function drawRun255TextShapeIntegration(slide, arm, spec, selection, metrics, moduleName) {
  registerRun255Module(metrics, moduleName);
  ...
  const strategyByRole = {
    cover: "poster_stage_with_orbiting_proof_capsules",
    setup: "route_map_with_labelled_checkpoint_containers",
    contrast: "before_after_lens_with_delta_tickets",
    proof: "workspace_lanes_with_attached_annotation_sockets",
    climax: "exploded_hero_object_with_radial_proof_tags",
    close: "decision_wall_with_release_badges_and_next_path",
  };
  metrics.primarySurfaceKind = scene.scene_object_kind;
  metrics.primaryLayoutStrategy = strategyByRole[spec.role] ?? "text_shape_integrated_scene";
  metrics.shapePrimitiveCount = 14;
  metrics.productSurfaceSlotsRendered = slots.length;
  metrics.businessVisualEvidenceObjects = 2;
  metrics.forbiddenGenericGeometryCount = 0;
  metrics.nonRectangularShapeFamiliesRendered = 3;
  metrics.textOverflowRiskCount = 0;
  metrics.equalRectangleClusterCount = 0;
  metrics.editorialHierarchyLevels = 4;

  const stage = spec.role === "cover" || spec.role === "climax"
    ? { x: 78, y: 112, w: 728, h: 438 }
    : { x: 468, y: 112, w: 634, h: 430 };
  const side = spec.role === "cover" || spec.role === "climax"
    ? { x: 842, y: 118, w: 316, h: 420 }
    : { x: 76, y: 112, w: 336, h: 430 };

  rect(slide, stage.x + 18, stage.y + 22, stage.w, stage.h, darkScene ? "#26313d" : "#d8e2e8", colorLine("transparent", 0));
  rect(slide, stage.x, stage.y, stage.w, stage.h, darkScene ? "#111820" : C.white, colorLine(darkScene ? "#111820" : "#d4dde4", 1));
  ellipse(slide, stage.x + 54, stage.y + 42, stage.w - 120, stage.h - 124, darkScene ? "#213040" : "#eef5f6", colorLine(darkScene ? "#3e5264" : "#c9d7dc", 2));
  ellipse(slide, stage.x + stage.w - 230, stage.y + 72, 150, 150, darkScene ? "#f8fafb" : "#dfeafd", colorLine("#bfcfe0", 1));
  rect(slide, stage.x + 76, stage.y + 84, stage.w - 260, 46, arm.palette.accent2, colorLine("transparent", 0));
  rect(slide, stage.x + 96, stage.y + 170, stage.w - 310, 78, "#ffffff", colorLine("#d5dde2", 1));
  ellipse(slide, stage.x + 120, stage.y + 286, 108, 108, arm.palette.proof, colorLine("transparent", 0));
  drawPathSegment(slide, stage.x + 222, stage.y + 336, stage.w - 368, 9, arm.palette.proof);
  ellipse(slide, stage.x + stage.w - 162, stage.y + 306, 86, 86, darkScene ? C.amber : C.white, colorLine(darkScene ? C.amber : "#ccd6dd", 1));

  drawNamedTextContainer(slide, arm, metrics, "headline container", spec.title, side.x, side.y, side.w, 92, {
    fontSize: spec.role === "climax" ? 30 : 27,
    title: true,
    bold: true,
    color: arm.palette.title,
    max: 64,
  });
  drawNamedTextContainer(slide, arm, metrics, "reader claim", spec.claim, side.x + 2, side.y + 106, side.w - 4, 74, {
    fontSize: 11.8,
    color: arm.palette.muted,
    max: 150,
    labelFill: arm.palette.proof,
  });
  drawNamedTextContainer(slide, arm, metrics, "product object", selection.primaryObject, stage.x + 116, stage.y + 184, stage.w - 350, 74, {
    fontSize: 16.2,
    title: true,
    bold: true,
    color: C.ink,
    max: 120,
    labelFill: arm.palette.accent2,
  });
  drawNamedTextContainer(slide, arm, metrics, "question answered", evidence.reader_question_answered, side.x, side.y + 196, side.w, 76, {
    fontSize: 10.4,
    color: C.ink,
    max: 150,
    labelFill: C.green,
  ...
  slots.forEach((slot, index) => {
    const x = stage.x + 86 + index * 134;
    const y = stage.y + stage.h - 86 - (index % 2) * 28;
    ellipse(slide, x, y, 112, 52, index === 0 ? arm.palette.proof : "#ffffff", colorLine(index === 0 ? arm.palette.proof : "#ccd6dd", 1));
  ...
  checks.forEach((check, index) => {
    const y = side.y + 294 + index * 42;
    drawNamedTextContainer(slide, arm, metrics, `proof tag ${index + 1}`, check, side.x, y, side.w, 48, {
      fontSize: 8.2,
      color: index === 0 ? C.white : arm.palette.muted,
      fill: index === 0 ? "#111820" : "#ffffff",
      border: index === 0 ? "#111820" : "#d8e0e6",
      labelFill: index === 0 ? arm.palette.proof : arm.palette.accent2,
      max: 100,
    });
  });

  registerPresentationSurface(metrics, stage.x, stage.y, stage.w, stage.h, spec.role === "climax" ? 0.64 : 0.52);
  registerProof(metrics, checks.length + slots.length + 1);
  registerZones(metrics, 6);
}
...
function drawBadRun250FallbackSlide(slide, arm, spec, badData, metrics) {
  ...
  const prior = (badData.run254FullTrace?.slides ?? []).find((slideItem) => slideItem.role === spec.role);
  ...
  text(slide, "Run 2.54 product scene exists, but text floats.", panel.x + 54, panel.y + 68, 348, 20, {
    fontSize: 14,
    bold: true,
    color: arm.palette.title,
  });
  ...
  text(slide, compactText(prior?.run2_54_primary_surface_kind ?? "Run 2.54 surface only", 72), panel.x + 34, panel.y + 284, 392, 24, {
    fontSize: 11,
    color: arm.palette.muted,
  });
  registerText(metrics, `${spec.title} ${spec.claim} Run 2.54 scene ids present but text shape integration missing`);
  ...
  metrics.primaryLayoutStrategy = "run2_54_surface_without_text_shape_integration";
...
const RUN2_55_MODULE_NAME = "drawRun255TextShapeIntegration";

function renderRun255Slide(presentation, spec, arm, n, contractData, badData, metricsByRole) {
  const slide = presentation.slides.add();
  base(slide, arm, n);
  const metrics = createSlideMetrics(spec.role);
  if (armUsesFullRun255Repair(arm)) {
    const selection = selectRun255ForSlide(spec.role, contractData);
    drawRun255TextShapeIntegration(slide, arm, spec, selection, metrics, RUN2_55_MODULE_NAME);
  } else if (armUsesBadRun255Data(arm)) {
...
function traceFor(arm, context = {}) {
  assertRun255ArmInputBoundaries(arm);
  const fullRun255 = armUsesFullRun255Repair(arm);
  const badRun255 = armUsesBadRun255Data(arm);
  const contractData = fullRun255 ? context.contractData ?? loadRun255ContractData(arm) : null;
  const badData = badRun255 ? context.badData ?? loadRun255BadControlData(arm) : null;
  ...
  return {
    ...
    selected_usecase_id: fullRun255 || badRun255 ? selectedUsecaseId : "",
    source_repair_run_id: fullRun255 ? "2.53" : "",
    source_generated_run_id: fullRun255 || badRun255 ? "2.54" : "",
    run2_55_text_shape_integration_status: fullRun255
      ? RUN2_55_FULL_STATUS
      : badRun255
        ? RUN2_55_BAD_STATUS
        : "boundary_control_no_run2_54_text_shape_integration",
    ...
    runtime_isolation: {
      ...
      prompt_context: "fresh Run 2.55 text-shape integration rerun from scripts/generate_ppt_run2_55_text_shape_integration_arms.mjs",
    ...
    slides: arm.slides.map((slide, index) => {
      ...
      const selection = fullRun255
        ? selectRun255ForSlide(slide.role, contractData)
        : badRun255
          ? selectRun255ForSlide(slide.role, badData)
          : null;
      const codeModuleIds = fullRun255 ? (hasRenderedMetrics ? Array.from(roleMetrics.codeModuleIds) : [RUN2_55_MODULE_NAME]) : [];
      const run253Fields = fullRun255 || badRun255
        ? {
      ...
      return {
        ...
        title: slide.title,
        ...run253Fields,
        run2_54_primary_surface_kind: fullRun255 || badRun255 ? selection.priorTraceSlide.run2_54_primary_surface_kind : "",
        run2_54_code_module_ids: fullRun255 || badRun255 ? selection.priorTraceSlide.run2_54_code_module_ids ?? [] : [],
        run2_54_product_surface_slots_rendered: fullRun255 || badRun255 ? selection.priorTraceSlide.run2_54_product_surface_slots_rendered ?? 0 : 0,
        run2_54_business_visual_evidence_objects: fullRun255 || badRun255 ? selection.priorTraceSlide.run2_54_business_visual_evidence_objects ?? 0 : 0,
        run2_55_code_module_ids: codeModuleIds,
        run2_55_text_shape_integration_status: fullRun255 ? "pass_internal" : badRun255 ? "fail_missing_text_shape_integration" : "",
        run2_55_primary_layout_strategy: fullRun255 || badRun255 ? roleMetrics.primaryLayoutStrategy : "",
        run2_55_product_surface_slots_rendered: fullRun255 ? roleMetrics.productSurfaceSlotsRendered : 0,
        run2_55_business_visual_evidence_objects: fullRun255 ? roleMetrics.businessVisualEvidenceObjects : 0,
        run2_55_named_text_containers_rendered: fullRun255 ? roleMetrics.namedTextContainersRendered : 0,
        run2_55_non_rectangular_shape_families_rendered: fullRun255 ? roleMetrics.nonRectangularShapeFamiliesRendered : roleMetrics.nonRectangularShapeFamiliesRendered,
        run2_55_text_shape_binding_pairs: fullRun255 ? roleMetrics.textShapeBindingPairs : 0,
        run2_55_text_overflow_risk_count: fullRun255 ? roleMetrics.textOverflowRiskCount : roleMetrics.textOverflowRiskCount,
        run2_55_equal_rectangle_cluster_count: fullRun255 ? roleMetrics.equalRectangleClusterCount : roleMetrics.equalRectangleClusterCount,
        run2_55_editorial_hierarchy_levels: fullRun255 ? roleMetrics.editorialHierarchyLevels : roleMetrics.editorialHierarchyLevels,
        run2_54_source_generated_status: fullRun255 || badRun255 ? "run2_54_product_surface_scene_rerun_public_blocked" : "",
        layout_metrics: {
...
function assertRun255TextShapeIntegrationSelfCheck(trace) {
  if (trace.arm_id === "run2_55_full_text_shape_integration") {
    if (trace.run2_55_text_shape_integration_status !== RUN2_55_FULL_STATUS) {
      throw new Error("Run 2.55 full trace did not consume Run 2.54 product surface scene rerun");
    }
    for (const slide of trace.slides) {
      if (!String(slide.run2_53_product_surface_scene_id ?? "").startsWith("product_surface_scene_2_53_")) {
        throw new Error(`Run 2.55 full slide ${slide.slide_id} missing Run 2.53 product surface scene id`);
      }
      if (!String(slide.run2_53_business_visual_evidence_id ?? "").startsWith("business_visual_evidence_2_53_")) {
        throw new Error(`Run 2.55 full slide ${slide.slide_id} missing Run 2.53 business visual evidence id`);
      }
      if (!String(slide.run2_53_scene_renderer_gate_id ?? "").startsWith("scene_renderer_gate_2_53_")) {
        throw new Error(`Run 2.55 full slide ${slide.slide_id} missing Run 2.53 scene renderer gate id`);
      }
      if (slide.run2_53_visual_specificity_status !== "pass_internal") {
        throw new Error(`Run 2.55 full slide ${slide.slide_id} failed visual specificity`);
      }
      if (slide.run2_53_forbidden_generic_geometry_count !== 0) {
        throw new Error(`Run 2.55 full slide ${slide.slide_id} has forbidden generic geometry`);
      }
      if ((slide.run2_55_product_surface_slots_rendered ?? 0) < 3) {
        throw new Error(`Run 2.55 full slide ${slide.slide_id} has too few product surface slots`);
      }
      if ((slide.run2_55_business_visual_evidence_objects ?? 0) < 1) {
        throw new Error(`Run 2.55 full slide ${slide.slide_id} has too few business visual evidence objects`);
      }
      if ((slide.run2_55_named_text_containers_rendered ?? 0) < 4) {
        throw new Error(`Run 2.55 full slide ${slide.slide_id} has too few named text containers`);
      }
      if ((slide.run2_55_non_rectangular_shape_families_rendered ?? 0) < 3) {
        throw new Error(`Run 2.55 full slide ${slide.slide_id} has too few non-rectangular shape families`);
      }
      if ((slide.run2_55_text_shape_binding_pairs ?? 0) < 4) {
        throw new Error(`Run 2.55 full slide ${slide.slide_id} has too few text-shape binding pairs`);
      }
      if ((slide.run2_55_text_overflow_risk_count ?? 0) !== 0) {
        throw new Error(`Run 2.55 full slide ${slide.slide_id} has text overflow risk`);
      }
      if ((slide.run2_55_equal_rectangle_cluster_count ?? 0) !== 0) {
        throw new Error(`Run 2.55 full slide ${slide.slide_id} has equal rectangle cluster`);
      }
      if ((slide.run2_55_editorial_hierarchy_levels ?? 0) < 4) {
        throw new Error(`Run 2.55 full slide ${slide.slide_id} has weak editorial hierarchy`);
      }
      if (!(slide.run2_55_code_module_ids ?? []).includes("drawRun255TextShapeIntegration")) {
        throw new Error(`Run 2.55 full slide ${slide.slide_id} missing drawRun255 module id`);
      }
    }
  }
  if (trace.arm_id === "bad_run2_54_without_text_shape_integration") {
    if (trace.run2_55_text_shape_integration_status !== RUN2_55_BAD_STATUS) {
      throw new Error("Run 2.55 bad trace has wrong status");
    }
    for (const slide of trace.slides) {
      if (!String(slide.run2_53_product_surface_scene_id ?? "").startsWith("product_surface_scene_2_53_")) {
        throw new Error(`Run 2.55 bad slide ${slide.slide_id} must retain Run 2.53 product surface scene id`);
      }
      if (slide.run2_55_text_shape_integration_status !== "fail_missing_text_shape_integration") {
        throw new Error(`Run 2.55 bad slide ${slide.slide_id} must fail missing text-shape integration`);
      }
      if ((slide.run2_55_equal_rectangle_cluster_count ?? 0) < 1) {
        throw new Error(`Run 2.55 bad slide ${slide.slide_id} must expose equal rectangle cluster`);
      }
    }
  }
}
...
function buildRun255FourArmSheet(built) {
  ...
  return buildNamedContactSheet(
    path.join(outRoot, "run2-55-four-arm-contact-sheet.png"),
    "Run 2.55 text-shape integration comparison",
    sheets,
    sheets.length,
    labels,
  );
...
async function buildArm(arm) {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "required proof objects: Run 2.53 product surface scene id, Run 2.54 generated trace, named text containers, non-rectangular shape families, text-shape binding pairs",
      "source requirements: full arm reads Run 2.54 generated result and full trace before text-shape redraw",
      "negative control: bad arm can reuse Run 2.54 generated result and trace, but cannot use Run 2.55 text-shape integration module",
  ...
  const contractData = armUsesFullRun255Repair(arm) ? loadRun255ContractData(arm) : null;
  const badData = armUsesBadRun255Data(arm) ? loadRun255BadControlData(arm) : null;
  const slides = arm.slides.map((slide, index) =>
    renderRun255Slide(presentation, slide, arm, index + 1, contractData, badData, metricsByRole),
  );
  ...
  assertRun255TextShapeIntegrationSelfCheck(trace);
  ...
  const manifest = {
    ...
    slides: slides.map((_, index) => ({
      ...
      source: "scripts/generate_ppt_run2_55_text_shape_integration_arms.mjs",
...
function writeRun255Result(runSummary) {
  const fullTrace = readJson(`outputs/${threadId}/presentations/ppt-run2-55-full-vulca/trace_manifest.json`);
  const badTrace = readJson(`outputs/${threadId}/presentations/ppt-run2-55-bad-without-text-shape-integration/trace_manifest.json`);
  const result = {
    schema_version: 1,
    run_id: RUN_ID,
    status: "run2_55_text_shape_integration_rerun_public_blocked",
    public_ready: false,
    selected_usecase_id: selectedUsecaseId,
    source_repair_run_id: "2.53",
    source_generated_run_id: "2.54",
    database_expansion: false,
    workflow_expansion: false,
    stage_policy: "repeat_same_five_layers_not_run3",
    input_chain: {
      run2_53_result: RUN2_55_INPUTS.run253Result,
      run2_53_product_surface_scene_memory: RUN2_55_INPUTS.run253Scenes,
      run2_53_business_visual_evidence_memory: RUN2_55_INPUTS.run253Evidence,
      run2_53_scene_renderer_workflow_gates: RUN2_55_INPUTS.run253Gates,
      run2_54_result: RUN2_55_INPUTS.run254Result,
      run2_54_full_trace: RUN2_55_INPUTS.run254FullTrace,
      commercial_usecase_bank: RUN2_55_INPUTS.commercialUsecaseBank,
      sources: RUN2_55_INPUTS.sources,
    },
    rerun: {
      generator: "scripts/generate_ppt_run2_55_text_shape_integration_arms.mjs",
      arms: armSpecs.map((arm) => arm.armId),
      best_internal_arm: "run2_55_full_text_shape_integration",
      best_internal_arm_verdict:
        "Run 2.54 product surface scene rerun drives a native redraw with named text containers and non-rectangular shape families",
    ...
    quality_delta: {
      target_layer: RUN2_55_POLICY,
      source_data_status: RUN2_55_FULL_STATUS,
      ...
      full_slides_with_product_surface_slots: fullTrace.slides.filter(
        (slide) => (slide.run2_55_product_surface_slots_rendered ?? 0) >= 3,
      ).length,
      ...
      full_slides_with_named_text_containers: fullTrace.slides.filter(
        (slide) => (slide.run2_55_named_text_containers_rendered ?? 0) >= 4,
      ).length,
      full_slides_with_non_rectangular_shape_families: fullTrace.slides.filter(
        (slide) => (slide.run2_55_non_rectangular_shape_families_rendered ?? 0) >= 3,
      ).length,
      full_slides_with_text_shape_binding_pairs: fullTrace.slides.filter(
        (slide) => (slide.run2_55_text_shape_binding_pairs ?? 0) >= 4,
      ).length,
      full_slides_without_text_overflow_risk: fullTrace.slides.filter(
        (slide) => (slide.run2_55_text_overflow_risk_count ?? 0) === 0,
      ).length,
      full_slides_without_equal_rectangle_clusters: fullTrace.slides.filter(
        (slide) => (slide.run2_55_equal_rectangle_cluster_count ?? 0) === 0,
      ).length,
      bad_control_slides_without_text_shape_integration: badTrace.slides.filter(
        (slide) =>
          slide.run2_55_text_shape_integration_status === "fail_missing_text_shape_integration" &&
          (slide.run2_55_equal_rectangle_cluster_count ?? 0) >= 1,
      ).length,
      repair_modules: [RUN2_55_MODULE_NAME],
    },
    control_boundary: {
      bad_run2_54_without_text_shape_integration:
        "may see Run 2.54 generated result and trace, including Run 2.53 scene ids, but must not use Run 2.55 named text containers, non-rectangular shape families, or text-shape binding pairs",
      prompt_only: "commercial_case_only_no_run2_54_or_text_shape_integration",
      run1_5_skill: "prior_baseline_no_run2_54_or_text_shape_integration",
    },
    visual_quality_boundary:
      "Run 2.55 repairs measurable text-shape integration; public video-grade aesthetics, native motion, and human release approval remain blocked",
    ...
    next_required_action:
      "audit_run2_55_text_shape_integration_against_visible_content_quality_then_continue_same_five_layers",
  };
  const resultJson = path.join(root, pack, "results", "run2_55_text_shape_integration_rerun_result.json");
  const resultMd = path.join(root, pack, "results", "run2_55_text_shape_integration_rerun_result.md");
...
async function main() {
  ...
  const fourArmSheet = buildRun255FourArmSheet(built);
  const fullSkillSeriesSheet = buildFullSkillSeriesSheet();
  const runSummary = {
    run_id: "run2_55_text_shape_integration_four_arms",
    selected_usecase_id: selectedUsecaseId,
    arms: armSpecs.map((arm) => arm.armId),
  ...
  writeJson(path.join(outRoot, "run2_55_text_shape_integration_rerun_summary.json"), runSummary);
  writeRun255Result(runSummary);
  return runSummary;
}
...
export {
  ...
  RUN2_55_INPUTS,
  armSpecs,
  assertRun255ArmInputBoundaries,
  assertRun255TextShapeIntegrationSelfCheck,
  drawRun255TextShapeIntegration,
  loadRun255ContractData,
  main,
  readRun255PackJsonForArm,
  registerRun255Module,
  selectRun255ForSlide,
  traceFor,
  validateRun255TextShapeInputs,
};
