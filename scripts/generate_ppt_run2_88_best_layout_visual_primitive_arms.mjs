...
const RUN_ID = "2.88";
const RESULT_JSON = `${pack}/results/run2_88_best_layout_visual_primitive_rerun_result.json`;
const RESULT_MD = `${pack}/results/run2_88_best_layout_visual_primitive_rerun_result.md`;
...
const INPUTS = {
  ...
  run285Result: "docs/product/ppt-run2-data-skill-quality/results/run2_85_design_motif_renderer_rerun_result.json",
};
const CONSUMED_SOURCES = [INPUTS.partRPlan, INPUTS.run285Result];
const REPAIR_FLAGS = [
...
const COPY_BY_ROLE = {
  cover: {
    ...
    subhead: "Run 2.88 starts from the strongest historical hero rhythm, then renders a richer product-theater primitive.",
...
const armSpecs = [
  ...
  {
    armId: "run2_88_full_best_layout_visual_primitives",
    slug: "ppt-run2-88-full-vulca",
    label: "Run 2.88 full best-layout visual primitives",
    mode: "full",
    release: "public_blocked",
    palette: { bg: C.paper, title: C.ink, muted: C.muted, accent: C.blue, accent2: C.green, surface: C.white },
  },
  {
    armId: "bad_without_best_layout_visual_primitives",
    slug: "ppt-run2-88-bad-without-best-layout-visual-primitives",
    label: "Bad missing best-layout primitives",
    mode: "negative",
...
function drawRicherProductSurface(slide, x, y, w, h, arm, variant = 0) {
  rect(slide, x + 34, y + 36, w, h, { color: "#cfc6b8" }, line("#cfc6b8", 0));
...
function drawRun287ProductTheaterSurface(slide, page, arm) {
...
function drawRun287EditorialTextField(slide, page, arm) {
...
function drawRun287BeforeAfterSurface(slide, page, arm) {
...
function drawRun287ModularMatrixWorkspace(slide, page, arm) {
...
function drawRun287OverlayStickerStack(slide, page, arm) {
...
function drawRun287DecisionMapBoard(slide, page, arm) {
...
function drawFullSlide(slide, page, arm) {
  ...
  if (page.renderer_function_name === "drawRun287EditorialTextField") drawRun287EditorialTextField(slide, page, arm);
  else if (page.renderer_function_name === "drawRun287BeforeAfterSurface") drawRun287BeforeAfterSurface(slide, page, arm);
  else if (page.renderer_function_name === "drawRun287ModularMatrixWorkspace") drawRun287ModularMatrixWorkspace(slide, page, arm);
  else if (page.renderer_function_name === "drawRun287OverlayStickerStack") drawRun287OverlayStickerStack(slide, page, arm);
  else if (page.renderer_function_name === "drawRun287DecisionMapBoard") drawRun287DecisionMapBoard(slide, page, arm);
  else drawRun287ProductTheaterSurface(slide, page, arm);
...
function drawComparisonSlide(slide, page, arm) {
  ...
  const body = arm.mode === "negative"
    ...
    : "This comparison arm is generated locally for viewer context. The Part S manifest validates the full arm only.";
...
function loadInputs() {
  const data = {
    ...
    run285Result: readJson(INPUTS.run285Result),
  };
  if (data.partRPlan.status !== "run2_87_best_layout_recovery_visual_primitive_plan_ready_public_blocked") {
    throw new Error("Run 2.88 requires the Run 2.87 best-layout visual primitive plan");
  }
  if (data.partRPlan.next_required_action !== "part_s_renderer_rerun_from_run2_87_best_layout_visual_primitive_plan") {
    throw new Error("Run 2.88 requires Part R to request the Part S renderer rerun");
  }
  if (data.run285Result.status !== "run2_85_design_motif_renderer_rerun_generated_public_blocked") {
    throw new Error("Run 2.88 requires the Run 2.85 generated renderer result");
  }
  return data;
}
...
function buildRenderModel(data) {
  ...
  return ROLES.map((role, index) => {
    ...
    if (!recovery || !primitive) throw new Error(`Run 2.88 missing Part R recovery record for role: ${role}`);
...
function traceForArm(arm, pages) {
  return {
    ...
    part_s_renderer_scope: arm.mode === "full"
      ? "renderer_best_layout_visual_primitives_consumes_run2_87_part_r_and_run2_85"
      : "comparison_arm_not_part_s_quality_manifest",
...
function buildRun288Sheet(built) {
  ...
  return buildNamedContactSheet(
    path.join(outRoot, "run2-88-four-arm-contact-sheet.png"),
    "Run 2.88 best-layout visual primitive comparison",
    sheets,
    sheets.length,
    labels,
  );
...
async function buildArm(arm, pages) {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      `arm: ${arm.armId}`,
      `part: Part S / Run ${RUN_ID}`,
      `consumed sources: ${arm.mode === "full" ? CONSUMED_SOURCES.join(", ") : "commercial_case.md only"}`,
      "full arm requirement: recover historical best layouts before drawing Run 2.87 visual primitives",
      "release boundary: public blocked until Part T visual quality evaluation",
      "",
    ].join("\n"),
  );
  ...
  const manifest = {
    ...
    slides: slides.map((_, index) => ({
      ...
      source: "scripts/generate_ppt_run2_88_best_layout_visual_primitive_arms.mjs",
...
function buildStandaloneHtml(pages, outPath) {
  const slideCards = pages.map((page) => `
    <section class="slide ${htmlEscape(page.role)}">
      <div class="kicker">Run 2.88 / ${htmlEscape(page.renderer_primitive_id)}</div>
...
<html lang="en" data-run-id="2.88">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Run 2.88 best-layout visual primitives</title>
  <style>
...
function buildResult(pages, built, fourArmSheet, standaloneHtml, data) {
  const full = built.find((item) => item.arm.armId === "run2_88_full_best_layout_visual_primitives");
  return {
    ...
    source_run2_85_renderer_result: {
      status: data.run285Result.status,
      next_required_action: data.run285Result.next_required_action,
      source_result: INPUTS.run285Result,
    },
    ...
    rendered_pages: pages.map((page) => ({
      ...
      historical_layout_recovered: page.historical_layout_recovered,
    ...
    renderer_best_layout_visual_primitive_checks: {
      pages_with_part_r_consumed: pages.length,
      pages_with_historical_layout_recovered: pages.filter((page) => page.historical_layout_recovered).length,
      pages_with_visual_primitive_rendered: pages.filter((page) => page.renderer_function_name.startsWith("drawRun287")).length,
...
function buildMarkdown(result) {
  return [
    "# Run 2.88 best-layout visual primitive renderer rerun",
    "",
    `Status: ${result.status}`,
    "",
    "Run 2.88 generated a four-arm comparison and updated the local viewer. It consumes Run 2.87 Part R and the Run 2.85 generated renderer result.",
    "",
    "Consumed source trace:",
    ...result.consumed_sources.map((source) => `- ${source}`),
    "",
    "Boundary: public release remains blocked. Part T must evaluate visual quality before any public-ready claim.",
    "",
...
async function main() {
  ...
  const standaloneHtml = path.join(outRoot, "ppt-run2-88-full-vulca", "output", "run2-88-best-layout-visual-primitives.html");
  ...
  writeJson(path.join(outRoot, "run2_88_best_layout_visual_primitive_rerun_summary.json"), {
    status: result.status,
