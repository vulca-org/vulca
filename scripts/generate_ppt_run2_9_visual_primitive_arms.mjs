...
const EXPECTED_RUN2_9_TRACE_FIELDS = [
...
const RUN2_9_INPUTS = {
  repair: `${pack}/run2_9_visual_primitive_repair.json`,
  modules: `${pack}/run2_9_executable_visual_modules.json`,
  gateMatrix: `${pack}/run2_9_visual_gate_matrix.json`,
};
const RUN2_9_RESTRICTED_INPUTS = Object.values(RUN2_9_INPUTS);

const baseSlides = [
  {
    role: "cover",
    title: "The deck stops looking like a report.",
    claim: "Run 2.9 converts tutorial learning into visual primitives before native PPT code is written.",
    trace: "commercial_case -> visual primitives -> native PPT",
  },
  {
    role: "setup",
...
const armSpecs = [
  {
    armId: "prompt_only",
    slug: "ppt-run2-9-prompt-only",
    label: "Prompt-only control",
    kicker: "RUN 2.9 / CONTROL",
    footer: "prompt_only | commercial case only | public blocked",
    release: "public_blocked",
    allowed: [`${pack}/commercial_case.md`],
    forbidden: [
      `${pack}/run2_9_visual_primitive_repair.json`,
      `${pack}/run2_9_executable_visual_modules.json`,
      `${pack}/run2_9_visual_gate_matrix.json`,
  ...
  {
    armId: "run1_5_skill",
    slug: "ppt-run2-9-run1-5-skill",
    label: "Run 1.5 baseline",
    kicker: "RUN 2.9 / RUN 1.5",
  ...
  {
    armId: "run2_9_full_skill",
    slug: "ppt-run2-9-full-vulca",
    label: "Run 2.9 full Vulca skill",
    kicker: "RUN 2.9 / FULL VULCA",
    footer: "run2_9_full_skill | visual primitives + executable modules + gate matrix | public blocked",
    release: "public_blocked",
    allowed: [
  ...
  {
    armId: "bad_visual_primitive_memory",
    slug: "ppt-run2-9-bad-visual-primitive-memory",
    label: "Bad visual-primitive boundary control",
    kicker: "RUN 2.9 / NEGATIVE CONTROL",
    footer: "bad_visual_primitive_memory | primitive repair only | internal comparison",
    ...
    forbidden: [
      ...
      `${pack}/skill_workflow.json`,
      `${pack}/results/trace_manifest_contract.json`,
      "manual Run 2.9 visual module repair before scoring",
    ],
    palette: {
      bg: "#f2efe1",
      rail: "#6d633e",
...
function drawRun29TypographicField(slide, arm, spec, module, metrics, opts = {}) {
  registerVisualModule(metrics, "drawRun29TypographicField");
...
function drawRun29EditorialSpread(slide, arm, spec, module, metrics, opts = {}) {
  registerVisualModule(metrics, "drawRun29EditorialSpread");
  const x = opts.x ?? 80;
  const y = opts.y ?? 118;
  const w = opts.w ?? 1040;
  const h = opts.h ?? 430;
...
function drawRun29LayeredProductSurface(slide, arm, spec, module, metrics, opts = {}) {
  registerVisualModule(metrics, "drawRun29LayeredProductSurface");
  const x = opts.x ?? 604;
  const y = opts.y ?? 134;
  const w = opts.w ?? 504;
  const h = opts.h ?? 352;
...
function drawRun29MotionStoryboard(slide, arm, spec, module, metrics, opts = {}) {
  registerVisualModule(metrics, "drawRun29MotionStoryboard");
  const x = opts.x ?? 100;
  const y = opts.y ?? 430;
  const w = opts.w ?? 880;
  const h = opts.h ?? 126;
...
function drawRun29ClimaxStage(slide, arm, spec, module, metrics, opts = {}) {
  registerVisualModule(metrics, "drawRun29ClimaxStage");
...
function assertArmInputBoundaries(arm) {
  ...
  if (arm.armId === "run2_9_full_skill") {
    for (const input of RUN2_9_RESTRICTED_INPUTS) assertAllowed(input);
    return;
  }
  if (arm.armId === "bad_visual_primitive_memory") {
    assertAllowed(RUN2_9_INPUTS.repair);
    assertForbidden(RUN2_9_INPUTS.modules);
    assertForbidden(RUN2_9_INPUTS.gateMatrix);
    return;
  }
  for (const input of RUN2_9_RESTRICTED_INPUTS) assertForbidden(input);
}

function readRun29JsonForArm(arm, relPath) {
  assertArmInputBoundaries(arm);
  if (arm.armId !== "run2_9_full_skill") {
    throw new Error(`${arm.armId} cannot read Run 2.9 visual-module input: ${relPath}`);
  }
...
function readRun29RepairForArm(arm) {
  assertArmInputBoundaries(arm);
  if (!arm.allowed.includes(RUN2_9_INPUTS.repair) || arm.forbidden.includes(RUN2_9_INPUTS.repair)) {
    throw new Error(`${arm.armId} input boundary does not permit reading ${RUN2_9_INPUTS.repair}`);
  }
  return readJson(RUN2_9_INPUTS.repair);
}

function run29VisualGateMatrixByRole(gateMatrix) {
  return new Map((gateMatrix?.gates ?? []).map((gate) => [gate.slide_role, gate]));
}

function run29PrimitiveRepairsById(repair) {
  return new Map((repair?.primitive_repairs ?? []).map((primitive) => [primitive.id, primitive]));
}

function run29VisualPrimitivesByRole(repair, gateMatrix) {
  const primitivesById = run29PrimitiveRepairsById(repair);
  const map = new Map();
  for (const gate of gateMatrix?.gates ?? []) {
    map.set(
      gate.slide_role,
      gate.visual_primitive_ids.map((id) => primitivesById.get(id)).filter(Boolean),
    );
  }
  return map;
}

function run29VisualModulesByRole(modules) {
  const map = new Map();
  for (const module of modules?.modules ?? []) {
    for (const role of module.applies_to_slide_roles ?? []) {
...
function run29CodeModulesByRole(gateMatrix) {
  const map = new Map();
  for (const gate of gateMatrix?.gates ?? []) {
...
function run29LayoutBudgetByRole(modules) {
  const map = new Map();
  for (const module of modules?.modules ?? []) {
    for (const role of module.applies_to_slide_roles ?? []) {
      const merged = { ...(map.get(role) ?? {}) };
      for (const [key, value] of Object.entries(module.code_binding?.layout_budget ?? {})) {
...
function run29VisualDeltaByRole() {
  return new Map(
    Object.entries({
      cover: "typographic field and editorial asymmetry replace Run 2.8 title-plus-surface boxes",
      setup: "editorial spread plus layered product surface replace equal setup panels",
      contrast: "one focal field and proof rail replace mirrored before-after boxes",
      proof: "layered surface and motion storyboard replace report dashboard proof",
      climax: "keynote-style stage replaces boxed hero-object report layout",
      close: "typographic decision handoff replaces checklist cards",
    }),
  );
}

function buildBadPrimitiveMemoryByRole(repair) {
  const primitives = repair?.primitive_repairs ?? [];
  return new Map([
    ["cover", primitives.filter((primitive) => primitive.target_slide_roles?.includes("cover")).slice(0, 1)],
    ["setup", primitives.filter((primitive) => primitive.target_slide_roles?.includes("setup")).slice(0, 2)],
    ["contrast", primitives.filter((primitive) => primitive.target_slide_roles?.includes("contrast")).slice(0, 1)],
    ["proof", primitives.filter((primitive) => primitive.target_slide_roles?.includes("proof")).slice(0, 2)],
    ["climax", primitives.filter((primitive) => primitive.target_slide_roles?.includes("climax")).slice(0, 2)],
    ["close", primitives.filter((primitive) => primitive.target_slide_roles?.includes("close")).slice(0, 2)],
  ]);
}

function loadRun29ContractData(arm) {
  const repair = readRun29JsonForArm(arm, RUN2_9_INPUTS.repair);
  const modules = readRun29JsonForArm(arm, RUN2_9_INPUTS.modules);
  const gateMatrix = readRun29JsonForArm(arm, RUN2_9_INPUTS.gateMatrix);
  const gateMatrixByRole = run29VisualGateMatrixByRole(gateMatrix);
  const visualPrimitivesByRole = run29VisualPrimitivesByRole(repair, gateMatrix);
  const visualModulesByRole = run29VisualModulesByRole(modules);
  const codeModulesByRole = run29CodeModulesByRole(gateMatrix);
  const layoutBudgetByRole = run29LayoutBudgetByRole(modules);
  const visualDeltaByRole = run29VisualDeltaByRole();
  return {
    repair,
    modules,
    gateMatrix,
    gateMatrixByRole,
    visualPrimitivesByRole,
    visualModulesByRole,
    codeModulesByRole,
    layoutBudgetByRole,
    visualDeltaByRole,
    status: "run2_9_visual_contract_ready",
  };
}

function renderRun29Full(arm) {
  const contract = loadRun29ContractData(arm);
  return {
    ...contract,
    status: "run2_9_visual_contract_ready",
  };
...
function renderRun29FullSlide(presentation, spec, arm, n, fullData, metricsByRole) {
  const slide = presentation.slides.add();
  base(slide, arm, n);
  const metrics = createSlideMetrics(spec.role);
  const roleModules = fullData.visualModulesByRole.get(spec.role) ?? [];
  const moduleByFunction = new Map(roleModules.map((module) => [module.code_binding?.function_name, module]));

  if (spec.role === "cover") {
    drawRun29TypographicField(slide, arm, spec, moduleByFunction.get("drawRun29TypographicField"), metrics, {
      x: 76,
      y: 126,
      w: 720,
      h: 220,
    ...
    drawRun29EditorialSpread(slide, arm, spec, moduleByFunction.get("drawRun29EditorialSpread"), metrics, {
      x: 82,
      y: 390,
      w: 960,
      h: 170,
      headline: "primitive -> module -> gate",
      note: "The full arm must show the learning path in composition, not just in trace.",
      headlineSize: 28,
      railLabels: ["visual primitive", "native module", "gate"],
    });
    ...
    drawRun29EditorialSpread(slide, arm, spec, moduleByFunction.get("drawRun29EditorialSpread"), metrics, {
      ...
      h: 390,
      headlineSize: 38,
      railLabels: ["choose", "compose", "render"],
    });
    drawRun29LayeredProductSurface(slide, arm, spec, moduleByFunction.get("drawRun29LayeredProductSurface"), metrics, {
      x: 676,
      y: 154,
      w: 430,
      h: 330,
      caption: "depth proves the workflow",
    });
    ...
    drawRun29EditorialSpread(slide, arm, spec, moduleByFunction.get("drawRun29EditorialSpread"), metrics, {
      ...
      w: 990,
      h: 448,
      headline: "Before: equal boxes. After: one visual argument.",
      note: "The gate fails if this still reads as a two-column report comparison.",
      headlineSize: 42,
      railLabels: ["before", "failure probe", "after"],
    });
    rect(slide, 818, 404, 166, 68, C.fog, colorLine("#cbd3d8", 1));
    rect(slide, 1012, 370, 76, 138, arm.palette.proof, colorLine(arm.palette.proof, 1));
  } else if (spec.role === "proof") {
    drawRun29LayeredProductSurface(slide, arm, spec, moduleByFunction.get("drawRun29LayeredProductSurface"), metrics, {
      x: 86,
      y: 130,
      w: 540,
      h: 352,
      caption: "data binds to native visual code",
    });
    drawRun29MotionStoryboard(slide, arm, spec, moduleByFunction.get("drawRun29MotionStoryboard"), metrics, {
      x: 660,
      y: 392,
      w: 430,
      h: 118,
    ...
    drawRun29ClimaxStage(slide, arm, spec, moduleByFunction.get("drawRun29ClimaxStage"), metrics);
    drawRun29MotionStoryboard(slide, arm, spec, moduleByFunction.get("drawRun29MotionStoryboard"), metrics, {
      x: 236,
      y: 530,
      w: 720,
...
function renderControlSlide(presentation, spec, arm, n, metricsByRole) {
  ...
  base(slide, arm, n);
  const metrics = createSlideMetrics(spec.role);
  const mode = arm.armId === "bad_visual_primitive_memory" ? "bad" : arm.armId === "run1_5_skill" ? "run1_5" : "prompt";
  ...
  drawControlGate(slide, arm, spec, metrics, {
    ...
    headline: arm.armId === "bad_visual_primitive_memory" ? "primitive only" : "public blocked",
    line: arm.armId === "bad_visual_primitive_memory" ? "no module/gate read" : "no Run 2.9 visual trace",
  });
  proofFooter(slide, spec, arm);
  metricsByRole.set(spec.role, metrics);
  return slide;
...
function traceFor(arm, context = {}) {
  assertArmInputBoundaries(arm);
  const fullRun29 = arm.armId === "run2_9_full_skill";
  const fullData = fullRun29 ? context.fullData ?? renderRun29Full(arm) : null;
  const gateByRole = fullData?.gateMatrixByRole ?? new Map();
  const visualPrimitivesByRole = fullData?.visualPrimitivesByRole ?? new Map();
  const visualModulesByRole = fullData?.visualModulesByRole ?? new Map();
  ...
  const badPrimitiveMemoryByRole = context.badPrimitiveMemoryByRole ?? new Map();
  const metricsByRole = context.metricsByRole ?? new Map();
  const hasRenderedMetrics =
    fullRun29 && arm.slides.every((slide) => metricsByRole.has(slide.role));
  return {
    schema_version: 1,
    arm_id: arm.armId,
    ...
    runtime_isolation: {
      ...
      prompt_context: "fresh Run 2.9 visual-primitive arm-specific generation from scripts/generate_ppt_run2_9_visual_primitive_arms.mjs",
    ...
    slides: arm.slides.map((slide, index) => {
      ...
      const rolePrimitives = visualPrimitivesByRole.get(slide.role) ?? [];
      const roleModules = visualModulesByRole.get(slide.role) ?? [];
      ...
      return {
        ...
        role: slide.role,
        title: slide.title,
        run2_9_contract_status: fullRun29
          ? hasRenderedMetrics
            ? "full_arm_native_generator_rendered"
            : "full_arm_contract_preview_not_rendered"
          : "boundary_control_not_run2_9_full",
        run2_9_visual_primitive_ids: fullRun29 ? rolePrimitives.map((primitive) => primitive.id) : [],
        run2_9_visual_module_ids: fullRun29 ? roleModules.map((module) => module.id) : [],
        run2_9_gate_matrix_ids: fullRun29 ? (gate ? [gate.id] : []) : [],
        run2_9_required_code_module_ids: fullRun29 ? gate?.required_code_modules ?? codeModulesByRole.get(slide.role) ?? [] : [],
        run2_9_code_module_ids: fullRun29
          ? hasRenderedMetrics
            ? actualCodeModuleIds
            : gate?.required_code_modules ?? codeModulesByRole.get(slide.role) ?? []
          : [],
        run2_9_boxiness_failure_probe: fullRun29 ? gate?.boxiness_failure_probe ?? "" : "",
        run2_9_visual_delta_from_run2_8: fullRun29 ? visualDeltaByRole.get(slide.role) ?? "" : "",
        run2_9_trace_origin: hasRenderedMetrics ? "actual_native_visual_module_calls" : fullRun29 ? "contract_preview_required_visual_modules" : "boundary_control",
        negative_control_visual_primitive_ids:
          arm.armId === "bad_visual_primitive_memory" ? (badPrimitiveMemoryByRole.get(slide.role) ?? []).map((primitive) => primitive.id) : [],
        layout_metrics: {
...
function assertRun29VisualGateSelfCheck(trace) {
  if (trace.arm_id !== "run2_9_full_skill") return;
  for (const slide of trace.slides) {
    if (slide.run2_9_contract_status !== "full_arm_native_generator_rendered") {
      throw new Error(`Run 2.9 full slide ${slide.slide_id} was not rendered with native module metrics`);
    }
    for (const field of EXPECTED_RUN2_9_TRACE_FIELDS) {
      ...
      if (empty) throw new Error(`Run 2.9 full slide ${slide.slide_id} missing ${field}`);
    }
    if (!slide.layout_budget_status?.within_budget) {
      throw new Error(`Run 2.9 full slide ${slide.slide_id} exceeded layout budget: ${slide.layout_budget_status.violations.join("; ")}`);
    }
    const actualCodeModules = new Set(slide.run2_9_code_module_ids ?? []);
    for (const requiredCodeModule of slide.run2_9_required_code_module_ids ?? []) {
      if (!actualCodeModules.has(requiredCodeModule)) {
        throw new Error(`Run 2.9 full slide ${slide.slide_id} did not call required module ${requiredCodeModule}`);
      }
    }
    if (slide.role === "climax") {
      if (!actualCodeModules.has("drawRun29ClimaxStage")) {
        throw new Error("Run 2.9 climax did not call drawRun29ClimaxStage");
      }
      const share = slide.layout_metrics?.hero_object_canvas_share ?? 0;
      if (!share) throw new Error("Run 2.9 climax missing hero object share");
    }
  }
}
...
function buildArmContract() {
  return armSpecs.map((arm) => ({
    ...
    label: arm.label,
    contract_status:
      arm.armId === "run2_9_full_skill"
        ? "run2_9_visual_contract_ready_requires_render_metrics"
        : "run2_9_boundary_control_contract_ready",
    allowed: arm.allowed,
...
function buildRun29FourArmSheet(built) {
  const sheets = built.map((item) => item.contactSheet).filter((file) => fs.existsSync(file));
  if (!sheets.length) return "";
  return buildNamedContactSheet(path.join(outRoot, "run2-9-four-arm-contact-sheet.png"), "Run 2.9 four-arm comparison", sheets, sheets.length);
}
...
async function buildArm(arm) {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "required proof objects: editorial spread, layered product surface, motion storyboard, climax stage, typographic field, release gate trace",
      "source requirements: commercial case always; bad control may read visual primitive repair only; full arm additionally requires executable visual modules and visual gate matrix",
  ...
  let fullData = null;
  let badPrimitiveMemoryByRole = new Map();
  if (arm.armId === "run2_9_full_skill") {
    fullData = loadRun29ContractData(arm);
  } else if (arm.armId === "bad_visual_primitive_memory") {
    badPrimitiveMemoryByRole = buildBadPrimitiveMemoryByRole(readRun29RepairForArm(arm));
  }

  const slides = arm.slides.map((slide, index) =>
    arm.armId === "run2_9_full_skill"
      ? renderRun29FullSlide(presentation, slide, arm, index + 1, fullData, metricsByRole)
  ...
  const trace = traceFor(arm, { fullData, badPrimitiveMemoryByRole, metricsByRole });
  assertRun29VisualGateSelfCheck(trace);
  ...
  const manifest = {
    ...
    slides: slides.map((_, index) => ({
      ...
      source: "scripts/generate_ppt_run2_9_visual_primitive_arms.mjs",
...
async function main() {
  ...
  const fourArmSheet = buildRun29FourArmSheet(built);
  const fullSkillSeriesSheet = buildFullSkillSeriesSheet();
  const runSummary = {
    run_id: "run2_9_visual_primitive_four_arms",
  ...
  writeJson(path.join(outRoot, "run2_9_visual_primitive_summary.json"), runSummary);
  return runSummary;
}
...
export {
  ...
  renderRun29Full,
  run29CodeModulesByRole,
  run29LayoutBudgetByRole,
  run29PrimitiveRepairsById,
  run29VisualDeltaByRole,
  run29VisualGateMatrixByRole,
  run29VisualModulesByRole,
  run29VisualPrimitivesByRole,
  traceFor,
};
