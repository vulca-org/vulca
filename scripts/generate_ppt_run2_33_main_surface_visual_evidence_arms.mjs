...
const EXPECTED_RUN2_33_TRACE_FIELDS = [
  ...
  "run2_33_synthesis_record_id",
  "run2_33_public_surface_mode",
  "run2_33_trace_surface_mode",
  "run2_33_presentation_module_id",
  "run2_33_chain_compression_policy",
  "run2_33_presentation_synthesis_execution_status",
  "run2_32_source_audit_status",
  "run2_32_top_next_layer_to_thicken",
  "run2_33_spine_min_font_size_target",
  "run2_33_climax_style_policy",
  "run2_33_visual_evidence_object_min_target",
  "run2_33_main_surface_visual_evidence_execution_status",
  "run2_33_code_module_ids",
];

const RUN2_33_INPUTS = {
  presentationSynthesis: `${pack}/run2_29_presentation_synthesis_memory.json`,
  presentationSynthesisAudit: `${pack}/results/run2_30_presentation_synthesis_audit.json`,
  spineClimaxRepairAudit: `${pack}/results/run2_32_spine_climax_repair_audit.json`,
  spineClimaxRepairRerun: `${pack}/results/run2_31_spine_climax_repair_rerun_result.json`,
...
const RUN2_33_DATA_INPUTS = [
  RUN2_33_INPUTS.presentationSynthesis,
  RUN2_33_INPUTS.presentationSynthesisAudit,
  RUN2_33_INPUTS.spineClimaxRepairAudit,
  RUN2_33_INPUTS.spineClimaxRepairRerun,
  RUN2_33_INPUTS.evidenceChain,
  RUN2_33_INPUTS.contentMemory,
  RUN2_33_INPUTS.visualAssets,
  RUN2_33_INPUTS.workflowGates,
];
...
const armSpecs = [
  {
    armId: "prompt_only",
    slug: "ppt-run2-33-prompt-only",
    label: "Prompt-only control",
    kicker: "RUN 2.33 / CONTROL",
    ...
    forbidden: [
      ...
      `${pack}/results/run2_30_presentation_synthesis_audit.json`,
      `${pack}/results/run2_32_spine_climax_repair_audit.json`,
      `${pack}/results/run2_31_spine_climax_repair_rerun_result.json`,
  ...
  {
    armId: "run1_5_skill",
    slug: "ppt-run2-33-run1-5-skill",
    label: "Run 1.5 baseline",
    kicker: "RUN 2.33 / RUN 1.5",
  ...
  {
    armId: "run2_33_full_main_surface_visual_evidence",
    slug: "ppt-run2-33-full-vulca",
    label: "Run 2.33 full main-surface visual-evidence",
    kicker: "RUN 2.33 / MAIN SURFACE + VISUAL EVIDENCE",
    footer: "run2_33_full_main_surface_visual_evidence | presentation-first surface + readable evidence spine | public blocked",
    release: "public_blocked",
    allowed: [
  ...
  {
    armId: "bad_main_surface_visual_evidence_memory",
    slug: "ppt-run2-33-bad-main-surface-visual-evidence-memory",
    label: "Bad main-surface visual-evidence memory control",
    kicker: "RUN 2.33 / NEGATIVE CONTROL",
    footer: "bad_main_surface_visual_evidence_memory | usecase label only | internal comparison",
    ...
    forbidden: [
      ...
      `${pack}/skill_workflow.json`,
      `${pack}/results/trace_manifest_contract.json`,
      "manual Run 2.33 pack repair before scoring",
    ],
    palette: {
      bg: "#f1eadb",
      rail: "#695d3a",
...
function registerRun233Module(metrics, functionName) {
...
function drawRun233LaunchField(slide, arm, spec, selection, metrics, opts = {}) {
  registerRun233Module(metrics, "drawRun233LaunchField");
...
function drawRun233SelectedRouteMap(slide, arm, spec, selection, metrics, opts = {}) {
  registerRun233Module(metrics, "drawRun233SelectedRouteMap");
  const x = opts.x ?? 82;
  const y = opts.y ?? 402;
  const w = opts.w ?? 1000;
...
function run233ContentEvidenceSurfaceGeometry({ x, y, w, h }) {
...
function drawRun233ContentEvidenceSurface(slide, arm, spec, selection, metrics, opts = {}) {
  registerRun233Module(metrics, "drawRun233ContentEvidenceSurface");
  const x = opts.x ?? 96;
  const y = opts.y ?? 126;
  const w = opts.w ?? 870;
  const h = opts.h ?? 368;
  const geom = run233ContentEvidenceSurfaceGeometry({ x, y, w, h });
...
function run233EvidenceChainSurfaceGeometry({ x, y, w, h }) {
  const compact = w < 760;
...
function drawRun233EvidenceChainSurface(slide, arm, spec, selection, metrics, opts = {}) {
  registerRun233Module(metrics, "drawRun233EvidenceChainSurface");
  const x = opts.x ?? 70;
  const y = opts.y ?? 112;
  const w = opts.w ?? 1068;
  const h = opts.h ?? 424;
  const geom = run233EvidenceChainSurfaceGeometry({ x, y, w, h });
...
function drawRun233WorkflowTracePanel(slide, arm, spec, selection, metrics, opts = {}) {
  registerRun233Module(metrics, "drawRun233WorkflowTracePanel");
  drawRun233EvidenceChainSurface(slide, arm, spec, selection, metrics, {
    x: opts.x ?? 72,
    y: opts.y ?? 108,
    w: opts.w ?? 720,
    h: opts.h ?? 418,
...
function drawRun233ClimaxEvidenceObject(slide, arm, spec, selection, metrics) {
  registerRun233Module(metrics, "drawRun233ClimaxEvidenceObject");
  drawRun233ClimaxProofObject(slide, arm, spec, selection, metrics);
...
function drawRun233ClimaxProofObject(slide, arm, spec, selection, metrics) {
  registerRun233Module(metrics, "drawRun233ClimaxProofObject");
...
function run233SpineDetail(selection, step) {
...
function drawRun233ReadableEvidenceSpine(slide, arm, spec, selection, metrics, opts = {}) {
  registerRun233Module(metrics, "drawRun233ReadableEvidenceSpine");
  const x = opts.x ?? 922;
  const y = opts.y ?? 124;
  const w = opts.w ?? 246;
  const h = opts.h ?? 394;
  ...
  steps.forEach((step, index) => {
    ...
    text(slide, run233SpineDetail(selection, step), x + 48, rowY + 22, w - 66, 34, {
      fontSize: 8,
      color: opts.dark ? "#bac8d0" : arm.palette.muted,
    });
    registerText(metrics, `${step} ${run233SpineDetail(selection, step)}`);
  });
  registerWorkflow(metrics, 4);
  registerVisualEvidence(metrics, 1);
}

function run233EvidenceLayerSlot(role) {
  return {
...
function drawRun233VisualEvidenceStoryboard(slide, arm, spec, selection, metrics, slot) {
  registerRun233Module(metrics, "drawRun233VisualEvidenceStoryboard");
...
function drawRun233MainSurfaceEvidenceLayer(slide, arm, spec, selection, metrics) {
  registerRun233Module(metrics, "drawRun233MainSurfaceEvidenceLayer");
  const slot = run233EvidenceLayerSlot(spec.role);
  ...
  drawRun233VisualEvidenceStoryboard(slide, arm, spec, selection, metrics, slot);
  registerText(metrics, "main-surface evidence visual evidence realism");
  registerProof(metrics, 1);
}

function drawRun233EditorialLaunchFrame(slide, arm, spec, selection, metrics) {
  registerRun233Module(metrics, "drawRun233EditorialLaunchFrame");
  ...
  drawRun233ReadableEvidenceSpine(slide, arm, spec, selection, metrics, { x: 898, y: 118, w: 264, h: 438 });
  registerProof(metrics, 2);
  registerZones(metrics, 3);
  registerHero(metrics, stage.x, stage.y, stage.w, stage.h);
}

function drawRun233AsymmetricProblemFrame(slide, arm, spec, selection, metrics) {
  registerRun233Module(metrics, "drawRun233AsymmetricProblemFrame");
  moduleLabel(slide, 76, 98, "presentation-first surface", arm);
  ...
  drawRun233ReadableEvidenceSpine(slide, arm, spec, selection, metrics, { x: 900, y: 124, w: 262, h: 420 });
...
function drawRun233BeforeAfterEditorialSpread(slide, arm, spec, selection, metrics) {
  registerRun233Module(metrics, "drawRun233BeforeAfterEditorialSpread");
...
function drawRun233ProductProofTheater(slide, arm, spec, selection, metrics) {
  registerRun233Module(metrics, "drawRun233ProductProofTheater");
  moduleLabel(slide, 76, 96, "presentation-first surface", arm);
...
function drawRun233HeroProofScene(slide, arm, spec, selection, metrics) {
  registerRun233Module(metrics, "drawRun233HeroProofScene");
  ...
  registerText(metrics, "public blocked");
  drawRun233ReadableEvidenceSpine(slide, arm, spec, selection, metrics, { x: 872, y: 154, w: 270, h: 392 });
  registerProof(metrics, 3);
  registerGate(metrics, 1);
  registerVisualEvidence(metrics, selection.assets.length);
  registerZones(metrics, 3);
...
function drawRun233DecisionHandoff(slide, arm, spec, selection, metrics) {
  registerRun233Module(metrics, "drawRun233DecisionHandoff");
  moduleLabel(slide, 76, 96, "presentation-first surface", arm);
...
function assertRun233ArmInputBoundaries(arm) {
  ...
  if (arm.armId === "run2_33_full_main_surface_visual_evidence") {
    for (const input of RUN2_33_DATA_INPUTS) assertAllowed(input);
    return;
  }
  for (const input of RUN2_33_DATA_INPUTS) assertForbidden(input);
}

function readRun233PackJsonForArm(arm, relPath) {
  assertRun233ArmInputBoundaries(arm);
...
function validateRun230PresentationSynthesisAudit(audit) {
  if (audit?.status !== "run2_30_presentation_synthesis_audit_public_blocked") {
    throw new Error("Run 2.33 source audit must be Run 2.30 presentation synthesis audit");
  }
...
function validateRun231SpineClimaxRepairRerun(result) {
  if (result?.status !== "run2_31_spine_climax_repair_rerun_public_blocked") {
    throw new Error("Run 2.33 prior rerun result must be Run 2.31 public-blocked spine/climax repair rerun");
  }
  if (
    result.run_id !== "2.31" ||
    ...
    throw new Error("Run 2.33 prior rerun result source mismatch");
  }
}

function validateRun232SpineClimaxRepairAudit(audit) {
  if (audit?.status !== "run2_32_spine_climax_repair_audit_public_blocked") {
    throw new Error("Run 2.33 source audit must be Run 2.32 spine/climax repair audit");
  }
  ...
  if (audit.repair_verification?.repair_target_closed !== true) {
    throw new Error("Run 2.32 audit must close the Run 2.31 spine/climax repair target before Run 2.33");
  }
...
function validateRun233EvidenceChainViewModel(model, contentMemory, workflowGates) {
  if (model?.status !== "run2_28_evidence_chain_view_model_ready_public_blocked") {
    throw new Error("Run 2.33 source evidence chain must be the Run 2.28 evidence-chain view model");
  }
...
function validateRun233PresentationSynthesisMemory(memory, evidenceChain) {
  if (memory?.status !== "run2_29_presentation_synthesis_memory_ready_public_blocked") {
    throw new Error("Run 2.33 must consume Run 2.29 presentation synthesis memory");
  }
  if (memory.selected_usecase_id !== selectedUsecaseId || memory.stage_policy !== "repeat_same_five_layers_not_run3") {
    throw new Error("Run 2.33 presentation synthesis usecase/stage mismatch");
  }
  if (memory.source_evidence_chain_layer !== "run2_28_evidence_chain_view_model.json") {
    throw new Error("Run 2.33 presentation synthesis must bind Run 2.28 evidence chain");
  }
  if (memory.surface_policy?.primary_reader_experience !== "presentation_first_surface") {
    throw new Error("Run 2.33 must render a presentation-first surface");
  }
  if (memory.surface_policy?.secondary_reviewer_experience !== "compressed_evidence_spine") {
    throw new Error("Run 2.33 must keep a readable evidence spine");
  }
  if (memory.surface_policy?.forbidden_primary_surface !== "four_column_audit_table") {
    throw new Error("Run 2.33 must forbid four-column audit table as primary surface");
  }
  ...
  if (records.length !== roles.length || records.map((record) => record.role).join(",") !== roles.join(",")) {
    throw new Error("Run 2.33 presentation synthesis must contain the six canonical slide roles in order");
  }
  ...
  for (const record of records) {
    ...
    if (!chain || record.evidence_chain_id !== chain.evidence_chain_id) {
      throw new Error(`Run 2.33 synthesis/evidence-chain mismatch for ${record.role}`);
    }
    if (record.source_native_surface_module_id !== chain.native_surface_module_id) {
      throw new Error(`Run 2.33 synthesis source module mismatch for ${record.role}`);
    }
    if (!String(record.presentation_module_id ?? "").startsWith("drawRun229")) {
      throw new Error(`Run 2.33 synthesis missing presentation module for ${record.role}`);
    }
    if (record.trace_surface_mode !== "manifest_and_html_viewer_full_chain_visible") {
      throw new Error(`Run 2.33 synthesis trace surface mode mismatch for ${record.role}`);
    }
    if (JSON.stringify(record.visible_on_slide_evidence_spine_steps ?? []) !== JSON.stringify(chainOrder)) {
      throw new Error(`Run 2.33 synthesis chain order mismatch for ${record.role}`);
    }
    for (const policy of [
      ...
      if (!(record.chain_compression_policy ?? []).includes(policy)) {
        throw new Error(`Run 2.33 synthesis missing policy ${policy} for ${record.role}`);
      }
    }
    for (const field of requiredTraceFields) {
      if (!(record.preserved_trace_fields ?? []).includes(field)) {
        throw new Error(`Run 2.33 synthesis missing preserved trace field ${field} for ${record.role}`);
      }
    }
  }
}

function run233RequiredModulesByRole(presentationSynthesis) {
  return new Map(
    (presentationSynthesis.slide_synthesis_records ?? []).map((record) => [
      record.role,
      [run233PresentationModuleIdForRole(record.role), "drawRun233MainSurfaceEvidenceLayer", "drawRun233ReadableEvidenceSpine"],
    ]),
  );
}

function run233PresentationModuleIdForRole(role) {
  return {
    cover: "drawRun233EditorialLaunchFrame",
    setup: "drawRun233AsymmetricProblemFrame",
    contrast: "drawRun233BeforeAfterEditorialSpread",
    proof: "drawRun233ProductProofTheater",
    climax: "drawRun233HeroProofScene",
    close: "drawRun233DecisionHandoff",
  }[role];
}

function loadRun233ContractData(arm) {
  const presentationSynthesis = readRun233PackJsonForArm(arm, RUN2_33_INPUTS.presentationSynthesis);
  const presentationSynthesisAudit = readRun233PackJsonForArm(arm, RUN2_33_INPUTS.presentationSynthesisAudit);
  const spineClimaxRepairAudit = readRun233PackJsonForArm(arm, RUN2_33_INPUTS.spineClimaxRepairAudit);
  const spineClimaxRepairRerun = readRun233PackJsonForArm(arm, RUN2_33_INPUTS.spineClimaxRepairRerun);
  const evidenceChain = readRun233PackJsonForArm(arm, RUN2_33_INPUTS.evidenceChain);
  const contentMemory = readRun233PackJsonForArm(arm, RUN2_33_INPUTS.contentMemory);
  const visualAssets = readRun233PackJsonForArm(arm, RUN2_33_INPUTS.visualAssets);
  const workflowGates = readRun233PackJsonForArm(arm, RUN2_33_INPUTS.workflowGates);
  validateRun224SingleUsecaseSchemas(contentMemory, visualAssets, workflowGates);
  validateRun230PresentationSynthesisAudit(presentationSynthesisAudit);
  validateRun231SpineClimaxRepairRerun(spineClimaxRepairRerun);
  validateRun232SpineClimaxRepairAudit(spineClimaxRepairAudit);
  validateRun233EvidenceChainViewModel(evidenceChain, contentMemory, workflowGates);
  validateRun233PresentationSynthesisMemory(presentationSynthesis, evidenceChain);
  return {
    presentationSynthesis,
    presentationSynthesisAudit,
    spineClimaxRepairAudit,
    spineClimaxRepairRerun,
    evidenceChain,
    contentMemory,
    visualAssets,
    workflowGates,
    requiredModulesByRole: run233RequiredModulesByRole(presentationSynthesis),
    status: "run2_33_main_surface_visual_evidence_contract_ready",
  };
}

function selectRun233ForSlide(role, contractData) {
  ...
  if (!synthesis) throw new Error(`Run 2.33 presentation synthesis missing role ${role}`);
  ...
  if (synthesis.evidence_chain_id !== chain.evidence_chain_id) {
    throw new Error(`Run 2.33 synthesis/evidence-chain mismatch for ${role}`);
  }
  ...
  return { content, assets, gate, chain, synthesis, repairModuleId: run233PresentationModuleIdForRole(role) };
}

function renderRun233FullSlide(presentation, spec, arm, n, fullData, metricsByRole) {
  const slide = presentation.slides.add();
  base(slide, arm, n);
  const metrics = createSlideMetrics(spec.role);
  const selection = selectRun233ForSlide(spec.role, fullData);

  const modules = {
    drawRun233EditorialLaunchFrame,
    drawRun233AsymmetricProblemFrame,
    drawRun233BeforeAfterEditorialSpread,
    drawRun233ProductProofTheater,
    drawRun233HeroProofScene,
    drawRun233DecisionHandoff,
  };
  const renderModule = modules[selection.repairModuleId];
  if (!renderModule) throw new Error(`Run 2.33 missing presentation module ${selection.repairModuleId}`);
  renderModule(slide, arm, spec, selection, metrics);
  drawRun233MainSurfaceEvidenceLayer(slide, arm, spec, selection, metrics);
...
function renderControlSlide(presentation, spec, arm, n, metricsByRole) {
  ...
  base(slide, arm, n);
  const metrics = createSlideMetrics(spec.role);
  const mode = arm.armId === "bad_main_surface_visual_evidence_memory" ? "bad" : arm.armId === "run1_5_skill" ? "run1_5" : "prompt";
  ...
  drawControlGate(slide, arm, spec, metrics, {
    ...
    headline: arm.armId === "bad_main_surface_visual_evidence_memory" ? "pack missing" : "public blocked",
    line: arm.armId === "bad_main_surface_visual_evidence_memory" ? "label only / no 2.33 synthesis" : "no Run 2.33 trace",
  });
...
function traceFor(arm, context = {}) {
  assertRun233ArmInputBoundaries(arm);
  const fullRun233 = arm.armId === "run2_33_full_main_surface_visual_evidence";
  const fullData = fullRun233 ? context.fullData ?? loadRun233ContractData(arm) : null;
  const requiredModulesByRole = fullData?.requiredModulesByRole ?? new Map();
  const metricsByRole = context.metricsByRole ?? new Map();
  const hasRenderedMetrics = fullRun233 && arm.slides.every((slide) => metricsByRole.has(slide.role));
  return {
    schema_version: 1,
    arm_id: arm.armId,
    render_style_arm_id: arm.armId,
    selected_usecase_id: fullRun233 ? selectedUsecaseId : "",
    prior_rerun_run_id: fullRun233 ? fullData.spineClimaxRepairRerun.run_id : "",
    prior_rerun_status: fullRun233 ? fullData.spineClimaxRepairRerun.status : "",
    evidence_chain_status: fullRun233 ? fullData.evidenceChain.status : "",
    presentation_synthesis_status: fullRun233 ? fullData.presentationSynthesis.status : "",
    ...
    runtime_isolation: {
      ...
      prompt_context: "fresh Run 2.33 main-surface visual-evidence arm-specific generation from scripts/generate_ppt_run2_33_main_surface_visual_evidence_arms.mjs",
      no_cross_arm_reuse: [
        ...
        "Run 2.30 audit in control arms",
        "Run 2.33 main-surface visual-evidence in control arms",
      ],
    },
    ...
    slides: arm.slides.map((slide, index) => {
      ...
      const selection = fullRun233 ? selectRun233ForSlide(slide.role, fullData) : null;
      return {
        slide_id: `slide_${String(index + 1).padStart(2, "0")}`,
        role: slide.role,
        title: slide.title,
        run2_33_contract_status: fullRun233
          ? hasRenderedMetrics
            ? "full_arm_native_generator_rendered"
            : "full_arm_contract_preview_not_rendered"
          : "boundary_control_not_run2_33_full",
        run2_24_selected_usecase_id: fullRun233 ? selection.content.selected_usecase_id : "",
        run2_24_content_memory_id: fullRun233 ? selection.content.content_memory_id : "",
        run2_24_visual_evidence_slot_ids: fullRun233 ? selection.content.visual_evidence_slot_ids : [],
        run2_24_content_density_gate_id: fullRun233 ? selection.gate.gate_id : "",
        run2_24_business_proof_points_visible: fullRun233 ? selection.content.business_proof_points.slice(0, 3) : [],
        run2_24_visual_evidence_asset_ids: fullRun233 ? selection.assets.map((asset) => asset.asset_id) : [],
        run2_24_public_surface_policy: fullRun233 ? selection.content.public_surface_policy : "",
        run2_28_evidence_chain_id: fullRun233 ? selection.chain.evidence_chain_id : "",
        run2_28_multimodal_source_record_ids: fullRun233 ? selection.chain.multimodal_source_record_ids : [],
        run2_28_extracted_design_rule: fullRun233 ? selection.chain.extracted_design_rule : "",
        run2_28_workflow_decision: fullRun233 ? selection.chain.workflow_decision : "",
        run2_28_native_surface_module_id: fullRun233 ? selection.chain.native_surface_module_id : "",
        run2_33_synthesis_record_id: fullRun233 ? selection.synthesis.synthesis_record_id : "",
        run2_33_public_surface_mode: fullRun233 ? selection.synthesis.public_surface_mode : "",
        run2_33_trace_surface_mode: fullRun233 ? selection.synthesis.trace_surface_mode : "",
        run2_33_presentation_module_id: fullRun233 ? selection.repairModuleId : "",
        run2_33_chain_compression_policy: fullRun233 ? selection.synthesis.chain_compression_policy : [],
        run2_33_presentation_synthesis_execution_status: fullRun233
          ? "presentation_first_surface_rendered_with_secondary_evidence_spine"
          : "",
        run2_32_source_audit_status: fullRun233 ? fullData.spineClimaxRepairAudit.status : "",
        run2_32_top_next_layer_to_thicken: fullRun233
          ? fullData.spineClimaxRepairAudit.quality_summary.top_next_layer_to_thicken
          : "",
        run2_33_spine_min_font_size_target: fullRun233 ? 8 : "",
        run2_33_climax_style_policy: fullRun233 ? "high_contrast_climax_with_shared_light_editorial_frame" : "",
        run2_33_visual_evidence_object_min_target: fullRun233 ? 2 : "",
        run2_33_main_surface_visual_evidence_execution_status: fullRun233
          ? "main_surface_information_density_and_visual_evidence_realism_thickened_before_native_ppt_generation"
          : "",
        run2_33_required_code_module_ids: fullRun233 ? requiredModulesByRole.get(slide.role) ?? [] : [],
        run2_33_code_module_ids: fullRun233
          ? hasRenderedMetrics
            ? actualCodeModuleIds
            : requiredModulesByRole.get(slide.role) ?? []
          : [],
        run2_33_bad_control_probe: fullRun233
          ? "bad_main_surface_visual_evidence_memory may receive the selected usecase label but forbids Run 2.29 presentation synthesis, Run 2.30 audit, Run 2.32 audit, Run 2.31 rerun, Run 2.28 evidence chain, Run 2.24 content memory, visual evidence assets, and workflow gates"
          : "boundary_control",
        negative_control_usecase_id: arm.armId === "bad_main_surface_visual_evidence_memory" ? selectedUsecaseId : "",
        layout_metrics: {
...
function assertRun233ContentVisualGateSelfCheck(trace) {
  if (trace.arm_id !== "run2_33_full_main_surface_visual_evidence") return;
  if (trace.selected_usecase_id !== selectedUsecaseId) throw new Error("Run 2.33 full trace did not lock the selected usecase");
  for (const slide of trace.slides) {
    if (slide.run2_33_contract_status !== "full_arm_native_generator_rendered") {
      throw new Error(`Run 2.33 full slide ${slide.slide_id} was not rendered with native module metrics`);
    }
    for (const field of EXPECTED_RUN2_33_TRACE_FIELDS) {
      ...
      if (empty) throw new Error(`Run 2.33 full slide ${slide.slide_id} missing ${field}`);
    }
    if ((slide.run2_24_visual_evidence_slot_ids ?? []).length < 2) {
      throw new Error(`Run 2.33 full slide ${slide.slide_id} missing at least two visual evidence slots`);
    }
    const actualCodeModules = new Set(slide.run2_33_code_module_ids ?? []);
    for (const requiredCodeModule of slide.run2_33_required_code_module_ids ?? []) {
      if (!actualCodeModules.has(requiredCodeModule)) {
        throw new Error(`Run 2.33 full slide ${slide.slide_id} did not call required module ${requiredCodeModule}`);
      }
    }
    if (!actualCodeModules.has(slide.run2_33_presentation_module_id)) {
      throw new Error(`Run 2.33 full slide ${slide.slide_id} did not call presentation synthesis module ${slide.run2_33_presentation_module_id}`);
    }
    if (!actualCodeModules.has("drawRun233ReadableEvidenceSpine")) {
      throw new Error(`Run 2.33 full slide ${slide.slide_id} did not preserve the readable evidence spine`);
    }
    if (!actualCodeModules.has("drawRun233MainSurfaceEvidenceLayer")) {
      throw new Error(`Run 2.33 full slide ${slide.slide_id} did not call the main-surface evidence layer`);
    }
    if (slide.run2_33_presentation_synthesis_execution_status !== "presentation_first_surface_rendered_with_secondary_evidence_spine") {
      throw new Error(`Run 2.33 full slide ${slide.slide_id} missing presentation-first execution status`);
    }
    if (slide.run2_32_top_next_layer_to_thicken !== "main_surface_information_density_and_visual_evidence_realism") {
      throw new Error(`Run 2.33 full slide ${slide.slide_id} did not consume the Run 2.32 main-surface visual-evidence target`);
    }
    if (!(Number(slide.run2_33_spine_min_font_size_target) >= 8)) {
      throw new Error(`Run 2.33 full slide ${slide.slide_id} did not preserve the readable spine font target`);
    }
    if (slide.run2_33_climax_style_policy !== "high_contrast_climax_with_shared_light_editorial_frame") {
      throw new Error(`Run 2.33 full slide ${slide.slide_id} did not preserve the climax style policy`);
    }
    if (!(Number(slide.run2_33_visual_evidence_object_min_target) >= 2)) {
      throw new Error(`Run 2.33 full slide ${slide.slide_id} missing visual evidence object target`);
    }
    if (
      slide.run2_33_main_surface_visual_evidence_execution_status !==
      "main_surface_information_density_and_visual_evidence_realism_thickened_before_native_ppt_generation"
    ) {
      throw new Error(`Run 2.33 full slide ${slide.slide_id} missing main-surface visual-evidence execution status`);
    }
    if ((slide.layout_metrics?.visual_evidence_objects ?? 0) < Number(slide.run2_33_visual_evidence_object_min_target)) {
      throw new Error(`Run 2.33 full slide ${slide.slide_id} did not render enough visual evidence objects`);
    }
    if (!(slide.run2_33_chain_compression_policy ?? []).includes("do_not_render_four_column_audit_table_as_primary_surface")) {
      throw new Error(`Run 2.33 full slide ${slide.slide_id} missing audit-table compression policy`);
    }
    if ((slide.layout_metrics?.workflow_objects ?? 0) < 4) {
      throw new Error(`Run 2.33 full slide ${slide.slide_id} does not expose the four-step workflow chain`);
    }
    if (slide.run2_24_selected_usecase_id !== selectedUsecaseId) {
      throw new Error(`Run 2.33 full slide ${slide.slide_id} selected the wrong usecase`);
    }
    if (slide.role === "climax" && !(slide.layout_metrics?.hero_object_canvas_share > 0.5)) {
      throw new Error("Run 2.33 climax missing large proof object stage");
    }
  }
}
...
function buildArmContract() {
  return armSpecs.map((arm) => ({
    ...
    label: arm.label,
    contract_status:
      arm.armId === "run2_33_full_main_surface_visual_evidence"
        ? "run2_33_main_surface_visual_evidence_contract_ready_requires_render_metrics"
        : "run2_33_boundary_control_contract_ready",
    allowed: arm.allowed,
...
function buildRun233FourArmSheet(built) {
  ...
  return buildNamedContactSheet(
    path.join(outRoot, "run2-33-four-arm-contact-sheet.png"),
    "Run 2.33 four-arm comparison",
    sheets,
    sheets.length,
    labels,
  );
...
async function buildArm(arm) {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "required proof objects: presentation-first surface, readable evidence spine, editable product proof object, release gate",
      "source requirements: commercial case always; full arm requires Run 2.32 spine/climax repair audit, Run 2.31 spine/climax repair rerun result, Run 2.30 presentation synthesis audit, Run 2.29 presentation synthesis, Run 2.28 evidence chain, Run 2.24 content memory, visual assets, and workflow gates before native PPT generation; bad control receives usecase label only",
  ...
  let fullData = null;
  if (arm.armId === "run2_33_full_main_surface_visual_evidence") {
    fullData = loadRun233ContractData(arm);
  }

  const slides = arm.slides.map((slide, index) =>
    arm.armId === "run2_33_full_main_surface_visual_evidence"
      ? renderRun233FullSlide(presentation, slide, arm, index + 1, fullData, metricsByRole)
  ...
  assertRun233ContentVisualGateSelfCheck(trace);
  ...
  const manifest = {
    ...
    slides: slides.map((_, index) => ({
      ...
      source: "scripts/generate_ppt_run2_33_main_surface_visual_evidence_arms.mjs",
...
function writeRun233Result(runSummary) {
  const result = {
    schema_version: 1,
    run_id: "2.33",
    status: "run2_33_main_surface_visual_evidence_rerun_public_blocked",
    public_ready: false,
    selected_usecase_id: selectedUsecaseId,
    source_audit_run_id: "2.32",
    stage_policy: "repeat_same_five_layers_not_run3",
    input_chain: {
      spine_climax_repair_audit: RUN2_33_INPUTS.spineClimaxRepairAudit,
      spine_climax_repair_rerun_result: RUN2_33_INPUTS.spineClimaxRepairRerun,
      presentation_synthesis_memory: RUN2_33_INPUTS.presentationSynthesis,
      presentation_synthesis_audit: RUN2_33_INPUTS.presentationSynthesisAudit,
      evidence_chain_view_model: RUN2_33_INPUTS.evidenceChain,
      content_memory: RUN2_33_INPUTS.contentMemory,
      visual_evidence_asset_memory: RUN2_33_INPUTS.visualAssets,
      content_visual_workflow_gates: RUN2_33_INPUTS.workflowGates,
      source_data_layer: RUN2_33_INPUTS.spineClimaxRepairAudit,
    },
    rerun: {
      generator: "scripts/generate_ppt_run2_33_main_surface_visual_evidence_arms.mjs",
      arms: armSpecs.map((arm) => arm.armId),
      best_internal_arm: "run2_33_full_main_surface_visual_evidence",
      best_internal_arm_verdict: "main_surface_information_density_and_visual_evidence_realism_thickened_before_native_ppt_generation",
    ...
    quality_delta: {
      target_layer: "main_surface_information_density_and_visual_evidence_realism",
      source_audit: RUN2_33_INPUTS.spineClimaxRepairAudit,
      source_repair_rerun: RUN2_33_INPUTS.spineClimaxRepairRerun,
      source_model: RUN2_33_INPUTS.presentationSynthesis,
      source_evidence_chain: RUN2_33_INPUTS.evidenceChain,
      chain_order: ["source evidence", "extracted design rule", "workflow decision", "generated slide surface"],
      replacement_focus: "make_main_slide_surface_carry_more_business_information_and_two_visible_visual_evidence_objects_per_slide",
      repair_modules: ["drawRun233MainSurfaceEvidenceLayer", "drawRun233VisualEvidenceStoryboard"],
      preserved_modules: ["drawRun233ReadableEvidenceSpine", "drawRun233HeroProofScene"],
      primary_surface_mode: "presentation-first surface",
      secondary_surface_mode: "readable evidence spine",
      forbidden_primary_surface: "four-column audit table",
      required_visible_chain_steps_per_full_slide: 4,
      visual_evidence_object_min_target: 2,
      source_repair_target_closed: true,
      spine_min_font_size_target: 8,
      climax_style_policy: "high_contrast_climax_with_shared_light_editorial_frame",
    },
    visual_quality_boundary:
      "main_surface_visual_evidence_proof_only_not_public_video_grade_aesthetic_or_human_release_approval",
    control_boundary: {
      bad_main_surface_visual_evidence_memory: "selected_usecase_label_only_without_run2_32_spine_climax_repair_audit_run2_31_rerun_run2_30_audit_run2_29_presentation_synthesis_run2_28_evidence_chain_run2_24_content_memory_visual_assets_or_workflow_gates",
    ...
    trace_manifest_requirements: EXPECTED_RUN2_33_TRACE_FIELDS,
    native_module_status: "actual_run2_33_presentation_module_calls_recorded_in_trace_manifest",
    ...
    next_required_action: "review_run2_33_outputs_for_main_surface_information_density_and_visual_evidence_realism_then_continue_thickening_same_five_layers",
  };
  const resultJson = path.join(root, pack, "results", "run2_33_main_surface_visual_evidence_rerun_result.json");
  const resultMd = path.join(root, pack, "results", "run2_33_main_surface_visual_evidence_rerun_result.md");
  writeJson(resultJson, result);
  fs.writeFileSync(
    resultMd,
    [
      "# Run 2.33 Main Surface Visual Evidence Rerun Result",
      "",
      "Status: rerun completed, public blocked.",
      "",
      "Run 2.33 is the generated four-arm rerun that consumes the Run 2.32 spine/climax repair audit, Run 2.31 spine/climax repair rerun result, Run 2.30 presentation synthesis audit, Run 2.29 presentation synthesis memory, Run 2.28 evidence-chain data, Run 2.24 single-usecase content memory, visual evidence asset memory, and content/visual workflow gates before native PPT code generation. It repeats the same five layers and does not advance to Run 3.0.",
      "",
      "The generator is `scripts/generate_ppt_run2_33_main_surface_visual_evidence_arms.mjs`.",
      "",
      "## Arms",
      "",
      "- `prompt_only`",
      ...
      "",
      "The negative control `bad_main_surface_visual_evidence_memory` may receive the selected usecase label, but it is blocked from reading the Run 2.32 audit, Run 2.31 rerun result, Run 2.30 audit, Run 2.29 presentation synthesis memory, Run 2.28 evidence chain data, Run 2.24 content memory, visual evidence assets, and workflow gates.",
      "",
      "## Result",
      "",
      "Best internal arm: `run2_33_full_main_surface_visual_evidence`.",
      "",
      "Verdict: `main_surface_information_density_and_visual_evidence_realism_thickened_before_native_ppt_generation`.",
      "",
      "Quality delta: `main_surface_information_density_and_visual_evidence_realism`. The full arm adds `drawRun233MainSurfaceEvidenceLayer` and `drawRun233VisualEvidenceStoryboard` so each slide must expose at least two visible visual-evidence objects while preserving the readable evidence spine and climax repair from Run 2.31.",
      "",
      "Public release remains blocked. This proves the Run 2.32 audit-driven main-surface visual-evidence thickening, not final public-video-grade visual quality or high-aesthetic human approval.",
      "",
...
async function main() {
  ...
  const fourArmSheet = buildRun233FourArmSheet(built);
  const fullSkillSeriesSheet = buildFullSkillSeriesSheet();
  const runSummary = {
    run_id: "run2_33_main_surface_visual_evidence_four_arms",
    selected_usecase_id: selectedUsecaseId,
    arms: armSpecs.map((arm) => arm.armId),
  ...
  writeJson(path.join(outRoot, "run2_33_main_surface_visual_evidence_rerun_summary.json"), runSummary);
  writeRun233Result(runSummary);
  return runSummary;
}
...
export {
  EXPECTED_RUN2_33_TRACE_FIELDS,
  RUN2_33_INPUTS,
  RUN2_33_DATA_INPUTS,
  armSpecs,
  assertRun233ArmInputBoundaries,
  assertRun233ContentVisualGateSelfCheck,
  buildArmContract,
  drawRun233LaunchField,
  drawRun233SelectedRouteMap,
  drawRun233ContentEvidenceSurface,
  ...
  drawRun233HeroProofScene,
  drawRun233DecisionHandoff,
  drawRun233EvidenceChainSurface,
  drawRun233WorkflowTracePanel,
  drawRun233ClimaxEvidenceObject,
  drawRun233ClimaxProofObject,
  loadRun233ContractData,
  main,
  readRun233PackJsonForArm,
  registerRun233Module,
  run233PresentationModuleIdForRole,
  run233RequiredModulesByRole,
  run233ContentEvidenceSurfaceGeometry,
  run233EvidenceChainSurfaceGeometry,
  selectRun233ForSlide,
  traceFor,
  ...
  validateRun233EvidenceChainViewModel,
  validateRun233PresentationSynthesisMemory,
};
