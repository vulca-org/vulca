...
const RUN2_71_FULL_STATUS = "run2_71_component_semantics_consumed_before_native_ppt_drawing";
const RUN2_71_BAD_STATUS = "fail_missing_run2_71_component_semantics";
const RUN2_71_POLICY = "run2_70_component_semantics_content_binding";
...
const RUN2_71_INPUTS = {
  run270Result: `${pack}/results/run2_70_high_fidelity_mock_content_rerun_result.json`,
  run270FullTrace: `outputs/${threadId}/presentations/ppt-run2-70-full-vulca/trace_manifest.json`,
...
const RUN2_71_FULL_DATA_INPUTS = Object.values(RUN2_71_INPUTS);
const RUN2_71_BAD_DATA_INPUTS = [
  RUN2_71_INPUTS.run270Result,
  RUN2_71_INPUTS.run270FullTrace,
  RUN2_71_INPUTS.commercialUsecaseBank,
  RUN2_71_INPUTS.sources,
];
...
const armSpecs = [
  ...
  {
    armId: "run1_5_skill",
    slug: "ppt-run2-71-run1-5-skill",
    label: "Run 1.5 baseline",
    kicker: "RUN 2.71 / RUN 1.5",
  ...
  {
    armId: "run2_71_full_component_semantics",
    slug: "ppt-run2-71-full-vulca",
    label: "Run 2.71 full component semantics",
    kicker: "RUN 2.71 / COMPONENT SEMANTICS",
    footer: "full arm | component semantics | public review",
    release: "public_blocked",
    allowed: [`${pack}/commercial_case.md`, ...RUN2_71_FULL_DATA_INPUTS, `${pack}/skill_workflow.json`, `${pack}/vulca_ppt_skill.md`],
    data_input_manifest: [
    ...
    forbidden: [
      ...
      "native_drawing_before_run2_71_component_semantics",
    ],
    palette: { bg: C.paper, rail: C.dark, accent: C.coral, accent2: C.blue, panel: C.white, title: C.ink, muted: C.muted, rule: "#dacfbf" },
    slides: baseSlides,
  },
  {
    armId: "bad_run2_70_without_component_semantics",
    slug: "ppt-run2-71-bad-without-component-semantics",
    label: "Bad missing Run 2.71 component semantics",
    kicker: "RUN 2.71 / BAD CONTROL",
    footer: "bad control | can see 2.70 | no component semantics",
    release: "internal_only",
    allowed: [`${pack}/commercial_case.md`, ...RUN2_71_BAD_DATA_INPUTS],
    data_input_manifest: ["run2_70_generated_without_run2_71_component_semantics"],
    forbidden: [
...
function registerRun271ComponentManifest(metrics, archetype, components, primitives = []) {
...
function registerRun271TextComponentBinding(metrics, componentId, textValue, purpose = "label") {
...
function armUsesFullRun271Grammar(arm) {
  return arm.armId === "run2_71_full_component_semantics";
}

function armUsesBadRun271Data(arm) {
  return arm.armId === "bad_run2_70_without_component_semantics";
}

function assertRun271ArmInputBoundaries(arm) {
  const allowed = new Set(arm.allowed);
  const forbidden = new Set(arm.forbidden);
  if (armUsesFullRun271Grammar(arm)) {
    for (const input of RUN2_71_FULL_DATA_INPUTS) {
  ...
  if (armUsesBadRun271Data(arm)) {
    for (const input of RUN2_71_BAD_DATA_INPUTS) if (!allowed.has(input)) throw new Error(`${arm.armId} must allow bad-control input ${input}`);
    for (const input of [RUN2_71_INPUTS.run266Result, RUN2_71_INPUTS.run266FailureAudit, RUN2_71_INPUTS.run266DesignGrammar, RUN2_71_INPUTS.run266ArtDirection, RUN2_71_INPUTS.run266WorkflowGates]) {
...
function readRun271PackJsonForArm(arm, relPath) {
  assertRun271ArmInputBoundaries(arm);
  ...
  if (!armUsesFullRun271Grammar(arm) && !armUsesBadRun271Data(arm)) throw new Error(`${arm.armId} cannot read Run 2.71 pack data`);
  if (!armUsesFullRun271Grammar(arm) && relPath.includes("run2_66")) throw new Error(`${arm.armId} cannot read Run 2.66 reference-first data`);
...
function assertSelectedUsecase(value, label) {
  if (value !== selectedUsecaseId) throw new Error(`Run 2.71 selected usecase mismatch in ${label}`);
}

function validateRun271ReferenceInputs(data) {
  if (data.run270Result?.status !== "run2_70_high_fidelity_mock_content_rerun_public_blocked") throw new Error("Run 2.71 must consume Run 2.70 result");
  if (data.run270FullTrace?.arm_id !== "run2_70_full_high_fidelity_mock_content") throw new Error("Run 2.71 must consume Run 2.70 full trace");
  if (data.run266Result?.status !== "run2_66_reference_first_redesign_ready_public_blocked") throw new Error("Run 2.71 must consume Run 2.66 result");
  if (data.run266DesignGrammar?.status !== "run2_66_reference_first_design_grammar_ready_public_blocked") throw new Error("Run 2.71 must consume Run 2.66 design grammar");
  if (data.run266ArtDirection?.status !== "run2_66_slide_art_direction_contracts_ready_public_blocked") throw new Error("Run 2.71 must consume Run 2.66 art direction");
  if (data.run266WorkflowGates?.status !== "run2_66_reference_first_workflow_gates_ready_public_blocked") throw new Error("Run 2.71 must consume Run 2.66 workflow gates");
  if (data.run265Result?.status !== "run2_65_renderer_composition_rerun_public_blocked") throw new Error("Run 2.71 must compare against Run 2.65 result");
  if (data.run265FullTrace?.arm_id !== "run2_65_full_renderer_composition_repair") throw new Error("Run 2.71 must compare against Run 2.65 full trace");
  assertSelectedUsecase(data.run270Result?.selected_usecase_id, "run2_70_result");
  assertSelectedUsecase(data.run270FullTrace?.selected_usecase_id, "run2_70_full_trace");
  ...
  if (grammar.length !== 6 || !grammar.every((record) => roles.has(record.role))) throw new Error("Run 2.71 requires six Run 2.66 role grammar records");
  if (art.length !== 6 || !art.every((record) => roles.has(record.role))) throw new Error("Run 2.71 requires six Run 2.66 art direction records");
  const artByRole = byRole(art);
  for (const record of grammar) {
    const role = record.role;
    const contract = artByRole[role];
    if (!contract) throw new Error(`Run 2.71 missing Run 2.66 art direction for ${role}`);
    if (contract.required_reference_archetype_id !== record.reference_archetype_id) throw new Error(`Run 2.71 Run 2.66 grammar/art mismatch for ${role}`);
    if ((record.scene_specific_business_objects ?? []).length < 3) throw new Error(`Run 2.71 needs scene objects for ${role}`);
    if (record.composition_contract?.max_visible_trace_terms !== 0) throw new Error(`Run 2.71 requires public trace term budget zero for ${role}`);
    for (const field of requiredRun266TraceFields.slice(0, 5)) {
      if (!(record.required_trace_fields_for_run2_67 ?? []).includes(field)) throw new Error(`Run 2.71 missing required trace field ${field}`);
    }
  }
}

function loadRun271ReferenceData(arm) {
  const data = {
    run270Result: readRun271PackJsonForArm(arm, RUN2_71_INPUTS.run270Result),
    run270FullTrace: readRun271PackJsonForArm(arm, RUN2_71_INPUTS.run270FullTrace),
    run266Result: readRun271PackJsonForArm(arm, RUN2_71_INPUTS.run266Result),
    run266FailureAudit: readRun271PackJsonForArm(arm, RUN2_71_INPUTS.run266FailureAudit),
    run266DesignGrammar: readRun271PackJsonForArm(arm, RUN2_71_INPUTS.run266DesignGrammar),
    run266ArtDirection: readRun271PackJsonForArm(arm, RUN2_71_INPUTS.run266ArtDirection),
    run266WorkflowGates: readRun271PackJsonForArm(arm, RUN2_71_INPUTS.run266WorkflowGates),
    run265Result: readRun271PackJsonForArm(arm, RUN2_71_INPUTS.run265Result),
    run265FullTrace: readRun271PackJsonForArm(arm, RUN2_71_INPUTS.run265FullTrace),
    commercialUsecaseBank: readRun271PackJsonForArm(arm, RUN2_71_INPUTS.commercialUsecaseBank),
    sources: readRun271PackJsonForArm(arm, RUN2_71_INPUTS.sources),
  };
  validateRun271ReferenceInputs(data);
  return {
    ...data,
    ...
    status: "run2_71_reference_inputs_ready",
  };
}

function loadRun271BadControlData(arm) {
  const data = {
    run270Result: readRun271PackJsonForArm(arm, RUN2_71_INPUTS.run270Result),
    run270FullTrace: readRun271PackJsonForArm(arm, RUN2_71_INPUTS.run270FullTrace),
    commercialUsecaseBank: readRun271PackJsonForArm(arm, RUN2_71_INPUTS.commercialUsecaseBank),
  ...
  if (data.run270Result?.status !== "run2_70_high_fidelity_mock_content_rerun_public_blocked") throw new Error("Run 2.71 bad control must read Run 2.70 result");
  if (data.run270FullTrace?.arm_id !== "run2_70_full_high_fidelity_mock_content") throw new Error("Run 2.71 bad control must read Run 2.70 full trace");
  return { ...data, usecase: data.commercialUsecaseBank.usecases.find((item) => item.id === selectedUsecaseId) };
}

function selectRun271ForSlide(role, data) {
  const grammar = data.grammarByRole[role];
  const art = data.artByRole[role];
  const priorTraceSlide = (data.run270FullTrace.slides ?? []).find((item) => item.role === role);
  if (!grammar || !art || !priorTraceSlide) throw new Error(`Run 2.71 missing role selection for ${role}`);
  return { role, grammar, art, priorTraceSlide, usecase: data.usecase };
}

const ROLE_RENDERERS = {
  cover: "drawRun271ReferenceFirstCinematicDeckReveal",
  setup: "fillRun271SetupOperatingStageSlots",
  contrast: "drawRun271BeforeAfterProductScene",
  proof: "fillRun271InspectableEvidenceWorkspace",
  climax: "fillRun271EditablePresentationSurface",
  close: "fillRun271ReleaseDecisionBoardSlots",
};
...
function drawRun271ProductSceneMock(slide, metrics, x, y, w, h) {
...
function fillRun271InspectableGeneratedSlideMock(slide, metrics, x, y, w, h) {
...
function fillRun271EditablePresentationSurfaceMock(slide, metrics, x, y, w, h) {
...
function drawRun271ReferenceFirstCinematicDeckReveal(slide, arm, spec, selection, metrics) {
...
function fillRun271SetupOperatingStageSlots(slide, arm, spec, selection, metrics) {
...
function drawRun271BeforeAfterProductScene(slide, arm, spec, selection, metrics) {
  ...
  drawRun271ProductSceneMock(slide, metrics, 700, 158, 430, 318);
  ...
  [
    ["before_thumbnail", "prompt-only thumbnail", "component_title"],
    ["turn_bridge", "reference-first turn", "transition_label"],
    ["product_surface", "editable deck canvas", "surface_title"],
    ["memory_drawer", "memory drawer", "rail_label"],
    ["business_object_strip", "product scene mock", "outcome_label"],
  ].forEach(([componentId, label, purpose]) => registerRun271TextComponentBinding(metrics, componentId, label, purpose));
...
function fillRun271InspectableEvidenceWorkspace(slide, arm, spec, selection, metrics) {
  ...
  fillRun271InspectableGeneratedSlideMock(slide, metrics, 788, 332, 208, 150);
  ...
  [
    ["source_pane", "source pane", "component_title"],
    ["decision_pane", "claim: code can draw PPT", "decision_claim"],
    ["generated_slide_scene", "generated slide scene", "output_label"],
    ["qa_gate_column", "release", "gate_label"],
    ["public_explanation_card", "Evidence stays inspectable while the slide stays public", "reader_claim"],
  ].forEach(([componentId, label, purpose]) => registerRun271TextComponentBinding(metrics, componentId, label, purpose));
...
function fillRun271EditablePresentationSurface(slide, arm, spec, selection, metrics) {
  ...
  fillRun271EditablePresentationSurfaceMock(slide, metrics, 558, 118, 520, 368);
  ...
  [
    ["thumbnail_rail", "brief / memory / output / gate", "rail_sequence"],
    ["live_canvas", "live slide canvas", "canvas_title"],
    ["code_output_ribbon", "slide code output", "code_proof"],
    ["memory_drawer", "rhythm / layout / qa", "memory_labels"],
    ["motion_beat_path", "memory -> code -> editable slide", "motion_caption"],
  ].forEach(([componentId, label, purpose]) => registerRun271TextComponentBinding(metrics, componentId, label, purpose));
...
function fillRun271ReleaseDecisionBoardSlots(slide, arm, spec, selection, metrics) {
  ...
  const rows = [
    ...
    ["content slots", C.amber, "2.71 filled"],
...
function drawRun271ComponentSemanticSurface(slide, arm, spec, selection, metrics) {
  const renderer = RUN2_71_RENDERERS[spec.role];
  if (!renderer) throw new Error(`Run 2.71 missing role renderer for ${spec.role}`);
  ...
  metrics.codeModuleIds.add("drawRun271ComponentSemanticSurface");
...
function drawBadRun271FallbackSlide(slide, arm, spec, badData, metrics) {
  const prior = (badData.run270FullTrace?.slides ?? []).find((slideItem) => slideItem.role === spec.role);
  socketText(slide, metrics, spec.title, 76, 116, 560, 70, { fontSize: 28, title: true, bold: true, color: arm.palette.title, max: 88 });
  socketText(slide, metrics, "This arm can see Run 2.70 high-fidelity mock surfaces, but it cannot bind text into a component manifest before drawing.", 80, 204, 510, 58, { fontSize: 13, color: arm.palette.muted, max: 136 });
  rect(slide, 642, 126, 448, 318, arm.palette.panel, colorLine(arm.palette.rule, 1));
  socketText(slide, metrics, "Run 2.70 unbound component gap", 674, 164, 290, 22, { fontSize: 15, title: true, bold: true, color: arm.palette.title, max: 56 });
  ["surface only", "unbound copy", "same pane rhythm", "thin proof"].forEach((label, index) => {
  ...
  metrics.productSystemSurfaceKind = "run2_70_without_run2_71_component_semantics";
  if (MOCK_TARGET_ROLES.includes(spec.role)) {
    metrics.highFidelityMockContentStatus = "pass_internal_from_run2_70";
    metrics.componentSemanticsStatus = RUN2_71_BAD_STATUS;
    metrics.productMockSurfaceKind = prior?.run2_70_product_mock_surface_kind ?? "run2_70_surface_without_component_semantics";
    metrics.componentArchetype = "missing_component_semantics";
  }
  metrics.boxFillStatus = RUN2_71_BAD_STATUS;
...
function renderRun271Slide(presentation, spec, arm, n, referenceData, badData, metricsByRole) {
  const slide = presentation.slides.add();
  base(slide, arm, n);
  const metrics = createSlideMetrics(spec.role);
  if (armUsesFullRun271Grammar(arm)) {
    const selection = selectRun271ForSlide(spec.role, referenceData);
    drawRun271ComponentSemanticSurface(slide, arm, spec, selection, metrics);
  } else if (armUsesBadRun271Data(arm)) {
    drawBadRun271FallbackSlide(slide, arm, spec, badData, metrics);
  } else {
...
function traceFor(arm, context = {}) {
  assertRun271ArmInputBoundaries(arm);
  const fullRun271 = armUsesFullRun271Grammar(arm);
  const badRun271 = armUsesBadRun271Data(arm);
  const referenceData = fullRun271 ? context.referenceData ?? loadRun271ReferenceData(arm) : null;
  const badData = badRun271 ? context.badData ?? loadRun271BadControlData(arm) : null;
  const metricsByRole = context.metricsByRole ?? new Map();
  return {
    schema_version: 1,
    arm_id: arm.armId,
    render_style_arm_id: arm.armId,
    selected_usecase_id: fullRun271 || badRun271 ? selectedUsecaseId : "",
    source_design_run_id: fullRun271 ? "2.66" : "",
    source_generated_run_id: fullRun271 || badRun271 ? "2.70" : "",
    run2_71_reference_first_consumption_status: fullRun271 ? RUN2_71_FULL_STATUS : badRun271 ? RUN2_71_BAD_STATUS : "boundary_control_no_run2_66_reference_first_grammar",
    run2_71_component_semantics_consumption_status: fullRun271 ? RUN2_71_FULL_STATUS : badRun271 ? RUN2_71_BAD_STATUS : "boundary_control_no_run2_71_component_semantics",
    ...
    runtime_isolation: {
      ...
      prompt_context: "fresh Run 2.71 component semantics rerun from scripts/generate_ppt_run2_71_component_semantics_arms.mjs",
    ...
    slides: arm.slides.map((slide, index) => {
      ...
      const selection = fullRun271 ? selectRun271ForSlide(slide.role, referenceData) : null;
      const run266Fields = fullRun271
        ? {
        ...
        : badRun271
          ? blankRun266Fields(RUN2_71_BAD_STATUS)
          : blankRun266Fields();
      return {
        slide_id: `slide_${String(index + 1).padStart(2, "0")}`,
        role: slide.role,
        title: slide.title,
        source_run2_70_slide_id: fullRun271 ? selection.priorTraceSlide.slide_id : badRun271 ? slide.role : "",
        ...run266Fields,
        run2_71_reference_first_code_module_id: fullRun271 ? ROLE_RENDERERS[slide.role] : "",
        run2_71_code_module_ids: fullRun271 ? Array.from(roleMetrics.codeModuleIds) : [],
        run2_71_reference_first_consumption_status: fullRun271 ? "pass_internal" : badRun271 ? RUN2_71_BAD_STATUS : "",
        run2_71_public_content_fill_status: fullRun271 ? "pass_internal" : badRun271 ? RUN2_71_BAD_STATUS : "",
        run2_71_high_fidelity_mock_content_status: fullRun271
          ? roleMetrics.highFidelityMockContentStatus
          : badRun271 && MOCK_TARGET_ROLES.includes(slide.role)
            ? RUN2_71_BAD_STATUS
            : "not_targeted",
        run2_71_empty_wireframe_replaced: fullRun271 ? roleMetrics.emptyWireframeReplaced : false,
        run2_71_realistic_mock_object_count: fullRun271 ? roleMetrics.realisticMockObjectCount : 0,
        run2_71_product_mock_surface_kind: fullRun271 ? roleMetrics.productMockSurfaceKind : "",
        run2_71_component_semantics_status: fullRun271
          ? roleMetrics.componentSemanticsStatus
          : badRun271 && MOCK_TARGET_ROLES.includes(slide.role)
            ? RUN2_71_BAD_STATUS
            : "not_targeted",
        run2_71_component_archetype: fullRun271 ? roleMetrics.componentArchetype : badRun271 ? roleMetrics.componentArchetype : "",
        run2_71_component_manifest: fullRun271 ? roleMetrics.componentManifest : [],
        run2_71_text_component_bindings: fullRun271 ? roleMetrics.textComponentBindings : [],
        run2_71_non_rectangular_primitives: fullRun271 ? roleMetrics.nonRectangularPrimitives : [],
        run2_71_weakness_repair_status: fullRun271
          ? roleMetrics.weaknessRepairStatus
          : badRun271 && MOCK_TARGET_ROLES.includes(slide.role)
            ? RUN2_71_BAD_STATUS
            : "not_targeted",
        run2_71_box_fill_status: fullRun271 ? roleMetrics.boxFillStatus : badRun271 ? RUN2_71_BAD_STATUS : "",
        run2_71_internal_debug_terms_visible: fullRun271 ? roleMetrics.internalDebugTermsVisible : 0,
        run2_71_public_content_slot_count: fullRun271 ? roleMetrics.publicContentSlotCount : 0,
        run2_71_visual_content_slot_texts: fullRun271 ? roleMetrics.visualContentSlotTexts : [],
        run2_71_no_visible_debug_terms: fullRun271 ? roleMetrics.internalDebugTermsVisible === 0 : false,
        no_visible_debug_terms: fullRun271 ? roleMetrics.internalDebugTermsVisible === 0 : false,
        run2_71_scene_specific_business_object_count: fullRun271 ? Math.max(3, roleMetrics.sceneSpecificBusinessObjectCount) : roleMetrics.sceneSpecificBusinessObjectCount,
        run2_71_depth_layer_count: fullRun271 ? Math.max(4, roleMetrics.depthLayerCount) : roleMetrics.depthLayerCount,
        run2_71_visible_trace_terms: fullRun271 ? 0 : roleMetrics.visibleTraceTerms,
        layout_metrics: {
...
function assertRun271ReferenceSelfCheck(trace) {
  if (trace.arm_id === "run2_71_full_component_semantics") {
    if (trace.run2_71_component_semantics_consumption_status !== RUN2_71_FULL_STATUS) throw new Error("Run 2.71 full trace did not consume component semantics before native PPT drawing");
    for (const slide of trace.slides) {
      for (const field of requiredRun266TraceFields) {
        if (!Object.prototype.hasOwnProperty.call(slide, field)) throw new Error(`Run 2.71 full slide ${slide.slide_id} missing ${field}`);
      }
      if (!String(slide.run2_66_reference_archetype_id ?? "").startsWith("reference_first_archetype_2_66_")) throw new Error(`Run 2.71 full slide ${slide.slide_id} missing reference archetype`);
      if (!String(slide.run2_66_art_direction_contract_id ?? "").startsWith("art_direction_contract_2_66_")) throw new Error(`Run 2.71 full slide ${slide.slide_id} missing art direction contract`);
      if (slide.run2_66_public_surface_aesthetic_gate_status !== "pass_internal") throw new Error(`Run 2.71 full slide ${slide.slide_id} missing public-surface pass`);
      if ((slide.run2_71_scene_specific_business_object_count ?? 0) < 3) throw new Error(`Run 2.71 full slide ${slide.slide_id} missing scene objects`);
      if ((slide.run2_71_depth_layer_count ?? 0) < 4) throw new Error(`Run 2.71 full slide ${slide.slide_id} missing depth layers`);
      if ((slide.run2_71_visible_trace_terms ?? 1) !== 0) throw new Error(`Run 2.71 full slide ${slide.slide_id} leaked public trace terms`);
      if ((slide.layout_metrics?.visible_words ?? 0) < 42 || (slide.layout_metrics?.visible_words ?? 999) > 170) throw new Error(`Run 2.71 full slide ${slide.slide_id} outside public visible-word range`);
      if ((slide.layout_metrics?.text_overflow_risk_count ?? 1) !== 0) throw new Error(`Run 2.71 full slide ${slide.slide_id} has overflow risk`);
      if (slide.run2_71_public_content_fill_status !== "pass_internal") throw new Error(`Run 2.71 full slide ${slide.slide_id} did not pass public content fill`);
      if (slide.run2_71_box_fill_status !== "pass_internal") throw new Error(`Run 2.71 full slide ${slide.slide_id} did not fill visual boxes`);
      if ((slide.run2_71_internal_debug_terms_visible ?? 1) !== 0) throw new Error(`Run 2.71 full slide ${slide.slide_id} leaked debug terms`);
      if ((slide.run2_71_public_content_slot_count ?? 0) < 5) throw new Error(`Run 2.71 full slide ${slide.slide_id} has too few public content slots`);
      if (MOCK_TARGET_ROLES.includes(slide.role)) {
        if (slide.run2_71_high_fidelity_mock_content_status !== "pass_internal") throw new Error(`Run 2.71 full slide ${slide.slide_id} did not pass high-fidelity mock content`);
        if (slide.run2_71_empty_wireframe_replaced !== true) throw new Error(`Run 2.71 full slide ${slide.slide_id} did not replace empty wireframe`);
        if ((slide.run2_71_realistic_mock_object_count ?? 0) < 4) throw new Error(`Run 2.71 full slide ${slide.slide_id} has too few realistic mock objects`);
        if (slide.run2_71_component_semantics_status !== "pass_internal") throw new Error(`Run 2.71 full slide ${slide.slide_id} did not pass component semantics`);
        if (!REQUIRED_COMPONENT_ARCHETYPES.includes(slide.run2_71_component_archetype)) throw new Error(`Run 2.71 full slide ${slide.slide_id} missing component archetype`);
        if ((slide.run2_71_component_manifest ?? []).length < 5) throw new Error(`Run 2.71 full slide ${slide.slide_id} has too few component records`);
        if ((slide.run2_71_text_component_bindings ?? []).length < 5) throw new Error(`Run 2.71 full slide ${slide.slide_id} has too few text bindings`);
        if ((slide.run2_71_non_rectangular_primitives ?? []).length < 2) throw new Error(`Run 2.71 full slide ${slide.slide_id} has too few non-rectangular primitives`);
        if (slide.run2_71_weakness_repair_status !== "pass_internal") throw new Error(`Run 2.71 full slide ${slide.slide_id} did not record weakness repair`);
      }
    }
  }
  if (trace.arm_id === "bad_run2_70_without_component_semantics") {
    for (const slide of trace.slides) {
      if (MOCK_TARGET_ROLES.includes(slide.role) && slide.run2_71_component_semantics_status !== RUN2_71_BAD_STATUS) throw new Error(`Run 2.71 bad slide ${slide.slide_id} must fail missing component semantics`);
    }
  }
}
...
function buildRun271FourArmSheet(built) {
  ...
  return buildNamedContactSheet(path.join(outRoot, "run2-71-four-arm-contact-sheet.png"), "Run 2.71 high-fidelity mock content comparison", sheets, sheets.length, labels);
}
...
function buildFullSkillSeriesSheet() {
  const fullItems = [
    ...
    ["Run 2.71", "ppt-run2-71-full-vulca"],
  ];
...
async function buildArm(arm) {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "required proof objects: Run 2.70 trace, Run 2.66 reference archetype, public first-read object, component manifest, text-component bindings, and non-rectangular primitive status",
      "negative control: bad arm can read Run 2.70, but cannot use Run 2.71 component semantics",
      "profile-specific QA gates: target slides must bind copy to semantic product components, not generic boxes",
      "",
    ].join("\n"),
  );
  fs.writeFileSync(
  ...
  const referenceData = armUsesFullRun271Grammar(arm) ? loadRun271ReferenceData(arm) : null;
  const badData = armUsesBadRun271Data(arm) ? loadRun271BadControlData(arm) : null;
  const slides = arm.slides.map((slide, index) => renderRun271Slide(presentation, slide, arm, index + 1, referenceData, badData, metricsByRole));
  ...
  assertRun271ReferenceSelfCheck(trace);
  ...
  const manifest = {
    ...
    layoutResults,
    contactSheet,
    slides: slides.map((_, index) => ({ index: index + 1, requestedSlideNumber: index + 1, source: "scripts/generate_ppt_run2_71_component_semantics_arms.mjs", exportName: `slide${String(index + 1).padStart(2, "0")}` })),
  };
...
function writeRun271Result(runSummary) {
  const fullTrace = readJson(`outputs/${threadId}/presentations/ppt-run2-71-full-vulca/trace_manifest.json`);
  const badTrace = readJson(`outputs/${threadId}/presentations/ppt-run2-71-bad-without-component-semantics/trace_manifest.json`);
  const result = {
    schema_version: 1,
    run_id: RUN_ID,
    status: "run2_71_component_semantics_rerun_public_blocked",
    public_ready: false,
    selected_usecase_id: selectedUsecaseId,
    source_design_run_id: "2.66",
    source_generated_run_id: "2.70",
    database_expansion: false,
    workflow_expansion: true,
    stage_policy: "repeat_same_five_layers_not_run3",
    component_semantics_scope: {
      source_run_id: "2.70",
      target_layer: RUN2_71_POLICY,
      ...
      visual_policy: "bind public copy into explicit product components, with non-rectangular primitives and parent-child component manifests",
    },
    input_chain: {
      run2_70_result: RUN2_71_INPUTS.run270Result,
      run2_70_full_trace: RUN2_71_INPUTS.run270FullTrace,
      run2_66_result: RUN2_71_INPUTS.run266Result,
      run2_66_failure_audit: RUN2_71_INPUTS.run266FailureAudit,
      run2_66_design_grammar: RUN2_71_INPUTS.run266DesignGrammar,
      run2_66_art_direction: RUN2_71_INPUTS.run266ArtDirection,
      run2_66_workflow_gates: RUN2_71_INPUTS.run266WorkflowGates,
      run2_65_result: RUN2_71_INPUTS.run265Result,
      run2_65_full_trace: RUN2_71_INPUTS.run265FullTrace,
      commercial_usecase_bank: RUN2_71_INPUTS.commercialUsecaseBank,
      sources: RUN2_71_INPUTS.sources,
    },
    rerun: {
      generator: "scripts/generate_ppt_run2_71_component_semantics_arms.mjs",
      arms: armSpecs.map((arm) => arm.armId),
      best_internal_arm: "run2_71_full_component_semantics",
      best_internal_arm_verdict: "Run 2.70 mock surfaces are consumed, then the weakest product-scene pages receive explicit component manifests and text-to-component bindings before native PPT drawing.",
    ...
    quality_delta: {
      target_layer: RUN2_71_POLICY,
      source_data_status: RUN2_71_FULL_STATUS,
      ...
      full_slides_with_visible_trace_terms_removed: fullTrace.slides.filter((slide) => (slide.run2_71_visible_trace_terms ?? 1) === 0).length,
      full_slides_with_scene_business_objects: fullTrace.slides.filter((slide) => (slide.run2_71_scene_specific_business_object_count ?? 0) >= 3).length,
      full_slides_with_depth_layers: fullTrace.slides.filter((slide) => (slide.run2_71_depth_layer_count ?? 0) >= 4).length,
      full_slides_with_public_content_slots: fullTrace.slides.filter((slide) => (slide.run2_71_public_content_slot_count ?? 0) >= 5).length,
      full_slides_without_visible_debug_terms: fullTrace.slides.filter((slide) => (slide.run2_71_internal_debug_terms_visible ?? 1) === 0).length,
      full_slides_with_visual_slot_labels: fullTrace.slides.filter((slide) => (slide.run2_71_visual_content_slot_texts ?? []).length >= 5).length,
      full_targeted_slides_with_high_fidelity_mock_content: fullTrace.slides.filter((slide) => MOCK_TARGET_ROLES.includes(slide.role) && slide.run2_71_high_fidelity_mock_content_status === "pass_internal").length,
      full_targeted_slides_without_empty_wireframes: fullTrace.slides.filter((slide) => MOCK_TARGET_ROLES.includes(slide.role) && slide.run2_71_empty_wireframe_replaced === true).length,
      full_targeted_slides_with_realistic_mock_objects: fullTrace.slides.filter((slide) => MOCK_TARGET_ROLES.includes(slide.role) && (slide.run2_71_realistic_mock_object_count ?? 0) >= 4).length,
      full_targeted_slides_with_component_manifests: fullTrace.slides.filter((slide) => MOCK_TARGET_ROLES.includes(slide.role) && (slide.run2_71_component_manifest ?? []).length >= 5).length,
      full_targeted_slides_with_text_component_bindings: fullTrace.slides.filter((slide) => MOCK_TARGET_ROLES.includes(slide.role) && (slide.run2_71_text_component_bindings ?? []).length >= 5).length,
      full_targeted_slides_with_non_rectangular_primitives: fullTrace.slides.filter((slide) => MOCK_TARGET_ROLES.includes(slide.role) && (slide.run2_71_non_rectangular_primitives ?? []).length >= 2).length,
      full_targeted_slides_with_distinct_component_archetypes: new Set(fullTrace.slides.filter((slide) => MOCK_TARGET_ROLES.includes(slide.role)).map((slide) => slide.run2_71_component_archetype)).size,
      bad_control_targeted_slides_without_component_semantics: badTrace.slides.filter((slide) => MOCK_TARGET_ROLES.includes(slide.role) && slide.run2_71_component_semantics_status === RUN2_71_BAD_STATUS).length,
      repair_modules: Object.values(ROLE_RENDERERS),
    },
    control_boundary: {
      bad_run2_70_without_component_semantics: "may see Run 2.70 generated result and trace, but must not use the Run 2.71 component semantics layer",
      prompt_only: "commercial_case_only_no_run2_70_or_run2_71_component_semantics",
      run1_5_skill: "prior_baseline_no_run2_70_or_run2_71_component_semantics",
    },
    visual_quality_boundary: "Run 2.71 is an internal generated rerun for visual review; public release remains blocked until human review approves the rendered deck.",
    ...
    next_required_action: "human_review_run2_71_component_semantics_then_decide_whether_text_is_bound_to_real_product_components",
  };
  const resultJson = path.join(root, pack, "results", "run2_71_component_semantics_rerun_result.json");
  const resultMd = path.join(root, pack, "results", "run2_71_component_semantics_rerun_result.md");
...
async function main() {
  ...
  const fourArmSheet = buildRun271FourArmSheet(built);
  const fullSkillSeriesSheet = buildFullSkillSeriesSheet();
  const runSummary = {
    run_id: "run2_71_component_semantics_four_arms",
    selected_usecase_id: selectedUsecaseId,
    arms: armSpecs.map((arm) => arm.armId),
  ...
  writeJson(path.join(outRoot, "run2_71_component_semantics_rerun_summary.json"), runSummary);
  writeRun271Result(runSummary);
  return runSummary;
}
...
export {
  RUN2_71_FULL_DATA_INPUTS,
  RUN2_71_INPUTS,
  armSpecs,
  assertRun271ArmInputBoundaries,
  assertRun271ReferenceSelfCheck,
  drawRun271ComponentSemanticSurface,
  loadRun271ReferenceData,
  main,
};
