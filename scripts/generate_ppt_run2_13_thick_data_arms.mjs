...
const EXPECTED_RUN2_13_TRACE_FIELDS = [
...
const RUN2_13_RESTRICTED_INPUTS = [RUN2_13_INPUTS.evidence, RUN2_13_INPUTS.memory, RUN2_13_INPUTS.gateSeed];

const baseSlides = [
  {
    role: "cover",
    title: "The deck stops looking like a report.",
    claim: "Run 2.13 converts tutorial learning into thick data before native PPT code is written.",
    trace: "commercial_case -> thick_data -> native PPT",
  },
  {
    role: "setup",
...
const armSpecs = [
  {
    armId: "prompt_only",
    slug: "ppt-run2-13-prompt-only",
    label: "Prompt-only control",
    kicker: "RUN 2.13 / CONTROL",
  ...
  {
    armId: "run1_5_skill",
    slug: "ppt-run2-13-run1-5-skill",
    label: "Run 1.5 baseline",
    kicker: "RUN 2.13 / RUN 1.5",
  ...
  {
    armId: "run2_13_full_skill",
    slug: "ppt-run2-13-full-vulca",
    label: "Run 2.13 full Vulca skill",
    kicker: "RUN 2.13 / FULL VULCA",
    footer: "run2_13_full_skill | 2.12 evidence + memory + workflow gate | public blocked",
    release: "public_blocked",
    allowed: [
  ...
  {
    armId: "bad_thick_data_memory",
    slug: "ppt-run2-13-bad-thick-data-memory",
    label: "Bad thick-data boundary control",
    kicker: "RUN 2.13 / NEGATIVE CONTROL",
    footer: "bad_thick_data_memory | sources only | internal comparison",
    ...
    forbidden: [
      ...
      `${pack}/skill_workflow.json`,
      `${pack}/results/trace_manifest_contract.json`,
      "manual Run 2.13 visual module repair before scoring",
    ],
    palette: {
      bg: "#f2efe1",
      rail: "#6d633e",
...
function registerRun213Module(metrics, functionName) {
...
function drawRun213TypeWhitespaceSystem(slide, arm, spec, module, metrics, opts = {}) {
  registerRun213Module(metrics, "drawRun213TypeWhitespaceSystem");
...
function drawRun213LaunchArcRoute(slide, arm, spec, module, metrics, opts = {}) {
  registerRun213Module(metrics, "drawRun213LaunchArcRoute");
  const x = opts.x ?? 80;
  const y = opts.y ?? 118;
  const w = opts.w ?? 1040;
  const h = opts.h ?? 430;
...
function drawRun213ProductTheater(slide, arm, spec, module, metrics, opts = {}) {
  registerRun213Module(metrics, "drawRun213ProductTheater");
  const x = opts.x ?? 604;
  const y = opts.y ?? 134;
  const w = opts.w ?? 504;
  const h = opts.h ?? 352;
...
function drawRun213WorkflowGateRail(slide, arm, spec, module, metrics, opts = {}) {
  registerRun213Module(metrics, "drawRun213WorkflowGateRail");
  const x = opts.x ?? 104;
  const y = opts.y ?? 352;
...
function drawRun213ProductDemoSequence(slide, arm, spec, module, metrics, opts = {}) {
  registerRun213Module(metrics, "drawRun213ProductDemoSequence");
  const x = opts.x ?? 100;
  const y = opts.y ?? 430;
  const w = opts.w ?? 880;
  const h = opts.h ?? 126;
...
function drawRun213MetricClimaxObject(slide, arm, spec, module, metrics, opts = {}) {
  registerRun213Module(metrics, "drawRun213MetricClimaxObject");
...
function assertArmInputBoundaries(arm) {
  ...
  if (arm.armId === "run2_13_full_skill") {
    for (const input of RUN2_13_RESTRICTED_INPUTS) assertAllowed(input);
    return;
  }
  if (arm.armId === "bad_thick_data_memory") {
    assertAllowed(RUN2_13_INPUTS.evidence);
    assertForbidden(RUN2_13_INPUTS.memory);
    assertForbidden(RUN2_13_INPUTS.gateSeed);
    assertForbidden(`${pack}/skill_workflow.json`);
    return;
  }
  for (const input of RUN2_13_RESTRICTED_INPUTS) assertForbidden(input);
}

function readRun213JsonForArm(arm, relPath) {
  assertArmInputBoundaries(arm);
  if (arm.armId !== "run2_13_full_skill") {
    throw new Error(`${arm.armId} cannot read Run 2.13 visual-module input: ${relPath}`);
  }
...
function readRun213EvidenceForArm(arm) {
  assertArmInputBoundaries(arm);
  if (!arm.allowed.includes(RUN2_13_INPUTS.evidence) || arm.forbidden.includes(RUN2_13_INPUTS.evidence)) {
    throw new Error(`${arm.armId} input boundary does not permit reading ${RUN2_13_INPUTS.evidence}`);
  }
  return readJson(RUN2_13_INPUTS.evidence);
}

function run213WorkflowGateSeedsByRole(gateSeed) {
  const map = new Map();
...
function run213EvidenceRecordsById(evidence) {
  return new Map((evidence?.records ?? []).map((record) => [record.id, record]));
}

function run213MemorySeedsById(memory) {
  return new Map((memory?.memory_seeds ?? []).map((seed) => [seed.id, seed]));
}

function run213EvidenceRecordsByRole(evidence, gateSeed) {
  const evidenceById = run213EvidenceRecordsById(evidence);
  const map = new Map();
  for (const gate of gateSeed?.gates ?? []) {
    for (const role of gate.slide_roles ?? []) {
      const bucket = map.get(role) ?? [];
...
function run213MemorySeedsByRole(memory) {
  const map = new Map();
...
function run213CodeModulesByRole() {
  return new Map(
    Object.entries({
      cover: ["drawRun213LaunchArcRoute", "drawRun213TypeWhitespaceSystem"],
      setup: ["drawRun213LaunchArcRoute", "drawRun213WorkflowGateRail"],
      contrast: ["drawRun213LaunchArcRoute", "drawRun213WorkflowGateRail"],
      proof: ["drawRun213ProductDemoSequence", "drawRun213WorkflowGateRail"],
      climax: ["drawRun213MetricClimaxObject", "drawRun213ProductDemoSequence"],
      close: ["drawRun213TypeWhitespaceSystem", "drawRun213WorkflowGateRail"],
    }),
  );
}

function run213ShapeBudgetByRole() {
  const baseBudget = {
    max_text_boxes: 18,
...
function run213VisualDeltaByRole() {
  return new Map(
    Object.entries({
      cover: "2.12 launch-arc memory creates one route plus type system instead of the Run 2.10 field/card hybrid",
      setup: "2.12 workflow gate rail exposes selection order before any feature-like surface appears",
      contrast: "2.12 typography and whitespace memory compresses before/after into one route",
      proof: "2.12 demo-sequence memory renders command, route, progress, and resolved state as native objects",
      climax: "2.12 metric memory gives one resolved state the protected canvas instead of another evidence grid",
      close: "2.12 gate seed turns the ending into a release handoff instead of a checklist",
    }),
  );
}
...
function loadRun213ContractData(arm) {
  const evidence = readRun213JsonForArm(arm, RUN2_13_INPUTS.evidence);
  const memory = readRun213JsonForArm(arm, RUN2_13_INPUTS.memory);
  const gateSeed = readRun213JsonForArm(arm, RUN2_13_INPUTS.gateSeed);
  const workflowGateSeedsByRole = run213WorkflowGateSeedsByRole(gateSeed);
  const evidenceRecordsByRole = run213EvidenceRecordsByRole(evidence, gateSeed);
  const memorySeedsByRole = run213MemorySeedsByRole(memory);
  const codeModulesByRole = run213CodeModulesByRole();
  const shapeBudgetByRole = run213ShapeBudgetByRole();
  const visualDeltaByRole = run213VisualDeltaByRole();
  ...
  return {
    ...
    densityGate,
    demoGate,
    status: "run2_13_thick_data_contract_ready",
  };
}

function renderRun213Full(arm) {
  const contract = loadRun213ContractData(arm);
  return {
    ...contract,
    status: "run2_13_thick_data_contract_ready",
  };
...
function renderRun213FullSlide(presentation, spec, arm, n, fullData, metricsByRole) {
  const slide = presentation.slides.add();
  base(slide, arm, n);
  const metrics = createSlideMetrics(spec.role);

  if (spec.role === "cover") {
    drawRun213TypeWhitespaceSystem(slide, arm, spec, null, metrics, {
      x: 82,
      y: 120,
      w: 760,
      h: 240,
    ...
    drawRun213LaunchArcRoute(slide, arm, spec, null, metrics, {
      x: 76,
      y: 382,
      w: 1020,
      h: 176,
      headline: "sources -> memory -> code",
      note: "The thick data must change structure, not only palette or box count.",
      headlineSize: 28,
    ...
    drawRun213LaunchArcRoute(slide, arm, spec, null, metrics, {
      ...
      w: 496,
      h: 330,
      headline: "memory before render",
      note: "2.12 gate requires selection before native code.",
      headlineSize: 34,
      railLabels: ["evidence", "memory", "render"],
    });
    drawRun213WorkflowGateRail(slide, arm, spec, null, metrics, {
      x: 88,
      y: 382,
    ...
    drawRun213LaunchArcRoute(slide, arm, spec, null, metrics, {
      ...
      headlineSize: 42,
      railLabels: ["Run 2.9", "failure", "Run 2.13"],
    });
    drawRun213WorkflowGateRail(slide, arm, spec, null, metrics, {
      x: 168,
      y: 372,
    ...
    drawRun213WorkflowGateRail(slide, arm, spec, null, metrics, {
      x: 86,
      y: 120,
      caption: "gate rail keeps the demo from becoming equal cards",
    });
    drawRun213ProductDemoSequence(slide, arm, spec, null, metrics, {
      x: 666,
      y: 372,
      w: 444,
      h: 126,
    ...
    drawRun213MetricClimaxObject(slide, arm, spec, null, metrics);
    drawRun213ProductDemoSequence(slide, arm, spec, null, metrics, {
      x: 236,
      y: 530,
      w: 720,
...
function renderControlSlide(presentation, spec, arm, n, metricsByRole) {
  ...
  base(slide, arm, n);
  const metrics = createSlideMetrics(spec.role);
  const mode = arm.armId === "bad_thick_data_memory" ? "bad" : arm.armId === "run1_5_skill" ? "run1_5" : "prompt";
  ...
  drawControlGate(slide, arm, spec, metrics, {
    ...
    headline: arm.armId === "bad_thick_data_memory" ? "primitive only" : "public blocked",
    line: arm.armId === "bad_thick_data_memory" ? "no module/gate read" : "no Run 2.13 visual trace",
  });
  proofFooter(slide, spec, arm);
  metricsByRole.set(spec.role, metrics);
  return slide;
...
function traceFor(arm, context = {}) {
  assertArmInputBoundaries(arm);
  const fullRun213 = arm.armId === "run2_13_full_skill";
  const fullData = fullRun213 ? context.fullData ?? renderRun213Full(arm) : null;
  ...
  const hasRenderedMetrics =
    fullRun213 && arm.slides.every((slide) => metricsByRole.has(slide.role));
  return {
    schema_version: 1,
    arm_id: arm.armId,
    ...
    runtime_isolation: {
      ...
      prompt_context: "fresh Run 2.13 thick-data arm-specific generation from scripts/generate_ppt_run2_13_thick_data_arms.mjs",
    ...
    slides: arm.slides.map((slide, index) => {
      ...
      return {
        ...
        role: slide.role,
        title: slide.title,
        run2_13_contract_status: fullRun213
          ? hasRenderedMetrics
            ? "full_arm_native_generator_rendered"
            : "full_arm_contract_preview_not_rendered"
          : "boundary_control_not_run2_13_full",
        run2_12_evidence_record_ids: fullRun213 ? roleEvidenceRecords.map((record) => record.id) : [],
        run2_12_memory_seed_ids: fullRun213 ? roleMemorySeeds.map((seed) => seed.id) : [],
        run2_12_workflow_gate_ids: fullRun213 ? roleGates.map((gateSeed) => gateSeed.id) : [],
        run2_12_required_code_module_ids: fullRun213 ? codeModulesByRole.get(slide.role) ?? [] : [],
        run2_12_code_module_ids: fullRun213
          ? hasRenderedMetrics
            ? actualCodeModuleIds
            : codeModulesByRole.get(slide.role) ?? []
          : [],
        run2_12_density_whitespace_gate: fullRun213 ? fullData.densityGate?.id ?? "gate_2_12_visual_density_and_whitespace_missing" : "",
        run2_12_demo_sequence_gate: fullRun213 ? fullData.demoGate?.id ?? "gate_2_12_demo_sequence_climax_missing" : "",
        run2_12_climax_object_gate: fullRun213 ? "memory_2_12_metric_to_climax_object" : "",
        run2_13_visual_delta_from_run2_10: fullRun213 ? visualDeltaByRole.get(slide.role) ?? "" : "",
        run2_13_negative_control_probe: fullRun213
          ? "bad_thick_data_memory reads evidence only; if it matches full arm, memory/workflow did not add value"
          : "boundary_control",
        run2_13_trace_origin: hasRenderedMetrics ? "actual_native_thick_data_module_calls" : fullRun213 ? "contract_preview_required_thick_data_modules" : "boundary_control",
        negative_control_evidence_record_ids:
          arm.armId === "bad_thick_data_memory" ? (badThickEvidenceByRole.get(slide.role) ?? []).map((source) => source.id) : [],
        layout_metrics: {
...
function assertRun213ThickDataGateSelfCheck(trace) {
  if (trace.arm_id !== "run2_13_full_skill") return;
  for (const slide of trace.slides) {
    if (slide.run2_13_contract_status !== "full_arm_native_generator_rendered") {
      throw new Error(`Run 2.13 full slide ${slide.slide_id} was not rendered with native module metrics`);
    }
    for (const field of EXPECTED_RUN2_13_TRACE_FIELDS) {
      ...
      if (empty) throw new Error(`Run 2.13 full slide ${slide.slide_id} missing ${field}`);
    }
    if (!slide.layout_budget_status?.within_budget) {
      throw new Error(`Run 2.13 full slide ${slide.slide_id} exceeded layout budget: ${slide.layout_budget_status.violations.join("; ")}`);
    }
    const actualCodeModules = new Set(slide.run2_12_code_module_ids ?? []);
    for (const requiredCodeModule of slide.run2_12_required_code_module_ids ?? []) {
      if (!actualCodeModules.has(requiredCodeModule)) {
        throw new Error(`Run 2.13 full slide ${slide.slide_id} did not call required module ${requiredCodeModule}`);
      }
    }
    if (slide.role === "climax") {
      if (!actualCodeModules.has("drawRun213MetricClimaxObject")) {
        throw new Error("Run 2.13 climax did not call drawRun213MetricClimaxObject");
      }
      const share = slide.layout_metrics?.hero_object_canvas_share ?? 0;
      if (!share) throw new Error("Run 2.13 climax missing hero object share");
    }
  }
}
...
function buildArmContract() {
  return armSpecs.map((arm) => ({
    ...
    label: arm.label,
    contract_status:
      arm.armId === "run2_13_full_skill"
        ? "run2_13_thick_data_contract_ready_requires_render_metrics"
        : "run2_13_boundary_control_contract_ready",
    allowed: arm.allowed,
...
function buildRun213FourArmSheet(built) {
  const sheets = built.map((item) => item.contactSheet).filter((file) => fs.existsSync(file));
  if (!sheets.length) return "";
  return buildNamedContactSheet(path.join(outRoot, "run2-13-four-arm-contact-sheet.png"), "Run 2.13 four-arm comparison", sheets, sheets.length);
}
...
async function buildArm(arm) {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "source requirements: commercial case always; bad control may read Run 2.13 thick-data sources only; full arm additionally requires thick-data memory and gate matrix",
  ...
  if (arm.armId === "run2_13_full_skill") {
    fullData = loadRun213ContractData(arm);
  } else if (arm.armId === "bad_thick_data_memory") {
    badThickEvidenceByRole = buildBadThickEvidenceByRole(readRun213EvidenceForArm(arm));
  }

  const slides = arm.slides.map((slide, index) =>
    arm.armId === "run2_13_full_skill"
      ? renderRun213FullSlide(presentation, slide, arm, index + 1, fullData, metricsByRole)
  ...
  assertRun213ThickDataGateSelfCheck(trace);
  ...
  const manifest = {
    ...
    slides: slides.map((_, index) => ({
      ...
      source: "scripts/generate_ppt_run2_13_thick_data_arms.mjs",
...
async function main() {
  ...
  const fourArmSheet = buildRun213FourArmSheet(built);
  const fullSkillSeriesSheet = buildFullSkillSeriesSheet();
  const runSummary = {
    run_id: "run2_13_thick_data_four_arms",
  ...
  writeJson(path.join(outRoot, "run2_13_thick_data_summary.json"), runSummary);
  return runSummary;
}
...
export {
  ...
  renderRun213Full,
  run213CodeModulesByRole,
  run213ShapeBudgetByRole,
  run213EvidenceRecordsById,
  run213EvidenceRecordsByRole,
  run213VisualDeltaByRole,
  run213WorkflowGateSeedsByRole,
  run213MemorySeedsById,
