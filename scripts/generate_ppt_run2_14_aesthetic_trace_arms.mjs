...
const EXPECTED_RUN2_14_TRACE_FIELDS = [
...
const RUN2_14_DATA_INPUTS = [RUN2_14_INPUTS.evidence, RUN2_14_INPUTS.memory, RUN2_14_INPUTS.gateSeed];
const RUN2_14_AESTHETIC_INPUTS = [
  RUN2_14_INPUTS.priorVisualSources,
  RUN2_14_INPUTS.priorVisualMemory,
  RUN2_14_INPUTS.priorVisualGateMatrix,
];
const RUN2_14_RESTRICTED_INPUTS = [...RUN2_14_DATA_INPUTS, ...RUN2_14_AESTHETIC_INPUTS];

const baseSlides = [
  {
    role: "cover",
    title: "The deck stops showing its machinery.",
    claim: "Run 2.14 keeps the data trace in the system and gives the slide surface back to the story.",
...
const armSpecs = [
  {
    armId: "prompt_only",
    slug: "ppt-run2-14-prompt-only",
    label: "Prompt-only control",
    kicker: "RUN 2.14 / CONTROL",
  ...
  {
    armId: "run1_5_skill",
    slug: "ppt-run2-14-run1-5-skill",
    label: "Run 1.5 baseline",
    kicker: "RUN 2.14 / RUN 1.5",
  ...
  {
    armId: "run2_14_full_skill",
    slug: "ppt-run2-14-full-vulca",
    label: "Run 2.14 full Vulca skill",
    kicker: "RUN 2.14 / FULL VULCA",
    footer: "run2_14_full_skill | 2.10 shell + 2.12 trace core | public blocked",
    release: "public_blocked",
    allowed: [
  ...
  {
    armId: "bad_visible_workflow_memory",
    slug: "ppt-run2-14-bad-visible-workflow-memory",
    label: "Bad visible-workflow control",
    kicker: "RUN 2.14 / NEGATIVE CONTROL",
    footer: "bad_visible_workflow_memory | 2.12 data without 2.10 shell | internal comparison",
    ...
    forbidden: [
      ...
      `${pack}/run2_10_visual_system_gate_matrix.json`,
      `${pack}/results/trace_manifest_contract.json`,
      "manual Run 2.14 visual module repair before scoring",
    ],
    palette: {
      bg: "#f2efe1",
      rail: "#6d633e",
...
function registerRun214Module(metrics, functionName) {
...
function drawRun214PresentationShell(slide, arm, spec, module, metrics, opts = {}) {
  registerRun214Module(metrics, "drawRun214PresentationShell");
...
function drawRun214HiddenWorkflowRoute(slide, arm, spec, module, metrics, opts = {}) {
  registerRun214Module(metrics, "drawRun214HiddenWorkflowRoute");
  const x = opts.x ?? 80;
  const y = opts.y ?? 118;
  const w = opts.w ?? 1040;
  const h = opts.h ?? 430;
...
function drawRun214CinematicProductTheater(slide, arm, spec, module, metrics, opts = {}) {
  registerRun214Module(metrics, "drawRun214CinematicProductTheater");
  const x = opts.x ?? 604;
  const y = opts.y ?? 134;
  const w = opts.w ?? 504;
  const h = opts.h ?? 352;
...
function drawRun214WorkflowGateRail(slide, arm, spec, module, metrics, opts = {}) {
  registerRun214Module(metrics, "drawRun214WorkflowGateRail");
  const x = opts.x ?? 104;
  const y = opts.y ?? 352;
...
function drawRun214QuietReleaseHandoff(slide, arm, spec, module, metrics, opts = {}) {
  registerRun214Module(metrics, "drawRun214QuietReleaseHandoff");
  const x = opts.x ?? 100;
  const y = opts.y ?? 430;
  const w = opts.w ?? 880;
  const h = opts.h ?? 126;
...
function drawRun214MetricRevealStage(slide, arm, spec, module, metrics, opts = {}) {
  registerRun214Module(metrics, "drawRun214MetricRevealStage");
...
function assertArmInputBoundaries(arm) {
  ...
  if (arm.armId === "run2_14_full_skill") {
    for (const input of RUN2_14_RESTRICTED_INPUTS) assertAllowed(input);
    return;
  }
  if (arm.armId === "bad_visible_workflow_memory") {
    for (const input of RUN2_14_DATA_INPUTS) assertAllowed(input);
    for (const input of RUN2_14_AESTHETIC_INPUTS) assertForbidden(input);
    return;
  }
  for (const input of RUN2_14_RESTRICTED_INPUTS) assertForbidden(input);
}

function readRun214JsonForArm(arm, relPath) {
  assertArmInputBoundaries(arm);
  if (arm.armId !== "run2_14_full_skill") {
    throw new Error(`${arm.armId} cannot read Run 2.14 visual-module input: ${relPath}`);
  }
...
function readRun214EvidenceForArm(arm) {
  assertArmInputBoundaries(arm);
  if (!arm.allowed.includes(RUN2_14_INPUTS.evidence) || arm.forbidden.includes(RUN2_14_INPUTS.evidence)) {
    throw new Error(`${arm.armId} input boundary does not permit reading ${RUN2_14_INPUTS.evidence}`);
  }
  return readJson(RUN2_14_INPUTS.evidence);
}

function run214WorkflowGateSeedsByRole(gateSeed) {
  const map = new Map();
...
function run214EvidenceRecordsById(evidence) {
  return new Map((evidence?.records ?? []).map((record) => [record.id, record]));
}

function run214MemorySeedsById(memory) {
  return new Map((memory?.memory_seeds ?? []).map((seed) => [seed.id, seed]));
}

function run214EvidenceRecordsByRole(evidence, gateSeed) {
  const evidenceById = run214EvidenceRecordsById(evidence);
  const map = new Map();
  for (const gate of gateSeed?.gates ?? []) {
    for (const role of gate.slide_roles ?? []) {
      const bucket = map.get(role) ?? [];
...
function run214MemorySeedsByRole(memory) {
  const map = new Map();
...
function run214CodeModulesByRole() {
  return new Map(
    Object.entries({
      cover: ["drawRun214PresentationShell", "drawRun214HiddenWorkflowRoute"],
      setup: ["drawRun214CinematicProductTheater", "drawRun214HiddenWorkflowRoute"],
      contrast: ["drawRun214PresentationShell", "drawRun214HiddenWorkflowRoute"],
      proof: ["drawRun214CinematicProductTheater", "drawRun214HiddenWorkflowRoute"],
      climax: ["drawRun214MetricRevealStage"],
      close: ["drawRun214PresentationShell", "drawRun214QuietReleaseHandoff"],
    }),
  );
}

function run214ShapeBudgetByRole() {
  const baseBudget = {
    max_text_boxes: 18,
...
function run214VisualDeltaByRole() {
  return new Map(
    Object.entries({
      cover: "Run 2.14 restores the Run 2.10 presentation shell while keeping Run 2.13 evidence trace in manifest fields.",
      ...
      proof: "Product theater carries the proof while trace details stay in run2_12_* and run2_14_* fields.",
...
function buildBadThickEvidenceByRole(evidenceData) {
  const records = evidenceData?.records ?? [];
  return new Map([
    ["cover", records.filter((record) => record.memory_seed_targets?.includes("memory_2_12_launch_arc_to_six_slide_route")).slice(0, 1)],
    ["setup", records.filter((record) => record.memory_seed_targets?.includes("memory_2_12_launch_arc_to_six_slide_route")).slice(0, 1)],
    ["contrast", records.filter((record) => record.memory_seed_targets?.includes("memory_2_12_typographic_whitespace_system")).slice(0, 1)],
    ["proof", records.filter((record) => record.memory_seed_targets?.includes("memory_2_12_product_demo_sequence_pacing")).slice(0, 1)],
    ["climax", records.filter((record) => record.memory_seed_targets?.includes("memory_2_12_metric_to_climax_object")).slice(0, 1)],
    ["close", records.filter((record) => record.workflow_gate_obligations?.length).slice(0, 1)],
  ]);
}

function loadRun214ContractData(arm) {
  const evidence = readRun214JsonForArm(arm, RUN2_14_INPUTS.evidence);
  const memory = readRun214JsonForArm(arm, RUN2_14_INPUTS.memory);
  const gateSeed = readRun214JsonForArm(arm, RUN2_14_INPUTS.gateSeed);
  const priorVisualSources = readRun214JsonForArm(arm, RUN2_14_INPUTS.priorVisualSources);
  const priorVisualMemory = readRun214JsonForArm(arm, RUN2_14_INPUTS.priorVisualMemory);
  const priorVisualGateMatrix = readRun214JsonForArm(arm, RUN2_14_INPUTS.priorVisualGateMatrix);
  const workflowGateSeedsByRole = run214WorkflowGateSeedsByRole(gateSeed);
  const evidenceRecordsByRole = run214EvidenceRecordsByRole(evidence, gateSeed);
  const memorySeedsByRole = run214MemorySeedsByRole(memory);
  const codeModulesByRole = run214CodeModulesByRole();
  const shapeBudgetByRole = run214ShapeBudgetByRole();
  const visualDeltaByRole = run214VisualDeltaByRole();
  ...
  return {
    ...
    aestheticShellInputIds: RUN2_14_AESTHETIC_INPUTS.map((input) => path.basename(input)),
    ...
    densityGate,
    demoGate,
    status: "run2_14_aesthetic_trace_contract_ready",
  };
}

function renderRun214Full(arm) {
  const contract = loadRun214ContractData(arm);
  return {
    ...contract,
    status: "run2_14_aesthetic_trace_contract_ready",
  };
...
function renderRun214FullSlide(presentation, spec, arm, n, fullData, metricsByRole) {
  const slide = presentation.slides.add();
  base(slide, arm, n);
  const metrics = createSlideMetrics(spec.role);

  if (spec.role === "cover") {
    drawRun214PresentationShell(slide, arm, spec, null, metrics, {
      x: 82,
      y: 120,
      w: 760,
      h: 240,
    ...
    drawRun214HiddenWorkflowRoute(slide, arm, spec, null, metrics, {
      x: 76,
      y: 382,
      w: 1020,
      h: 176,
    ...
    drawRun214CinematicProductTheater(slide, arm, spec, null, metrics, {
      x: 596,
      y: 128,
      w: 496,
      h: 330,
    ...
    drawRun214MetricRevealStage(slide, arm, spec, null, metrics);
    drawRun214QuietReleaseHandoff(slide, arm, spec, null, metrics, {
      x: 236,
      y: 530,
      w: 720,
      h: 78,
...
function renderControlSlide(presentation, spec, arm, n, metricsByRole) {
  ...
  base(slide, arm, n);
  const metrics = createSlideMetrics(spec.role);
  const mode = arm.armId === "bad_visible_workflow_memory" ? "bad" : arm.armId === "run1_5_skill" ? "run1_5" : "prompt";
  ...
  drawControlGate(slide, arm, spec, metrics, {
    ...
    headline: arm.armId === "bad_visible_workflow_memory" ? "primitive only" : "public blocked",
    line: arm.armId === "bad_visible_workflow_memory" ? "no module/gate read" : "no Run 2.14 visual trace",
  });
  proofFooter(slide, spec, arm);
  metricsByRole.set(spec.role, metrics);
  return slide;
...
function traceFor(arm, context = {}) {
  assertArmInputBoundaries(arm);
  const fullRun214 = arm.armId === "run2_14_full_skill";
  const fullData = fullRun214 ? context.fullData ?? renderRun214Full(arm) : null;
  ...
  const badThickEvidenceByRole = context.badThickEvidenceByRole ?? new Map();
  const metricsByRole = context.metricsByRole ?? new Map();
  const hasRenderedMetrics =
    fullRun214 && arm.slides.every((slide) => metricsByRole.has(slide.role));
  return {
    schema_version: 1,
    arm_id: arm.armId,
    ...
    runtime_isolation: {
      ...
      prompt_context: "fresh Run 2.14 aesthetic-trace arm-specific generation from scripts/generate_ppt_run2_14_aesthetic_trace_arms.mjs",
    ...
    slides: arm.slides.map((slide, index) => {
      ...
      return {
        ...
        role: slide.role,
        title: slide.title,
        run2_14_contract_status: fullRun214
          ? hasRenderedMetrics
            ? "full_arm_native_generator_rendered"
            : "full_arm_contract_preview_not_rendered"
          : "boundary_control_not_run2_14_full",
        run2_12_evidence_record_ids: fullRun214 ? roleEvidenceRecords.map((record) => record.id) : [],
        run2_12_memory_seed_ids: fullRun214 ? roleMemorySeeds.map((seed) => seed.id) : [],
        run2_12_workflow_gate_ids: fullRun214 ? roleGates.map((gateSeed) => gateSeed.id) : [],
        run2_14_aesthetic_source_run_id: fullRun214 ? "2.10" : "",
        run2_14_aesthetic_shell_input_ids: fullRun214 ? fullData.aestheticShellInputIds ?? [] : [],
        run2_14_surface_policy: fullRun214 ? "manifest_only_trace_public_surface" : "",
        run2_14_visible_workflow_suppression: fullRun214 ? "visible_workflow_suppression_pass" : "",
        run2_14_required_code_module_ids: fullRun214 ? codeModulesByRole.get(slide.role) ?? [] : [],
        run2_14_code_module_ids: fullRun214
          ? hasRenderedMetrics
            ? actualCodeModuleIds
            : codeModulesByRole.get(slide.role) ?? []
          : [],
        run2_14_visual_delta_from_run2_13: fullRun214 ? visualDeltaByRole.get(slide.role) ?? "" : "",
        run2_14_bad_control_probe: fullRun214
          ? "bad_visible_workflow_memory uses Run 2.12 data/workflow without the Run 2.10 aesthetic shell; if it matches full arm, aesthetic shell did not add value"
          : "boundary_control",
        run2_14_trace_origin: hasRenderedMetrics ? "actual_native_aesthetic_trace_module_calls" : fullRun214 ? "contract_preview_required_aesthetic_trace_modules" : "boundary_control",
        negative_control_evidence_record_ids:
          arm.armId === "bad_visible_workflow_memory" ? (badThickEvidenceByRole.get(slide.role) ?? []).map((source) => source.id) : [],
        layout_metrics: {
...
function assertRun214AestheticTraceGateSelfCheck(trace) {
  if (trace.arm_id !== "run2_14_full_skill") return;
  for (const slide of trace.slides) {
    if (slide.run2_14_contract_status !== "full_arm_native_generator_rendered") {
      throw new Error(`Run 2.14 full slide ${slide.slide_id} was not rendered with native module metrics`);
    }
    for (const field of EXPECTED_RUN2_14_TRACE_FIELDS) {
      ...
      if (empty) throw new Error(`Run 2.14 full slide ${slide.slide_id} missing ${field}`);
    }
    if (!slide.layout_budget_status?.within_budget) {
      throw new Error(`Run 2.14 full slide ${slide.slide_id} exceeded layout budget: ${slide.layout_budget_status.violations.join("; ")}`);
    }
    const actualCodeModules = new Set(slide.run2_14_code_module_ids ?? []);
    for (const requiredCodeModule of slide.run2_14_required_code_module_ids ?? []) {
      if (!actualCodeModules.has(requiredCodeModule)) {
        throw new Error(`Run 2.14 full slide ${slide.slide_id} did not call required module ${requiredCodeModule}`);
      }
    }
    if (slide.role === "climax") {
      if (!actualCodeModules.has("drawRun214MetricRevealStage")) throw new Error("Run 2.14 climax did not call drawRun214MetricRevealStage");
      const share = slide.layout_metrics?.hero_object_canvas_share ?? 0;
      if (!share) throw new Error("Run 2.14 climax missing hero object share");
    }
    if (slide.run2_14_surface_policy !== "manifest_only_trace_public_surface") {
      throw new Error(`Run 2.14 full slide ${slide.slide_id} did not keep trace manifest-only`);
    }
  }
}
...
function buildArmContract() {
  return armSpecs.map((arm) => ({
    ...
    label: arm.label,
    contract_status:
      arm.armId === "run2_14_full_skill"
        ? "run2_14_aesthetic_trace_contract_ready_requires_render_metrics"
        : "run2_14_boundary_control_contract_ready",
    allowed: arm.allowed,
...
function buildRun214FourArmSheet(built) {
  const sheets = built.map((item) => item.contactSheet).filter((file) => fs.existsSync(file));
  if (!sheets.length) return "";
  return buildNamedContactSheet(path.join(outRoot, "run2-14-four-arm-contact-sheet.png"), "Run 2.14 four-arm comparison", sheets, sheets.length);
}
...
async function buildArm(arm) {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "source requirements: commercial case always; bad control may read Run 2.14 aesthetic-trace sources only; full arm additionally requires aesthetic-trace memory and gate matrix",
  ...
  let fullData = null;
  let badThickEvidenceByRole = new Map();
  if (arm.armId === "run2_14_full_skill") {
    fullData = loadRun214ContractData(arm);
  } else if (arm.armId === "bad_visible_workflow_memory") {
    badThickEvidenceByRole = buildBadThickEvidenceByRole(readRun214EvidenceForArm(arm));
  }

  const slides = arm.slides.map((slide, index) =>
    arm.armId === "run2_14_full_skill"
      ? renderRun214FullSlide(presentation, slide, arm, index + 1, fullData, metricsByRole)
  ...
  const trace = traceFor(arm, { fullData, badThickEvidenceByRole, metricsByRole });
  assertRun214AestheticTraceGateSelfCheck(trace);
  ...
  const manifest = {
    ...
    slides: slides.map((_, index) => ({
      ...
      source: "scripts/generate_ppt_run2_14_aesthetic_trace_arms.mjs",
...
async function main() {
  ...
  const fourArmSheet = buildRun214FourArmSheet(built);
  const fullSkillSeriesSheet = buildFullSkillSeriesSheet();
  const runSummary = {
    run_id: "run2_14_aesthetic_trace_four_arms",
  ...
  writeJson(path.join(outRoot, "run2_14_aesthetic_trace_summary.json"), runSummary);
  return runSummary;
}
...
export {
  ...
  renderRun214Full,
  run214CodeModulesByRole,
  run214ShapeBudgetByRole,
  run214EvidenceRecordsById,
  run214EvidenceRecordsByRole,
  run214VisualDeltaByRole,
  run214WorkflowGateSeedsByRole,
  run214MemorySeedsById,
  run214MemorySeedsByRole,
  traceFor,
};
