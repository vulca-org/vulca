...
function renderFull(slide, spec, arm, n) {
  if (spec.role === "cover") {
    ...
    text(slide, "Run 2.6R adds production reference decompositions, aesthetic memory v2, and native visual modules.", 78, 382, 610, 58, {
      fontSize: 17,
      color: "#bed0c8",
    });
...
const baseSlides = [
  ...
  },
  {
    ...
    cards: [
      ...
      ["Run 2.6R", "not used", "forbidden input"],
    ],
    sideTitle: "Boundary",
  ...
  },
  {
    ...
    sideTitle: "Next",
    sideBody: "Compare against Run 1.5, Run 2.6R full, and bad aesthetic memory.",
...
const fullSlides = [
  {
    ...
    claim: "Run 2.6R turns reference decompositions into native production modules.",
...
const armSpecs = [
  {
    armId: "prompt_only",
    slug: "ppt-run2-6r-prompt-only",
    label: "Prompt-only control",
    kicker: "RUN 2.6R / CONTROL",
  ...
  {
    armId: "run1_5_skill",
    slug: "ppt-run2-6r-run1-5-skill",
    label: "Run 1.5 evidence-heavy baseline",
    kicker: "RUN 2.6R / BASELINE",
    footer: "run1_5_skill | prior product-lab workflow | public blocked",
    boundaryTitle: "prior workflow",
    boundaryBody: "Uses Run 1.5 workflow and the commercial case; no Run 2.6R motion or sequence data.",
    ...
    slides: baseSlides.map((slide, index) => ({
      ...
      trace: "Run 1.5 workflow; no Run 2.6R motion grammar",
    })),
  },
  {
    armId: "run2_6r_visual_repair_full_skill",
    slug: "ppt-run2-6r-full-vulca",
    label: "Run 2.6R full Vulca skill",
    kicker: "RUN 2.6R / FULL VULCA",
    footer: "run2_6r_visual_repair_full_skill | production modules + native sequence contracts | public blocked",
  ...
  },
  {
    ...
    slug: "ppt-run2-6r-bad-aesthetic-memory",
    label: "Bad aesthetic memory negative control",
    kicker: "RUN 2.6R / NEGATIVE CONTROL",
    ...
    slides: baseSlides.map((slide, index) => ({
      ...
      trace: "valid Run 2.6R data; bad aesthetic memory replacement",
    })),
  },
];

function sequenceStepsForSlide(arm, role) {
  if (!["run2_6r_visual_repair_full_skill", "bad_aesthetic_memory"].includes(arm.armId)) return [];
  const ids = motionByRole[role]?.sequence_component_ids ?? [];
  return ids.flatMap((id) => sequenceStepsById.get(id) ?? []);
}

function traceFor(arm) {
  const workflowEligible = ["run2_6r_visual_repair_full_skill", "bad_aesthetic_memory"].includes(arm.armId);
  const fullWorkflow = arm.armId === "run2_6r_visual_repair_full_skill";
  const productionEligible = ["run2_6r_visual_repair_full_skill", "bad_aesthetic_memory"].includes(arm.armId);
  const fullProduction = arm.armId === "run2_6r_visual_repair_full_skill";
  const repairEligible = arm.armId === "run2_6r_visual_repair_full_skill";
  return {
    schema_version: 1,
    arm_id: arm.armId,
    render_style_arm_id: arm.armId,
    generation_brief: `${pack}/generation_briefs/${arm.armId === "run2_6r_visual_repair_full_skill" ? "run2_skill" : arm.armId}.md`,
    ...
    runtime_isolation: {
      ...
      prompt_context: "fresh Run 2.6R visual-repair arm-specific generation from scripts/generate_ppt_run2_6r_visual_repair_arms.mjs",
    ...
    slides: arm.slides.map((slide, index) => {
      ...
      return {
        ...
        aesthetic_move_ids:
          arm.armId === "run2_6r_visual_repair_full_skill"
        ...
        source_brand_sanitization: fullWorkflow
          ...
          : arm.armId === "bad_aesthetic_memory"
            ...
            : "not applicable; Run 2.6R data forbidden for this arm",
...
async function buildArm(arm) {
  ...
  const manifest = {
    ...
    slides: slides.map((_, index) => ({
      ...
      source: "scripts/generate_ppt_run2_6r_visual_repair_arms.mjs",
