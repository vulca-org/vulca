...
const EXPECTED_RUN2_31_TRACE_FIELDS = [
  ...
  "run2_31_synthesis_record_id",
  "run2_31_public_surface_mode",
  "run2_31_trace_surface_mode",
  "run2_31_presentation_module_id",
  "run2_31_chain_compression_policy",
  "run2_31_presentation_synthesis_execution_status",
  ...
  "run2_31_code_module_ids",
];

const RUN2_31_INPUTS = {
  ...
  priorRerun: `${pack}/results/run2_29_presentation_synthesis_rerun_result.json`,
};
const RUN2_31_DATA_INPUTS = [
  RUN2_31_INPUTS.presentationSynthesis,
  RUN2_31_INPUTS.presentationSynthesisAudit,
  RUN2_31_INPUTS.evidenceChain,
  RUN2_31_INPUTS.contentMemory,
  RUN2_31_INPUTS.visualAssets,
  RUN2_31_INPUTS.workflowGates,
  RUN2_31_INPUTS.priorRerun,
];
...
const armSpecs = [
  {
    armId: "prompt_only",
    slug: "ppt-run2-31-prompt-only",
    label: "Prompt-only control",
    kicker: "RUN 2.31 / CONTROL",
    ...
    forbidden: [
      ...
      `${pack}/results/run2_29_presentation_synthesis_rerun_result.json`,
  ...
  {
    armId: "run1_5_skill",
    slug: "ppt-run2-31-run1-5-skill",
    label: "Run 1.5 baseline",
    kicker: "RUN 2.31 / RUN 1.5",
  ...
  {
    armId: "run2_31_full_spine_climax_repair",
    slug: "ppt-run2-31-full-vulca",
    label: "Run 2.31 full spine/climax repair",
    kicker: "RUN 2.31 / SPINE + CLIMAX REPAIR",
    footer: "run2_31_full_spine_climax_repair | presentation-first surface + readable evidence spine | public blocked",
    release: "public_blocked",
    allowed: [
  ...
  {
    armId: "bad_spine_climax_repair_memory",
    slug: "ppt-run2-31-bad-spine-climax-repair-memory",
    label: "Bad spine/climax memory control",
    kicker: "RUN 2.31 / NEGATIVE CONTROL",
    footer: "bad_spine_climax_repair_memory | usecase label only | internal comparison",
    ...
    forbidden: [
      ...
      `${pack}/skill_workflow.json`,
      `${pack}/results/trace_manifest_contract.json`,
      "manual Run 2.31 pack repair before scoring",
    ],
    palette: {
      bg: "#f1eadb",
      rail: "#695d3a",
...
function registerRun231Module(metrics, functionName) {
...
function drawRun231LaunchField(slide, arm, spec, selection, metrics, opts = {}) {
  registerRun231Module(metrics, "drawRun231LaunchField");
...
function drawRun231SelectedRouteMap(slide, arm, spec, selection, metrics, opts = {}) {
  registerRun231Module(metrics, "drawRun231SelectedRouteMap");
  const x = opts.x ?? 82;
  const y = opts.y ?? 402;
  const w = opts.w ?? 1000;
...
function run231ContentEvidenceSurfaceGeometry({ x, y, w, h }) {
...
function drawRun231ContentEvidenceSurface(slide, arm, spec, selection, metrics, opts = {}) {
  registerRun231Module(metrics, "drawRun231ContentEvidenceSurface");
  const x = opts.x ?? 96;
  const y = opts.y ?? 126;
  const w = opts.w ?? 870;
  const h = opts.h ?? 368;
  const geom = run231ContentEvidenceSurfaceGeometry({ x, y, w, h });
...
function run231EvidenceChainSurfaceGeometry({ x, y, w, h }) {
  const compact = w < 760;
...
function drawRun231EvidenceChainSurface(slide, arm, spec, selection, metrics, opts = {}) {
  registerRun231Module(metrics, "drawRun231EvidenceChainSurface");
  const x = opts.x ?? 70;
  const y = opts.y ?? 112;
  const w = opts.w ?? 1068;
  const h = opts.h ?? 424;
  const geom = run231EvidenceChainSurfaceGeometry({ x, y, w, h });
...
function drawRun231WorkflowTracePanel(slide, arm, spec, selection, metrics, opts = {}) {
  registerRun231Module(metrics, "drawRun231WorkflowTracePanel");
  drawRun231EvidenceChainSurface(slide, arm, spec, selection, metrics, {
    x: opts.x ?? 72,
    y: opts.y ?? 108,
    w: opts.w ?? 720,
    h: opts.h ?? 418,
...
function drawRun231ClimaxEvidenceObject(slide, arm, spec, selection, metrics) {
  registerRun231Module(metrics, "drawRun231ClimaxEvidenceObject");
  drawRun231ClimaxProofObject(slide, arm, spec, selection, metrics);
...
function drawRun231ClimaxProofObject(slide, arm, spec, selection, metrics) {
  registerRun231Module(metrics, "drawRun231ClimaxProofObject");
...
function run231SpineDetail(selection, step) {
...
function drawRun231ReadableEvidenceSpine(slide, arm, spec, selection, metrics, opts = {}) {
  registerRun231Module(metrics, "drawRun231ReadableEvidenceSpine");
  const x = opts.x ?? 922;
  const y = opts.y ?? 124;
  const w = opts.w ?? 246;
  const h = opts.h ?? 394;
  ...
  steps.forEach((step, index) => {
    ...
    text(slide, run231SpineDetail(selection, step), x + 48, rowY + 22, w - 66, 34, {
      fontSize: 8,
      color: opts.dark ? "#bac8d0" : arm.palette.muted,
    });
    registerText(metrics, `${step} ${run231SpineDetail(selection, step)}`);
  });
  registerWorkflow(metrics, 4);
  registerVisualEvidence(metrics, 1);
}

function drawRun231EditorialLaunchFrame(slide, arm, spec, selection, metrics) {
  registerRun231Module(metrics, "drawRun231EditorialLaunchFrame");
  ...
  drawRun231ReadableEvidenceSpine(slide, arm, spec, selection, metrics, { x: 898, y: 118, w: 264, h: 438 });
  registerProof(metrics, 2);
  registerZones(metrics, 3);
  registerHero(metrics, stage.x, stage.y, stage.w, stage.h);
}

function drawRun231AsymmetricProblemFrame(slide, arm, spec, selection, metrics) {
  registerRun231Module(metrics, "drawRun231AsymmetricProblemFrame");
  moduleLabel(slide, 76, 98, "presentation-first surface", arm);
  ...
  drawRun231ReadableEvidenceSpine(slide, arm, spec, selection, metrics, { x: 900, y: 124, w: 262, h: 420 });
...
function drawRun231BeforeAfterEditorialSpread(slide, arm, spec, selection, metrics) {
  registerRun231Module(metrics, "drawRun231BeforeAfterEditorialSpread");
...
function drawRun231ProductProofTheater(slide, arm, spec, selection, metrics) {
  registerRun231Module(metrics, "drawRun231ProductProofTheater");
  moduleLabel(slide, 76, 96, "presentation-first surface", arm);
...
function drawRun231HeroProofScene(slide, arm, spec, selection, metrics) {
  registerRun231Module(metrics, "drawRun231HeroProofScene");
  ...
  registerText(metrics, "public blocked");
  drawRun231ReadableEvidenceSpine(slide, arm, spec, selection, metrics, { x: 872, y: 154, w: 270, h: 392 });
  registerProof(metrics, 3);
  registerGate(metrics, 1);
  registerVisualEvidence(metrics, selection.assets.length);
  registerZones(metrics, 3);
...
function drawRun231DecisionHandoff(slide, arm, spec, selection, metrics) {
  registerRun231Module(metrics, "drawRun231DecisionHandoff");
  moduleLabel(slide, 76, 96, "presentation-first surface", arm);
...
function assertRun231ArmInputBoundaries(arm) {
  ...
  if (arm.armId === "run2_31_full_spine_climax_repair") {
    for (const input of RUN2_31_DATA_INPUTS) assertAllowed(input);
    return;
  }
  for (const input of RUN2_31_DATA_INPUTS) assertForbidden(input);
}

function readRun231PackJsonForArm(arm, relPath) {
  assertRun231ArmInputBoundaries(arm);
...
function validateRun229PriorRerunResult(result) {
  if (result?.status !== "run2_29_presentation_synthesis_rerun_public_blocked") {
    throw new Error("Run 2.31 prior rerun result must be Run 2.29 public-blocked presentation-synthesis rerun");
  }
  if (result.run_id !== "2.29" || result.rerun?.best_internal_arm !== "run2_29_full_presentation_synthesis") {
    throw new Error("Run 2.31 prior rerun result source mismatch");
  }
}
...
function validateRun230PresentationSynthesisAudit(audit) {
  if (audit?.status !== "run2_30_presentation_synthesis_audit_public_blocked") {
    throw new Error("Run 2.31 source audit must be Run 2.30 presentation synthesis audit");
  }
...
function validateRun231EvidenceChainViewModel(model, contentMemory, workflowGates) {
  if (model?.status !== "run2_28_evidence_chain_view_model_ready_public_blocked") {
    throw new Error("Run 2.31 source evidence chain must be the Run 2.28 evidence-chain view model");
  }
...
function validateRun231PresentationSynthesisMemory(memory, evidenceChain) {
  if (memory?.status !== "run2_29_presentation_synthesis_memory_ready_public_blocked") {
    throw new Error("Run 2.31 must consume Run 2.29 presentation synthesis memory");
  }
  if (memory.selected_usecase_id !== selectedUsecaseId || memory.stage_policy !== "repeat_same_five_layers_not_run3") {
    throw new Error("Run 2.31 presentation synthesis usecase/stage mismatch");
  }
  if (memory.source_evidence_chain_layer !== "run2_28_evidence_chain_view_model.json") {
    throw new Error("Run 2.31 presentation synthesis must bind Run 2.28 evidence chain");
  }
  if (memory.surface_policy?.primary_reader_experience !== "presentation_first_surface") {
    throw new Error("Run 2.31 must render a presentation-first surface");
  }
  if (memory.surface_policy?.secondary_reviewer_experience !== "compressed_evidence_spine") {
    throw new Error("Run 2.31 must keep a readable evidence spine");
  }
  if (memory.surface_policy?.forbidden_primary_surface !== "four_column_audit_table") {
    throw new Error("Run 2.31 must forbid four-column audit table as primary surface");
  }
  ...
  if (records.length !== roles.length || records.map((record) => record.role).join(",") !== roles.join(",")) {
    throw new Error("Run 2.31 presentation synthesis must contain the six canonical slide roles in order");
  }
  ...
  for (const record of records) {
    ...
    if (!chain || record.evidence_chain_id !== chain.evidence_chain_id) {
      throw new Error(`Run 2.31 synthesis/evidence-chain mismatch for ${record.role}`);
    }
    if (record.source_native_surface_module_id !== chain.native_surface_module_id) {
      throw new Error(`Run 2.31 synthesis source module mismatch for ${record.role}`);
    }
    if (!String(record.presentation_module_id ?? "").startsWith("drawRun229")) {
      throw new Error(`Run 2.31 synthesis missing presentation module for ${record.role}`);
    }
    if (record.trace_surface_mode !== "manifest_and_html_viewer_full_chain_visible") {
      throw new Error(`Run 2.31 synthesis trace surface mode mismatch for ${record.role}`);
    }
    if (JSON.stringify(record.visible_on_slide_evidence_spine_steps ?? []) !== JSON.stringify(chainOrder)) {
      throw new Error(`Run 2.31 synthesis chain order mismatch for ${record.role}`);
    }
    for (const policy of [
      ...
      if (!(record.chain_compression_policy ?? []).includes(policy)) {
        throw new Error(`Run 2.31 synthesis missing policy ${policy} for ${record.role}`);
      }
    }
    for (const field of requiredTraceFields) {
      if (!(record.preserved_trace_fields ?? []).includes(field)) {
        throw new Error(`Run 2.31 synthesis missing preserved trace field ${field} for ${record.role}`);
      }
    }
  }
}

function run231RequiredModulesByRole(presentationSynthesis) {
  return new Map(
    (presentationSynthesis.slide_synthesis_records ?? []).map((record) => [
      record.role,
      [run231PresentationModuleIdForRole(record.role), "drawRun231ReadableEvidenceSpine"],
    ]),
  );
}

function run231PresentationModuleIdForRole(role) {
  return {
    cover: "drawRun231EditorialLaunchFrame",
    setup: "drawRun231AsymmetricProblemFrame",
    contrast: "drawRun231BeforeAfterEditorialSpread",
    proof: "drawRun231ProductProofTheater",
    climax: "drawRun231HeroProofScene",
    close: "drawRun231DecisionHandoff",
  }[role];
}

function loadRun231ContractData(arm) {
  const presentationSynthesis = readRun231PackJsonForArm(arm, RUN2_31_INPUTS.presentationSynthesis);
  const presentationSynthesisAudit = readRun231PackJsonForArm(arm, RUN2_31_INPUTS.presentationSynthesisAudit);
  const evidenceChain = readRun231PackJsonForArm(arm, RUN2_31_INPUTS.evidenceChain);
  const contentMemory = readRun231PackJsonForArm(arm, RUN2_31_INPUTS.contentMemory);
  const visualAssets = readRun231PackJsonForArm(arm, RUN2_31_INPUTS.visualAssets);
  const workflowGates = readRun231PackJsonForArm(arm, RUN2_31_INPUTS.workflowGates);
  const priorRerun = readRun231PackJsonForArm(arm, RUN2_31_INPUTS.priorRerun);
  validateRun224SingleUsecaseSchemas(contentMemory, visualAssets, workflowGates);
  validateRun229PriorRerunResult(priorRerun);
  validateRun230PresentationSynthesisAudit(presentationSynthesisAudit);
  validateRun231EvidenceChainViewModel(evidenceChain, contentMemory, workflowGates);
  validateRun231PresentationSynthesisMemory(presentationSynthesis, evidenceChain);
  return {
    ...
    workflowGates,
    priorRerun,
    requiredModulesByRole: run231RequiredModulesByRole(presentationSynthesis),
    status: "run2_31_spine_climax_repair_contract_ready",
  };
}

function selectRun231ForSlide(role, contractData) {
  ...
  if (!synthesis) throw new Error(`Run 2.31 presentation synthesis missing role ${role}`);
  ...
  if (synthesis.evidence_chain_id !== chain.evidence_chain_id) {
    throw new Error(`Run 2.31 synthesis/evidence-chain mismatch for ${role}`);
  }
  ...
  return { content, assets, gate, chain, synthesis, repairModuleId: run231PresentationModuleIdForRole(role) };
}

function renderRun231FullSlide(presentation, spec, arm, n, fullData, metricsByRole) {
  const slide = presentation.slides.add();
  base(slide, arm, n);
  const metrics = createSlideMetrics(spec.role);
  const selection = selectRun231ForSlide(spec.role, fullData);

  const modules = {
    drawRun231EditorialLaunchFrame,
    drawRun231AsymmetricProblemFrame,
    drawRun231BeforeAfterEditorialSpread,
    drawRun231ProductProofTheater,
    drawRun231HeroProofScene,
    drawRun231DecisionHandoff,
  };
  const renderModule = modules[selection.repairModuleId];
  if (!renderModule) throw new Error(`Run 2.31 missing presentation module ${selection.repairModuleId}`);
...
function renderControlSlide(presentation, spec, arm, n, metricsByRole) {
  ...
  base(slide, arm, n);
  const metrics = createSlideMetrics(spec.role);
  const mode = arm.armId === "bad_spine_climax_repair_memory" ? "bad" : arm.armId === "run1_5_skill" ? "run1_5" : "prompt";
  ...
  drawControlGate(slide, arm, spec, metrics, {
    ...
    headline: arm.armId === "bad_spine_climax_repair_memory" ? "pack missing" : "public blocked",
    line: arm.armId === "bad_spine_climax_repair_memory" ? "label only / no 2.31 synthesis" : "no Run 2.31 trace",
  });
...
function traceFor(arm, context = {}) {
  assertRun231ArmInputBoundaries(arm);
  const fullRun231 = arm.armId === "run2_31_full_spine_climax_repair";
  const fullData = fullRun231 ? context.fullData ?? loadRun231ContractData(arm) : null;
  const requiredModulesByRole = fullData?.requiredModulesByRole ?? new Map();
  const metricsByRole = context.metricsByRole ?? new Map();
  const hasRenderedMetrics = fullRun231 && arm.slides.every((slide) => metricsByRole.has(slide.role));
  return {
    schema_version: 1,
    arm_id: arm.armId,
    render_style_arm_id: arm.armId,
    selected_usecase_id: fullRun231 ? selectedUsecaseId : "",
    prior_rerun_run_id: fullRun231 ? fullData.priorRerun.run_id : "",
    prior_rerun_status: fullRun231 ? fullData.priorRerun.status : "",
    evidence_chain_status: fullRun231 ? fullData.evidenceChain.status : "",
    presentation_synthesis_status: fullRun231 ? fullData.presentationSynthesis.status : "",
    ...
    runtime_isolation: {
      ...
      prompt_context: "fresh Run 2.31 spine/climax repair arm-specific generation from scripts/generate_ppt_run2_31_spine_climax_repair_arms.mjs",
      no_cross_arm_reuse: [
        ...
        "Run 2.31 spine/climax repair in control arms",
      ],
    },
    ...
    slides: arm.slides.map((slide, index) => {
      ...
      const selection = fullRun231 ? selectRun231ForSlide(slide.role, fullData) : null;
      return {
        slide_id: `slide_${String(index + 1).padStart(2, "0")}`,
        role: slide.role,
        title: slide.title,
        run2_31_contract_status: fullRun231
          ? hasRenderedMetrics
            ? "full_arm_native_generator_rendered"
            : "full_arm_contract_preview_not_rendered"
          : "boundary_control_not_run2_31_full",
        run2_24_selected_usecase_id: fullRun231 ? selection.content.selected_usecase_id : "",
        run2_24_content_memory_id: fullRun231 ? selection.content.content_memory_id : "",
        run2_24_visual_evidence_slot_ids: fullRun231 ? selection.content.visual_evidence_slot_ids : [],
        run2_24_content_density_gate_id: fullRun231 ? selection.gate.gate_id : "",
        run2_24_business_proof_points_visible: fullRun231 ? selection.content.business_proof_points.slice(0, 3) : [],
        run2_24_visual_evidence_asset_ids: fullRun231 ? selection.assets.map((asset) => asset.asset_id) : [],
        run2_24_public_surface_policy: fullRun231 ? selection.content.public_surface_policy : "",
        run2_28_evidence_chain_id: fullRun231 ? selection.chain.evidence_chain_id : "",
        run2_28_multimodal_source_record_ids: fullRun231 ? selection.chain.multimodal_source_record_ids : [],
        run2_28_extracted_design_rule: fullRun231 ? selection.chain.extracted_design_rule : "",
        run2_28_workflow_decision: fullRun231 ? selection.chain.workflow_decision : "",
        run2_28_native_surface_module_id: fullRun231 ? selection.chain.native_surface_module_id : "",
        run2_31_synthesis_record_id: fullRun231 ? selection.synthesis.synthesis_record_id : "",
        run2_31_public_surface_mode: fullRun231 ? selection.synthesis.public_surface_mode : "",
        run2_31_trace_surface_mode: fullRun231 ? selection.synthesis.trace_surface_mode : "",
        run2_31_presentation_module_id: fullRun231 ? selection.repairModuleId : "",
        run2_31_chain_compression_policy: fullRun231 ? selection.synthesis.chain_compression_policy : [],
        run2_31_presentation_synthesis_execution_status: fullRun231
          ? "presentation_first_surface_rendered_with_secondary_evidence_spine"
          : "",
        run2_30_source_audit_status: fullRun231 ? fullData.presentationSynthesisAudit.status : "",
        run2_30_top_next_layer_to_thicken: fullRun231
          ? fullData.presentationSynthesisAudit.quality_summary.top_next_layer_to_thicken
          : "",
        run2_31_spine_min_font_size_target: fullRun231 ? 8 : "",
        run2_31_climax_style_policy: fullRun231 ? "high_contrast_climax_with_shared_light_editorial_frame" : "",
        run2_31_spine_climax_repair_execution_status: fullRun231
          ? "spine_readability_and_climax_consistency_repaired_before_native_ppt_generation"
          : "",
        run2_31_required_code_module_ids: fullRun231 ? requiredModulesByRole.get(slide.role) ?? [] : [],
        run2_31_code_module_ids: fullRun231
          ? hasRenderedMetrics
            ? actualCodeModuleIds
            : requiredModulesByRole.get(slide.role) ?? []
          : [],
        run2_31_bad_control_probe: fullRun231
          ? "bad_spine_climax_repair_memory may receive the selected usecase label but forbids Run 2.29 presentation synthesis, Run 2.30 audit, Run 2.28 evidence chain, Run 2.24 content memory, visual evidence assets, workflow gates, and Run 2.29 prior rerun result"
          : "boundary_control",
        negative_control_usecase_id: arm.armId === "bad_spine_climax_repair_memory" ? selectedUsecaseId : "",
        layout_metrics: {
...
function assertRun231ContentVisualGateSelfCheck(trace) {
  if (trace.arm_id !== "run2_31_full_spine_climax_repair") return;
  if (trace.selected_usecase_id !== selectedUsecaseId) throw new Error("Run 2.31 full trace did not lock the selected usecase");
  for (const slide of trace.slides) {
    if (slide.run2_31_contract_status !== "full_arm_native_generator_rendered") {
      throw new Error(`Run 2.31 full slide ${slide.slide_id} was not rendered with native module metrics`);
    }
    for (const field of EXPECTED_RUN2_31_TRACE_FIELDS) {
      ...
      if (empty) throw new Error(`Run 2.31 full slide ${slide.slide_id} missing ${field}`);
    }
    if ((slide.run2_24_visual_evidence_slot_ids ?? []).length < 2) {
      throw new Error(`Run 2.31 full slide ${slide.slide_id} missing at least two visual evidence slots`);
    }
    const actualCodeModules = new Set(slide.run2_31_code_module_ids ?? []);
    for (const requiredCodeModule of slide.run2_31_required_code_module_ids ?? []) {
      if (!actualCodeModules.has(requiredCodeModule)) {
        throw new Error(`Run 2.31 full slide ${slide.slide_id} did not call required module ${requiredCodeModule}`);
      }
    }
    if (!actualCodeModules.has(slide.run2_31_presentation_module_id)) {
      throw new Error(`Run 2.31 full slide ${slide.slide_id} did not call presentation synthesis module ${slide.run2_31_presentation_module_id}`);
    }
    if (!actualCodeModules.has("drawRun231ReadableEvidenceSpine")) {
      throw new Error(`Run 2.31 full slide ${slide.slide_id} did not call readable evidence spine`);
    }
    if (slide.run2_31_presentation_synthesis_execution_status !== "presentation_first_surface_rendered_with_secondary_evidence_spine") {
      throw new Error(`Run 2.31 full slide ${slide.slide_id} missing presentation-first execution status`);
    }
    if (slide.run2_30_top_next_layer_to_thicken !== "spine_readability_and_climax_consistency") {
      throw new Error(`Run 2.31 full slide ${slide.slide_id} did not consume the Run 2.30 repair target`);
    }
    if (!(Number(slide.run2_31_spine_min_font_size_target) >= 8)) {
      throw new Error(`Run 2.31 full slide ${slide.slide_id} missing readable spine font target`);
    }
    if (slide.run2_31_climax_style_policy !== "high_contrast_climax_with_shared_light_editorial_frame") {
      throw new Error(`Run 2.31 full slide ${slide.slide_id} missing climax style policy`);
    }
    if (
      slide.run2_31_spine_climax_repair_execution_status !==
      "spine_readability_and_climax_consistency_repaired_before_native_ppt_generation"
    ) {
      throw new Error(`Run 2.31 full slide ${slide.slide_id} missing spine/climax repair execution status`);
    }
    if (!(slide.run2_31_chain_compression_policy ?? []).includes("do_not_render_four_column_audit_table_as_primary_surface")) {
      throw new Error(`Run 2.31 full slide ${slide.slide_id} missing audit-table compression policy`);
    }
    if ((slide.layout_metrics?.workflow_objects ?? 0) < 4) {
      throw new Error(`Run 2.31 full slide ${slide.slide_id} does not expose the four-step workflow chain`);
    }
    if (slide.run2_24_selected_usecase_id !== selectedUsecaseId) {
      throw new Error(`Run 2.31 full slide ${slide.slide_id} selected the wrong usecase`);
    }
    if (slide.role === "climax" && !(slide.layout_metrics?.hero_object_canvas_share > 0.5)) {
      throw new Error("Run 2.31 climax missing large proof object stage");
    }
  }
}
...
function buildArmContract() {
  return armSpecs.map((arm) => ({
    ...
    label: arm.label,
    contract_status:
      arm.armId === "run2_31_full_spine_climax_repair"
        ? "run2_31_spine_climax_repair_contract_ready_requires_render_metrics"
        : "run2_31_boundary_control_contract_ready",
    allowed: arm.allowed,
...
function buildRun231FourArmSheet(built) {
  ...
  return buildNamedContactSheet(
    path.join(outRoot, "run2-31-four-arm-contact-sheet.png"),
    "Run 2.31 four-arm comparison",
    sheets,
    sheets.length,
    labels,
  );
...
async function buildArm(arm) {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "source requirements: commercial case always; full arm requires Run 2.29 presentation synthesis, Run 2.30 audit, Run 2.28 evidence chain, Run 2.24 content memory, visual assets, workflow gates, and Run 2.29 prior rerun result before native PPT generation; bad control receives usecase label only",
  ...
  let fullData = null;
  if (arm.armId === "run2_31_full_spine_climax_repair") {
    fullData = loadRun231ContractData(arm);
  }

  const slides = arm.slides.map((slide, index) =>
    arm.armId === "run2_31_full_spine_climax_repair"
      ? renderRun231FullSlide(presentation, slide, arm, index + 1, fullData, metricsByRole)
  ...
  assertRun231ContentVisualGateSelfCheck(trace);
  ...
  const manifest = {
    ...
    slides: slides.map((_, index) => ({
      ...
      source: "scripts/generate_ppt_run2_31_spine_climax_repair_arms.mjs",
...
function writeRun231Result(runSummary) {
  const result = {
    schema_version: 1,
    run_id: "2.31",
    status: "run2_31_spine_climax_repair_rerun_public_blocked",
    public_ready: false,
    selected_usecase_id: selectedUsecaseId,
    source_audit_run_id: "2.30",
    stage_policy: "repeat_same_five_layers_not_run3",
    input_chain: {
      presentation_synthesis_memory: RUN2_31_INPUTS.presentationSynthesis,
      presentation_synthesis_audit: RUN2_31_INPUTS.presentationSynthesisAudit,
      evidence_chain_view_model: RUN2_31_INPUTS.evidenceChain,
      content_memory: RUN2_31_INPUTS.contentMemory,
      visual_evidence_asset_memory: RUN2_31_INPUTS.visualAssets,
      content_visual_workflow_gates: RUN2_31_INPUTS.workflowGates,
      prior_rerun_result: RUN2_31_INPUTS.priorRerun,
      source_data_layer: RUN2_31_INPUTS.presentationSynthesisAudit,
    },
    rerun: {
      generator: "scripts/generate_ppt_run2_31_spine_climax_repair_arms.mjs",
      arms: armSpecs.map((arm) => arm.armId),
      best_internal_arm: "run2_31_full_spine_climax_repair",
      best_internal_arm_verdict: "spine_readability_and_climax_consistency_repaired_before_native_ppt_generation",
    ...
    quality_delta: {
      target_layer: "spine_readability_and_climax_consistency",
      source_audit: RUN2_31_INPUTS.presentationSynthesisAudit,
      source_model: RUN2_31_INPUTS.presentationSynthesis,
      source_evidence_chain: RUN2_31_INPUTS.evidenceChain,
      prior_rerun_result: RUN2_31_INPUTS.priorRerun,
      chain_order: ["source evidence", "extracted design rule", "workflow decision", "generated slide surface"],
      replacement_focus: "make_the_secondary_evidence_spine_readable_and_normalize_climax_into_the_shared_light_editorial_frame",
      repair_modules: ["drawRun231ReadableEvidenceSpine", "drawRun231HeroProofScene"],
    ...
    visual_quality_boundary:
      "spine_climax_repair_proof_only_not_public_video_grade_aesthetic_or_human_release_approval",
    control_boundary: {
      bad_spine_climax_repair_memory: "selected_usecase_label_only_without_run2_29_presentation_synthesis_run2_30_audit_run2_28_evidence_chain_run2_24_content_memory_visual_assets_workflow_gates_or_run2_29_prior_rerun_result",
    ...
    trace_manifest_requirements: EXPECTED_RUN2_31_TRACE_FIELDS,
    native_module_status: "actual_run2_31_presentation_module_calls_recorded_in_trace_manifest",
    ...
    next_required_action: "review_run2_31_outputs_for_readable_spine_and_climax_consistency_then_continue_thickening_same_five_layers",
  };
  const resultJson = path.join(root, pack, "results", "run2_31_spine_climax_repair_rerun_result.json");
  const resultMd = path.join(root, pack, "results", "run2_31_spine_climax_repair_rerun_result.md");
...
async function main() {
  ...
  const fourArmSheet = buildRun231FourArmSheet(built);
  const fullSkillSeriesSheet = buildFullSkillSeriesSheet();
  const runSummary = {
    run_id: "run2_31_spine_climax_repair_four_arms",
    selected_usecase_id: selectedUsecaseId,
    arms: armSpecs.map((arm) => arm.armId),
  ...
  writeJson(path.join(outRoot, "run2_31_spine_climax_repair_rerun_summary.json"), runSummary);
  writeRun231Result(runSummary);
  return runSummary;
}
...
export {
  EXPECTED_RUN2_31_TRACE_FIELDS,
  RUN2_31_INPUTS,
  RUN2_31_DATA_INPUTS,
  armSpecs,
  assertRun231ArmInputBoundaries,
  assertRun231ContentVisualGateSelfCheck,
  buildArmContract,
  drawRun231LaunchField,
  drawRun231SelectedRouteMap,
  drawRun231ContentEvidenceSurface,
  ...
  drawRun231HeroProofScene,
  drawRun231DecisionHandoff,
  drawRun231EvidenceChainSurface,
  drawRun231WorkflowTracePanel,
  drawRun231ClimaxEvidenceObject,
  drawRun231ClimaxProofObject,
  loadRun231ContractData,
  main,
  readRun231PackJsonForArm,
  registerRun231Module,
  run231PresentationModuleIdForRole,
  run231RequiredModulesByRole,
  run231ContentEvidenceSurfaceGeometry,
  run231EvidenceChainSurfaceGeometry,
  selectRun231ForSlide,
  traceFor,
  validateRun224SingleUsecaseSchemas,
  validateRun229PriorRerunResult,
  validateRun230PresentationSynthesisAudit,
  validateRun231EvidenceChainViewModel,
  validateRun231PresentationSynthesisMemory,
};
