...
const RUN_ID = "2.75";
const RESULT_JSON = `${pack}/results/run2_75_renderer_repair_rerun_result.json`;
const RESULT_MD = `${pack}/results/run2_75_renderer_repair_rerun_result.md`;
...
const RUN2_75_INPUTS = {
  ...
  visualQualityEvaluation: "docs/product/ppt-run2-data-skill-quality/results/run2_74_visual_quality_evaluation.json",
};
const CONSUMED_SOURCES = Object.values(RUN2_75_INPUTS);
const REPAIR_FLAGS = [
...
const VISUAL_DENSITY_PROFILE_BY_ROLE = {
  cover: "cover_product_surface_depth_dense_edge_labels",
  setup: "setup_field_route_with_visible_product_destination",
  contrast: "contrast_rebalanced_before_after_product_states",
  proof: "proof_populated_evidence_workspace",
  climax: "climax_finished_product_reveal_surface",
  close: "close_large_decision_nodes_connector_endpoints",
};
const PRODUCT_SURFACE_DETAIL_COUNT_BY_ROLE = {
  cover: 9,
  setup: 8,
  contrast: 8,
  proof: 11,
...
const CONNECTOR_OR_EDGE_COUNT_BY_ROLE = {
  cover: 5,
  setup: 5,
  contrast: 4,
  proof: 5,
...
const armSpecs = [
  ...
  {
    armId: "run2_75_full_renderer_repair",
    slug: "ppt-run2-75-full-vulca",
    label: "Run 2.75 full renderer repair",
    mode: "full",
    release: "public_blocked",
    palette: { bg: C.paper, accent: C.coral, accent2: C.blue, title: C.ink, muted: C.muted, surface: C.white },
  },
  {
    armId: "bad_run2_75_without_h_repair",
    slug: "ppt-run2-75-bad-without-h-repair",
    label: "Bad missing Run 2.75 H repair",
    mode: "negative",
...
function loadInputs() {
  const data = {
    scenePlanExpansion: readJson(RUN2_75_INPUTS.scenePlanExpansion),
    rendererInputValidation: readJson(RUN2_75_INPUTS.rendererInputValidation),
    visualGrammarModules: readJson(RUN2_75_INPUTS.visualGrammarModules),
    rendererAdapterContracts: readJson(RUN2_75_INPUTS.rendererAdapterContracts),
    textBindingStrategy: readJson(RUN2_75_INPUTS.textBindingStrategy),
    visualQualityEvaluation: readJson(RUN2_75_INPUTS.visualQualityEvaluation),
  };
  if (data.rendererInputValidation.status !== "run2_73_renderer_input_validation_passed_public_blocked") {
    throw new Error("Run 2.75 renderer repair requires the passed Run 2.73 renderer input validation");
  }
  if (data.textBindingStrategy.status !== "run2_73_text_binding_strategy_ready_public_blocked") {
    throw new Error("Run 2.75 renderer repair requires the Run 2.73 text binding strategy");
  }
  if (data.visualQualityEvaluation.status !== "run2_74_visual_quality_evaluation_public_blocked") {
    throw new Error("Run 2.75 renderer repair requires the Run 2.74 visual quality evaluation");
  }
  if (data.visualQualityEvaluation.next_required_action !== "part_i_renderer_repair_from_visual_quality_evaluation") {
    throw new Error("Run 2.75 renderer repair requires Part H to request Part I renderer repair");
  }
  return data;
}
...
function buildRenderModel(data) {
  ...
  const hByRole = indexByRole(data.visualQualityEvaluation.role_assessments);
  ...
  return ROLES.map((role, index) => {
    ...
    const hRepair = hByRole.get(role);
    if (!scene || !validation || !grammar || !adapter || !textBinding || !hRepair) {
      throw new Error(`Run 2.75 missing renderer input record for role: ${role}`);
    }
    const module = adapter.visual_grammar_binding?.module_id ?? grammar.primary_visual_grammar_module;
    if (module !== MODULE_BY_ROLE[role]) throw new Error(`Run 2.75 module mismatch for ${role}: ${module}`);
    if (validation.status !== "pass") throw new Error(`Run 2.75 D3 validation not pass for ${role}`);
    ...
    const sockets = flattenSockets(textBinding).map((socket) => {
      if (!BOUND_VISUAL_OBJECT_TYPES.has(socket.bound_visual_object_type)) {
        throw new Error(`Run 2.75 socket has invalid visual object type: ${socket.bound_visual_object_type}`);
      }
      if (!known.has(socket.bound_source_id)) {
        throw new Error(`Run 2.75 socket ${socket.socket_id} references unknown D2/E2 id: ${socket.bound_source_id}`);
      }
      return {
    ...
    return {
      ...
      layout_signature: `${textBinding.layout_signature}_renderer_repair`,
      visual_density_profile: VISUAL_DENSITY_PROFILE_BY_ROLE[role],
      ...
      h_repair_source: {
        root_cause_layer: hRepair.root_cause_layer,
        repair_instruction: hRepair.repair_instruction,
        delta_vs_2_72: hRepair.delta_vs_2_72,
        text_visual_fusion: hRepair.text_visual_fusion,
        report_like_risk: hRepair.report_like_risk,
      },
      renderer_repair_directives_applied: REPAIR_FLAGS,
      product_surface_detail_count: PRODUCT_SURFACE_DETAIL_COUNT_BY_ROLE[role],
      connector_or_edge_binding_count: CONNECTOR_OR_EDGE_COUNT_BY_ROLE[role],
...
function drawControlSlide(slide, page, arm) {
  ...
  const title = arm.mode === "negative"
    ? `${page.role}: H repair directives withheld`
    : arm.mode === "baseline"
  ...
  textSocket(slide, "This comparison arm is generated locally for viewer context. The Part I manifest validates the full arm only.", { x: 100, y: 218, w: 430, h: 58 }, { fontSize: 13, color: arm.palette.muted, max: 128 });
  page.proof_labels.slice(0, 3).forEach((label, index) => {
...
function traceForArm(arm, pages) {
  return {
    ...
    part_i_renderer_scope: arm.mode === "full" ? "renderer_repair_consumes_d2_d3_e_e2_f_and_h_visual_quality_evaluation" : "comparison_arm_not_part_i_quality_manifest",
...
function buildRun273Sheet(built) {
  ...
  return buildNamedContactSheet(path.join(outRoot, "run2-75-four-arm-contact-sheet.png"), "Run 2.75 renderer repair comparison", sheets, sheets.length, labels);
}
...
async function buildArm(arm, pages) {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      `arm: ${arm.armId}`,
      `part: Part I / Run ${RUN_ID}`,
      `consumed sources: ${arm.mode === "full" ? CONSUMED_SOURCES.join(", ") : "commercial_case.md only"}`,
      "full arm requirement: apply Part H repair directives while preserving visual grammar modules and text sockets",
      "release boundary: public blocked until Part J visual evaluation",
      "",
    ].join("\n"),
  );
  ...
  const manifest = {
    ...
    slides: slides.map((_, index) => ({
      ...
      source: "scripts/generate_ppt_run2_75_renderer_repair_arms.mjs",
...
<html lang="en" data-run-id="2.75">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Run 2.75 Renderer Repair</title>
  <style>
...
function writeResult(pages, built, fourArmSheet, standaloneHtml, visualQualityEvaluation) {
  const full = built.find((item) => item.arm.armId === "run2_75_full_renderer_repair");
  ...
  const renderedPages = pages.map((page) => ({
    ...
    h_repair_source: page.h_repair_source,
    renderer_repair_directives_applied: page.renderer_repair_directives_applied,
    product_surface_detail_count: page.product_surface_detail_count,
    connector_or_edge_binding_count: page.connector_or_edge_binding_count,
  ...
  const result = {
    ...
    source_h_evaluation: {
      ...
      primary_layer: visualQualityEvaluation.root_cause_summary?.primary_layer,
      next_required_action: visualQualityEvaluation.next_required_action,
      source_result: RUN2_75_INPUTS.visualQualityEvaluation,
    },
    ...
    renderer_repair_checks: {
      ...
      pages_with_h_repair_directive_consumed: renderedPages.filter((page) => page.renderer_repair_directives_applied.includes("h_repair_instruction_consumed")).length,
      pages_with_concrete_product_surface: renderedPages.filter((page) => page.product_surface_detail_count >= 5).length,
      pages_with_stronger_connector_or_edge_binding: renderedPages.filter((page) => page.connector_or_edge_binding_count >= 3).length,
      distinct_visual_density_profiles: new Set(renderedPages.map((page) => page.visual_density_profile)).size,
      public_quality_verdict_started: false,
    },
    preliminary_viewer_observations: [
      "Generated full arm applies the Part H renderer repair directives to the A-F scene model.",
      "Viewer is updated for local inspection, but Part J must still evaluate visual quality.",
    ],
    release_boundary: "public_blocked_until_part_j_visual_quality_evaluation_and_human_release_approval",
  ...
  fs.writeFileSync(
    ...
    [
      "# Run 2.75 Renderer Repair Rerun",
      "",
      "Status: generated locally, public blocked.",
      "",
      "Run 2.75 consumes the 2.73 A-F inputs plus the Part H visual quality evaluation before native PPT drawing.",
      "",
      "Best internal arm: `run2_75_full_renderer_repair`.",
      "",
      "Viewer check: generated output is available for local browser inspection. Part J must still evaluate visual quality.",
      "",
      "Generated outputs:",
      "",
...
async function main() {
  ...
  const full = built.find((item) => item.arm.armId === "run2_75_full_renderer_repair");
  const standaloneHtml = path.join(full.workspace, "output", "run2-75-renderer-repair.html");
  buildStandaloneHtml(pages, standaloneHtml);
  writeJson(path.join(outRoot, "run2_75_renderer_repair_rerun_summary.json"), {
    run_id: "run2_75_renderer_repair_four_arms",
    arms: built.map((item) => item.arm.armId),
    consumed_sources: CONSUMED_SOURCES,
    combined_contact_sheet: repoRelative(fourArmSheet),
...
if (process.argv[1] && path.resolve(process.argv[1]) === __filename) {
  ...
  process.stdout.write(`${JSON.stringify(result.renderer_repair_manifest, null, 2)}\n`);
}
