...
function renderFull(slide, spec, arm, n) {
  if (spec.role === "cover") {
    ...
    text(slide, "Run 2.5 adds production reference decompositions, aesthetic memory v2, and native visual modules.", 78, 382, 610, 58, {
      fontSize: 17,
      color: "#bed0c8",
    });
...
function renderSlide(presentation, spec, arm, n) {
  ...
  base(slide, arm, n);
  if (arm.armId === "run2_5_full_skill") {
...
const baseSlides = [
  ...
  },
  {
    ...
    cards: [
      ...
      ["Run 2.5", "not used", "forbidden input"],
    ],
    sideTitle: "Boundary",
  ...
  },
  {
    ...
    sideTitle: "Next",
    sideBody: "Compare against Run 1.5, Run 2.5 full, and bad aesthetic memory.",
...
const fullSlides = [
  {
    ...
    claim: "Run 2.5 turns reference decompositions into native production modules.",
...
const armSpecs = [
  {
    armId: "prompt_only",
    slug: "ppt-run2-5-prompt-only",
    label: "Prompt-only control",
    kicker: "RUN 2.5 / CONTROL",
  ...
  {
    armId: "run1_5_skill",
    slug: "ppt-run2-5-run1-5-skill",
    label: "Run 1.5 evidence-heavy baseline",
    kicker: "RUN 2.5 / BASELINE",
    footer: "run1_5_skill | prior product-lab workflow | public blocked",
    boundaryTitle: "prior workflow",
    boundaryBody: "Uses Run 1.5 workflow and the commercial case; no Run 2.5 motion or sequence data.",
    ...
    slides: baseSlides.map((slide, index) => ({
      ...
      trace: "Run 1.5 workflow; no Run 2.5 motion grammar",
    })),
  },
  {
    armId: "run2_5_full_skill",
    slug: "ppt-run2-5-full-vulca",
    label: "Run 2.5 full Vulca skill",
    kicker: "RUN 2.5 / FULL VULCA",
    footer: "run2_5_full_skill | production modules + native sequence contracts | public blocked",
  ...
  },
  {
    ...
    slug: "ppt-run2-5-bad-aesthetic-memory",
    label: "Bad aesthetic memory negative control",
    kicker: "RUN 2.5 / NEGATIVE CONTROL",
    ...
    forbidden: [`${pack}/aesthetic_memory.json`, `${pack}/aesthetic_memory_v2.json`, `${pack}/visual_production_modules.json`, `${pack}/slide_archetypes.json`, "docs/product/ppt-run1-5-product-lab/", "manual aesthetic repair before scoring"],
    ...
    slides: baseSlides.map((slide, index) => ({
      ...
      trace: "valid Run 2.5 data; bad aesthetic memory replacement",
    })),
  },
];

function sequenceStepsForSlide(arm, role) {
  if (!["run2_5_full_skill", "bad_aesthetic_memory"].includes(arm.armId)) return [];
  const ids = motionByRole[role]?.sequence_component_ids ?? [];
  return ids.flatMap((id) => sequenceStepsById.get(id) ?? []);
}

function traceFor(arm) {
  const productionEligible = ["run2_5_full_skill", "bad_aesthetic_memory"].includes(arm.armId);
  const fullProduction = arm.armId === "run2_5_full_skill";
  return {
    schema_version: 1,
    arm_id: arm.armId,
    render_style_arm_id: arm.armId,
    generation_brief: `${pack}/generation_briefs/${arm.armId === "run2_5_full_skill" ? "run2_skill" : arm.armId}.md`,
    ...
    runtime_isolation: {
      ...
      prompt_context: "fresh Run 2.5 arm-specific generation from scripts/generate_ppt_run2_5_arms.mjs",
...
async function buildArm(arm) {
  ...
  const manifest = {
    ...
    slides: slides.map((_, index) => ({
      ...
      source: "scripts/generate_ppt_run2_5_arms.mjs",
