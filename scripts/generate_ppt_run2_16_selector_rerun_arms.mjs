...
const EXPECTED_RUN2_16_TRACE_FIELDS = [
...
const RUN2_16_INPUTS = {
  evidence: `${pack}/run2_12_thick_multimodal_evidence.json`,
  memory: `${pack}/run2_12_design_memory_seed.json`,
  gateSeed: `${pack}/run2_12_workflow_gate_seed.json`,
  priorVisualSources: `${pack}/run2_10_visual_system_sources.json`,
  priorVisualMemory: `${pack}/run2_10_visual_system_memory.json`,
  priorVisualGateMatrix: `${pack}/run2_10_visual_system_gate_matrix.json`,
  selectorSources: `${pack}/run2_15_layout_selector_sources.json`,
  selectorMemory: `${pack}/run2_15_layout_module_memory.json`,
  selectorGateMatrix: `${pack}/run2_15_layout_selector_gate_matrix.json`,
};
const RUN2_16_DATA_INPUTS = [RUN2_16_INPUTS.evidence, RUN2_16_INPUTS.memory, RUN2_16_INPUTS.gateSeed];
const RUN2_16_AESTHETIC_INPUTS = [
  RUN2_16_INPUTS.priorVisualSources,
  RUN2_16_INPUTS.priorVisualMemory,
  RUN2_16_INPUTS.priorVisualGateMatrix,
];
const RUN2_16_SELECTOR_INPUTS = [
  RUN2_16_INPUTS.selectorSources,
  RUN2_16_INPUTS.selectorMemory,
  RUN2_16_INPUTS.selectorGateMatrix,
];
const RUN2_16_RESTRICTED_INPUTS = [...RUN2_16_DATA_INPUTS, ...RUN2_16_AESTHETIC_INPUTS, ...RUN2_16_SELECTOR_INPUTS];

const baseSlides = [
  {
    role: "cover",
    title: "The deck stops showing its machinery.",
    claim: "Run 2.16 keeps the data trace in the system and gives the slide surface back to the story.",
    trace: "2.10 visual shell + 2.13 trace core",
  },
  {
    role: "setup",
  ...
  },
  {
    ...
    trace: "visual delta from Run 2.13",
  },
  {
    role: "proof",
...
const armSpecs = [
  {
    armId: "prompt_only",
    slug: "ppt-run2-16-prompt-only",
    label: "Prompt-only control",
    kicker: "RUN 2.16 / CONTROL",
    footer: "prompt_only | commercial case only | public blocked",
    release: "public_blocked",
    allowed: [`${pack}/commercial_case.md`],
    forbidden: [
      `${pack}/run2_12_thick_multimodal_evidence.json`,
      `${pack}/run2_12_design_memory_seed.json`,
      `${pack}/run2_12_workflow_gate_seed.json`,
      `${pack}/run2_10_visual_system_sources.json`,
      `${pack}/run2_10_visual_system_memory.json`,
      `${pack}/run2_10_visual_system_gate_matrix.json`,
      `${pack}/run2_15_layout_selector_sources.json`,
      `${pack}/run2_15_layout_module_memory.json`,
      `${pack}/run2_15_layout_selector_gate_matrix.json`,
  ...
  {
    armId: "run1_5_skill",
    slug: "ppt-run2-16-run1-5-skill",
    label: "Run 1.5 baseline",
    kicker: "RUN 2.16 / RUN 1.5",
  ...
  {
    armId: "run2_16_full_skill",
    slug: "ppt-run2-16-full-vulca",
    label: "Run 2.16 full Vulca skill",
    kicker: "RUN 2.16 / FULL VULCA",
    footer: "run2_16_full_skill | 2.15 selector + 2.10 shell | public blocked",
    release: "public_blocked",
    allowed: [
  ...
  },
  {
    ...
    slug: "ppt-run2-16-bad-selector-memory",
    label: "Bad selector-memory control",
    kicker: "RUN 2.16 / NEGATIVE CONTROL",
    footer: "bad_selector_memory | selector sources without module memory/gates | internal comparison",
    ...
    forbidden: [
      ...
      `${pack}/skill_workflow.json`,
      `${pack}/results/trace_manifest_contract.json`,
      "manual Run 2.16 selector module repair before scoring",
    ],
    palette: {
      bg: "#f2efe1",
      rail: "#6d633e",
...
function registerRun216Module(metrics, functionName) {
...
function drawRun216EditorialCoverField(slide, arm, spec, module, metrics, opts = {}) {
  registerRun216Module(metrics, "drawRun216EditorialCoverField");
...
function drawRun216BeforeAfterRoute(slide, arm, spec, module, metrics, opts = {}) {
  registerRun216Module(metrics, "drawRun216BeforeAfterRoute");
  const x = opts.x ?? 80;
  const y = opts.y ?? 118;
  const w = opts.w ?? 1040;
  const h = opts.h ?? 430;
...
function drawRun216ProductTheaterStage(slide, arm, spec, module, metrics, opts = {}) {
  registerRun216Module(metrics, "drawRun216ProductTheaterStage");
  const x = opts.x ?? 604;
  const y = opts.y ?? 134;
  const w = opts.w ?? 504;
  const h = opts.h ?? 352;
...
function drawRun216DenseEvidenceCompression(slide, arm, spec, module, metrics, opts = {}) {
  registerRun216Module(metrics, "drawRun216DenseEvidenceCompression");
  const x = opts.x ?? 104;
  const y = opts.y ?? 352;
...
function drawRun216QuietReleaseHandoff(slide, arm, spec, module, metrics, opts = {}) {
  registerRun216Module(metrics, "drawRun216QuietReleaseHandoff");
  const x = opts.x ?? 100;
  const y = opts.y ?? 430;
  const w = opts.w ?? 880;
  const h = opts.h ?? 126;
...
function drawRun216MetricRevealStage(slide, arm, spec, module, metrics, opts = {}) {
  registerRun216Module(metrics, "drawRun216MetricRevealStage");
...
function assertArmInputBoundaries(arm) {
  ...
  if (arm.armId === "run2_16_full_skill") {
    for (const input of RUN2_16_RESTRICTED_INPUTS) assertAllowed(input);
    return;
  }
  if (arm.armId === "bad_selector_memory") {
    assertAllowed(RUN2_16_INPUTS.selectorSources);
    for (const input of RUN2_16_DATA_INPUTS) assertForbidden(input);
    for (const input of RUN2_16_AESTHETIC_INPUTS) assertForbidden(input);
    assertForbidden(RUN2_16_INPUTS.selectorMemory);
    assertForbidden(RUN2_16_INPUTS.selectorGateMatrix);
    return;
  }
  for (const input of RUN2_16_RESTRICTED_INPUTS) assertForbidden(input);
}

function readRun216JsonForArm(arm, relPath) {
  assertArmInputBoundaries(arm);
  if (arm.armId !== "run2_16_full_skill") {
    throw new Error(`${arm.armId} cannot read Run 2.16 selector/full-arm input: ${relPath}`);
  }
...
function readRun216SelectorJsonForArm(arm, relPath) {
  assertArmInputBoundaries(arm);
  if (!arm.allowed.includes(relPath) || arm.forbidden.includes(relPath)) {
    throw new Error(`${arm.armId} input boundary does not permit reading ${relPath}`);
  }
  if (arm.armId !== "run2_16_full_skill" && relPath !== RUN2_16_INPUTS.selectorSources) {
    throw new Error(`${arm.armId} cannot read Run 2.15 selector memory or gate matrix: ${relPath}`);
  }
  return readJson(relPath);
}

function validateRun215SelectorSchemas(selectorSources, selectorMemory, selectorGateMatrix) {
  if (selectorSources?.status !== "run2_15_layout_selector_sources_ready") {
    throw new Error("Run 2.15 selector sources schema/status mismatch");
  }
  if (selectorMemory?.status !== "run2_15_layout_module_memory_ready") {
    throw new Error("Run 2.15 selector memory schema/status mismatch");
  }
  if (selectorGateMatrix?.status !== "run2_15_layout_selector_gate_matrix_ready") {
    throw new Error("Run 2.15 selector gate matrix schema/status mismatch");
  }
  if (!Array.isArray(selectorSources.records) || selectorSources.records.length < 6) {
    throw new Error("Run 2.15 selector sources must contain records");
  }
  if (!Array.isArray(selectorMemory.modules) || selectorMemory.modules.length < 6) {
    throw new Error("Run 2.15 selector memory must contain modules");
  }
  if (!Array.isArray(selectorGateMatrix.gates) || selectorGateMatrix.gates.length < 6) {
    throw new Error("Run 2.15 selector gate matrix must contain gates");
  }
  const sourceIds = new Set(selectorSources.records.map((record) => record.record_id));
  for (const module of selectorMemory.modules) {
    if (!module.module_id || !Array.isArray(module.slide_roles) || !Array.isArray(module.source_record_ids)) {
      throw new Error(`Run 2.15 selector module is incomplete: ${module.module_id ?? "unknown"}`);
    }
    for (const sourceId of module.source_record_ids) {
      if (!sourceIds.has(sourceId)) throw new Error(`Run 2.15 selector module ${module.module_id} references missing source ${sourceId}`);
    }
  }
  const moduleIds = new Set(selectorMemory.modules.map((module) => module.module_id));
  for (const gate of selectorGateMatrix.gates) {
    if (!gate.gate_id || !Array.isArray(gate.candidate_module_ids) || !Array.isArray(gate.trace_fields)) {
      throw new Error(`Run 2.15 selector gate is incomplete: ${gate.gate_id ?? "unknown"}`);
    }
    for (const moduleId of gate.candidate_module_ids) {
      if (!moduleIds.has(moduleId)) throw new Error(`Run 2.15 selector gate ${gate.gate_id} references missing module ${moduleId}`);
    }
  }
}

function selectRun215LayoutModulesForSlide(role, selectorMemory, selectorGateMatrix) {
  const roleModules = (selectorMemory?.modules ?? []).filter((module) => module.slide_roles?.includes(role));
  const primaryModules = roleModules.filter((module) => module.module_family !== "dense_evidence_compression");
  const denseModules = roleModules.filter((module) => module.module_family === "dense_evidence_compression");
  const modules = role === "proof" ? [...primaryModules, ...denseModules] : primaryModules.length ? primaryModules : denseModules;
  const gates = (selectorGateMatrix?.gates ?? []).filter((gate) => {
    if (gate.slide_role === role) return true;
    if (gate.slide_role === "all") return true;
    return gate.candidate_module_ids?.some((moduleId) => modules.some((module) => module.module_id === moduleId));
  });
  if (!modules.length) throw new Error(`Run 2.15 selector returned no layout module for role ${role}`);
  if (!gates.length) throw new Error(`Run 2.15 selector returned no gate for role ${role}`);
  return { modules, gates };
}

function run216WorkflowGateSeedsByRole(gateSeed) {
  const map = new Map();
  for (const gate of gateSeed?.gates ?? []) {
    for (const role of gate.slide_roles ?? []) {
...
function run216EvidenceRecordsById(evidence) {
  return new Map((evidence?.records ?? []).map((record) => [record.id, record]));
}

function run216MemorySeedsById(memory) {
  return new Map((memory?.memory_seeds ?? []).map((seed) => [seed.id, seed]));
}

function run216EvidenceRecordsByRole(evidence, gateSeed) {
  const evidenceById = run216EvidenceRecordsById(evidence);
  const map = new Map();
  for (const gate of gateSeed?.gates ?? []) {
    for (const role of gate.slide_roles ?? []) {
      const bucket = map.get(role) ?? [];
      for (const id of gate.evidence_record_ids ?? []) {
        const record = evidenceById.get(id);
        if (record && !bucket.some((item) => item.id === record.id)) bucket.push(record);
      }
      map.set(role, bucket);
    }
  }
...
function run216MemorySeedsByRole(memory) {
  const map = new Map();
  for (const seed of memory?.memory_seeds ?? []) {
    for (const role of seed.applies_to_slide_roles ?? []) {
      const bucket = map.get(role) ?? [];
      bucket.push(seed);
      map.set(role, bucket);
    }
...
function run216CodeModulesByRole() {
  return new Map(
    Object.entries({
      cover: ["drawRun216EditorialCoverField", "drawRun216BeforeAfterRoute"],
      setup: ["drawRun216ProductTheaterStage", "drawRun216BeforeAfterRoute"],
      contrast: ["drawRun216EditorialCoverField", "drawRun216BeforeAfterRoute"],
      proof: ["drawRun216ProductTheaterStage", "drawRun216DenseEvidenceCompression"],
      climax: ["drawRun216MetricRevealStage"],
      close: ["drawRun216EditorialCoverField", "drawRun216QuietReleaseHandoff"],
    }),
  );
}

function run216ShapeBudgetByRole() {
  const baseBudget = {
    max_text_boxes: 18,
...
function run216VisualDeltaByRole() {
  return new Map(
    Object.entries({
      cover: "Run 2.16 restores the Run 2.10 presentation shell while keeping Run 2.13 evidence trace in manifest fields.",
      setup: "Workflow selection becomes an off-surface composition rule instead of a visible gate rail.",
      contrast: "The before-after comparison becomes editorial again, not a diagram of the generator.",
      proof: "Product theater carries the proof while trace details stay in run2_12_* and run2_16_* fields.",
      climax: "The resolved metric/product state owns the stage; gate evidence is hidden from the slide surface.",
      close: "Release remains blocked through trace and QA, but the slide presents a quiet handoff.",
    }),
  );
}

function buildBadSelectorSourcesByRole(selectorSources) {
  const records = selectorSources?.records ?? [];
  return new Map([
    ["cover", records.filter((record) => record.source_family === "editorial_cover").slice(0, 1)],
    ["setup", records.filter((record) => record.source_family === "product_theater").slice(0, 1)],
    ["contrast", records.filter((record) => record.source_family === "before_after_route").slice(0, 1)],
    ["proof", records.filter((record) => record.source_family === "product_theater").slice(0, 1)],
    ["climax", records.filter((record) => record.source_family === "metric_reveal").slice(0, 1)],
    ["close", records.filter((record) => record.source_family === "quiet_handoff").slice(0, 1)],
  ]);
}

function loadRun216ContractData(arm) {
  const evidence = readRun216JsonForArm(arm, RUN2_16_INPUTS.evidence);
  const memory = readRun216JsonForArm(arm, RUN2_16_INPUTS.memory);
  const gateSeed = readRun216JsonForArm(arm, RUN2_16_INPUTS.gateSeed);
  const priorVisualSources = readRun216JsonForArm(arm, RUN2_16_INPUTS.priorVisualSources);
  const priorVisualMemory = readRun216JsonForArm(arm, RUN2_16_INPUTS.priorVisualMemory);
  const priorVisualGateMatrix = readRun216JsonForArm(arm, RUN2_16_INPUTS.priorVisualGateMatrix);
  const selectorSources = readRun216SelectorJsonForArm(arm, RUN2_16_INPUTS.selectorSources);
  const selectorMemory = readRun216SelectorJsonForArm(arm, RUN2_16_INPUTS.selectorMemory);
  const selectorGateMatrix = readRun216SelectorJsonForArm(arm, RUN2_16_INPUTS.selectorGateMatrix);
  validateRun215SelectorSchemas(selectorSources, selectorMemory, selectorGateMatrix);
  const workflowGateSeedsByRole = run216WorkflowGateSeedsByRole(gateSeed);
  const evidenceRecordsByRole = run216EvidenceRecordsByRole(evidence, gateSeed);
  const memorySeedsByRole = run216MemorySeedsByRole(memory);
  const codeModulesByRole = run216CodeModulesByRole();
  const shapeBudgetByRole = run216ShapeBudgetByRole();
  const visualDeltaByRole = run216VisualDeltaByRole();
  const densityGate = gateSeed.gates.find((gate) => gate.id === "gate_2_12_visual_density_and_whitespace");
  const demoGate = gateSeed.gates.find((gate) => gate.id === "gate_2_12_demo_sequence_climax");
  return {
    evidence,
    memory,
    gateSeed,
    priorVisualSources,
    priorVisualMemory,
    priorVisualGateMatrix,
    selectorSources,
    selectorMemory,
    selectorGateMatrix,
    aestheticShellInputIds: RUN2_16_AESTHETIC_INPUTS.map((input) => path.basename(input)),
    selectorInputIds: RUN2_16_SELECTOR_INPUTS.map((input) => path.basename(input)),
    ...
    densityGate,
    demoGate,
    status: "run2_16_selector_rerun_contract_ready",
  };
}

function renderRun216Full(arm) {
  const contract = loadRun216ContractData(arm);
  return {
    ...contract,
    status: "run2_16_selector_rerun_contract_ready",
  };
...
function renderRun216FullSlide(presentation, spec, arm, n, fullData, metricsByRole) {
  const slide = presentation.slides.add();
  base(slide, arm, n);
  const metrics = createSlideMetrics(spec.role);

  if (spec.role === "cover") {
    drawRun216EditorialCoverField(slide, arm, spec, null, metrics, {
      x: 82,
      y: 120,
      w: 760,
      h: 240,
    ...
    drawRun216BeforeAfterRoute(slide, arm, spec, null, metrics, {
      x: 76,
      y: 382,
      w: 1020,
      h: 176,
    ...
    drawRun216ProductTheaterStage(slide, arm, spec, null, metrics, {
      x: 596,
      y: 128,
      w: 496,
      h: 330,
    ...
    drawRun216DenseEvidenceCompression(slide, arm, spec, null, metrics, {
      x: 666,
      y: 372,
      w: 444,
      h: 126,
    ...
    drawRun216MetricRevealStage(slide, arm, spec, null, metrics);
    drawRun216QuietReleaseHandoff(slide, arm, spec, null, metrics, {
      x: 236,
      y: 530,
      w: 720,
      h: 78,
...
function renderControlSlide(presentation, spec, arm, n, metricsByRole) {
  ...
  drawControlGate(slide, arm, spec, metrics, {
    ...
    line: arm.armId === "bad_selector_memory" ? "no module/gate read" : "no Run 2.16 selector trace",
  });
  proofFooter(slide, spec, arm);
  metricsByRole.set(spec.role, metrics);
  return slide;
...
function traceFor(arm, context = {}) {
  assertArmInputBoundaries(arm);
  const fullRun216 = arm.armId === "run2_16_full_skill";
  const fullData = fullRun216 ? context.fullData ?? renderRun216Full(arm) : null;
  ...
  const badSelectorSourcesByRole = context.badSelectorSourcesByRole ?? new Map();
  const metricsByRole = context.metricsByRole ?? new Map();
  const hasRenderedMetrics =
    fullRun216 && arm.slides.every((slide) => metricsByRole.has(slide.role));
  return {
    schema_version: 1,
    arm_id: arm.armId,
    ...
    runtime_isolation: {
      ...
      prompt_context: "fresh Run 2.16 selector-rerun arm-specific generation from scripts/generate_ppt_run2_16_selector_rerun_arms.mjs",
    ...
    slides: arm.slides.map((slide, index) => {
      ...
      const roleSelector = fullRun216
        ? selectRun215LayoutModulesForSlide(slide.role, fullData.selectorMemory, fullData.selectorGateMatrix)
        : { modules: [], gates: [] };
      const selectedModuleIds = roleSelector.modules.map((module) => module.module_id);
      const selectedGateIds = roleSelector.gates.map((gate) => gate.gate_id);
      const selectedModuleFamilies = roleSelector.modules.map((module) => module.module_family);
      return {
        slide_id: `slide_${String(index + 1).padStart(2, "0")}`,
        role: slide.role,
        title: slide.title,
        run2_16_contract_status: fullRun216
          ? hasRenderedMetrics
            ? "full_arm_native_generator_rendered"
            : "full_arm_contract_preview_not_rendered"
          : "boundary_control_not_run2_16_full",
        run2_12_evidence_record_ids: fullRun216 ? roleEvidenceRecords.map((record) => record.id) : [],
        run2_12_memory_seed_ids: fullRun216 ? roleMemorySeeds.map((seed) => seed.id) : [],
        run2_12_workflow_gate_ids: fullRun216 ? roleGates.map((gateSeed) => gateSeed.id) : [],
        run2_15_selected_layout_module_ids: fullRun216 ? selectedModuleIds : [],
        run2_15_selector_gate_ids: fullRun216 ? selectedGateIds : [],
        run2_15_trace_visibility_policy: fullRun216 ? "manifest_viewer_qa_only_for_public_surface" : "",
        run2_15_text_resilience_result: fullRun216
          ? selectedModuleFamilies.includes("dense_evidence_compression")
            ? "selector_chose_compression_or_split_over_unreadable_shrink"
            : "selector_text_budget_within_readable_range"
          : "",
        run2_15_trace_hidden_result: fullRun216 ? "trace_hidden_from_public_slide_surface_pass" : "",
        run2_15_product_surface_probe: fullRun216
          ? selectedModuleFamilies.includes("product_theater_stage")
            ? "native_layered_product_surface_selected_no_copied_screenshot"
            : "product_surface_not_primary_for_slide_role"
          : "",
        run2_15_metric_reveal_climax_probe: fullRun216
          ? slide.role === "climax"
            ? "metric_reveal_climax_module_selected"
            : "not_climax_slide"
          : "",
        run2_15_bad_selector_control_probe: fullRun216
          ? "bad_selector_memory forbids layout module memory and selector gate matrix"
          : "boundary_control",
        run2_16_aesthetic_source_run_id: fullRun216 ? "2.10" : "",
        run2_16_aesthetic_shell_input_ids: fullRun216 ? fullData.aestheticShellInputIds ?? [] : [],
        run2_16_selector_input_ids: fullRun216 ? fullData.selectorInputIds ?? [] : [],
        run2_16_selector_execution_status: fullRun216 ? "selector_gate_matrix_executed_before_native_ppt_generation" : "",
        run2_16_surface_policy: fullRun216 ? "manifest_only_trace_public_surface" : "",
        run2_16_visible_workflow_suppression: fullRun216 ? "visible_workflow_suppression_pass" : "",
        run2_16_required_code_module_ids: fullRun216 ? codeModulesByRole.get(slide.role) ?? [] : [],
        run2_16_code_module_ids: fullRun216
          ? hasRenderedMetrics
            ? actualCodeModuleIds
            : codeModulesByRole.get(slide.role) ?? []
          : [],
        run2_16_visual_delta_from_run2_13: fullRun216 ? visualDeltaByRole.get(slide.role) ?? "" : "",
        run2_16_bad_control_probe: fullRun216
          ? "bad_selector_memory uses Run 2.15 selector sources without layout module memory or selector gate matrix; if it matches full arm, selector did not add value"
          : "boundary_control",
        run2_16_trace_origin: hasRenderedMetrics ? "actual_native_selector_rerun_module_calls" : fullRun216 ? "contract_preview_required_selector_rerun_modules" : "boundary_control",
        negative_control_selector_source_ids:
          arm.armId === "bad_selector_memory" ? (badSelectorSourcesByRole.get(slide.role) ?? []).map((source) => source.record_id) : [],
        layout_metrics: {
...
function assertRun216SelectorGateSelfCheck(trace) {
  if (trace.arm_id !== "run2_16_full_skill") return;
  for (const slide of trace.slides) {
    if (slide.run2_16_contract_status !== "full_arm_native_generator_rendered") {
      throw new Error(`Run 2.16 full slide ${slide.slide_id} was not rendered with native module metrics`);
    }
    for (const field of EXPECTED_RUN2_16_TRACE_FIELDS) {
      ...
      if (empty) throw new Error(`Run 2.16 full slide ${slide.slide_id} missing ${field}`);
    }
    if (!slide.layout_budget_status?.within_budget) {
      throw new Error(`Run 2.16 full slide ${slide.slide_id} exceeded layout budget: ${slide.layout_budget_status.violations.join("; ")}`);
    }
    const actualCodeModules = new Set(slide.run2_16_code_module_ids ?? []);
    for (const requiredCodeModule of slide.run2_16_required_code_module_ids ?? []) {
      if (!actualCodeModules.has(requiredCodeModule)) {
        throw new Error(`Run 2.16 full slide ${slide.slide_id} did not call required module ${requiredCodeModule}`);
      }
    }
    if (slide.role === "climax") {
      if (!actualCodeModules.has("drawRun216MetricRevealStage")) throw new Error("Run 2.16 climax did not call drawRun216MetricRevealStage");
      const share = slide.layout_metrics?.hero_object_canvas_share ?? 0;
      if (!share) throw new Error("Run 2.16 climax missing hero object share");
    }
    if (slide.run2_16_surface_policy !== "manifest_only_trace_public_surface") {
      throw new Error(`Run 2.16 full slide ${slide.slide_id} did not keep trace manifest-only`);
    }
  }
}
...
function buildArmContract() {
  return armSpecs.map((arm) => ({
    ...
    label: arm.label,
    contract_status:
      arm.armId === "run2_16_full_skill"
        ? "run2_16_selector_rerun_contract_ready_requires_render_metrics"
        : "run2_16_boundary_control_contract_ready",
    allowed: arm.allowed,
...
function buildRun216FourArmSheet(built) {
  const sheets = built.map((item) => item.contactSheet).filter((file) => fs.existsSync(file));
  if (!sheets.length) return "";
  return buildNamedContactSheet(path.join(outRoot, "run2-16-four-arm-contact-sheet.png"), "Run 2.16 four-arm comparison", sheets, sheets.length);
}
...
async function buildArm(arm) {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "source requirements: commercial case always; bad control may read Run 2.16 selector-rerun sources only; full arm additionally requires selector-rerun memory and gate matrix",
  ...
  let fullData = null;
  let badSelectorSourcesByRole = new Map();
  if (arm.armId === "run2_16_full_skill") {
    fullData = loadRun216ContractData(arm);
  } else if (arm.armId === "bad_selector_memory") {
    const selectorSources = readRun216SelectorJsonForArm(arm, RUN2_16_INPUTS.selectorSources);
    badSelectorSourcesByRole = buildBadSelectorSourcesByRole(selectorSources);
  }

  const slides = arm.slides.map((slide, index) =>
    arm.armId === "run2_16_full_skill"
      ? renderRun216FullSlide(presentation, slide, arm, index + 1, fullData, metricsByRole)
  ...
  const trace = traceFor(arm, { fullData, badSelectorSourcesByRole, metricsByRole });
  assertRun216SelectorGateSelfCheck(trace);
  ...
  const manifest = {
    ...
    slides: slides.map((_, index) => ({
      ...
      source: "scripts/generate_ppt_run2_16_selector_rerun_arms.mjs",
...
async function main() {
  ...
  const fourArmSheet = buildRun216FourArmSheet(built);
  const fullSkillSeriesSheet = buildFullSkillSeriesSheet();
  const runSummary = {
    run_id: "run2_16_selector_rerun_four_arms",
  ...
  writeJson(path.join(outRoot, "run2_16_selector_rerun_summary.json"), runSummary);
  return runSummary;
}
...
export {
  ...
  main,
  registerRun216Module,
  registerVisualModule,
  renderRun216Full,
  readRun216SelectorJsonForArm,
  run216CodeModulesByRole,
  run216ShapeBudgetByRole,
  run216EvidenceRecordsById,
  run216EvidenceRecordsByRole,
  run216VisualDeltaByRole,
  run216WorkflowGateSeedsByRole,
  run216MemorySeedsById,
  run216MemorySeedsByRole,
