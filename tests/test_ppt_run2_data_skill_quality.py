...
EXPECTED_RUN2_N_RESULT = (
    PACK / "results" / "run2_80_visual_quality_evaluation.json"
)
EXPECTED_RUN2_N_SCRIPT = ROOT / "scripts" / "audit_ppt_run2_80_visual_quality_evaluation.py"
EXPECTED_RUN2_N_QUESTIONS = {
...
EXPECTED_RUN2_N_ROOT_CAUSE_LAYERS = {
    "renderer",
    "visual_grammar",
    "text_binding",
    "content",
}
EXPECTED_RUN2_O1_RESULT = PACK / "run2_81_text_composition_typography_plan.json"
EXPECTED_RUN2_O1_REQUIRED_INPUTS = [
...
EXPECTED_RUN2_O1_FORBIDDEN_PATTERNS = {
    "floating labels",
...
EXPECTED_RUN2_O2_REQUIRED_INPUTS = [
    "docs/product/ppt-run2-data-skill-quality/results/run2_80_visual_quality_evaluation.json",
...
EXPECTED_RUN2_O2_RESULT = (
    PACK / "results" / "run2_82_renderer_product_surface_text_composition_rerun_result.json"
)
EXPECTED_RUN2_O2_SCRIPT = ROOT / "scripts" / "generate_ppt_run2_82_product_surface_text_composition_arms.mjs"
EXPECTED_RUN2_O2_REPAIR_FLAGS = {
...
EXPECTED_RUN2_P0_RESULT = (
    PACK / "results" / "run2_83_workflow_taxonomy_bias_audit.json"
)
EXPECTED_RUN2_P0_SCRIPT = ROOT / "scripts" / "audit_ppt_run2_83_workflow_taxonomy_bias.py"
EXPECTED_RUN2_P0_REQUIRED_INPUTS = [
...
EXPECTED_RUN2_P0_MISSING_TAXONOMY = {
    "motif_id",
    "motif_family",
    "layout_recipe",
    "spatial_relation",
...
EXPECTED_RUN2_P0_PRESERVED_GATES = {
    "traceability",
    "source_availability",
...
EXPECTED_RUN2_P2_RESULT = (
    PACK / "results" / "run2_85_design_motif_renderer_rerun_result.json"
)
EXPECTED_RUN2_P2_SCRIPT = ROOT / "scripts" / "generate_ppt_run2_85_design_motif_renderer_arms.mjs"
EXPECTED_RUN2_P2_REQUIRED_INPUTS = [
    "docs/product/ppt-run2-data-skill-quality/run2_84_design_motif_taxonomy_style_router_plan.json",
    "docs/product/ppt-run2-data-skill-quality/results/run2_82_renderer_product_surface_text_composition_rerun_result.json",
]
EXPECTED_RUN2_P2_REPAIR_FLAGS = {
...
EXPECTED_RUN2_S_RESULT = (
    PACK / "results" / "run2_88_best_layout_visual_primitive_rerun_result.json"
)
EXPECTED_RUN2_S_SCRIPT = ROOT / "scripts" / "generate_ppt_run2_88_best_layout_visual_primitive_arms.mjs"
EXPECTED_RUN2_S_REQUIRED_INPUTS = [
...
EXPECTED_RUN2_T_RESULT = (
    PACK / "results" / "run2_89_visual_quality_evaluation.json"
)
EXPECTED_RUN2_T_SCRIPT = ROOT / "scripts" / "audit_ppt_run2_89_visual_quality_evaluation.py"
EXPECTED_RUN2_T_QUESTIONS = {
...
EXPECTED_RUN2_T_ROOT_CAUSE_LAYERS = {
...
EXPECTED_RUN2_U_RESULT = (
    PACK / "results" / "run2_90_renderer_asset_surface_composition_rerun_result.json"
)
EXPECTED_RUN2_U_SCRIPT = ROOT / "scripts" / "generate_ppt_run2_90_renderer_asset_surface_composition_arms.mjs"
EXPECTED_RUN2_U_REQUIRED_INPUTS = [
...
EXPECTED_RUN2_V_RESULT = (
    PACK / "results" / "run2_91_visual_quality_evaluation.json"
)
EXPECTED_RUN2_V_SCRIPT = ROOT / "scripts" / "audit_ppt_run2_91_visual_quality_evaluation.py"
EXPECTED_RUN2_V_QUESTIONS = {
...
EXPECTED_RUN2_V_ROOT_CAUSE_LAYERS = {
    "text_visual_integration",
    "object_bound_typography",
    "caption_anchor_binding",
    "proof_object_embedding",
    "composition_rhythm",
    "renderer_surface_materiality",
}
EXPECTED_RUN2_W_RESULT = (
    PACK / "results" / "run2_92_renderer_text_visual_binding_repair_rerun_result.json"
)
EXPECTED_RUN2_W_SCRIPT = ROOT / "scripts" / "generate_ppt_run2_92_renderer_text_visual_binding_repair_arms.mjs"
EXPECTED_RUN2_W_REQUIRED_INPUTS = [
...
def test_run2_80_visual_quality_evaluation_compares_2_79_against_2_77_with_gemini(
    tmp_path: Path,
) -> None:
    assert EXPECTED_RUN2_N_SCRIPT.exists(), "missing Part N visual quality evaluation script"
    script = EXPECTED_RUN2_N_SCRIPT.read_text(encoding="utf-8")
    assert_contains(
        script,
        [
            "run2_79_renderer_art_direction_repair_rerun_result.json",
            "run2_78_visual_quality_evaluation.json",
            "run2_77_visual_grammar_renderer_repair_rerun_result.json",
            "gemini-3.5-flash",
            "ppt-run2-77-full-vulca/preview/contact-sheet.png",
    ...
    result_json = tmp_path / "run2_80_visual_quality_evaluation.json"
    result_md = tmp_path / "run2_80_visual_quality_evaluation.md"

    completed = subprocess.run(
        [
            sys.executable,
            str(EXPECTED_RUN2_N_SCRIPT),
            "--result-json",
            str(result_json),
            "--result-md",
            str(result_md),
    ...
    audit = load_json(result_json)
    report = result_md.read_text(encoding="utf-8")
    assert "run2_80_visual_quality_evaluation_public_blocked" in completed.stdout
    assert audit["artifact_id"] == "run2_80_visual_quality_evaluation"
    assert audit["part"] == "Part N"
    assert audit["run_id"] == "2.80"
    assert audit["status"] == "run2_80_visual_quality_evaluation_public_blocked"
    assert audit["creates_new_ppt_deck"] is False
    assert audit["starts_renderer_rerun"] is False
    assert audit["updates_html_viewer"] is False
    assert audit["public_ready"] is False
    assert audit["public_release_started"] is False
    assert audit["quality_claim_boundary"] == "part_n_evaluation_only_no_public_release_no_renderer_rerun"
    assert audit["source_runs"] == {
    ...
    input_chain = audit["input_chain"]
    assert input_chain["run2_79_result"].endswith("run2_79_renderer_art_direction_repair_rerun_result.json")
    assert input_chain["run2_78_l_evaluation"].endswith("run2_78_visual_quality_evaluation.json")
    assert input_chain["run2_77_result"].endswith("run2_77_visual_grammar_renderer_repair_rerun_result.json")
    assert input_chain["run2_77_full_contact_sheet"].endswith("ppt-run2-77-full-vulca/preview/contact-sheet.png")
    assert input_chain["run2_79_full_contact_sheet"].endswith("ppt-run2-79-full-vulca/preview/contact-sheet.png")

    closure = audit["viewer_comparison_closure"]
    assert closure["viewer_latest_run_id"] == "2.92"
    assert closure["viewer_can_compare_2_77_and_2_79"] is True
    assert closure["run2_77_full_preview_count"] == 6
    assert closure["run2_79_full_preview_count"] == 6
    assert closure["browser_check_required_for_handoff"] is True
    ...
    assert gemini["review_count"] == 1
    assert gemini["used_for_verdict"] is True
    assert "minimalist" in " ".join(gemini["run2_79_findings"])
    assert "product surface" in " ".join(gemini["run2_79_risks"])

    questions = audit["evaluation_questions"]
    assert set(questions) == EXPECTED_RUN2_N_QUESTIONS
    assert questions["is_2_79_better_than_2_77"]["answer"] == (
        "mixed_annotation_down_but_product_surface_absent_public_blocked"
    )
    assert questions["did_2_79_reduce_wireframe_and_annotation"]["answer"] == (
        "partial_debug_annotations_removed_but_typographic_wireframe_remains"
    )
    assert questions["did_2_79_fix_small_label_problem"]["answer"] == (
        "no_labels_still_tiny_and_spatially_scattered"
    )
    assert questions["did_2_79_create_concrete_product_surface"]["answer"] == (
        "no_product_surface_not_visibly_realized"
    )
    assert questions["does_2_79_reach_public_video_presentation_direction"]["answer"] == (
        "no_public_blocked"
    )
    ...
    assert assessment["design_quality_gate"] == "blocked"
    assert assessment["public_video_readiness"] == "blocked"
    assert assessment["global_delta_vs_2_77"] == (
        "debug_annotations_down_but_product_surface_absent_and_small_labels_remain"
    )
    assert assessment["top_blocker"] == (
        "product_surface_not_visibly_realized_and_slides_read_as_sparse_text_wireframes"
    )
    assert assessment["next_layer_to_fix"] == "renderer_product_surface_realization"
    ...
    for record in audit["role_assessments"]:
        assert record["visual_grammar_module"] == EXPECTED_RUN2_E_PAGE_MODULE_MAP[record["role"]]
        assert record["root_cause_layer"] in EXPECTED_RUN2_N_ROOT_CAUSE_LAYERS
        assert record["delta_vs_2_77"] in {"improved", "partial", "unchanged", "regressed"}
        assert record["wireframe_reduction"] in {"strong", "partial", "weak", "none"}
        assert record["label_hierarchy"] in {"improved", "partial", "weak"}
        assert record["product_surface_realization"] in {"strong", "partial", "weak", "absent"}
        assert record["public_video_direction"] in {"no", "partial", "yes"}
        assert record["next_repair_instruction"]
        assert record["trace_support"]["art_direction_scene"]
        assert record["trace_support"]["label_count"] <= 3
        assert record["trace_support"]["debug_annotation_count"] == 0

    assert audit["root_cause_summary"]["primary_layer"] == "renderer_product_surface_realization"
    assert audit["root_cause_summary"]["not_primary_layer"] == "data_absence"
    assert audit["no_new_renderer_proof"]["new_pptx_created"] is False
    assert audit["no_new_renderer_proof"]["new_html_created"] is False
    assert audit["next_required_action"] == "part_o_renderer_product_surface_repair_from_n_evaluation"
    assert_contains(
        report,
        [
            "Run 2.80 Visual Quality Evaluation",
            "2.79 vs 2.77",
            "debug annotations down",
...
def test_run2_80_records_visual_quality_evaluation_result() -> None:
    audit = load_json(EXPECTED_RUN2_N_RESULT)
    report = (
        PACK / "results" / "run2_80_visual_quality_evaluation.md"
    ).read_text(encoding="utf-8")
    ...
    assert audit["public_release_started"] is False
    assert audit["visual_quality_assessment"]["design_quality_gate"] == "blocked"
    assert audit["visual_quality_assessment"]["global_delta_vs_2_77"] == (
        "debug_annotations_down_but_product_surface_absent_and_small_labels_remain"
    )
    assert audit["viewer_comparison_closure"]["viewer_latest_run_id"] == "2.79"
    assert len(audit["role_assessments"]) == 6
    assert "product surface absent" in report


def test_run2_81_text_composition_typography_plan_consolidates_existing_typography_memory() -> None:
    plan = load_json(EXPECTED_RUN2_O1_RESULT)

    assert plan["artifact_id"] == "run2_81_text_composition_typography_plan"
    assert plan["part"] == "Part O1"
    assert plan["run_id"] == "2.81"
    assert plan["status"] == "run2_81_text_composition_typography_plan_ready_public_blocked"
    assert plan["stage_policy"] == (
        "part_o1_text_composition_typography_plan_only_no_renderer_rerun_no_public_release"
    )
    assert plan["creates_new_ppt_deck"] is False
    assert plan["starts_renderer_rerun"] is False
    assert plan["updates_html_viewer"] is False
    assert plan["public_release_started"] is False
    assert plan["public_ready"] is False
    assert plan["consumed_sources"] == EXPECTED_RUN2_O1_REQUIRED_INPUTS

    source_paths = {source["path"] for source in plan["source_inputs"]}
    assert set(EXPECTED_RUN2_O1_REQUIRED_INPUTS) <= source_paths
    assert all(source["available"] is True for source in plan["source_inputs"])
    assert plan["source_n_evaluation"]["status"] == "run2_80_visual_quality_evaluation_public_blocked"
    assert plan["source_n_evaluation"]["top_blocker"] == (
        "product_surface_not_visibly_realized_and_slides_read_as_sparse_text_wireframes"
    )

    contract = plan["global_text_composition_contract"]
    assert set(contract["required_canvas_text_blocks"]) == {
        "headline_block",
        "subhead_block",
        "proof_sentence",
        "object_caption",
    }
    assert set(contract["global_forbidden_patterns"]) >= EXPECTED_RUN2_O1_FORBIDDEN_PATTERNS
    assert contract["traceability_default_route"] == "viewer_metadata_and_speaker_notes"
    assert contract["slide_canvas_traceability_allowed"] is False
    assert contract["minimum_main_text_region_count"] == 1

    records = plan["page_text_composition_records"]
    assert [record["role"] for record in records] == EXPECTED_RUN2_74_SLIDE_STORY_ROLES
    assert [record["slide_index"] for record in records] == [1, 2, 3, 4, 5, 6]
    for record in records:
        role = record["role"]
        assert record["visual_grammar_module"] == EXPECTED_RUN2_E_PAGE_MODULE_MAP[role]
        assert record["text_composition_id"] == f"text_composition_2_81_{role}"
        assert record["source_text_binding_id"] == f"text_binding_2_73_{role}"
        assert record["source_n_role_assessment"]["repair_required"] is True
        assert set(record["forbidden_patterns"]) >= EXPECTED_RUN2_O1_FORBIDDEN_PATTERNS

        for block_name in ["headline_block", "subhead_block", "proof_sentence", "object_caption"]:
            block = record[block_name]
            assert block["copy_intent"]
            assert block["object_anchor_required"] is True
            assert block["bound_visual_object_type"] in {
                "product surface",
                "evidence object",
                "comparison object",
                "decision object",
            }
            assert block["capacity"]["max_words"] > 0
            assert block["capacity"]["max_lines"] > 0
            assert block["typography"]["min_font_size"] >= 12
            assert block["canvas_route"] == "slide_canvas"

        label_policy = record["label_policy"]
        assert label_policy["max_visible_labels"] <= 3
        assert label_policy["min_font_size"] >= 12
        assert label_policy["must_attach_to_visible_object"] is True
        assert label_policy["floating_labels_allowed"] is False
        assert label_policy["overflow_route"] == "viewer_metadata_and_speaker_notes"

        route = record["viewer_note_route"]
        assert route["target"] == "viewer_metadata_and_speaker_notes"
        assert route["traceability_on_canvas"] is False
        assert route["speaker_note_allowed"] is True

    summary = plan["traceability_summary"]
    assert summary["page_text_composition_record_count"] == 6
    assert summary["source_input_count"] == len(EXPECTED_RUN2_O1_REQUIRED_INPUTS)
    assert summary["renderer_rerun_started"] is False
    assert plan["next_required_action"] == (
        "part_o2_renderer_rerun_from_text_composition_and_product_surface_repair"
    )


def test_run2_82_product_surface_text_composition_rerun_consumes_o1_and_updates_viewer() -> None:
    assert EXPECTED_RUN2_O2_SCRIPT.exists(), "missing Part O2 product surface/text composition renderer script"
    script = EXPECTED_RUN2_O2_SCRIPT.read_text(encoding="utf-8")
    assert_contains(
        script,
        [
            *EXPECTED_RUN2_O2_REQUIRED_INPUTS,
            "run2_82_renderer_product_surface_text_composition_rerun_result.json",
            "run2_82_renderer_product_surface_text_composition_rerun_result.md",
            "build_ppt_run_html_viewer.py",
        ],
    )

    o1 = load_json(EXPECTED_RUN2_O1_RESULT)
    n_eval = load_json(EXPECTED_RUN2_N_RESULT)
    run279 = load_json(EXPECTED_RUN2_M_RESULT)
    result = load_json(EXPECTED_RUN2_O2_RESULT)
    o1_by_role = {record["role"]: record for record in o1["page_text_composition_records"]}
    n_by_role = {record["role"]: record for record in n_eval["role_assessments"]}
    run279_by_role = {record["role"]: record for record in run279["rendered_pages"]}

    assert result["artifact_id"] == "run2_82_renderer_product_surface_text_composition_rerun_result"
    assert result["part"] == "Part O2"
    assert result["run_id"] == "2.82"
    assert result["status"] == "run2_82_renderer_product_surface_text_composition_rerun_generated_public_blocked"
    assert result["public_ready"] is False
    assert result["public_release_started"] is False
    assert result["quality_claim_boundary"] == (
        "renderer_product_surface_text_composition_generated_viewer_check_only_no_part_p_quality_verdict"
    )
    assert result["consumed_sources"] == EXPECTED_RUN2_O2_REQUIRED_INPUTS
    assert result["source_n_evaluation"]["status"] == n_eval["status"]
    assert result["source_o1_text_composition_plan"]["status"] == o1["status"]

    manifest = result["renderer_product_surface_text_composition_manifest"]
    assert manifest["generator"] == "scripts/generate_ppt_run2_82_product_surface_text_composition_arms.mjs"
    assert manifest["consumed_sources"] == EXPECTED_RUN2_O2_REQUIRED_INPUTS
    assert manifest["best_internal_arm"] == "run2_82_full_product_surface_text_composition"
    assert manifest["viewer_update"]["latest_run_id"] == "2.82"
    assert manifest["viewer_update"]["viewer_can_reference_new_run"] is True
    ...
    for page in pages:
        role = page["role"]
        assert page["visual_grammar_module"] == EXPECTED_RUN2_E_PAGE_MODULE_MAP[role]
        assert page["source_o1_text_composition_id"] == o1_by_role[role]["text_composition_id"]
        assert page["source_n_repair_instruction"] == n_by_role[role]["next_repair_instruction"]
        assert page["source_run2_79_page"]["art_direction_scene"] == run279_by_role[role]["art_direction_scene"]
        assert set(page["renderer_repair_directives_applied"]) >= EXPECTED_RUN2_O2_REPAIR_FLAGS
        assert page["concrete_product_surface_visible"] is True
        assert page["product_surface_type"] in {
        ...
        assert page["text_hierarchy"] == "headline_subhead_proof_caption"
        assert set(page["text_composition_blocks_applied"]) == {
            "headline_block",
            "subhead_block",
            "proof_sentence",
            "object_caption",
        }
        assert page["floating_label_count"] == 0
        assert page["label_count"] <= 3
        assert page["min_visible_label_font_size"] >= 12
        assert page["canvas_word_count"] >= 24
        assert page["source_trace_terms_visible_on_canvas"] == []
        assert page["public_polish_claimed"] is False

    checks = result["renderer_product_surface_text_composition_checks"]
    assert checks["pages_with_o1_text_composition_consumed"] == 6
    assert checks["pages_with_concrete_product_surface"] == 6
    assert checks["pages_with_text_hierarchy_applied"] == 6
    assert checks["pages_with_floating_labels_removed"] == 6
    assert checks["pages_with_traceability_routed_off_canvas"] == 6
    assert checks["public_quality_verdict_started"] is False
    ...
    assert "data-run-id=\"2.82\"" in html_output.read_text(encoding="utf-8")

    viewer_data = build_data(ROOT / "outputs" / DEFAULT_THREAD_ID / "presentations", viewer_output)
    run = next(run for run in viewer_data["runs"] if run["id"] == "2.82")
    assert run["fullArm"]["id"] == "run2_82_full_product_surface_text_composition"
    assert len(run["fullArm"]["slides"]) == 6
    assert result["next_required_action"] == "part_p_visual_quality_evaluation_for_run2_82"


def test_run2_83_workflow_taxonomy_bias_audit_preserves_rigor_and_identifies_design_gap() -> None:
    assert EXPECTED_RUN2_P0_SCRIPT.exists(), "missing Part P0 workflow/taxonomy bias audit script"
    script = EXPECTED_RUN2_P0_SCRIPT.read_text(encoding="utf-8")
    assert_contains(
        script,
        [
            *EXPECTED_RUN2_P0_REQUIRED_INPUTS,
            "run2_83_workflow_taxonomy_bias_audit.json",
            "run2_83_workflow_taxonomy_bias_audit.md",
            "design_motif_layer",
        ],
    )

    audit = load_json(EXPECTED_RUN2_P0_RESULT)
    assert audit["artifact_id"] == "run2_83_workflow_taxonomy_bias_audit"
    assert audit["part"] == "Part P0"
    assert audit["run_id"] == "2.83"
    assert audit["status"] == "run2_83_workflow_taxonomy_bias_audit_ready_public_blocked"
    assert audit["stage_policy"] == (
        "part_p0_audit_only_no_renderer_rerun_no_viewer_update_no_public_release"
    )
    assert audit["creates_new_ppt_deck"] is False
    assert audit["starts_renderer_rerun"] is False
    assert audit["updates_html_viewer"] is False
    assert audit["public_release_started"] is False
    assert audit["public_ready"] is False
    assert audit["quality_claim_boundary"] == (
        "workflow_taxonomy_bias_audit_only_no_visual_quality_pass_no_public_release"
    )
    assert audit["consumed_sources"] == EXPECTED_RUN2_P0_REQUIRED_INPUTS

    source_paths = {source["path"] for source in audit["source_inputs"]}
    assert set(EXPECTED_RUN2_P0_REQUIRED_INPUTS) <= source_paths
    assert all(source["available"] is True for source in audit["source_inputs"])

    preservation = audit["engineering_rigor_preservation"]
    assert set(preservation["preserve_existing_gates"]) >= EXPECTED_RUN2_P0_PRESERVED_GATES
    assert preservation["do_not_weaken_traceability"] is True
    assert preservation["public_release_gate_remains_blocked"] is True
    assert preservation["design_layer_adds_to_engineering_layer"] is True

    summary = audit["taxonomy_bias_summary"]
    assert summary["primary_bias"] == "engineering_constraint_labels_over_design_motif_labels"
    assert summary["root_cause"] == "design_motif_taxonomy_missing_between_tutorial_memory_and_renderer_adapter"
    assert "design_motif_layer" in summary["required_correction"]

    layers = audit["layer_bias_records"]
    assert {layer["layer_id"] for layer in layers} == EXPECTED_RUN2_P0_LAYER_IDS
    for layer in layers:
        assert layer["engineering_strength"]
        assert layer["design_signal_loss"]
        assert layer["impact_on_ppt"]
        assert layer["must_preserve"]
        assert layer["needs_new_design_layer"] is True
        assert layer["bias_direction"] in {
    ...
    pattern = audit["run2_series_pattern"]
    assert [stage["stage_range"] for stage in pattern] == [
        "2.7-2.18",
        "2.24-2.42",
        "2.43-2.64",
        "2.73-2.82",
    ]
    assert all(stage["effect_on_ppt"] for stage in pattern)

    missing = audit["missing_design_taxonomy"]
    assert {item["field"] for item in missing} == EXPECTED_RUN2_P0_MISSING_TAXONOMY
    assert all(item["why_needed"] for item in missing)
    assert all(item["renderer_contract_implication"] for item in missing)

    next_layer = audit["required_next_layer"]
    assert next_layer["layer_id"] == "design_motif_layer"
    assert set(next_layer["must_add_fields"]) >= EXPECTED_RUN2_P0_MISSING_TAXONOMY
    assert next_layer["must_preserve_engineering_gates"] is True
    assert next_layer["must_not_replace_validator"] is True

    assert audit["no_new_renderer_proof"] == {
        "new_ppt_created": False,
        "new_html_created": False,
        "viewer_updated": False,
    }
    assert audit["next_required_action"] == "part_p1_design_motif_taxonomy_and_style_router_plan"

    report = (
        PACK / "results" / "run2_83_workflow_taxonomy_bias_audit.md"
    ).read_text(encoding="utf-8")
    assert_contains(
        report,
        [
            "engineering constraint labels",
            "design motif layer",
            "public blocked",
...
def test_run2_85_design_motif_renderer_rerun_consumes_p1_and_updates_viewer() -> None:
    assert EXPECTED_RUN2_P2_SCRIPT.exists(), "missing Part P2 design motif renderer script"
    script = EXPECTED_RUN2_P2_SCRIPT.read_text(encoding="utf-8")
    assert_contains(
        script,
        [
            *EXPECTED_RUN2_P2_REQUIRED_INPUTS,
            "run2_85_design_motif_renderer_rerun_result.json",
            "run2_85_design_motif_renderer_rerun_result.md",
            "build_ppt_run_html_viewer.py",
            "bad_run2_85_without_design_motif_layer",
        ],
    )

    p1 = load_json(EXPECTED_RUN2_P1_RESULT)
    o2 = load_json(EXPECTED_RUN2_O2_RESULT)
    result = load_json(EXPECTED_RUN2_P2_RESULT)
    motif_by_id = {motif["motif_id"]: motif for motif in p1["design_motif_taxonomy"]}
    binding_by_role = {binding["role"]: binding for binding in p1["page_role_motif_bindings"]}
    o2_by_role = {page["role"]: page for page in o2["rendered_pages"]}

    assert result["artifact_id"] == "run2_85_design_motif_renderer_rerun_result"
    assert result["part"] == "Part P2"
    assert result["run_id"] == "2.85"
    assert result["status"] == "run2_85_design_motif_renderer_rerun_generated_public_blocked"
    assert result["public_ready"] is False
    assert result["public_release_started"] is False
    assert result["quality_claim_boundary"] == (
        "design_motif_renderer_generated_viewer_check_only_no_part_q_quality_verdict"
    )
    assert result["consumed_sources"] == EXPECTED_RUN2_P2_REQUIRED_INPUTS
    assert result["source_p1_design_motif_plan"]["status"] == p1["status"]
    assert result["source_o2_renderer_result"]["status"] == o2["status"]

    manifest = result["renderer_design_motif_manifest"]
    assert manifest["generator"] == "scripts/generate_ppt_run2_85_design_motif_renderer_arms.mjs"
    assert manifest["consumed_sources"] == EXPECTED_RUN2_P2_REQUIRED_INPUTS
    assert manifest["best_internal_arm"] == "run2_85_full_design_motif_style_router"
    assert manifest["viewer_update"]["latest_run_id"] == "2.85"
    assert manifest["viewer_update"]["viewer_can_reference_new_run"] is True
    assert set(manifest["arms"]) == {
        "prompt_only",
        "run1_5_skill",
    ...
    pptx_output = ROOT / outputs["pptx"]
    viewer_output = ROOT / outputs["ppt_run_viewer"]
    four_arm_sheet = ROOT / outputs["four_arm_contact_sheet"]
    assert html_output.exists()
    assert pptx_output.exists()
    assert viewer_output.exists()
    assert four_arm_sheet.exists()
    ...
    for page in pages:
        ...
        motif = motif_by_id[binding["primary_motif_id"]]
        assert page["visual_grammar_module"] == EXPECTED_RUN2_E_PAGE_MODULE_MAP[role]
        assert page["source_p1_primary_motif_id"] == binding["primary_motif_id"]
        assert page["source_p1_fallback_motif_id"] == binding["fallback_motif_id"]
        assert page["source_o2_product_surface_type"] == o2_by_role[role]["product_surface_type"]
        assert page["motif_family"] == motif["motif_family"]
        assert page["style_family"] == binding["style_family"]
        assert page["scenario"] == binding["scenario"]
        assert page["visual_density"] in {"sparse", "balanced", "dense", "climax"}
        assert set(page["renderer_repair_directives_applied"]) >= EXPECTED_RUN2_P2_REPAIR_FLAGS
        assert set(page["preserved_visual_effects_rendered"]) & EXPECTED_RUN2_P1_PRESERVED_EFFECTS
        assert set(page["motif_fidelity_checks"]) >= set(binding["required_motif_fidelity_checks"])
        assert page["motif_family_visible"] is True
        assert page["not_rectangle_only"] is True
        assert page["text_integrated_with_shape"] is True
        assert page["concrete_product_surface_visible"] is True
        assert page["text_hierarchy"] == "motif_aware_headline_subhead_proof_caption"
        assert page["floating_label_count"] == 0
        assert page["label_count"] <= 3
        assert page["min_visible_label_font_size"] >= 12
        assert page["source_trace_terms_visible_on_canvas"] == []
    ...
    checks = result["renderer_design_motif_checks"]
    assert checks["pages_with_p1_motif_consumed"] == 6
    assert checks["pages_with_motif_family_visible"] == 6
    assert checks["pages_with_not_rectangle_only"] == 6
    assert checks["pages_with_text_integrated_with_shape"] == 6
    assert checks["pages_with_traceability_routed_off_canvas"] == 6
    assert checks["public_quality_verdict_started"] is False
    ...
    assert "data-run-id=\"2.85\"" in html_output.read_text(encoding="utf-8")

    viewer_data = build_data(ROOT / "outputs" / DEFAULT_THREAD_ID / "presentations", viewer_output)
    run = next(run for run in viewer_data["runs"] if run["id"] == "2.85")
...
def test_run2_88_best_layout_visual_primitive_rerun_consumes_r_and_updates_viewer() -> None:
    assert EXPECTED_RUN2_S_SCRIPT.exists(), "missing Part S best-layout visual primitive renderer script"
    script = EXPECTED_RUN2_S_SCRIPT.read_text(encoding="utf-8")
    assert_contains(
        script,
        [
            *EXPECTED_RUN2_S_REQUIRED_INPUTS,
            "run2_88_best_layout_visual_primitive_rerun_result.json",
            "run2_88_best_layout_visual_primitive_rerun_result.md",
            "build_ppt_run_html_viewer.py",
            "bad_without_best_layout_visual_primitives",
            "drawRun287ProductTheaterSurface",
            "drawRun287OverlayStickerStack",
        ],
    )

    plan = load_json(EXPECTED_RUN2_R_PLAN)
    run285 = load_json(EXPECTED_RUN2_P2_RESULT)
    result = load_json(EXPECTED_RUN2_S_RESULT)
    primitive_by_role = {
        record["role"]: record["renderer_primitive_id"]
        for record in plan["page_layout_recovery_records"]
    }

    assert result["artifact_id"] == "run2_88_best_layout_visual_primitive_rerun_result"
    assert result["part"] == "Part S"
    assert result["run_id"] == "2.88"
    assert result["status"] == "run2_88_best_layout_visual_primitive_rerun_generated_public_blocked"
    assert result["public_ready"] is False
    assert result["public_release_started"] is False
    assert result["quality_claim_boundary"] == (
        "best_layout_visual_primitive_renderer_generated_viewer_check_only_no_quality_verdict"
    )
    assert result["consumed_sources"] == EXPECTED_RUN2_S_REQUIRED_INPUTS
    assert result["source_part_r_plan"]["status"] == plan["status"]
    assert result["source_run2_85_renderer_result"]["status"] == run285["status"]

    manifest = result["renderer_best_layout_visual_primitive_manifest"]
    assert manifest["generator"] == "scripts/generate_ppt_run2_88_best_layout_visual_primitive_arms.mjs"
    assert manifest["consumed_sources"] == EXPECTED_RUN2_S_REQUIRED_INPUTS
    assert manifest["best_internal_arm"] == "run2_88_full_best_layout_visual_primitives"
    assert manifest["viewer_update"]["latest_run_id"] == "2.88"
    assert manifest["viewer_update"]["viewer_can_reference_new_run"] is True
    assert set(manifest["arms"]) == EXPECTED_RUN2_S_ARMS
    ...
    for page in pages:
        role = page["role"]
        assert page["visual_grammar_module"] == EXPECTED_RUN2_E_PAGE_MODULE_MAP[role]
        assert page["renderer_primitive_id"] == primitive_by_role[role]
        assert page["renderer_function_name"].startswith("drawRun287")
        assert set(page["renderer_repair_directives_applied"]) >= EXPECTED_RUN2_S_REPAIR_FLAGS
        assert set(page["anti_regression_gates"]) >= {
            "not_rectangle_only",
            "text_integrated_with_shape",
            "collision_avoidance",
            "motif_fidelity_visible",
        }
        assert page["historical_layout_recovered"] is True
        assert page["not_rectangle_only"] is True
        assert page["text_integrated_with_shape"] is True
        assert page["collision_avoidance_passed"] is True
        assert page["traceability_on_canvas"] is False
        assert page["floating_label_count"] == 0
        assert page["label_count"] <= 3
        assert page["min_visible_label_font_size"] >= 12
        assert page["public_polish_claimed"] is False

    checks = result["renderer_best_layout_visual_primitive_checks"]
    assert checks["pages_with_part_r_consumed"] == 6
    assert checks["pages_with_historical_layout_recovered"] == 6
    assert checks["pages_with_visual_primitive_rendered"] == 6
    assert checks["pages_with_collision_avoidance"] == 6
    assert checks["pages_with_traceability_routed_off_canvas"] == 6
    assert checks["public_quality_verdict_started"] is False
    ...
    assert "data-run-id=\"2.88\"" in html_output.read_text(encoding="utf-8")

    viewer_data = build_data(ROOT / "outputs" / DEFAULT_THREAD_ID / "presentations", viewer_output)
    run = next(run for run in viewer_data["runs"] if run["id"] == "2.88")
    assert run["fullArm"]["id"] == "run2_88_full_best_layout_visual_primitives"
    assert len(run["fullArm"]["slides"]) == 6
    assert result["next_required_action"] == "part_t_visual_quality_evaluation_for_run2_88"


def test_run2_89_visual_quality_evaluation_compares_2_88_against_2_85_with_gemini(
    tmp_path: Path,
) -> None:
    assert EXPECTED_RUN2_T_SCRIPT.exists(), "missing Part T visual quality evaluation script"
    script = EXPECTED_RUN2_T_SCRIPT.read_text(encoding="utf-8")
    assert_contains(
        script,
        [
            "run2_88_best_layout_visual_primitive_rerun_result.json",
    ...
    result_json = tmp_path / "run2_89_visual_quality_evaluation.json"
    result_md = tmp_path / "run2_89_visual_quality_evaluation.md"

    completed = subprocess.run(
        [
            sys.executable,
            str(EXPECTED_RUN2_T_SCRIPT),
            "--result-json",
            str(result_json),
            "--result-md",
            str(result_md),
    ...
    audit = load_json(result_json)
    report = result_md.read_text(encoding="utf-8")
    assert "run2_89_visual_quality_evaluation_public_blocked" in completed.stdout
    assert audit["artifact_id"] == "run2_89_visual_quality_evaluation"
    assert audit["part"] == "Part T"
    assert audit["run_id"] == "2.89"
    assert audit["status"] == "run2_89_visual_quality_evaluation_public_blocked"
    assert audit["creates_new_ppt_deck"] is False
    assert audit["starts_renderer_rerun"] is False
    assert audit["updates_html_viewer"] is False
    assert audit["public_ready"] is False
    assert audit["public_release_started"] is False
    assert audit["quality_claim_boundary"] == "part_t_evaluation_only_no_public_release_no_renderer_rerun"
    assert audit["source_runs"] == {
    ...
    input_chain = audit["input_chain"]
    assert input_chain["run2_88_result"].endswith("run2_88_best_layout_visual_primitive_rerun_result.json")
    assert input_chain["run2_87_plan"].endswith("run2_87_best_layout_recovery_visual_primitive_plan.json")
    assert input_chain["run2_85_result"].endswith("run2_85_design_motif_renderer_rerun_result.json")
    assert input_chain["run2_85_full_contact_sheet"].endswith("ppt-run2-85-full-vulca/preview/contact-sheet.png")
    assert input_chain["run2_88_full_contact_sheet"].endswith("ppt-run2-88-full-vulca/preview/contact-sheet.png")
    assert input_chain["run2_88_four_arm_contact_sheet"].endswith("run2-88-four-arm-contact-sheet.png")

    closure = audit["viewer_comparison_closure"]
    assert closure["viewer_latest_run_id"] == "2.92"
    assert closure["viewer_can_compare_2_85_and_2_88"] is True
    assert closure["run2_85_full_preview_count"] == 6
    assert closure["run2_88_full_preview_count"] == 6
    assert closure["run2_88_arm_count"] == 4
    assert closure["browser_check_required_for_handoff"] is True

    gemini = audit["gemini_agent_review_summary"]
    assert gemini["tool"] == "gemini-agent artifact-review"
    assert gemini["model"] == "gemini-3.5-flash"
    assert gemini["review_count"] == 1
    assert gemini["used_for_verdict"] is True
    assert "structurally identical" in " ".join(gemini["run2_88_findings"])
    assert "wireframe-like" in " ".join(gemini["run2_88_risks"])
    assert "Slide 06" in " ".join(gemini["run2_88_risks"])

    questions = audit["evaluation_questions"]
    assert set(questions) == EXPECTED_RUN2_T_QUESTIONS
    assert questions["is_2_88_better_than_2_85"]["answer"] == (
        "partial_semantic_framing_up_visual_delta_small_public_blocked"
    )
    assert questions["did_2_88_recover_best_layout_visual_primitives"]["answer"] == (
        "partial_primitives_named_but_layout_engine_positions_remain_similar"
    )
    assert questions["did_2_88_keep_text_heavy_readability"]["answer"] == (
        "yes_text_hierarchy_readable_but_sparse_and_wireframe"
    )
    assert questions["did_2_88_preserve_modular_matrix_and_sticker_effects"]["answer"] == (
        "partial_slide_04_05_preserved_but_not_high_fidelity"
    )
    assert questions["does_2_88_fix_late_2_series_aesthetic_bottleneck"]["answer"] == (
        "no_visual_execution_still_wireframe_like"
    )
    assert questions["does_2_88_reach_public_video_presentation_direction"]["answer"] == "no_public_blocked"
    assert questions["which_layer_needs_next_repair"]["answer"] == (
        "renderer_asset_surface_and_composition_detail"
    )

    assessment = audit["visual_quality_assessment"]
    assert assessment["design_quality_gate"] == "blocked"
    assert assessment["public_video_readiness"] == "blocked"
    assert assessment["global_delta_vs_2_85"] == (
        "best_layout_language_up_but_visual_structure_mostly_unchanged"
    )
    assert assessment["top_blocker"] == "layout_primitive_names_changed_but_visual_execution_remains_wireframe_like"
    assert assessment["next_layer_to_fix"] == "renderer_asset_surface_and_composition_detail"

    assert len(audit["role_assessments"]) == 6
    assert [record["role"] for record in audit["role_assessments"]] == EXPECTED_RUN2_74_SLIDE_STORY_ROLES
    assert sum(1 for record in audit["role_assessments"] if record["repair_required"]) == 6
    for record in audit["role_assessments"]:
        assert record["visual_grammar_module"] == EXPECTED_RUN2_E_PAGE_MODULE_MAP[record["role"]]
        assert record["root_cause_layer"] in EXPECTED_RUN2_T_ROOT_CAUSE_LAYERS
        assert record["delta_vs_2_85"] in {"improved", "partial", "unchanged", "regressed"}
        assert record["best_layout_recovery"] in {"strong", "partial", "weak"}
        assert record["visual_primitive_fidelity"] in {"strong", "partial", "weak"}
        assert record["text_composition"] in {"strong", "partial", "rigid"}
        assert record["public_video_direction"] in {"no", "partial", "yes"}
        assert record["trace_support"]["renderer_primitive_id"].startswith("primitive_2_87_")
        assert record["trace_support"]["renderer_function_name"].startswith("drawRun287")
        assert record["trace_support"]["label_count"] <= 3
        assert record["trace_support"]["not_rectangle_only"] is True
        assert record["trace_support"]["text_integrated_with_shape"] is True

    summary = audit["root_cause_summary"]
    assert summary["primary_layer"] == "renderer_asset_surface_and_composition_detail"
    assert summary["not_primary_layer"] == "data_absence"
    assert summary["late_2_series_failure_mode"] == (
        "semantic_contracts_and_primitive_names_changed_faster_than_visible_composition"
    )
    assert audit["no_new_renderer_proof"]["new_pptx_created"] is False
    assert audit["no_new_renderer_proof"]["new_html_created"] is False
    assert audit["next_required_action"] == (
        "part_u_renderer_asset_surface_composition_repair_from_t_evaluation"
    )
    assert_contains(
        report,
        [
...
def test_run2_89_records_visual_quality_evaluation_result() -> None:
    audit = load_json(EXPECTED_RUN2_T_RESULT)
    report = (
        PACK / "results" / "run2_89_visual_quality_evaluation.md"
    ).read_text(encoding="utf-8")
    ...
    assert audit["public_release_started"] is False
    assert audit["visual_quality_assessment"]["design_quality_gate"] == "blocked"
    assert audit["visual_quality_assessment"]["global_delta_vs_2_85"] == (
        "best_layout_language_up_but_visual_structure_mostly_unchanged"
    )
    assert audit["viewer_comparison_closure"]["viewer_latest_run_id"] == "2.88"
    assert len(audit["role_assessments"]) == 6
    assert "layout-primitive language improved faster than visible composition" in report


def test_run2_90_renderer_asset_surface_composition_rerun_consumes_t_and_updates_viewer() -> None:
    assert EXPECTED_RUN2_U_SCRIPT.exists(), "missing Part U renderer asset surface composition script"
    script = EXPECTED_RUN2_U_SCRIPT.read_text(encoding="utf-8")
    assert_contains(
        script,
        [
            *EXPECTED_RUN2_U_REQUIRED_INPUTS,
            "run2_90_renderer_asset_surface_composition_rerun_result.json",
            "run2_90_renderer_asset_surface_composition_rerun_result.md",
            "build_ppt_run_html_viewer.py",
            "bad_without_asset_surface_composition",
            "drawRun290ProductSurfaceHero",
            "drawRun290EvidenceMatrixSurface",
            "drawRun290StickerStageSurface",
        ],
    )

    evaluation = load_json(EXPECTED_RUN2_T_RESULT)
    run288 = load_json(EXPECTED_RUN2_S_RESULT)
    result = load_json(EXPECTED_RUN2_U_RESULT)

    assert result["artifact_id"] == "run2_90_renderer_asset_surface_composition_rerun_result"
    assert result["part"] == "Part U"
    assert result["run_id"] == "2.90"
    assert result["status"] == "run2_90_renderer_asset_surface_composition_rerun_generated_public_blocked"
    assert result["public_ready"] is False
    assert result["public_release_started"] is False
    assert result["quality_claim_boundary"] == (
        "asset_surface_composition_renderer_generated_viewer_check_only_no_quality_verdict"
    )
    assert result["consumed_sources"] == EXPECTED_RUN2_U_REQUIRED_INPUTS
    assert result["source_t_evaluation"]["status"] == evaluation["status"]
    assert result["source_run2_88_renderer_result"]["status"] == run288["status"]
    assert result["source_t_evaluation"]["next_required_action"] == (
        "part_u_renderer_asset_surface_composition_repair_from_t_evaluation"
    )

    manifest = result["renderer_asset_surface_composition_manifest"]
    assert manifest["generator"] == "scripts/generate_ppt_run2_90_renderer_asset_surface_composition_arms.mjs"
    assert manifest["consumed_sources"] == EXPECTED_RUN2_U_REQUIRED_INPUTS
    assert manifest["best_internal_arm"] == "run2_90_full_asset_surface_composition"
    assert manifest["viewer_update"]["latest_run_id"] == "2.90"
    assert manifest["viewer_update"]["viewer_can_reference_new_run"] is True
    assert set(manifest["arms"]) == EXPECTED_RUN2_U_ARMS
    ...
    assert [page["role"] for page in pages] == EXPECTED_RUN2_74_SLIDE_STORY_ROLES
    assert [page["slide_index"] for page in pages] == [1, 2, 3, 4, 5, 6]
    assert len({page["asset_surface_composition_id"] for page in pages}) == 6
    for page in pages:
        role = page["role"]
        assert page["visual_grammar_module"] == EXPECTED_RUN2_E_PAGE_MODULE_MAP[role]
        assert page["source_t_root_cause_layer"] in EXPECTED_RUN2_T_ROOT_CAUSE_LAYERS
        assert page["renderer_function_name"].startswith("drawRun290")
        assert set(page["renderer_repair_directives_applied"]) >= EXPECTED_RUN2_U_REPAIR_FLAGS
        assert set(page["anti_regression_gates"]) >= {
        ...
        assert page["asset_surface_rendered"] is True
        assert page["composition_detail_added"] is True
        assert page["wireframe_like"] is False
        assert page["collision_avoidance_passed"] is True
        assert page["traceability_on_canvas"] is False
        assert page["floating_label_count"] == 0
        assert page["label_count"] <= 3
        assert page["min_visible_label_font_size"] >= 12
        assert page["surface_detail_count"] >= 8
        assert page["filled_surface_count"] >= 4
        assert page["mock_asset_count"] >= 2
        assert page["public_polish_claimed"] is False

    checks = result["renderer_asset_surface_composition_checks"]
    assert checks["pages_with_t_evaluation_consumed"] == 6
    assert checks["pages_with_asset_surface_rendered"] == 6
    assert checks["pages_with_composition_detail_added"] == 6
    assert checks["pages_with_wireframe_reduction"] == 6
    assert checks["pages_with_collision_avoidance"] == 6
    assert checks["pages_with_traceability_routed_off_canvas"] == 6
    assert checks["public_quality_verdict_started"] is False
    ...
    assert "data-run-id=\"2.90\"" in html_output.read_text(encoding="utf-8")

    viewer_data = build_data(ROOT / "outputs" / DEFAULT_THREAD_ID / "presentations", viewer_output)
    run = next(run for run in viewer_data["runs"] if run["id"] == "2.90")
    assert run["fullArm"]["id"] == "run2_90_full_asset_surface_composition"
    assert len(run["fullArm"]["slides"]) == 6
    assert result["next_required_action"] == "part_v_visual_quality_evaluation_for_run2_90"


def test_run2_91_visual_quality_evaluation_compares_2_90_against_2_88(
    tmp_path: Path,
) -> None:
    assert EXPECTED_RUN2_V_SCRIPT.exists(), "missing Part V visual quality evaluation script"
    script = EXPECTED_RUN2_V_SCRIPT.read_text(encoding="utf-8")
    assert_contains(
        script,
        [
            "run2_90_renderer_asset_surface_composition_rerun_result.json",
            "run2_89_visual_quality_evaluation.json",
            "run2_88_best_layout_visual_primitive_rerun_result.json",
            "ppt-run2-88-full-vulca/preview/contact-sheet.png",
    ...
    result_json = tmp_path / "run2_91_visual_quality_evaluation.json"
    result_md = tmp_path / "run2_91_visual_quality_evaluation.md"

    completed = subprocess.run(
        [
            sys.executable,
            str(EXPECTED_RUN2_V_SCRIPT),
            "--result-json",
            str(result_json),
            "--result-md",
            str(result_md),
    ...
    audit = load_json(result_json)
    report = result_md.read_text(encoding="utf-8")
    assert "run2_91_visual_quality_evaluation_public_blocked" in completed.stdout
    assert audit["artifact_id"] == "run2_91_visual_quality_evaluation"
    assert audit["part"] == "Part V"
    assert audit["run_id"] == "2.91"
    assert audit["status"] == "run2_91_visual_quality_evaluation_public_blocked"
    assert audit["creates_new_ppt_deck"] is False
    assert audit["starts_renderer_rerun"] is False
    assert audit["updates_html_viewer"] is False
    assert audit["public_ready"] is False
    assert audit["public_release_started"] is False
    assert audit["quality_claim_boundary"] == "part_v_evaluation_only_no_public_release_no_renderer_rerun"
    assert audit["source_runs"] == {
    ...
    assert input_chain["run2_89_t_evaluation"].endswith("run2_89_visual_quality_evaluation.json")
    assert input_chain["run2_88_result"].endswith("run2_88_best_layout_visual_primitive_rerun_result.json")
    assert input_chain["run2_88_full_contact_sheet"].endswith("ppt-run2-88-full-vulca/preview/contact-sheet.png")
    assert input_chain["run2_90_full_contact_sheet"].endswith("ppt-run2-90-full-vulca/preview/contact-sheet.png")
    assert input_chain["run2_90_four_arm_contact_sheet"].endswith("run2-90-four-arm-contact-sheet.png")

    closure = audit["viewer_comparison_closure"]
    assert closure["viewer_latest_run_id"] == "2.92"
    assert closure["viewer_can_compare_2_88_and_2_90"] is True
    assert closure["run2_88_full_preview_count"] == 6
    assert closure["run2_90_full_preview_count"] == 6
    assert closure["run2_90_arm_count"] == 4

    gemini = audit["gemini_agent_review_summary"]
    assert gemini["model"] == "gemini-3.5-flash"
    assert gemini["used_for_verdict"] is True
    assert gemini["review_count"] == 1
    assert "surface visibility" in " ".join(gemini["run2_90_findings"])
    assert "text-visual" in " ".join(gemini["run2_90_risks"])

    assert set(audit["evaluation_questions"]) == EXPECTED_RUN2_V_QUESTIONS
    assert audit["evaluation_questions"]["is_2_90_better_than_2_88"]["answer"] == (
        "yes_asset_surface_visibility_up_public_still_blocked"
    )
    assert audit["evaluation_questions"]["did_2_90_integrate_text_with_visual_objects"]["answer"] == (
        "no_text_visual_binding_still_weak"
    )
    assert audit["evaluation_questions"]["which_layer_needs_next_repair"]["answer"] == (
        "object_bound_typography_and_text_visual_integration"
    )
    ...
    assert assessment["design_quality_gate"] == "blocked"
    assert assessment["public_video_readiness"] == "blocked"
    assert assessment["global_delta_vs_2_88"] == (
        "asset_surface_visibility_up_text_visual_integration_still_split"
    )
    assert assessment["top_blocker"] == "text_blocks_and_product_surfaces_remain_parallel_not_integrated"
    assert assessment["next_layer_to_fix"] == "object_bound_typography_and_text_visual_integration"
    ...
    assert {record["root_cause_layer"] for record in role_assessments} <= EXPECTED_RUN2_V_ROOT_CAUSE_LAYERS
    assert all(record["repair_required"] is True for record in role_assessments)
    assert any(record["asset_surface_delta"] == "strong" for record in role_assessments)
    assert any(record["text_visual_integration"] == "weak" for record in role_assessments)
    for record in role_assessments:
        role = record["role"]
        assert record["visual_grammar_module"] == EXPECTED_RUN2_E_PAGE_MODULE_MAP[role]
        assert record["trace_support"]["renderer_function_name"].startswith("drawRun290")
        assert record["trace_support"]["asset_surface_rendered"] is True
        assert record["trace_support"]["traceability_on_canvas"] is False
        assert record["next_repair_instruction"]

    assert audit["root_cause_summary"]["primary_layer"] == "object_bound_typography_and_text_visual_integration"
    assert "text_visual_integration" in audit["root_cause_summary"]["secondary_layers"]
    assert audit["no_new_renderer_proof"]["new_pptx_created"] is False
    assert audit["no_new_renderer_proof"]["new_html_created"] is False
    assert audit["no_new_renderer_proof"]["starts_renderer_rerun"] is False
    assert audit["next_required_action"] == "part_w_renderer_text_visual_binding_repair_from_v_evaluation"
    assert_contains(
        report,
        [
...
def test_run2_91_records_visual_quality_evaluation_result() -> None:
    audit = load_json(EXPECTED_RUN2_V_RESULT)
    report = (
        PACK / "results" / "run2_91_visual_quality_evaluation.md"
    ).read_text(encoding="utf-8")
    ...
    assert audit["public_release_started"] is False
    assert audit["visual_quality_assessment"]["design_quality_gate"] == "blocked"
    assert audit["visual_quality_assessment"]["global_delta_vs_2_88"] == (
        "asset_surface_visibility_up_text_visual_integration_still_split"
    )
    assert audit["viewer_comparison_closure"]["viewer_latest_run_id"] == "2.90"
    assert len(audit["role_assessments"]) == 6
    assert "text and visual systems still behave as parallel layers" in report


def test_run2_92_renderer_text_visual_binding_rerun_consumes_v_and_updates_viewer() -> None:
    assert EXPECTED_RUN2_W_SCRIPT.exists(), "missing Part W renderer text-visual binding script"
    script = EXPECTED_RUN2_W_SCRIPT.read_text(encoding="utf-8")
    assert_contains(
        script,
        [
            *EXPECTED_RUN2_W_REQUIRED_INPUTS,
            "run2_92_renderer_text_visual_binding_repair_rerun_result.json",
            "run2_92_renderer_text_visual_binding_repair_rerun_result.md",
            "build_ppt_run_html_viewer.py",
            "bad_without_text_visual_binding",
            "drawRun292ProductSurfaceHero",
            "drawRun292EvidenceWorkspace",
            "drawRun292DecisionBoard",
            "text_visual_binding_records",
        ],
    )

    evaluation = load_json(EXPECTED_RUN2_V_RESULT)
    run290 = load_json(EXPECTED_RUN2_U_RESULT)
    o1_plan = load_json(PACK / "run2_81_text_composition_typography_plan.json")
    result = load_json(EXPECTED_RUN2_W_RESULT)

    assert result["artifact_id"] == "run2_92_renderer_text_visual_binding_repair_rerun_result"
    assert result["part"] == "Part W"
    assert result["run_id"] == "2.92"
    assert result["status"] == "run2_92_renderer_text_visual_binding_repair_rerun_generated_public_blocked"
    assert result["public_ready"] is False
    assert result["public_release_started"] is False
    assert result["quality_claim_boundary"] == (
        "text_visual_binding_renderer_generated_viewer_check_only_no_quality_verdict"
    )
    assert result["consumed_sources"] == EXPECTED_RUN2_W_REQUIRED_INPUTS
    assert result["source_v_evaluation"]["status"] == evaluation["status"]
    assert result["source_run2_90_renderer_result"]["status"] == run290["status"]
    assert result["source_o1_text_composition_plan"]["status"] == o1_plan["status"]
    assert result["source_v_evaluation"]["next_required_action"] == (
        "part_w_renderer_text_visual_binding_repair_from_v_evaluation"
    )

    manifest = result["renderer_text_visual_binding_manifest"]
    assert manifest["generator"] == "scripts/generate_ppt_run2_92_renderer_text_visual_binding_repair_arms.mjs"
    assert manifest["consumed_sources"] == EXPECTED_RUN2_W_REQUIRED_INPUTS
    assert manifest["best_internal_arm"] == "run2_92_full_text_visual_binding"
    assert manifest["viewer_update"]["latest_run_id"] == "2.92"
    assert manifest["viewer_update"]["viewer_can_reference_new_run"] is True
    assert set(manifest["arms"]) == EXPECTED_RUN2_W_ARMS
    ...
    pages = result["rendered_pages"]
    binding_records = result["text_visual_binding_records"]
    assert [page["role"] for page in pages] == EXPECTED_RUN2_74_SLIDE_STORY_ROLES
    assert [record["role"] for record in binding_records] == EXPECTED_RUN2_74_SLIDE_STORY_ROLES
    assert [page["slide_index"] for page in pages] == [1, 2, 3, 4, 5, 6]
    assert len({page["text_visual_binding_id"] for page in pages}) == 6

    for page, binding in zip(pages, binding_records, strict=True):
        role = page["role"]
        assert binding["role"] == role
        assert page["visual_grammar_module"] == EXPECTED_RUN2_E_PAGE_MODULE_MAP[role]
        assert page["source_v_root_cause_layer"] in EXPECTED_RUN2_V_ROOT_CAUSE_LAYERS
        assert page["renderer_function_name"].startswith("drawRun292")
        assert set(page["renderer_repair_directives_applied"]) >= EXPECTED_RUN2_W_REPAIR_FLAGS
        assert set(page["anti_regression_gates"]) >= {
            "object_bound_typography",
            "caption_anchor_binding",
            "proof_object_embedding",
        ...
        assert page["asset_surface_rendered"] is True
        assert page["object_bound_typography_applied"] is True
        assert page["caption_anchor_binding_applied"] is True
        assert page["proof_sentence_embedded_in_visual_object"] is True
        assert page["text_visual_binding_repaired"] is True
        assert page["traceability_on_canvas"] is False
        assert page["floating_label_count"] == 0
        assert page["label_count"] <= 2
        assert page["min_visible_label_font_size"] >= 13
        assert set(page["text_blocks_bound_to_objects"]) == EXPECTED_RUN2_W_TEXT_BLOCKS
        assert page["visual_anchor_count"] >= 4
        assert page["public_polish_claimed"] is False
        assert binding["text_visual_binding_id"] == page["text_visual_binding_id"]
        assert binding["source_o1_text_composition_id"].startswith("text_composition_2_81_")
        assert set(binding["bound_blocks"]) == EXPECTED_RUN2_W_TEXT_BLOCKS
        assert binding["floating_labels_allowed"] is False
        assert binding["traceability_on_canvas"] is False
        assert binding["binding_strategy"] in {
    ...
    checks = result["renderer_text_visual_binding_checks"]
    assert checks["pages_with_v_evaluation_consumed"] == 6
    assert checks["pages_with_run2_90_asset_surface_preserved"] == 6
    assert checks["pages_with_o1_text_composition_consumed"] == 6
    assert checks["pages_with_object_bound_typography"] == 6
    assert checks["pages_with_caption_anchor_binding"] == 6
    assert checks["pages_with_proof_embedded_in_visual_object"] == 6
    assert checks["pages_with_traceability_routed_off_canvas"] == 6
    assert checks["public_quality_verdict_started"] is False
    ...
    assert_contains(
        ...
        [
            '"latestRunId": "2.92"',
            "Run 2.92",
            "ppt-run2-92-full-vulca",
            "run2_92_renderer_text_visual_binding_repair_rerun_result.json",
        ],
    )
    assert "data-run-id=\"2.92\"" in html_output.read_text(encoding="utf-8")

    viewer_data = build_data(ROOT / "outputs" / DEFAULT_THREAD_ID / "presentations", viewer_output)
    run = next(run for run in viewer_data["runs"] if run["id"] == "2.92")
