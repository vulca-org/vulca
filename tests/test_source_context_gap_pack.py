...
def test_source_context_gap_pack_prioritizes_remaining_v3_router_gaps(tmp_path):
    ...
    assert report["summary"] == {
        "remaining_gap_count": 15,
        "source_context_gap_closable_count": 15,
        "tiny_model_dispatch_recoverable_count": 12,
        "still_agent_required_after_source_context_count": 3,
    }
    assert report["counts_by_case_type"] == {
        "decompose_case": 3,
        "layer_generate_case": 6,
        "redraw_case": 6,
    }
    ...
    assert report["counts_by_gap_task"] == {
        "attach_reviewed_source_artifact": 2,
...
def test_source_context_gap_pack_cli_writes_summary(tmp_path):
    ...
    assert "Remaining source-context gaps: 15" in result.stdout
    assert "Tiny-model dispatch recoverable after source context: 12" in result.stdout
    ...
    assert report["summary"]["remaining_gap_count"] == 15
