/**
 * SketchUploadNode — Drop zone + preview, outputs SKETCH.
 */

import { memo, useState } from 'react';
import { Handle, Position, type NodeProps } from '@xyflow/react';

interface SketchUploadNodeData {
  [key: string]: unknown;
  imageUrl?: string;
}

function SketchUploadNodeComponent({ data, selected }: NodeProps & { data: SketchUploadNodeData }) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5 px-2 py-1.5 border-b border-gray-100 dark:border-gray-700">
        ...
        <span className="text-[10px] font-semibold text-gray-700 dark:text-gray-300">Sketch</span>
      </div>
      <div
        className="p-2"
        onDrop={handleDrop}
      ...
      >
        {preview ? (
          <img src={preview} alt="Sketch" className="w-full h-20 object-contain rounded" />
        ) : (
          <div className="w-full h-20 border-2 border-dashed border-gray-200 dark:border-gray-600 rounded flex items-center justify-center">
            <span className="text-[9px] text-gray-400">Drop sketch here</span>
          </div>
        )}
      </div>
      <Handle
...
export default memo(SketchUploadNodeComponent);
