...
interface ModelImportNodeData {
  [key: string]: unknown;
}

function ModelImportNodeComponent({ selected }: NodeProps & { data: ModelImportNodeData }) {
  return (
    <div
      className={[
    ...
    >
      <div className="flex items-center gap-1.5 px-2 py-1.5 border-b border-gray-100 dark:border-gray-700">
        ...
        <span className="text-[10px] font-semibold text-gray-700 dark:text-gray-300">3D Model</span>
      ...
      <div className="p-2">
        <div className="w-full h-16 bg-gray-100 dark:bg-gray-700 rounded flex items-center justify-center">
          <span className="text-[9px] text-gray-400">.obj / .glb</span>
        </div>
      </div>
      <Handle
        type="source"
        position={Position.Right}
        className="!w-2.5 !h-2.5 !bg-[#6B8E7A] !border-white dark:!border-gray-800 !border-2"
      />
    </div>
  );
}

export default memo(ModelImportNodeComponent);
