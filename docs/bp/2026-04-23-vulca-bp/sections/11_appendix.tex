...
\paragraph{v1.5 交付版当前状态}

本版本为交付终版。红色 \TBD{} 仅保留\keyword{少量结构必需的占位符}（如样例行的格式提示），
不代表必填阻塞项；\keyword{Jindong Wang 顾问关系书面材料}将在 DD 数据室提供。

\emph{后续若扩展}（非阻塞，按需）：
创始人可选个人化内容、行业顾问具名、政府对接具体渠道、LOI 客户清单、股权结构微调——
可在 Pre-A 前随实际进展补入，不影响天使轮与孵化器申报使用。
...
\paragraph{共享字段提示}
GitHub star / PyPI 下载在\keyword{第 4 章（获客目标）}与\keyword{第 7 章（路线图）}两处为同源数字；
如后续根据实际数据调整，请保持两处一致。
