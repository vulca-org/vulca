...
本 BP 中所有\TBD{红框}字段汇总，便于定稿前一次扫完（按正文出现顺序）：

\begin{itemize}
  \item \keyword{第 1 章}：本轮募资金额、估值口径、本轮诉求一句话；
  \item \keyword{第 2 章}：市场规模三项数据源（Gartner / a16z / 信通院 / 国家文物局口径）；
  \item \keyword{第 4 章}：SaaS 月订阅定价 / 席、文化评估 API 单次定价、On-Prem 首年许可 + 年度维护定价、早期 SaaS GitHub star + PyPI 目标、灯塔客户候选名单、政府对接渠道、\keyword{学术 / 会议差旅预算}；
  \item \keyword{第 6 章}：创始人 bio、核心成员、学术顾问 / 行业顾问具名、目标团队规模、公司主体形式、股权比例、ESOP 池、投资人股比与董事席位、IP 归属；
  \item \keyword{第 7 章}：各季度 GitHub star / PyPI 下载 / Cloud MAU / 付费账户数 / ARR 具体数字（与第 4 章同源，避免两处数字不一致）；
  \item \keyword{第 8 章}：本轮募资金额最终校准 → 用资预算比例、三年 ARR 保守/中性/乐观具体数字、定价假设、每 1000 MAU 推理成本敏感性；
  \item \keyword{第 9 章}：国产算力（Ascend / 寒武纪）适配时间、算法备案判定结论、专利申报清单（是否全选）、商标类别（第 9 / 41 / 42 类）；社会效益 6 项年度目标数；
  \item \keyword{第 10 章}：内容安全机制细节（黑白名单、敏感类别检测算法、审核日志策略）。
\end{itemize}

\paragraph{共享字段提示}
GitHub star / PyPI 下载在 \keyword{第 4 章（获客目标）} 与 \keyword{第 7 章（路线图）} 两处都有 \TBD{}——
定稿时必须\keyword{两处同源填写}，避免数字不一致被 DD 察觉。
...
\begin{enumerate}
  ...
  A：漏斗见第 \ref{sec:roadmap} 章（7.4 节）——曝光 / 知晓 / 尝试 / 激活 / 留存 / 付费意向 / 付费转化七级；配合 dev.to 月度深度文 + Xiaohongshu 双语种子内容 + Claude Code 插件生态生态位 + ACM MM 2026 学术引流。如果某一级转化不达预期，优先加码 \TBD{具体渠道组合}并降级目标。
  ...
  A：\TBD{当前处于前期接触阶段，LOI 数量需你确认。建议本轮交割前至少锁定 1–2 家\keyword{非排他性意向书}作为 DD 实体证据，写入数据室}。
