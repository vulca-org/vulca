...
\paragraph{必须你亲自填入或确认的红色 TBD（v1.4 后仍剩 4 处）}
\begin{enumerate}
  \item \keyword{创始人简历可选续写字段}（§I CV 底部）——奖项 / 志愿 / 关键突破等（非必填）。
  \item \keyword{创始人此前学历}（§I CV 教育栏）——硕士 / 本科学校、专业、毕业年份、学位论文题目。
...
\emph{已通过 v1.3 / v1.4 完成的原 TBD}：创始人 bio（已填入 §6 + §I 真实信息）、联合创始人槽位（已以 Qiufeng Yi 补齐）、学术合作导师（Prof.~Natasha Lushetich + Dr.~Ramon Ruiz-Dolz + Prof.~Jindong Wang 均已具名）。

\paragraph{已默认、请扫读的琥珀 presumed（约 47 处）}
...
用资预算 ¥600 万分科目比例；T+0 锚点；Q\&A 答案锚；Jindong Wang 顾问关系书面材料备于 DD 数据室等。
...
\presumed{此前} & \TBD{请补充：硕士 / 本科学校、专业、毕业年份、学位论文题目（如适用）} \\
\midrule
