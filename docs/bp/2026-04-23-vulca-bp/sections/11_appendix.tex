...
\subsection*{G. 待确认字段清单（两色分级）}

本 BP 中的待确认字段分两类：

\begin{itemize}
  \item \TBD{红色 TBD 框}\quad — 真需你提供的\keyword{身份 / 关系 / 战略诉求}信息，AI 无法替代，不能编造；
  \item \presumed{琥珀 presumed 框}\quad — AI 已用\keyword{行业标准默认值或保守口径}填入，请扫读后确认或调整，不是阻塞项。
\end{itemize}

\paragraph{必须你亲自填入的红色 TBD（共 5-7 处）}
\begin{enumerate}
  \item \keyword{创始人真实简历}（§6.2）——姓名 / 学历 / 职业经历 / 代表作 / 个人动机；投资人最关心的首页信息。
  \item \keyword{联合创始人或早期核心成员}（§6.2）——若已有，具名列出；否则可按默认"关键岗位补齐"叙事保留。
  \item \keyword{学术合作导师 / 学校}（§6.3）——ACM MM 2026 合作具名；需对方同意。
  \item \keyword{行业顾问}（§6.3）——文博 / 设计 / 出版 2-3 位具名。
  \item \keyword{政府 / 行业具名渠道}（§4.4）——如已有，具体接触过的部门与项目。
  \item \keyword{行业灯塔客户 LOI}（§11 附录 H Q4）——是否已有非排他性意向书；具体客户名。
  \item \keyword{股权结构最终数字}（§6.5）——默认方案见表，最终由你与法务顾问锁定。
\end{enumerate}

\paragraph{已默认、请扫读的琥珀 presumed（约 25 处）}
覆盖：募资金额 ¥ 600 万 / post-money ¥ 3500 万；SaaS ¥299/席/月 + API ¥0.5/次 + On-Prem ¥30-80 万首年；
市场规模三组数据的行业公开口径；星标 / PyPI / ARR 三年三档具体数字；社会效益 6 项指标年度目标数；
差旅 ¥15-20 万 / 年；国产算力时间表；内容安全三层防线；商标 9/41/42 类全选；专利 2 件；
用资预算 ¥600 万分科目比例；T+0 锚点；Q\&A 答案锚等。

\paragraph{共享字段提示}
GitHub star / PyPI 下载在\keyword{第 4 章（获客目标）}与\keyword{第 7 章（路线图）}两处同步——
如果你调整其中一处，请保持两处一致。

\subsection*{H. DD 问答预演（VC 与评审视角）}

以下 5 组问题是\keyword{天使 VC + 孵化器评审}最可能追问的"必杀题"。
预先把答案锚点写进 BP 附录，而非到现场临场组织，体现团队的\keyword{自我审视能力}。

\begin{enumerate}
  \item \keyword{Q：从目前 6 stars / 82 views 到 Q4 2026 1500 stars + 30 付费账户，这中间是怎么发生的？}\\
  A：漏斗见第 \ref{sec:roadmap} 章（7.4 节）——曝光 / 知晓 / 尝试 / 激活 / 留存 / 付费意向 / 付费转化七级；配合 dev.to 月度深度文 + Xiaohongshu 双语种子内容 + Claude Code 插件生态生态位 + ACM MM 2026 学术引流。如果某一级转化不达预期，优先加码的渠道组合：\keyword{dev.to 付费推广 + 针对 agent 创业项目的 Discord/Slack 置换投放 + 1-2 场线上 AI 开发者 meetup 主题分享} \presumed{默认策略：以内容驱动为主，付费推广为辅}，必要时降级目标。
  ...
  \item \keyword{Q：第一个愿意付费的博物馆 / 出版社 / 设计集团是谁？有 LOI 吗？}\\
  A：目前处于前期接触阶段，候选行业包括博物馆数字化、出版教辅、品牌设计集团、高校艺术研究中心。\TBD{建议本轮交割前至少锁定 1-2 家\keyword{非排他性意向书（LOI）}作为 DD 实体证据写入数据室；具体接触进度由你补入}
...
\paragraph{说明}
上述 Q\&A 旨在帮助 DD 准备与申报答辩演练，不构成正式承诺。
\TBD{问题 4 的 LOI 状态需你在正式对外版本前填入}。

\subsection*{I. 创始人简历（Founder CV）}
...
\TBD{此前} & 请补充：硕士 / 本科学校、专业、毕业年份、学位论文题目（如适用） \\
\midrule
...
ACL 2026（审稿中） & \textbf{一作} \textperiodcentered{} \textit{VULCA-Bench: A Benchmark for Culturally-Aware Visual Understanding at Five Levels} \textperiodcentered{} arXiv 2601.07986 \\
arXiv 2026 & \textbf{一作} \textperiodcentered{} \textit{Art Critique: Cross-Cultural Expert-Level Critique Evaluation with VLMs} \textperiodcentered{} arXiv 2601.07984 \\
\midrule
...
生态位 & Claude Code 插件市场 \code{vulca-org/vulca-plugin}；PyPI \code{vulca} \\
\midrule
\multicolumn{2}{@{}l}{\heiti\color{vulcaPrimary}学术与资源网络} \\
\midrule
主要学术顾问 & Prof.~Jindong Wang（William \& Mary 助理教授，前 MSRA 资深研究员 2019–2024，23000+ citations, H-index 54，World Top 2\% Highly Cited Scientists） \\
...
学术赛事 & ACM MM 2026 EmoArt Challenge Track 1 参赛（提交 2026-06-10） \\
\midrule
...
\TBD{可选续写} & 奖项、志愿经历、个人关键突破等，可按需续写 \\
\bottomrule
\end{tabularx}
\end{center}

\presumed{以上简历按公开信息（University of Dundee DJCAD profile / ACL Anthology / GitHub / PyPI）整理；
\TBD 处为公开信息未覆盖但简历通常需要的字段，按需补充}
