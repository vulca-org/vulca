...
\begin{itemize}
  ...
  \item \keyword{品牌商标}：\code{Vulca} 名称与 logo 建议\keyword{国内第 9 类（软件）、第 41 类（教育培训）、第 42 类（SaaS/设计服务）}全部注册，条件允许时追加 Madrid 国际注册。
\end{itemize}
...
\begin{enumerate}
  ...
  \item \keyword{算力自主可控}\quad Apple Silicon MPS 已端到端验证；CUDA 适配在 2026 Q4 前完成；国产算力首轮适配路线：\keyword{2027 Q1 完成 Ascend POC，2027 Q3 完成寒武纪适配}（与国产算力政策窗口对齐）；
...
《生成式 AI 服务管理办法》第 11 条（算法备案） & 默认判定：\keyword{Cloud 版本（面向公众服务）需备案}；\keyword{SDK + On-Prem（不面向公众）按具体交付形态办理} \\
...
未来 18 个月内的 IP 申请计划如下，预算约 ¥ 15–25 万，与第 \ref{sec:financials} 章法务预算对齐。
...
\midrule
\keyword{服务机构数} & 博物馆 / 出版社 / 品牌 / 教育机构使用 Vulca（免费社区或商业部署均计入） & $\ge$ 20 家 \\
\keyword{数字化作品数} & 通过 Vulca 分层 / 编辑 / 评估的文化作品累计数 & $\ge$ 5000 件 \\
\keyword{非遗 / 馆藏 POC 案例} & 公开发表的非遗复现、馆藏数字化 POC 项目数 & $\ge$ 3 个 \\
\keyword{学术 / 教育课时} & 高校艺术 / 设计教学中使用 Vulca 的课时数（合作院校披露） & $\ge$ 200 课时 \\
\keyword{社区贡献 YAML} & 开源社区贡献的新文化传统定义数 & $\ge$ 3 个（跨机构） \\
\keyword{就业拉动} & 团队直接雇佣 + 生态合作方兼职 & $\ge$ 8 人全职 + 10 人兼职 \\
\bottomrule
\end{tabularx}
\end{center}
...
\begin{itemize}
  ...
  \item \keyword{未成年人 / 敏感内容}\quad 默认安全策略：\keyword{三层防线}——(i) 输入端关键词黑名单拦截；(ii) 上游 provider 自带审核回调；(iii) 输出端二分类敏感检测（默认模型为 CLIP-based 开源检测器）；未成年人场景提供"教学白名单"模式。所有拦截事件进入审计日志；Cloud 正式上线前完成安全评估并落实备案判断。
\end{itemize}
