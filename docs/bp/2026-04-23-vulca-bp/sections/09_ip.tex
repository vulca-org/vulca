...
\begin{enumerate}
  ...
  \item \keyword{算力自主可控}\quad Apple Silicon MPS 已端到端验证；CUDA 适配在 2026 Q4 前完成；\TBD{国产算力（Ascend / 寒武纪）首轮适配时间建议 2027 Q1}；
...
《生成式 AI 服务管理办法》第 11 条（算法备案） & \TBD{判断：面向公众服务的 Cloud 版本需备案；纯 SDK + On-Prem 不面向公众，不需备案。正式申报时由法务顾问最终判定} \\
...
\midrule
\keyword{服务机构数} & 博物馆 / 出版社 / 品牌 / 教育机构使用 Vulca（免费社区或商业部署均计入） & \TBD{$\ge$ 20 家} \\
\keyword{数字化作品数} & 通过 Vulca 分层 / 编辑 / 评估的文化作品累计数 & \TBD{$\ge$ 5000 件} \\
\keyword{非遗 / 馆藏 POC 案例} & 公开发表的非遗复现、馆藏数字化 POC 项目数 & \TBD{$\ge$ 3 个} \\
\keyword{学术 / 教育课时} & 高校艺术 / 设计教学中使用 Vulca 的课时数（合作院校披露） & \TBD{$\ge$ 200 课时} \\
\keyword{社区贡献 YAML} & 开源社区贡献的新文化传统定义数 & \TBD{$\ge$ 3 个（跨机构）} \\
\keyword{就业拉动} & 团队直接雇佣 + 生态合作方兼职 & \TBD{$\ge$ 8 人全职 + 10 人兼职} \\
\bottomrule
\end{tabularx}
\end{center}
