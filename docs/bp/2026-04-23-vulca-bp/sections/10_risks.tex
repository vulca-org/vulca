...
\begin{itemize}
  ...
  \item \keyword{内容安全机制}：\TBD{在内容生成路径上设置关键词黑名单、敏感类别检测；正式产品上线前完成安全评估。}；
...
\begin{center}
\small
\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{3.4cm}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}p{2.0cm}@{}}
\toprule
\rowcolor{tableHead}
\heiti 风险类别 & \heiti 当前级别评估 & \heiti 趋势 \\
\midrule
模型依赖 & 低（多 provider + 本地路径兜底） & 持平 \\
市场教育 & 中（agent 概念仍在扩散中） & 向下 \\
政策合规 & 中（本地链路抵消大部分） & 持平 \\
人才集中 & \keyword{高}（solo 起家，天使轮扩团队是关键用途） & \keyword{融资后下降} \\
竞品突然进场 & 低（尚未观察到对位竞品） & 持平 \\
\bottomrule
\end{tabularx}
\end{center}
