...
Each arm must include a persisted `trace_manifest.json` under `outputs/` before comparison. The manifest is the audit trail for memory selection, density counts, deletion/routing decisions, asset provenance, QA outcomes, and release-gate status.
