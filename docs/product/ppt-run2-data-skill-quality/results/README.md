...
Run 2.14 is the latest generated PPT rerun. It combines the Run 2.10 visual-system aesthetic shell with the Run 2.13 thick-data trace core, then updates the HTML viewer to latest `2.14`.

Current gate: `2_10_aesthetic_shell_recovered_with_2_13_trace_core_hidden_from_slide_surface`. Run 2.14 keeps the project public blocked because the editorial layouts are still partly hardcoded, dynamic text-length robustness is unproven, and no human release approval is recorded.
...
`run2_14_full_skill` is now the best internal arm for proving the current product loop because it keeps data/workflow trace truthfulness while hiding visible workflow machinery from the public slide surface. The `bad_visible_workflow_memory` negative control confirms that Run 2.12 data/workflow without the Run 2.10 aesthetic shell still trends toward beige box/checklist output. The result remains public blocked pending reusable layout-module selection, text-length robustness, native render inspection, source-brand sanitization review, Gemini and human visual review, motion/render support, and human approval.
