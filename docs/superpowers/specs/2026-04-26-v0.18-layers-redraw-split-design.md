...
4. **Layer write**: each instance becomes a flat sibling layer named `<label>_<i>`, ordered by DINO `det_score` descending (the sort happens at NMS stage in `_nms_bboxes` before SAM runs). `<label>_0` is the most confident detection.
...
digraph multi_instance_flow {
    ...
    sort [shape=box, label="sort by det_score desc\n(NMS stage, before SAM)"];
...
| DINO returns 2–8 bbox | Write N layers `<label>_0..N-1`, sorted by DINO det_score desc | (no flag) |
...
| 7 | `test_naming_sorted_by_det_score_desc` | 3 bboxes with distinct scores | `lanterns_0.det_score > lanterns_1 > lanterns_2` |
