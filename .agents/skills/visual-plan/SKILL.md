...
| 12 | Stale lockfile (jsonl mtime > 300s OR lockfile.started_at > 300s + no jsonl) | Auto-recover silently: `os.unlink(lockfile)`. Append Notes: `[stale-lock-recovery] previous pid <N> abandoned at <iso>; reclaimed at <now>. Resuming from iter <K>.` Handoff string at Phase 4 appends ` (recovered from stale lock at iter <K>)` suffix. **No user prompt; NOT stderr** (Codex skills don't surface stderr). | helper |
...
- Sibling skills: `.Codex/skills/visual-brainstorm/SKILL.md` (v0.17.4), `.Codex/skills/visual-spec/SKILL.md` (v0.17.5)
