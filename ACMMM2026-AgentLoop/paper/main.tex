...
Current text-to-image models generate culturally decontextualized art, routinely violating tradition-specific conventions---rendering \emph{xieyi} ink paintings with photorealistic textures or Islamic geometric patterns with figurative elements.
We present \system{}, a multi-agent pipeline that grounds art generation in cultural knowledge through five cognitive layers (\Lx{1}--\Lx{5}), from Visual Perception to Philosophical Aesthetics.
Its core mechanism is a \emph{cultural pipeline router} that selects tradition-specific dimension weights across 9 art traditions, enabling evaluation to reflect each tradition's priorities.
Controlled ablation across 480 runs (16 conditions $\times$ 30 tasks) yields three findings:
(1)~Cultural routing alone improves the mean score by $+0.036$ ($d = 0.95$), accounting for 95\% of the full effect, with gains concentrated in \Lx{5} and \Lx{3};
(2)~VLM-based scoring replacing CLIP yields $+0.077$ ($d = 2.10$), making LLM critics largely redundant;
(3)~Generator upgrade (FLUX$\to$NB2) raises the peak to $\mathbf{0.917}$ ($+0.107$, $d = 2.97$).
Effect decomposition attributes 73\% of improvement to the evaluation framework and 27\% to the generator, while multi-round self-correction adds only $+0.002$ (ns)---confirming diminishing returns as generator quality improves.
...
Similarly, Chinese xieyi painting privileges expressive emptiness (\emph{liu bai}) as a philosophical statement, not merely a composition choice; photorealistic gap-filling violates the art form's core epistemology.

Current models cannot make these distinctions because they primarily optimize for visual fidelity rather than cultural coherence.
...
Broader surveys document persistent geo-cultural biases that resist dataset-level mitigation~\cite{wan2024survey}, while multilingual evaluation reveals that models collapse diverse cultural prompts into culturally neutral outputs~\cite{ventura2025cultural, shi2025culture}.
...
\begin{enumerate}
    ...
    \item \textbf{VLM-Based Evaluation Resolves the Scoring Bottleneck.} Replacing CLIP with per-layer VLM scoring yields $+0.077$ ($d = 2.10$), while revealing that LLM critics become functionally redundant under high-fidelity VLM evaluation---making VLM routing without LLM critic the strongest single configuration ($0.887$).

    \item \textbf{Empirical Diminishing-Returns Law.} Controlled ablation across 480 runs (16 conditions $\times$ 30 tasks) decomposes improvement into 73\% evaluation framework and 27\% generator upgrade (FLUX$\to$NB2, peak $0.917$). Multi-round self-correction benefit inversely correlates with generator quality ($r = -0.81$): SD1.5 $+0.022$, FLUX $+0.002$ ns, NB2 $+0.002$ ns.
\end{enumerate}
...
Multi-agent architectures have shown promise in code generation~\cite{hong2024metagpt}, scientific discovery~\cite{boiko2023autonomous}, and creative writing~\cite{qian2024chatdev}.
...
On the generation side, Culture-Specific LoRA~\cite{chen2025culture} fine-tunes diffusion models with tradition-specific adapters to improve cultural authenticity---a complementary \emph{generation-side} approach to our \emph{evaluation-side} routing.
However, none of these multi-agent systems incorporate cultural knowledge bases or tradition-specific evaluation routing grounded in domain-specific criteria.

\paragraph{Self-Correction in Generation.}
...
TIFA~\cite{hu2023tifa} measures text-image faithfulness via VQA; VIEScore~\cite{ku2024viescore} uses MLLMs for explainable image scoring; CoFiVLA~\cite{niu2025cofivla} aligns coarse and fine visual-language features for aesthetic assessment; MLLM-as-a-Judge~\cite{chen2024mllmjudge} benchmarks multimodal judgment, confirming pairwise comparison reliable but scoring granularity challenging.
...
\textbf{\Lx{5} Philosophical Aesthetics}: transcendent value---whether the work achieves qualities like wabi-sabi impermanence, sublime terror, or xieyi's ``meaning beyond form'' (\emph{yi zai bi xian}). The most difficult layer to evaluate and the one where current AI systems fail most dramatically.
...
Given that prompt engineering alone cannot enforce structural cultural constraints~\cite{jeong2025culturetrip, ventura2025cultural, shi2025culture} (\S\ref{sec:intro}), the remaining path is an external evaluation pipeline.
But why four agents rather than a single LLM that retrieves, generates, evaluates, and decides?
...
\begin{figure*}[t]
    \centering
    \input{figures/architecture_inline}
    \caption{The \system{} pipeline. The Scout retrieves cultural evidence, the Draft generates candidates via pluggable backends (SD1.5/FLUX/NB2) grounded in an EvidencePack, the Critic evaluates across \Lx{1}--\Lx{5} using a hybrid rule+LLM+VLM approach, and the Queen decides whether to accept, rerun with a FixItPlan, or request more evidence. Dashed arrows show self-correction loops; the Cultural Router adjusts \Lx{1}--\Lx{5} weights per tradition.}
...
(2)~\textbf{Terminology lookup} from a curated dictionary of 200+ cultural terms with L-level associations (e.g., \emph{cun fa} tagged as \Lx{2}-relevant);
...
\smallskip
\noindent
While the self-correction mechanism above provides architectural completeness, our ablation (\S\ref{sec:analysis}) identifies the Cultural Pipeline Router---not iterative correction---as the system's most consequential design choice, accounting for the vast majority of observed quality gains.
...
\midrule
Chinese Xieyi   & .10 & .15 & .25 & .20 & \textbf{.30} \\
Chinese Gongbi  & .15 & \textbf{.30} & .25 & .15 & .15 \\
Japanese        & .15 & .20 & .20 & .20 & \textbf{.25} \\
Islamic         & .25 & \textbf{.30} & .20 & .15 & .10 \\
African         & .15 & .20 & \textbf{.30} & .20 & .15 \\
Western Acad.   & .20 & \textbf{.25} & .15 & \textbf{.25} & .15 \\
S.\ Asian       & .15 & .20 & \textbf{.25} & .15 & \textbf{.25} \\
...
The router also selects pipeline variants: \emph{chinese\_xieyi} uses atomic (single-pass) generation honoring the \emph{yiqi} principle; \emph{western\_academic} uses three-step progressive refinement.
...
D & FLUX & Rule+LLM & $\checkmark$ & $\checkmark$ & .80 \\
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=\textwidth,height=0.92\textheight,keepaspectratio]{figures/qualitative.pdf}
    \caption{Qualitative comparison across six key conditions (A, C, D, H, G, G++) spanning four experimental phases. Three representative tasks (Taboo, Poetic, Cultural) show progressive improvement from SD\,1.5 baseline (A) through FLUX (C$\to$D), VLM scoring (H), to NB2 generation (G/G++). Scores and $\Delta$ vs.\ FLUX baseline (C) shown below each image.}
...
D & \textbf{.880}\,{\scriptsize(.045)} & \textbf{.852}\,{\scriptsize(.016)} & \textbf{.809}\,{\scriptsize(.082)} & \textbf{.847} \\
\midrule
...
D & FLUX / Full Pipeline & .847 & +.038 & $<$.001*** & 0.94 & .800 & .766 & .872 & \textbf{.964} & .839 \\
...
H & FLUX / VLM / Single          & .887 & +.077 & $<$.001*** & 2.10 & .870 & .848 & .909 & .928 & .871 \\
...
G & NB2 / VLM / Single            & .915 & +.105 & $<$.001*** & 3.65 & .896 & .888 & \textbf{.950} & .930 & .900 \\
G\textsuperscript{+} & NB2 / VLM+Evid / Single   & .908 & +.098 & $<$.001*** & 2.02 & .891 & .876 & .939 & .931 & .892 \\
G\textsuperscript{++} & NB2 / VLM / Multi         & \textbf{.917} & \textbf{+.107} & $<$.001*** & 2.97 & \textbf{.899} & \textbf{.889} & .946 & .930 & \textbf{.909} \\
\midrule
...
Across all 30 tasks, G\textsuperscript{+} shows negative deltas on 18/30 tasks (mean $\Delta = -0.007$), with degradation concentrating in tasks where NB2's baseline score already exceeds $0.92$---consistent with NB2 having already encoded sufficient cultural knowledge internally, such that the additional ${\sim}$2K-token EvidencePack competes with rather than complements NB2's internal representations.
A single taboo task illustrates the extreme case: scoring $0.915$ under G but dropping to $0.725$ under G\textsuperscript{+} ($\Delta = -0.190$), with \Lx{2} collapsing from $0.875$ to $0.525$, suggesting attention dilution that degrades technical execution.
...
The same image scores differently under routed vs.\ unrouted evaluation because the weighting reflects the tradition's actual priorities.
However, re-weighting alone does not fully explain the effect: routing cascades through the entire pipeline---it changes which evidence the Scout retrieves (e.g., searching for \emph{liu bai} terminology rather than generic composition rules), how the Draft constructs evidence-grounded prompts, and which dimensions the Critic prioritizes in its per-layer evaluation.
The result is both a measurement improvement (better-calibrated evaluation) and a generation improvement (more culturally grounded output).
...
This contrasts with \emph{generation-side} approaches that build cultural knowledge into the generation process itself---for example, Culture-Specific LoRA~\cite{chen2025culture} fine-tunes tradition-specific adapters, while models like NB2 produce internal ``thought images'' before final output, performing implicit self-evaluation within the same forward pass.
...
The full 480-run experiment costs approximately~\$15 (FLUX API, NB2 API, SD1.5/RTX~2070, Gemini~Flash).
...
Effect decomposition confirms that cultural art quality is primarily accessible through evaluation-side improvement rather than generator upgrade alone.

Four findings carry implications beyond our specific system.
First, \emph{routing dominates iteration}: tradition-specific dimension weighting accounts for 95\% of the observed improvement ($d = 0.95$), while multi-round self-correction adds only $+0.002$ (ns)---a diminishing return that intensifies as generator quality improves (SD1.5: $+0.022$; FLUX/NB2: $+0.002$ ns).
...
Third, \emph{evaluation modality constrains component design}: VLM-based scoring outperforms CLIP by $+0.041$, but shows substantial functional overlap with LLM critics (I$\to$K: $-0.049$, $p = .029$ uncorrected), making VLM routing without LLM critic the most efficient configuration.
Fourth, \emph{framework contribution outweighs generator contribution}: the 73:27 decomposition (evaluation framework vs.\ generator upgrade) demonstrates that cultural art quality is primarily accessible through evaluation-side improvement---an architectural insight that generalizes beyond specific generators.
...
\begin{enumerate}
    ...
    \item \textbf{Human evaluation}: Results rely on automatic \Lx{1}--\Lx{5} metrics scored by VLM or rule-based methods. While the large effect sizes ($d = 0.95$--$2.97$) and per-dimension consistency suggest genuine quality differences, human evaluation with cultural domain experts---ideally through forced-choice comparison between routed vs.\ unrouted outputs across traditions---is needed to confirm that score improvements correspond to perceptible quality gains and to calibrate VLM-based metrics against expert judgment.
...
The evaluation tasks are drawn from \vulca{}-Bench~\cite{yu2025vulcabench}.
